...
Fix the bottom row and find the maximum area.
...
Iterate from left to right and use a monotonic stack to calculate the area for every bottom row. Update the nearest tree for each column for every bottom row.
