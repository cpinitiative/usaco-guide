...
Fix the bottom row of the rectangle. Then apply the solution for [Max Histogram Area](https://usaco.guide/solutions/lc-largest-rectangle-in-histogram).
...
Update the nearest tree for every column for each new bottom row. Iterate from left to right and use a monotonic stack to calculate the area for every bottom row.
