...
author: Melody Yu (Video), Neo Wang (C++), Brad Ma (Java)
---
...
public class HoofPaperScissors {
	...
	public static void main (String[] args) throws IOException {
		...
		for (int currentGame = 1; currentGame <= numGames; currentGame++) {  // for each game
			// for each switch amount
			for (int numberOfSwitches = 1; numberOfSwitches < maximumSwitches + 2; numberOfSwitches++) {  
				// try switching to the three possibilities
				int add1 = 0;
				if (moves[currentGame] == 0) {
					add1 = 1;
				}
				// take the maximum of the game before + add (win or lose)
				// and switching from the other two options 
				dp[currentGame][numberOfSwitches][0] = Math.max(dp[currentGame - 1][numberOfSwitches][0] 
				+ add1, Math.max(dp[currentGame - 1][numberOfSwitches - 1][1] + add1, 
				dp[currentGame - 1][numberOfSwitches - 1][2] + add1));

				int add2 = 0;
				if (moves[currentGame] == 1) {
					add2 = 1;
				}
				dp[currentGame][numberOfSwitches][1] = Math.max(dp[currentGame - 1][numberOfSwitches][1] 
				+ add2, Math.max(dp[currentGame - 1][numberOfSwitches - 1][0] + add2, 
				dp[currentGame - 1][numberOfSwitches - 1][2] + add2));

				int add3 = 0;
				if (moves[currentGame] == 2) {
					add3 = 1;
				}
				dp[currentGame][numberOfSwitches][2] = Math.max(dp[currentGame - 1][numberOfSwitches][2] + 
				add3, Math.max(dp[currentGame - 1][numberOfSwitches - 1][0] 
				+ add3, dp[currentGame - 1][numberOfSwitches - 1][1] + add3));

				if (currentGame == numGames) {  // check maximum if it is the last row
					maximum = Math.max(maximum, Math.max(dp[numGames][numberOfSwitches][0], 
					Math.max(dp[numGames][numberOfSwitches][1], dp[numGames][numberOfSwitches][2])));
				}
			}
		}
		pw.println(maximum);
