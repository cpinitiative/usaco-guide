...
def dijkstra(graph, source):
    ...
    dist[source] = 0  # setting distance of source to itself as 0
    ...
    while mnpq:
        ...
        for v, w in graph[cur]:  # if not visited, iterate through all neighbours of current node
            if vis[v]:
                continue
            if (w + dist[cur]) < dist[v]:  # if (weight between cur and v + dist of cur from source) < dist of v from source
...
for i in range(ne):  # reading node1,node2,dist between them and updating into graph
    ...
    graph[u].append((v, w))
    graph[v].append((u, w))
d = dijkstra(graph, 1)  # calling algorithm with source as 1
if d[-1] == -1:
    print(-1)
    exit()
path = [nv]
