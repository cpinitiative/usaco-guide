...
To improve this solution, notice that for some $i$, we have $j = A[k]$ only if $k$ is a child of an ancestor of $i$. Since there are only $\mathcal{O}(\log N)$ ancestors of $i$ and each has at most 2 children, this allows us to cut the complexity down to $\mathcal{O}(N \log^2N)$! For convenience, we still refer to the original DP state in the rest of this solution.

However, this DP still uses slightly too much memory. There are two things we need to do to fix this:
