...
The most important observation is that there are two routes we can take:
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		jiro.push_back(Card{type == "ATK", strength});
	}
	...
	for (int i = 0; i < std::min(n, m); i++) {
		...
		if (!jiro[i].type || ciel[i].strength <= jiro[i].strength){
			break;
		}
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < m; j++) {
			if (!done[j] && (
				...
				if (jiro[i].type) {
					appr_2 += ciel[j].strength - jiro[i].strength;
				}
	...
	for (int i = 0; i < m; i++) {
		if (!done[i]) {
			appr_2 += ciel[i].strength;
		}
	}

	std::cout << std::max(appr_2, appr1) << endl;
}
```
</CPPSection>
</LanguageSection>
