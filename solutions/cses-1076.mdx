...
We can directly use an [order statistic tree ](https://codeforces.com/blog/entry/11080) (C++) to get the median while sliding the window across the array.
```cpp
#include<bits/stdc++.h>
...
```
Alternatively, if you do not use C++ (or if you do), you can use a fenwick tree to simulate the aforementioned order statistic tree/indexed set. Clearly, the values of the array would need to be compressed. There will also be an extra $\log$ factor resulting from binary searching on the fenwick tree, yielding a total time complexity of $\mathcal{O}(n\log^2{n})$. 
