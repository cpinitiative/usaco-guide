...
Our goal is to find the length of the longest subarray that has a sum of less than or equal to $t$. In the first sample case, the subarray that has $[1,2,1]$ sums to $5$ and has a length of $3$. In the second sample case, any of the subarrays with length $1$ will work.
