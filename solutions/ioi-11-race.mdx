...
We perform small to large merging to find an $\texttt lca$ $i$ such that $sum(u, root) + sum(v, root) - 2 \cdot sum(i, root) = s$, and $dist(u, root) + dist(v, root) - 2 \cdot dist(i, root) = s$ is minimized. 
...
map<ll, ll> info[maxn];
ll dist[maxn];
ll sum[maxn];  // define these relative to root
int N;
int K;
int u;
int v;
