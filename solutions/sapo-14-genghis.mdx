---
id: sapo-14-TheStablesOfGenghisKhan
source: SAPO 2014
...
$$\texttt{lowestOps}[a][b]=\texttt{lowestOps}[a][c]+\texttt{lowestOps}[c][b]+(stableHorses[b]-stableHorses[a]-1)$$
...
int main() {
	...
	// add two "out-of-bounds" horses for later reasons
	...
	/*
	 * this[i][j] = min operations if we only consider horses
	 * from that exist in the index interval (i, j)
	 * ex: this[1][4] and stable_horses = [0, 1, 3, 7, 10, 11]
	...
	for (int i = 0; i < stable_num + 2; i++) {
		// not sure if the first two assignments are necessary
		lowest_ops[i][i] = 0;
		if (i + 1 < stable_num + 2) {
