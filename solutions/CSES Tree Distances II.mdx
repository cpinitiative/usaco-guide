...
It's easy to find the sum of distances from a single node - just root the tree at that node, do a DFS, and add the depths of each other node to the answer. Unfortunately, $n$ can go up to $2 \cdot 10^5$, so we can't just do this for each node.
...
This gives us a nice way to transition from node 1's answer to node 2's answer using only $n$ and the size of node 2's subtree! Observe that the change in the answer is exactly $n - 2(\text{node 2's subtree size})$.
