...
Notice that any two adjacent characters can be turned into any single character or removed:

If the two characters are the same, simply remove them. 
...
As such, if $O + W$ is even and $C + W$ is odd, we can always reduce our given substring to one C.
