...
When we are performing our BFS, we only have to keep track of the distance between the current amount of milk in pails $X$ and $Y$. That way, we can check all nodes with distance $\le K$.
In our matrix, the current amount of milk in $X$ is $i$, and $Y$ is $j$.

For every step, we simulate all the possible actions.

1. Fill either pail completely to the top: set $i$ or $j$ to $X$ and $Y$ respectively.
2. Empty either pail: set $i$ or $j$ to $0$.
3. Pour the contents of each pail until you either run out or reach the bucket size.

To solve 3, we can find the pour to be $\min(i, X - j)$ or $\min(j, Y-i)$. Then add/subtract this quantity to each pail appropriately.
