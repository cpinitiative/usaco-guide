...
To perform operation 3, we can find the amount poured to be $\min(i, X-j)$ or $\min(j, Y-i)$ depending on which pail you pour from. Then add/subtract this quantity to each pail appropriately.
