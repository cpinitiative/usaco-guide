...
item $j$ represent the apple. Therefore, $|x_i - x_j| \le t_j - t_i$. After extracting the absolute value, we get these two 
inequality.
...
We represnt each item as a point $(x_i - t_i, x_i + t_i)$. According to the inequalities above, in order for a cow to catch the 
...
struct Item {
	int q, t, x, n; // q is the type (cow/apple), t is time of entry, x is position, and n is the amount
