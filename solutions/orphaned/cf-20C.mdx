...
We can implement Dijkstra's Shortest Path algorithm to find the shortest
distances between node $1$ and node $n$. To find the path that yields the
shortest path between nodes $1$ and $n$, we should backtrack from node $n$
...
$n$ is $\infty$, then we should print $-1$.
