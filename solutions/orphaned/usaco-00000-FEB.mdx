...
I came up with ~30 test cases, but the exact number doesn't really matter. You can always come up with more later.
...
int main() {
	...
	if (num_beginning_F == n) {
		...
		// mn doesn't change, since we can always alternate the beginning / ending properly
		mx += num_beginning_F;
		mx += num_ending_F;
	}
