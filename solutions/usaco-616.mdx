...
int main() {
	...
	// run a simple brute force algo, checking each possible barn we can open
	...
	for(int open_barn = 0; open_barn < n; open_barn++) {
		int curdist = 0; 
		// stores the current distance if barn i was the opened barn
...
public class cbarn {
	public static void main(String[] args) throws IOException {
		cbarn.Kattio io = new cbarn.Kattio("cbarn");
		...
		// the answer cannot exceed N * N * 100, 
		// since there are at most 100N cows, each of which can move at most N
		int ans = n * n * 100;
