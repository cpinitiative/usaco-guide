...
public class SumOfThreeValues {
	static class comparator implements Comparator<Group> {
	...
	public static void main (String[] args) {
		...
		for (int i = 1; i <= n; i++) {
			...
			group.value = io.nextInt();
			group.index = i;

			data[i - 1] = group;
		}

		Arrays.sort(data, new comparator());  // make the values in increasing order
		for (int i = 0; i < n; i++) {
			int left = 0;
			int right = n - 1;
			...
			while (left != right) {
				...
				// if the left and the right are not i, their values sum up to the target
