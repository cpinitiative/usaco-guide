...
# store the input in tuples that hold values and indices
...
for i in range(n):
    ...
    while left < right:
        ...
        # if the values sum to the target and aren't from the same index, we have found the answer
...
# print impossible if no answer has been found
print("IMPOSSIBLE")
```
</PySection>
