...
**Main Idea:**
Different from a classic HLD problem, the nesting of functions does not follow the associative property.
...
We can then maintain two segment trees that stores the equivalent function that results from moving from an arbitrary node $u$ to $v$ and from node $v$ to $u$ respectively.
...
void build(int p, int l, int r){
	if(l == r){
		tree[p].fi = tree2[p].fi = val[l].fi;
		tree[p].se = tree2[p].se = val[l].se;
		return;
	}
	int mid = (l+r)>>1;
	build(lc,l,mid);
...
signed main(){
	...
	for(int i = 1; i <= N; ++i){
		int a,b; cin >> a >> b;
		ori[i].fi = a; ori[i].se = b;
	}
	for(int i = 1; i < N; ++i){
		int a,b; cin >> a >> b; ++a; ++b; //Change coordinates from [0,N) to [1,N]
	...
	for(int i = 1; i <= Q; ++i){
		int k; cin >> k;
		if(k == 0){
			int p,c,d; cin >> p >> c >> d; ++p;
			val[dfn[p]].fi = c; val[dfn[p]].se = d; //Potential Problem
