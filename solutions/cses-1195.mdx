...
public class FlightDiscount {
	...
	static long[] minCost(int from, ArrayList<Pair>[] neighbors) {
		...
		while (!frontier.isEmpty()) {
			...
			int curr = currState.first;
			if (minCost[curr] != currState.second) {
				continue;
			}
			for (Pair p : neighbors[curr]) {
				long newCost = minCost[curr] + p.second;
				if (newCost < minCost[p.first]) {
					minCost[p.first] = newCost;
					frontier.add(new Pair(p.first, newCost));
				}
			}
		}
		return minCost;
