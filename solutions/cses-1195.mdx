...
public class FlightDiscount {
	// used to represent both positions in Dijkstra's and edges
	...
	public static void main(String[] args) {
		...
		for (int c = 0; c < cityNum; c++) {
			...
			for (Pair n : neighbors[c]) {
				if (endCosts[n.to] == Long.MAX_VALUE) {
					continue;
				}
				totalMin = Math.min(
					totalMin, startCosts[c] + (n.cost / 2) + endCosts[n.to]
				);
			}
		}
		io.print(totalMin);
	...
	static long[] minCost(int from, ArrayList<Pair>[] neighbors) {
		long[] minCost = new long[neighbors.length];
		Arrays.fill(minCost, Long.MAX_VALUE);
		minCost[from] = 0;
		PriorityQueue<Pair> frontier = new PriorityQueue<>();
		frontier.add(new Pair(from, 0));
		while (!frontier.isEmpty()) {
			Pair currState = frontier.poll();
			int curr = currState.to;
			if (minCost[curr] != currState.cost) {
				continue;
			}
			for (Pair p : neighbors[curr]) {
				long newCost = minCost[curr] + p.cost;
				if (newCost < minCost[p.to]) {
					minCost[p.to] = newCost;
					frontier.add(new Pair(p.to, newCost));
				}
			}
		}
		return minCost;
