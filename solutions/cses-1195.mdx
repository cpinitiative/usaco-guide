...
public class FlightDiscount {
	...
	static ArrayList<ArrayList<Flight>> neighbors = new ArrayList<>();
	static ArrayList<ArrayList<Flight>> reverseNeighbors = new ArrayList<>();
	...
	public static void main(String[] args) throws IOException {
		...
		for (int c = 0; c < cityNum; c++) {
			...
			for (Flight e : neighbors.get(c)) {
				min_cost = Math.min(min_cost, dis1[c] + dis2[e.to] + e.cost / 2);
			}
		}
	...
	public static long[] minDist(int start, ArrayList<ArrayList<Flight>> neighbors) {
		long[] min_dist = new long[neighbors.size()];
