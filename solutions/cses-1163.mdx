...
For each traffic light that is added, we want to find the largest current gap
between traffic lights.

First, let's get our values. We'll use a set to represent the street itself, its
starting and ending points, and every traffic light along it. We'll also use an
integer array to store and read the positions of the traffic lights, since we
can't read values from sets in Java. We'll store our outputs in another array
too.

Now, to actually solve the problem! We'll start by trying to find the maximum
...
Once we've gone through and removed every traffic light and added every max gap
to our output array, we'll iterate through the output array and print out each
element. Boom-we're done!

<Warning>
We have to use some nonstandard io (input/output) to pass all test cases in the allowed time on CSES, due to it's slow grader. These are collapsed, and not particularily relevent to the overall function of the program.
</Warning>
```java
import java.io.*;
import java.util.*;

public class TrafficLights { public static void main(String[] args) throws IOException { 
	//BeginCodeSnip{io inits} BufferedReader r = new
	BufferedReader(new InputStreamReader(System.in)); PrintWriter pw = new
	PrintWriter(System.out); StringTokenizer st = new StringTokenizer(r.readLine());
	//EndCodeSnip{}
    	...
    	//BeginCodeSnip{More io fiddling}
    	r.close();
    	//EndCodeSnip{}
    	...
    	//BeginCodeSnip{Print out the array indexes quickly}
