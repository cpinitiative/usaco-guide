...
data structure. To check if a given $M^*$ value is feasible, we calculate the
maximum height of all four regions for each possible $x$ partition with maximum
$M^*$ cows in the region. We then go through every $x$ coordinate and check if
the sum of the heights of the southern and northern regions is larger than the
height of the farm. As there are each two regions in the north and south, we
always take the minimum between north-west/north-east and south-west/south-east.
If there is such an $x$ partition that leads to the sum being larger than the
height of the farm, then there is one solution with the current $x$ and $y$
partition.
...
vector<int> calc_max_height(int max_cows, const vector<pair<int, int>> &cows) {
	...
	for (int x = 0, i = 0; x < cows.size(); x++) {
		...
		/*
		 * if the maximum allowed number of cows in the region is exceeded, decrease
...
int main() {
	...
	while (l < h) {
		...
		for (int x = 0; x < N - 1; x++) {
			 ...
			 * larger than the height of the farm, then there is one solution with the
			 * current maximum M, i.e. mid
			 */
			if (min(west_south_height[x], east_south_height[N - 2 - x]) + 1 +
					min(west_north_height[x], east_north_height[N - 2 - x]) + 1 >= N) {
				possible = true;
				break;
			}
		}
