...
data structure. To check if a given $M^*$, the maximum number of cows in any of
the four regions, is feasible, we calculate the maximum height of all four
regions for each possible $x$ partition with maximum $M^*$ cows in the region.
We then go through every $x$ coordinate and check if the sum of the heights of
the southern and northern regions is larger than the height of the farm. As
there are each two regions in the north and south, we always take the minimum
between north-west/north-east and south-west/south-east. If there is such an $x$
partition that leads to the sum being larger than the height of the farm, then
there is one solution with the current $x$ and $y$ partition.
