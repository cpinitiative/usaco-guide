...
Let $\texttt{dp}[i][j]$ be the minimum residual area with a rectangular slab size of $i X j$. Initialize $\texttt{dp}[i][j]$ as $i \cdot j$ and set $\texttt{dp}[i][j]$ as 0, where $x_i, y_i$ are the width and heights of the desired plate sizes, respectively. Check every sub-rectangle with either $i$ or $j$ fixed (but not both) and store the residual area of the optimal sub-rectangle.
