...
Associate a subset $S_q\subset \{1,\ldots,12\}$ with each $q\in [1,N]$ such that if $B$ receives a pair $(q,h)$, $B$ should answer yes iff $h\in S_q$ (and no otherwise).

</Spoiler>

<Spoiler title="Hint 2">

It must be the case that $h\in S_x \setminus S_y$. In other words, no pair $(x,y)$ with $x\neq y$ exists such that $S_x$ contains $S_y$.
