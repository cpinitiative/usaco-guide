...
public class Billboard {
	public static void main(String[] args) throws IOException {
		...
		// Figure out how many corners are covered
		int numCoveredCorners = 0;
		if (isInRectangle(backBLX, backBLY, frontBLX, frontBLY, frontTRX, frontTRY)) numCoveredCorners++;
		if (isInRectangle(backTRX, backBLY, frontBLX, frontBLY, frontTRX, frontTRY)) numCoveredCorners++;
		if (isInRectangle(backTRX, backTRY, frontBLX, frontBLY, frontTRX, frontTRY)) numCoveredCorners++;
		if (isInRectangle(backBLX, backTRY, frontBLX, frontBLY, frontTRX, frontTRY)) numCoveredCorners++;

		if (numCoveredCorners <= 1) {
			// if 1 or 0 corners are covered, the tarp is simply just the area of the rectangle
			out.println((backTRX - backBLX) * (backTRY - backBLY));
		}  else if (numCoveredCorners == 2) {
			// if exactly two corners are covered, we need to figure out the exposed area.
			// calculate the area of the intersection (the covered area), and subtract it from the total area.
			...
			int totalAreaOfBackRectangle = (backTRX - backBLX) * (backTRY - backBLY);
			int areaOfIntersection = (intersectionTRX - intersectionBLX) * (intersectionTRY - intersectionBLY);

			out.println(totalAreaOfBackRectangle - areaOfIntersection);
		}  else {
			// all 4 corners are covered
			out.println(0);
		}
	...
	/**
	 * @returns whether or not (x, y) is in the rect bounded by bottom left corner (rectBLX, rectBLY) and top right corner (rectTRX, rectTRY)
	 */
	public static boolean isInRectangle(int x, int y, int rectBLX, int rectBLY, int rectTRX, int rectTRY) {
		return (x >= rectBLX && x <= rectTRX && y >= rectBLY && y <= rectTRY);
	}
