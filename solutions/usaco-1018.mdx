...
filein = open('help.in', 'r') 
n = int(filein.readline())

#If the segment is open the value will be +ve else -ve
a = [0] * (2 * n + 1)
for i in range(n):
...
open_segs = 0 #number of open segments
ans = 0
for i in range(1, 2 * n + 1):
	open_segs += a[i]
	if a[i] == 1:
		ans = (ans + pow[n - open_segs]) % mod

#writing the output to the file
fileout = open('help.out', 'w')
