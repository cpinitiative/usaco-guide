...
This leads us to the solution: for each cow, count the number of cows that can pass to her. Call this `reach[i]` for a cow `i`. Then the answer is the number of cows with $0$ cows passing to her (the number of `i` such that `reach[i]=0`). However, we forgot the case where there are exactly two adjacent cows passing to each other, and they each have exactly one cow passing to them. In that case, the number of cows passing to each of them is nonzero, but they need $1$ ball from Farmer John initially since they are secluded from the passings of the rest of the cows. To do this, we can keep an array `to` to keep track of which cow `to[i]` each cow `i` passes to. My (C++) code below takes $\mathcal{O}(n\log{n})$ time, but $\mathcal{O}(N^2)$ passes as well (the Python solution) . 
