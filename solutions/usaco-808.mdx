...
We can do this first by finding the number of times every cow passes a ball. 
	...
	int ballsNeeded = 0;
  	...
  	for (int i=0; i<N; i++) {
		...
		// This cow only passes to 1 other cow forever, so give it a ball
   		if (i < passToCow(i) && passToCow(passToCow(i)) == i && cowsPassingTo[i] == 1 && cowsPassingTo[passToCow(i)] == 1) {
			ballsNeeded++;
		}
  	}
