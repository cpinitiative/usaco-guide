...
Sets and Maps are introduced [here](https://usaco.guide/bronze/intro-sets?lang=py). Furthermore, using (an unordered set)[https://usaco.guide/gold/unordered?lang=py] will enable an $\mathcal{O}(n)$ complexity, although $\texttt{chash}$ is necessary to avoid a TLE verdict. 
## Implementation
