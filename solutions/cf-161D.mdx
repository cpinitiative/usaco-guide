...
2. There is a path from node $b$ to node $c$ (two nodes which are both in the subtree of $a$) that passes through node $a$.
The sum of the lengths of the paths from $b$ to $a$, and from $c$ to $a$, should be $K$.
	

If $N$ is the number of nodes and $K$ is the desired distance, we can define a DP array where $\texttt{DP[i][j]}$ is the number of nodes in the subtree of $i$ with a distance of $j$ from node $i$.

So we want to run DFS on the tree, and for each node, calculate the answer for case 1 and case 2, and then update the DP values by adding in the distances from the children.
