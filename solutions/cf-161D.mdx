...
If $N$ is the number of nodes, $K$ is the desired distance, we can define a DP matrix $\texttt{DP[N][K]}$ where $\texttt{DP[i][j]}$ is the number of nodes in the subtree of $i$ with a distance of $j$ from node $i$.
