...
signed main(){
    // uncomment before submitting to usaco
    ...
    //outgoing bool array, stores if we go out from this array or not,initialised with false
    vector<bool> outgoing(n,false);

    //take in input
    for(int i = 0;i<n-1;i++){
        int u,v;cin>>u>>v;
        //making nodes zero-indexed
        u--,v--;

        //we can go out from u
        outgoing[u] = true;
    }

    //if there is exactly one node which we cant get out of, we say thats the answer
    ...
    for(int i = 0;i<n;i++){
        ...
        if(!outgoing[i]){
            if(ans == -1){
                //first node to have 0 outgoing edges
                ans = i+1;//1-based so i + 1 
            }else{
                //second node to have 0 outgoing edges so, there are 2 sinks, so no answer
                cout<<-1;
                return 0;
            }
        }
