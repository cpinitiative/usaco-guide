...
struct Bound { int min_r, max_r, min_c, max_c; };
...
int main() {
	...
	int possFirst = width * width - visible.size();
	...
	if (visible.size() > 1) {
		...
		for (int r = 0; r < width; r++) {
			for (int c = 0; c < width; c++) {
				 ...
				 */
				if (rect_num[r][c] > 1) {
					valid_colors.erase(canvas[r][c]);
				}
			}
		}
		possFirst += valid_colors.size();
	}

	std::ofstream("art.out") << possFirst << endl;;
}
```
