...
Let's first give each child their own DSU data structure to store which cities are connected with their railroads,
...
runtime of about $\mathcal{O}(KM)$, which is too much given the bounds.

To find the first child that can accept a railroad faster, let's try using binary search!
...
Now that we've finally proved that we can use binary search to find the first child that can accept a railroad,
we can use it to reduce the factor of $M$ to $\log M$ instead, which makes the algorithm fast enough.
