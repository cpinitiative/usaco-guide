...
Otherwise, we can answer queries with the principle of inclusion-exclusion. The answer is simply $\sum\limits_{a\in A} super[B \text{ OR } a] (-1)^{|a|}$, which we can calculate in $O(2^{|A|})$

The answer is also $\sum\limits_{b\in B} sub[C \text{ OR } b] (-1)^{|B|-|b|}$, which we can calculate in $O(2^{|B|})$. They can be computed efficiently with submask enumeration.
```
