...
Let $dp[i][j][mask]$ be the number of ways to tile the grid so that:
...
For example, the following state would contribute toward $dp[4][2][10101_2]$:
![](<CSES Counting Tilings/state-example.PNG>)

We now have two cases when calculating $dp[i][j][mask]$:
- The $j$-th bit of mask is 0, meaning that cell $(i, j)$ is **covered**.
    - Case 1: we used a vertical tile to cover it.
        - Cell $(i - 1, j)$ must have been empty before this, so there are $dp[i - 1][j][mask \oplus 2^j]$ ways to do this.
    - Case 2: we used a horizontal tile to cover it.
        ...
        - Cell $(i, j - 1)$ must have been empty before this, while cells $(i - 1, j - 1)$ and $(i - 1, j)$ must have been covered,
          so there are $dp[i][j - 2][mask]$ to do this.
- The $j$-th bit of the mask is 1, meaning that cell $(i, j)$ is **empty**.
    - Cell $(i - 1, j)$ must have been covered before this, so there are $dp[i - 1][j][mask \oplus 2^j]$ ways to do this.
...
the first two dimensions of the DP array, as $dp[i][j]$ depends only on $dp[i][j - 1]$ and $dp[i][j - 2]$.
