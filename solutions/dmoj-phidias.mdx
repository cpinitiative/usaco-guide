...
pi a[max_N+1];
int dp[max_W+1][max_W+1]; //dp[i][j] = minimum waste for a rectangle with width i and height j 
int N,W,H;
...
int main(){
	...
	for(int i = 1; i <= W; ++i){
		for(int j = 1; j <= H; ++j){
			for(int k = 1; k <= N; ++k){
                //If the rectangle is not big enough
				if(i < a[k].f || j < a[k].s)continue;
                //Divide the larger rectangle into three parts as illustrated previously
