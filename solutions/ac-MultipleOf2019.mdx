...
For example, in the string $$1234$$ and the modulus of $$3$$, the suffix array would be
...
It also means that if both $${S_0}(mod$$ $$M)=$$ $${S_3}(mod$$ $$M)$$ (both suffixes have the same remainders), subtracting would lead to $${S_0}(mod$$ $$M)-$$ $${S_3}(mod$$ $$M)=0$$, meaning a substring that makes a number that is divisible by $$M$$ or $$2019$$. 

So with $$1234$$, $${S_0-{S_3}=1234(mod}$$ $$3)-4(mod$$ $$3)=1-1=0$$. Because they substract to zero, it means the substring from index $$0$$ to $$3$$ ($$3$$ is exclusive) leads to a number divisible by the modulus. (The substring is "$$123$$" which is divisble by 3). 

However, we would still need to go through every pair of possible suffixes (i, j). 

To take it one step further, **we do not need to know the specific indicies, we just need to know the amount of times a certain remainder occurs.** 

Thus, we can use $${N \choose K}$$ formula, and choose 2 numbers that are divisible by the same reminder of 2019, where $$N=$$ the amount of times a certain remainder occurs, and $$K=$$ 2 (because we are picking 2 indicies $$i$$ and $$j$$). 

We can then store all the counts of all the remainders in an array from $$0...2018$$. Then, we can add all $${N \choose 2}$$ and that is our answer. 

So in our example, our array would look like this

$$[2, 3, 0]$$

Where each index represents the count of a certain remainder. 
* There is $$1$$ suffixes with a remainder of $$0$$ when dividing by $$3$$, $$[234, 0]$$
* There is $$3$$ suffixes with a remainder of $$1$$ when dividing by 3, $$[1234, 34, 4]$$
* And there are no suffixes with a remainder of 2

So, the sum of all of these numbers would be $${2 \choose 2} + {3 \choose 2}=1+3=4$$ 

(If $$N<K$$ then, it should be $$0$$, not $$1$$ since there are not enough suffixes that can make a pair)
