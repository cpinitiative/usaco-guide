...
## Explanation
Firstly, in order to store sections of the string, we can make a suffix array with the condition that at index $$i$$ in suffix array $${S}$$
...
This is useful because in $\mathcal{O}(1)$ time, when we use a range query, we can capture any substring with just two indices $$i$$ and $$j$$. So if $${S_0} = 1234 $$ and $${S_3=4}$$, this means $${S_0}-{S_3} = 123 * {10^1}$$ or $$1230$$. 
...
[Errichto Explanation](https://www.youtube.com/watch?v=83yW2Pp6HMk)
## Implementation
