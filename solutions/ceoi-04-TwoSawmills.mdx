...
Because $x$ and $c$ are functions of $b$, we can hold them constant. Note that $y_a + m_a x$ forms a linear function of $x$. We can use [convex hull trick](https://usaco.guide/plat/convex-hull-trick?lang=cpp) to efficiently query the minimum value of a group of linear functions. The answer is simply $\max_{2 \leq b \leq N}g(b)$. 
...
**Time Complexity:** $\mathcal{O}(n)$ or $\mathcal{O}(n \log n)$ depending on the implementation of CHT. Because the following solution uses [LineContainer](https://usaco.guide/adv/line-container), it runs in $\mathcal{O}(n \log n)$.
