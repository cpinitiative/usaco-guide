...
int main() {
	...
	for (int i = 0; i < n; i++) { // iterating through n lines to input the data in
		int start; cin >> start; 
		int end; cin >> end;
		int amt; cin >> amt;
		event[start] += amt; 
		// simulating the events by adding/subtracting the amount from si and ti
		event[end] -= amt; // ^
	}
	...
	for (int i = 0; i <= maxtime; i++) {
		...
		res = max(res, cur); 
		// saving the curr in increasing order to get the 
		// highest value we can for the current bucket
	}
