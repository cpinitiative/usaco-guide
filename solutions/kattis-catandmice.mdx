...
public final class CatAndMice {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int m = 0; m < miceNum; m++) {
			mice[m] = Arrays.stream(read.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
		}
	...
	private static boolean allEatable(int[] start, int[][] mice, double speedRed, double initSpeed) {
		double[][] minTime = new double[1 << mice.length][mice.length];
		for (int s = 0; s < (1 << mice.length); s++) {
			Arrays.fill(minTime[s], Double.MAX_VALUE);  // MAX_VALUE will be used as a placeholder for invalid values
		}
		...
		for (int s = 1; s < (1 << mice.length); s++) {
			...
			for (int end = 0; end < mice.length; end++) {
				...
				if (prev == 0) {
					...
					minTime[s][end] = initSpeed * mice[end][2] >= dist ? dist / initSpeed : Double.MAX_VALUE;
					continue;
				}
