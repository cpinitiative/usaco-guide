...
public class ParsasHumongousTree {
	...
	private static void calculate_dp(int u, int par) {
		for (int v : adj.get(u)) {
			...
			dp[u][0] += mx_beauty(u, v, false);
			dp[u][1] += mx_beauty(u, v, true);
		}
	}

	// Return optimal beauty value for vertex u with subtree v.
	private static long mx_beauty(int u, int v, boolean type) {
		if (!type) {
