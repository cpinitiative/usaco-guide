...
segment tree (N.B. not a Fenwick tree, as the GCD function has no inverse).
...
Although BBSTs use 4 times less memory than segment trees, a BBST of BBSTs 
(e.g. a [range tree](https://en.wikipedia.org/wiki/Range_tree)) is rather
unpleasant to implement. However, a segment tree of BBSTs is much nicer to
implement, and is good enough to score 100 points!
...
Essentially, we don't instantiate nodes in the segment tree if they are not
leaf nodes and only contain a single leaf node in their subtree, as those nodes
are redundant. What we end up with is a segment tree with $N$ leaves and $2N - 1$
nodes. See
