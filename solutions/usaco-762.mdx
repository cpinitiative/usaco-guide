...
int main() {
        ...
        ll current_question, current_score, idx;
        for (int i = n - 2; i > 0; i--) {
                current_question = questions[i];

                total_score[i] = current_question + total_score[i + 1]; // Calculating sum of scores from index i to N
                minimum_score[i] = min(minimum_score[i + 1], current_question); //minimum score from scores in indexes i to N

                current_score = total_score[i] - minimum_score[i]; //excluding the least score question to calculate average
                idx = (n - (i + 1));

                if (current_score * right > left * idx) {
                        left = current_score;
                        right = idx;
                        max_score_values = {i};
                } else if (current_score * right == left * idx) {
