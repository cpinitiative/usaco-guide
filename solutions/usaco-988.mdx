...
title: Photoshoot
author: Ananth Kashyap, Ben Dodge
---

[Official Analysis (C++)](http://www.usaco.org/current/data/sol_photo_bronze_jan20.html)

We'll start by saying that for each $i$ in range $[1, N]$, $a_1 = i$. For all other values of $a_i$, we are given $b_i = a_i + a_{i+1}$ for $[1, N)$, or $a_i = b_{i -1} - a_{i - 1}$ for $[2, N]$. We will then check if these values of $a_i$ produce a valid permutation. 
...
Since the `in` operator in Python takes $\mathcal{O}(N)$ time, this solution is $\mathcal{O}(N^3)$.
</Info>
...
public class Photoshoot {
	public static void main(String[] args) throws IOException {
		...
		Set<Integer> isItThere = new HashSet<>();
