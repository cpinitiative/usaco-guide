...
We'll start by saying that for each $i$ in range $[1, N]$, $a_1 = i.$ For all other values of $a_i,$ we're given $b_i = a_i + a_{i+1}$ for $[1, N)$, or $a_i = b_{i -1} - a_{i - 1}$ for $[2, N]$. We'll then see if these values of $a_i$ produce a valid permutation. 
