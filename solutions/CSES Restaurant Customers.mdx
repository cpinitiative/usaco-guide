...
A naïve approach is to create an array $\texttt{ctr}$, where $\texttt{ctr}[i]$ is the number of intervals that cover each point $i$. We can do this by looping through each interval $[a,b]$ and increasing $\texttt{ctr}[i]$ by $1$ for each index in $a \leq i \leq b$.
...
In this problem, our $x$ is fixed at $1$. As a result, when we encounter a starting point, we can increment by $1$, and for an endpoint, decrement by $1$. We actually cannot compute the prefix sum array directly since $V \leq 10^9$, and we will run out of memory when creating an array of size $V$.
