...
title: Teleportation
author: Ananth Kashyap, Tanmoy Das, Thamognya Kodi, Rameez Parwez
---

[Official Analysis (C++)](http://www.usaco.org/current/data/sol_teleport_bronze_feb18.html)

## Explanation

We can consider three possibilities:

- Farmer John did not use any teleporter to transport the manure, so he travels a distance of $|a - b|$.
- Farmer John travels to point x from a, teleports the manure to y, then travels to b, for a distance of $|a - x| + |b - y|$.
- Farmer John travels to point y from a, teleports the manure to x, then travels to b, for a distance of $|a - y| + |b - x|$.

We can calculate the minimum distance he needs to haul the manure
