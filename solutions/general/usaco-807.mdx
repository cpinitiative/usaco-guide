...
```py
# This is second approach using python
# Take in data using Python file i/o system
file_in = open("teleport.in")
data = file_in.read().strip().split("\n")
start, end, open_teleport, close_teleport = map(int, data[0].split(" "))

# create a variable with 0 initialization
count = 0

# check if from the given number one is greater or not
if open_teleport > close_teleport:
# here the close_teleport is less than open_teleport so we substract close_teleport from start with absoulte value 
    count = abs(start - close_teleport)
# then add subtraction of open_teleport and end with absoulte value
    count += abs(open_teleport - end)
else:
# here the open_teleport is less than close_teleport so we substract open_teleport from start with absoulte value 
    count = abs(start - open_teleport)
# then add subtraction of open_teleport and end with absoulte value
    count += abs(end - close_teleport)

print(count, file=open("teleport.out", "w"))
```
