...
struct SuffixAuto {
	struct State {
		int len, link, pos;
		int next[26];
		State(int _len = 0, int _link = -1, int _pos = -1) : len(_len), link(_link), pos(_pos) {
...
int Q, ans[100005];	// answer for query
vector<pii> Z[200005]; // queries per state
vector<int> T[200005]; // suffix link tree
