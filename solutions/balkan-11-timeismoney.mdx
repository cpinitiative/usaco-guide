...
For the 40% $t = c$ cases, one can simply solve the problem by constructing a minimum spanning tree with e.g. $t$ as the weights for the edges. Let $T = \sum t$ and $C = \sum c$. $V = T \cdot C = T^2$ will be minimized when $T$ is minimized. 
...
/**
 * Calculate the signed area of the triangle ABP. This value is negative if point
