...
int main() {
	...
	for (int mask = 0; mask < 1 << right_arr.size(); mask++) {
		 ...
		 */
		best =
			max(best, *prev(left_sums.lower_bound(m - current_sum)) + current_sum);
	}
...
public class MaxSubseq {
	public static void main(String[] args) throws IOException {
		...
		for (int mask = 0; mask < 1 << rightArr.length; mask++) {
			...
			/*
			 * a possible new maximum value is the sum of currentSum and the largest
			 * value below m - currentSum from the combinations in leftArr
			 */
			best = Math.max(best,
				currentSum + leftSums.headSet(m - currentSum).last());
		}
