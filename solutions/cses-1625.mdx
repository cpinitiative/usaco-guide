...
public class GridPaths {
	...
	static boolean[][] onPath = new boolean[9][9]; //added border to all four sides so a 7x7 becomes a 9x9
	...
	public static int tryPath(int pathIdx, int curR, int curC) {
		...
		if (pathIdx == p.length) return 0; //visited all cells, but didn't end up in the correct locaiton
    ...
    // now search for dead ends (Optimization 4)
			...
			&& (!onPath[curR][curC - 1])) { // potential dead end on the left:
			int nxtR = curR;
			int nxtC = curC - 1;
			ret += tryPath(pathIdx + 1, nxtR, nxtC);
		}
			...
			&& (onPath[curR - 1][curC + 1] || onPath[curR + 1][curC + 1] )
			&& (!onPath[curR][curC + 1])) { // potential dead end on the right:
			int nxtR = curR;
			int nxtC = curC + 1;
			ret += tryPath(pathIdx + 1, nxtR, nxtC);
		}
		else if ((curR > 2) && onPath[curR - 2][curC] && onPath[curR - 1][curC - 1]
			&& (!onPath[curR - 1][curC])) { // potential dead end upwards
