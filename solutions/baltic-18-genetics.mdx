...
A naive method would be to iterate through each pair of clones and somehow calculate in near $\mathcal{O}(1)$ time
the number of positions at which their genomes differ.
This seems to be virtually impossible, so we should probably try a different method entirely.

What if for each clone, we somehow managed to check them against all the other clones at the same time?
...
we calculate the sum of the weights of these clones instead.
...
int main() {
	...
	for (int d = 0; d < dna_num; d++) {
		// for each difference, add the sum of th
