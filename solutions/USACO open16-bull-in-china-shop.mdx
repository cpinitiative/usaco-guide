...
The official solution doesn't check if any '#' fall outside the $N \times N$ grid, which is wrong.

</Warning>

As we iterate the pieces, we find the left, right, top and bottom sides of the piece and store them in the $\texttt{side}$ array. When we are shifting, we shift $\texttt{piece[i]}$ $\texttt{idy}$ units to the left and $\texttt{idx}$ units up, and shift $\texttt{piece[j]}$ $\texttt{jdy}$ units to the left and $\texttt{jdx}$ units up. This will send the piece at $(x + \texttt{idx}, y + \texttt{idy})$ to $(x, y)$ and the piece at $(x + \texttt{jdx}, y + \texttt{jdy})$ to $(x, y)$. So, we see that $\texttt{idy}$ must be bounded by $\texttt{right} - n$ and $\texttt{left}$, and $\texttt{idx}$ must be bounded by $\texttt{bottom} - n$ and $\texttt{top}$, with $\texttt{jdy}$ and $\texttt{jdx}$ bounded similarly. Because of that, every '#' will fall inside the $N \times N$ grid.
...
int main() 
{
	...
	for (int i = 1; i <= k; i++)
	{
		for (int j = i + 1; j <= k; j++)
		{
			for (int idx = side[i][3] - n; idx <= side[i][2]; idx++)
			{
				for (int idy = side[i][1] - n; idy <= side[i][0]; idy++)
				{
					for (int jdx = side[j][3] - n; jdx <= side[j][2]; jdx++)
					{
						for (int jdy = side[j][1] - n; jdy <= side[j][0]; jdy++)
						{
							bool good = true;
							for (int x = 0; x < n; x++)
							{
