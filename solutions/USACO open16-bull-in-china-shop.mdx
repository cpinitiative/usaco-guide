...
As we read in the pieces, we find the left, right, top and bottom sides of the piece and store this in the $\texttt{side}$ array. When we are shifting, we shift $\texttt{piece[i]}$ $\texttt{idy}$ units to the left and $\texttt{idx}$ units up, and shift $\texttt{piece[j]}$ $\texttt{jdy}$ units to the left and $\texttt{jdx}$ units up. This will send the piece at $(x + \texttt{idx}, y + \texttt{idy})$ to $(x, y)$ and the piece at $(x + \texttt{jdx}, y + \texttt{jdy})$ to $(x, y)$. So, we see that $\texttt{idy}$ must be bounded by $\texttt{right} - n$ and $\texttt{left}$, and $\texttt{idx}$ must be bounded by $\texttt{bottom} - n$ and $\texttt{top}$, with $\texttt{jdy}$ and $\texttt{jdx}$ bounded similarly.
