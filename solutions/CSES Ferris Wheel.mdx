...
Since each gondola can contain either 1 or 2 children, starting with the heaviest child, we can either
1. pair the heaviest child with the lightest child if possible; or
2. only include the heaviest child.

Alternatively, we can start with the lightest child. For each gondola, we pair the lightest child with
the heaviest child possible without exceeding the weight limit. Those that are left each get their own gondola. 

**Time Complexity:** $\mathcal{O}(n\log n)$ (from sorting)

This solution starts with the lightest child:
