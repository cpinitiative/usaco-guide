...
struct PersistentSegmentTree {
	...
	/*
	Updates the tree at a given position by incrementing the frequency of the specific number added. 
	It creates new nodes for every segment along the path from the root to the target position, 
	preserving the previous version of the tree while modifying only the necessary parts.
	*/ 
