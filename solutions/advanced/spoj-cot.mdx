	...
	*/
	int update(int v, int l, int r, int pos) {
		...
		if (l == r) {
			...
			node_value[new_node] = node_value[v] + 1; 
			return new_node;
		}
	...
	int query(int a, int b, int anc, int pr, int l, int r, int k) {
		...
		int left_subtree_count = node_value[left_child[a]] + node_value[left_child[b]] - node_value[left_child[anc]] - node_value[left_child[pr]];

		if (left_subtree_count >= k) {
			return query(left_child[a], left_child[b], left_child[anc], left_child[pr], l, m, k);
		} else {
			return query(right_child[a], right_child[b], right_child[and], right_child[pr], m + 1, r, k - left_subtree_count);
		}
	}
};
...
int main() {
	...
	for (int i = 0; i < m; i++) {
		...
		cout << compr[pst.query(g.roots[a], g.roots[b], g.roots[common], g.roots[pr], 1, MAX_NODES_GRAPH, k)] << '\n';
	}
}
```
