...
It takes the persistent segment tree of the parent node, adds the value of the current node by adding nodes to the persistent segment tree of the parent node, and keeps the root node of the new tree. This way we we don't need to build a completely new tree and can just modify the nodes that require a change
...
We subtract the frequencies of nodes from the root node to the LCA and its parent because we count the nodes twice from the root node to the LCA. In addition, since the LCA is on the path of u to v, we subtract the frequencies from the LCA and parents of the LCA to consider the value of the LCA.
