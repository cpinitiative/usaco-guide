...
After running the brute force solution, one can observe that the period of how matrix looks like is $\min(2^i, 2^i \geq n && 2^i \geq m)$.
...
Also, if $C(n, i)$ and $C(n, j)$ are odd, then $C(n, i \OR j)$ is odd as well, so we can reduce the problem to SOS DP, where $SOS[i]$ = xorsum of elements which are in positions included by $i$.

For each bit of the period, $SOS[i] = SOS[i - 2^position] \oplus SOS[i]$, if $i$ has the bit on position equal to $1$.

For other approaches, you can also check out the user solutions on [Infoarena](https://infoarena.ro/monitor?task=xortransform), just make sure to press that popup you receive after you first click on a solution.
