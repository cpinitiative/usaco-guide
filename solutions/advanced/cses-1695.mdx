...
public class PoliceChase { static final int N = 505; static int n, m; static
int[] lev = new int[N]; static int[][] c = new int[N][N]; static int[] seen =
new int[N]; static Queue<Integer> q = new LinkedList<>();

    // BFS to find level of each node
    static boolean bfs() {
    	Arrays.fill(lev, 1, n + 1, -1);
    	lev[1] = 0;
    	q.add(1);
    	while (!q.isEmpty()) {
    		int u = q.poll();
    ...
    public static void main(String[] args) {
    	...
    	while (bfs()) {
    		Arrays.fill(seen, 1, n + 1, 1);
    		while (dfs(1))
    			;
    	}

    	Arrays.fill(lev, 1, n + 1, 0);
    	dfs2(1);
