...
reach. Note that it may be the case that each source cannot be matched to a
...
To handle this, we can just connect them to the new graph by adding an edge.
...
int main(){
	...
	for(int sink: sinks){
		if(!vis[sink]){
			unmatched_sinks.push_back(sink);
		}
	}
	...
	for(int i = 0; i < matched_sources.size(); i++){
		ans.push_back({matched_sinks[i], matched_sources[(i + 1) % matched_sources.size()]});
	}
