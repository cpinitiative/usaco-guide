...
reach. The sink it matches with can be arbitrary but it should only travel through vertices that have not been visited yet (since we are returning as soon as we find a sink, it will be a straight path to it). Note that it may be the case that each source cannot be matched to a
