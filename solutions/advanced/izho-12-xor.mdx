...
Firstly, the original problem can be simplified sightly. In the question, we are asked to find the longest interval that satisfies this inequality.
$$
...
**Time Complexity:** $\mathcal{O}(N\log10^9)$
...
struct Trie {
	...
	int find_smallest_index(int x, int y, int bit = 30) { 
		...
		int x_bit = (x >> bit) & 1, y_bit = (y >> bit) & 1; // get bit-th bits of x and y

		for(int p_bit = 0; p_bit <= 1; p_bit++) {
			if (!children[p_bit]) continue; // not possible (no values like this)
