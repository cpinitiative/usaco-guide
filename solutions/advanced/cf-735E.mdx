...
[Official Editorial](https://codeforces.com/blog/entry/48659)

Solution described by editorial is terrible, but we can do better! This runs in
$\mathcal{O}(NK)$ time. (proof?)
 ...
 * also see https://github.com/ecnerwala/cp-book/blob/master/src/num/modnum.hpp
...
void dfs(int x, int y) {
	...
	trav(t, adj[x]) if (t != y) {
		...
		vmi YES, NO;
		F0R(i, sz(yes[x]))
		F0R(j, sz(yes[t])) ad(YES, min(i, j), yes[x][i] * yes[t][j]);
		F0R(i, sz(no[x]))
		F0R(j, sz(no[t])) ad(NO, max(i, j), no[x][i] * no[t][j]);
