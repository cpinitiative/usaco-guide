...
Then, by xoring all these stack sizes, we can tell whether we win or lose on this subtree.
...
int main() {
  ...
  auto dfs = [&] (auto self, int u, int p = 0) -> int {
    ...
    for (int v : g[u]) if (v ^ p) {
      // Adding 1 because the edge from u to v increases the size of each stack by 1
      sg ^= self(self, v, u) + 1;
    }
    return sg;
  };
