...
The brute force solution is to directly construct our string. We create 
...
However, as our string's length can grow beyond $2^{100000}$ (e.g. through doubling our string each time), directly constructing our tree is infeasible. The most problematic inefficiency 
...
the number of leaf nodes. To remedy this, we compress all chains, as chains do not affect 
our answer. Because each node in our tree has either $0$ or more than $1$ child, the number of total 
...
uses pointers, but using arrays and indices instead of pointers is also valid. We also process queries in reverse to make constructing the data structure easier.
