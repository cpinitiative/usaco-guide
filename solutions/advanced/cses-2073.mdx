...
2. **Lazy Reversal:** Physically reversing every node in `T2` would be too slow. Instead, we use a lazy boolean flag, `rev`. We simply toggle this flag at the root of `T2` in $\mathcal{O}(1)$ time. 
...
struct ImplicitTreap {
    ...
    void build(const string &s) {
        for (char c : s)
            root = merge(root, new Node(c, rng()));
    }
