...
For each edge of the convex polygon, this set of points at distance at least `d` from edge can be represented as a **half-plane** whose boundary line is at distance `d` from edge (*on its inward side*).
...
**Time Complexity:** $\mathcal{O}(N * \log N * \log(R / \varepsilon))$

where  
- $N$ is the number of vertices,  
- $R$ is the search range 
- $\varepsilon$ is the required precision (here, $\varepsilon = 10^{-12}$).
...
struct HalfPlane {
	...
	ld ang;
	HalfPlane() {}
	HalfPlane(Point a, Point b) : p(a), dir(b - a) { ang = atan2l(dir.y, dir.x); }
	bool outside(const Point &r) const { return dir.cross(r - p) < -EPS; }
};
...
vector<Point> halfPlaneIntersection(vector<HalfPlane> &h) {
	...
	while (dq.size() > 2 && dq[0].outside(intersect(dq.back(), dq[dq.size() - 2])))
		dq.pop_back();
