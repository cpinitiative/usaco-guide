...
int main() {
	...
	//creating a vector to store cow locations
	vector<int> a;
	for (int i=0; i<3; ++i) {
		int b; cin >> b;
		a.push_back(b);
	...
	//min number of moves can either be 0, 1, or 2. 0 only if they are already consecutive, 
	//1 if there is a difference of 2 b/w any two numbers, and 2 in all other cases.
	if (a[0] == a[2] - 2) {
		cout << 0 << endl;
