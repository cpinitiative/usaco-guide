...
vector<vector<bool>> visited; //this will be true if the corresponding location of the grid has been visited atleast once

//right bottom left top
int drow[4] = {0, 1, 0, -1};
int dcol[4] = {1, 0, -1, 0};
vector<pair<int, int>> wpts; //this will contain all the waypoint
...
void dfs(int row, int col, int mid)
{
        ...
        for (int i = 0; i <= 3; i++)
        {
                ...
                if ((newRow >= 0 && newRow < r && newCol >= 0 && newCol < c) && !visited[newRow][newCol] && (abs(grid[newRow][newCol] - grid[row][col]) <= mid))
...
bool check()
{
        for (auto i : wpts)
        {
                if (visited[i.first][i.second] == false) //checking if all the waypoints were visited in the last dfs, or not
                        return false;
        }
        return true;
}
...
int main()
{
        ...
        while (lo < hi)
        {
                ...
                if (check()) //checking if all the waypoints were visited or not
                {
                        hi = mid;
                }
                else
