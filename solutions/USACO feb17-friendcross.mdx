...
To count the number of unfriendly crossings, instead of keeping track of the count for each BIT node, we maintain a sorted set representing the cow breeds. For a breed $b$, we query the number of breeds in the range $[1, b-k-1]$ and $[b+k+1, n]$. We can use a [Order Statistic Tree
](https://usaco.guide/gold/PURS?lang=cpp#order-statistic-tree) in C++ to represent the cow breeds. Since each update takes $\log N$ operations, it would allocate $\log N$ nodes each update, thus resulting in $\mathcal O(N\log N)$ space and $\mathcal O(N\log^2 N)$ time.
