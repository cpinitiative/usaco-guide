...
int main() {
  ...
  for(int black_sheet = 0; black_sheet < 2; black_sheet++) {
    ...
    // since the the white sheet has to be either covered by a black sheet completely
    // or it has to be covered by two black sheets with an intersection line

    // lets check whether or not it would be possible for the seperator line to be vertical
    if(y3 <= y1 and y4 >= y2) {
      // the y axis bounds of the rectangle are completely engulfed by the black sheet
      // this is a problem!
      // now we need to update the bounds for our x axis
      // for the left bound of the x axis, check whether or not it is covered
      if(x1 >= x3 and x1 <= x4) {
        // now we can tell the left bound is engulfed, and that
        // it needs to be update to the right bound of our current black sheet
        x1 = x4;
      }
    ...
    // now apply similar logic for if the seperator line were to be horizontal
    if(x3 <= x1 and x4 >= x2) {
  ...
  if(x2 > x1 and y2 > y1) {
    // since it is not degenerate, any point inside our current rectangle works as a possible point
    cout << "YES\n";
  }
  else {
    // one of the sides of the rectangle is degenerate, so there is not visible point
    cout << "NO\n";
  }
