...
We can turn this into a graph problem, or more precisely, a tree. Let's add an edge from $u$ to $v$ if the maximum length of the subarray that starts at $v$ has to end at $u-1$, and so the next subarray must start at $u$. Remember the array is cyclic, so $u$ is not necessarily greater than $v$. This can be found using basic two pointers.

We can use binary lifting to find the start index of any next subarray in $\log(n)$ time. We can also track the number of elements between two subarrays.

Finally, we can binary search over the number of subarrays for the minimum. If we can use $k$ subarrays to cover the whole array, then we must also be able to use $k+1$ subarrays.
...
int main(){
	...
	/*
	 * two pointers to find the start index (l) of every
	 * next subarray (r)
	 */
	sum = 0;
