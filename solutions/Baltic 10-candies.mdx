...
title: Baltic OI 2010 Candies
author: Andi Qu
---

The official solution runs in $O(BN^2 \log N)$ time, but we can use bitsets to solve this problem in $O(BN^3 / 64)$ time.
...
We can do knapsack DP $N$ times (considering discarding each package) to find this package in $O(BN^3 / 64)$ time.
...
This knapsack DP runs in $O(BN^3 / 64)$ as well, which is fast enough for 100 points.
