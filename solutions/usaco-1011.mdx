	...
	for(int i = 0; i < n; i++) {
		for(int j = 0; j < n; j++) {
			for(int k = 0; k < n; k++) {
				...
				if(y[i] ==y [j] && x[i] == x[k]) {
					/** 
					 * Since we need to output two times the
					 * area and the formula for the area
					 * of a triangle is 1/2 * base * height, 
					 * we can omit the 1/2 because it anyways
					 * gets multiplied by 2, which cancels it out. 
					 * Finding the base is the difference
					 * between both x points and the height is 
					 * the difference between both y points;
					 * it is a right triangle because both 
					 * sides are parallel to the x and y axis,
					 * so no advanced math is required to find 
					 * the height.
					 */
