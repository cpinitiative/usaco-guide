...
Since extracting edge data is so difficult, we probably want to send the original graph plus some extra vertices. Now, the main problem becomes identifying the node numbers.

</Spoiler>

<Spoiler title="Hint 2">

$2^{10} \approx 1000$, so this suggests that we should encode bitwise data somehow.
...
In addition to the original graph, send 10 extra "bit-nodes", where bit-node $i$ and node $j$ are connected only if the $i$-th bit of $j$ is 1. If we can identify these bit-nodes and their numbers, then we are done.

Connect bit-node $i$ with bit-node $i + 1$ to form a "chain" so that we can order them if we can find them.
...
The degree of the first extra node will be $N + 1$ while the degree of the second extra node will be 1. This means that we can uniquely identify them and thus also the bit-nodes!

We still need to be able to deduce which end of the bit-node "chain" is bit-node 1 though. The key observation here is that bit-node 1 will always have a greater degree than bit-node 10. We can thus identify both the bit-nodes and their numbers.
