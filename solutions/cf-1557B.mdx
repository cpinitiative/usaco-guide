...
void solve() {
	...
	map<int, int> next; // Map from one value to the value after it in the sorted array
	sort(sarr, sarr + n);
	for (int i = 0; i < n - 1; i++) {
	...
	int ans = 1;
	for (int i = 1; i < n; i++) {
		// If the previous element in the unsorted array is not the same as the previous element in the sorted array, we need at least 1 more segment
...
public class MoamenSubarrays {
	public static void main (String[] args) throws IOException {
		...
		for (int t = 0; t < testNum; t++) {
			...
			TreeMap<Integer, Integer> next = new TreeMap<>(); // Map from one value to the value after it in the sorted array
			for (int i = 0; i < n - 1; i++) {
...
for _ in range(int(input())):
	...
	next = {} # Map (dict) from one value to the value after it in the sorted array
	for i in range(n - 1):
		next[sarr[i]] = sarr[i + 1]
