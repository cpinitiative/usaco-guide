...
We create a 2D array of booleans which states if $A$ has a ranking higher than $B$ in any point of time. We read every ranking, then for all $\frac{N(N-1)}{2}$ pairs we set $b[A][B]=\text{true}$.
