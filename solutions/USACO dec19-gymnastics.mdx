...
### C++ Implementation
We create a 2D array of booleans which states if A has a ranking higher than B in any point of time. We use $\mathcal{O}(K \cdot N^2)$ to create this array. Then we use $\mathcal{O}(N^2)$ to iterate through all pairs and check if they satisfy the condition. The overall time complexity is hence $\mathcal{O}(K \cdot N^2)$. 
