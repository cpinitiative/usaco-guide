...
We read every ranking, then for all $\frac{n(n+1)}{2}$ pairs we set $\texttt{b}[A][B]=\text{true}$.

Then we iterate though all pairs in $\mathcal{O}(N^2)$ time and check if they satisfy the condition. At least one of $\texttt{b}[A][B]$ and $\texttt{b}[B][A]$ must be true, and hence we only need to check if they are both true. If one of them is false, we increment our $\texttt{count}$ by 1.
