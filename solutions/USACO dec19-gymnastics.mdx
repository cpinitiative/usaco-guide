...
Then we iterate though all pairs in $\mathcal{O}(n^2)$ time and check if they satisfy the condition. At least one of $b[A][B]$ and $b[B][A]$ must be true, and hence we only need to check if they are both true. If they are not both true, we increment our $\texttt{count}$ by 1.
