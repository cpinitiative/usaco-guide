...
We can use the Amogus Trick] o represent this problem. Create a disjoint set union of $2n$ nodes where node $i$ represents the state where the operation is performed on $i$ and $i + n$ represents the state where operation is not performed on $i$.
...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = i + 1; j < n; j++) { // Only check each pair once and diagnals don't matter
      if (a[i][j] == a[j][i]) continue; // If they are the same we should not merge either way because it could affect later merges
      else if (a[i][j] > a[j][i]) {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = i; j < n; j++) {
      // If a[i][j] is part of a diagonal or the pair is equal (a[i][j] == a[j][i]), manually set it to be the same as the original array
