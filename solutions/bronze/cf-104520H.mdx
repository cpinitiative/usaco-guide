...
int main() {
	...
	for (int i = 0; i < len; i++) {
		subarr_num[i] = (long long)(i + 1) * (len - i);
	}

	vector<long long> actual_a(len);
	for (int i = 0; i < len; i++) {
		actual_a[i] = subarr_num[i] * a[i];
	}
...
public class Permutator {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < len; i++) {
			subarrNum[i] = (long) (i + 1) * (len - i);
		}

		long[] actualA = new long[len];
		for (int i = 0; i < len; i++) {
			actualA[i] = subarrNum[i] * a[i];
		}
...
subarr_num = [(i + 1) * (len - i) for i in range(len)]
