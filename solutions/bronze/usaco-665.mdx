...
We start by reading in the small grid and the scale factor.

We then generate a larger grid by repeating each character from the small grid both horizontally and vertically, so that every character becomes a block of identical characters in the final output.

We repeat each of the $N$ characters per row $K$ consecutive times, and repeat each of the $M$ rows in the grid $K$ consecutive times as well.
