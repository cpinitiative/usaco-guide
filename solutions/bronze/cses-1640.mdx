...
are less than or greater than the target sum, finding the two indices in $\mathcal{O}(N)$ time. However, we do have to sort the values in order to use two pointers, bringing our total complexity to $\mathcal{O}(N \log N)$.
