...
Let's represent each given pair as a 4-letter string (city prefix + state code) and use a `map` to track
frequencies. For each string we check if it's mirror exists or not and increase our count based on that.
