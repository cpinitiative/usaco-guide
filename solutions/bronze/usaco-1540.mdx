...
$M=0$ if and only if the string must already be square. For example, WCOCOWWCOCOW is WCOCOW repeated twice, requiring no splits.
...
It can be proven that $M \le 2$ is also always true, and this is because any two possible substrings (COW, OWC, WCO) will all have a substring of length 2 in common and one letter left over.

Because of this, you can keep one pointer at the start, and one in the center of the string. And for every $3$ length substring, take the common 2-letter substring of both pointers into operation 2, and the remainder goes to operation 1.
...
```cpp
#include <bits/stdc++.h>
#define NEWLINE (t == 0 ? "" : "\n") // this exists to avoid trailing newlines
