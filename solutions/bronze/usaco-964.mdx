...
We go through all possible substring lengths $k$ starting from $1$. For each length, we use a map to track how many times each substring of length $k$ appears. If all substrings of that length appear only once, we know we've found our answer and print $k$. This works because once we find a length where all substrings are unique, we don't need to check for longer lengths.
