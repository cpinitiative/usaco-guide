...
This specific case of n = 3 can also be solved in $ \mathcal{O}(1) $, by simply
considering the 2 possible cases.
...
void solve(void) {
	using point = pair<pair<int, int>, char>;
#define x first.first
#define y first.second
#define ch second

	auto print = [](const point a) {
		for (int i = 0; i < a.y; i++) {
	...
	// the logos can be rotated -> normalize to make the case work simpler
	...
	if (a.x == b.x && a.x == c.x) {
		if (a.y + b.y + c.y == a.x) {  // if they actually forms a square
			cout << a.x << "\n";
	...
	// put a as the logo with the largest x
	if (c.x > b.x) swap(b, c);
	if (b.x > a.x) swap(a, b);
