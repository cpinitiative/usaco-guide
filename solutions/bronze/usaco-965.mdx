...
For each permutation we then check if it's valid.
To ensure that the alphabetically earliest ordering is selected, though, we must iterate in lexicographical order.
