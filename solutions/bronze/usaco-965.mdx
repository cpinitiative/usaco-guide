...
author: Benjamin Qi, Kevin Sheng, Melody Yu, Ryan Chou, David Zhou
---
...
Since we have $8$ cows, we can build every single permutation of them, which comes out to $40320$ possible orderings. After that, we can loop through the $N$ milking constraints and see if a satisfactory permutation exists. To ensure that the alphabetically earliest ordering is selected, we can arrange our cows in alphabetical order. The permutations are thus processed in alphabetical order.
