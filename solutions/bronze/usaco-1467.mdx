		...
		if (a == 0 && b == 0) {
			...
			for (int x = 0; x < d; x++)
				for (int y = 0; y < d; y++) {
					...
					if (ny < d && nx < d &&
					    superimposed[ny][nx] != 'W') {  // otherwise it has been deleted
						leadsTo.insert({x, y});
			...
			for (int x = 0; x < d; x++)
				for (int y = 0; y < d; y++) {
					...
					if (nx < 0 || ny < 0 ||
					    superimposed[ny][nx] ==
					        'W') {  // Not possible that nothing led to us
						invalid = true;
						break;
					}

					if (image2.count(
					        {nx, ny})) {  // whoever leaded to us could not have been
						                  // image2, they were removed
						image2.erase({nx, ny});
					}
				}
