...
[Official Analysis (C++, Python)](https://usaco.org/current/data/sol_prob1_bronze_jan25.html)

## Explanation

### Case 1: $A = 0$ and $B = 0$
Since no stars are moving, they can only disappear in image 2.
- `B` means the star stayed across both images
- `G` means the star existed **only in the first image**, and then disappeared in the second image.
- `W` means in either picture there was no star.
To count how many stars were in image 2, we count how many `B`s and `G`s are present.
...
### Case 2: $A \ne 0$ or $B \ne 0$
When $A$ and $B$ are non-zero we can tell that star $(i, j)$ moved to star $(i+A, j+B)$. So for every star (top-left to bottom-right) $(i, j)$
if a $(i+A, j+B)$ is present, we can tell that $(i+A, j+B)$ was shifted and belongs to image 2.
...
For example, when $A = 1$ and $B = 2$, we know the star at $(1, 1)$ moved to $(2, 3)$.
<center>
<table className="text-center">
	<tbody>
		<tr>
...
**Time Complexity:** $\mathcal{O}(T\cdot D^2)$
...
int main() {
	...
	while (t--) {
		...
		if (a == 0 && b == 0) {
			...
			vector<vector<bool>> image2(1000, vector<bool>(1000));
			...
			for (int x = 0; x < d; x++)
				for (int y = 0; y < d; y++) {
					...
					if (ny < d && nx < d && superimposed[ny][nx] != 'W') {
						// do not ignore B cells
			...
			for (int x = 0; x < d; x++)
				for (int y = 0; y < d; y++) {
					...
					// Not possible that nothing led to this
					if (nx < 0 || ny < 0 || superimposed[ny][nx] == 'W') {
						invalid = true;
						break;
					}

					// whoever leaded to us could not have been image2, they were
					// removed
					if (image2[nx][ny]) { image2[nx][ny] = false; }
				}

			// Count how many stars in image 2
			int image2numstars = 0;
			for (int x = 0; x < d; x++)
				for (int y = 0; y < d; y++) { image2numstars += image2[x][y]; }
