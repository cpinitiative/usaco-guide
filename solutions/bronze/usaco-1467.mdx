...
### Case 2: $A \ne 0$ or $B \ne 0$
> This will begin with some simple logic and extend later (for `B` stars).

When $A$ and $B$ are non-zero we can tell that star $(i, j)$ moved to star $(i+A, j+B)$.

So for every star (top-left to bottom-right) $(i, j)$ if a $(i+A, j+B)$ is present, we can tell that $(i+A, j+B)$ was shifted and belongs to image 2.
		...
		if (a == 0 && b == 0) {
			...
			for (int x = 0; x < n; x++)
				for (int y = 0; y < n; y++) {
                    ...
                    // whoever leaded to us could not have been image2, they were removed
					if (image2[nx][ny]) {
						image2[nx][ny] = false;
					}
				}
