...
The answer to the previous hint is that the possible pairings are every 
combination of a letter from A to Z with every other letter. To check 
if the pairing is a win, review every possible row, column, or diagonal,
and check if it only contains the two letters. 
