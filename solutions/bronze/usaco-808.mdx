...
Suppose that instead of cow $i$ passing to the nearest cow, cow $i$ passes to
cow $p_i$ where the array $a_1,a_2,\dots,a_N$ is given as input ($1\le a_i\le N$).
In other words, the passing graph can be any [functional graph](/silver/func-graphs).

Solve the problem in $\mathcal{O}(N)$ time.
