...
We start with our list of $N$ heights and make a sorted copy of it. Then we compare the original list to the sorted list and let $K$ be the number of positions where they differ.

Because removing exactly one cow must make the list sorted, those $K$ out-of-place cows form an almost-sorted sequence, so it takes exactly $K-1$ moves to put everyone back in order (and if $K\le1$, we need zero moves).
