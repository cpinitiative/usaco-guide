...
Let's try to find a case where can't form a proper evolutionary tree instead!
</Spoiler>

<Spoiler title="Hint 2">
Find a minimal case that forces us to make an improper evolutionary tree.
</Spoiler>

<Spoiler title="Hint 2 (Answer)">
This would happen if we had a subpopulation with trait $A$, one with $B$, and one with $A$ and $B$.

This doesn't work because this forces the cow with $A$ and the cow with $B$ to be in different subtrees, and so the cow with $A$ and $B$ would have to develop $A$ or $B$ randomly (depending on which subtree it stems from).
</Spoiler>
