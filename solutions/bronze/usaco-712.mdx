...
For every cow $i$, we check which cows have one entry/exit point between cow $i$'s entry and exit point.
Then, we can add each of these cows as a pair with cows $N$ to a list of crossings, making sure we have no repeats.
Then we can just output the length of this list of crossings to get our final answer.
	...
	for (int i = 0; i < crossings.size(); i++) {
		...
		if (first_found[c_id] == -1) {
			...
			for (int c : appearances) {
				...
				// Sort to make sure only {a, b} is stored, not {b, a} as well
