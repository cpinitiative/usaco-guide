...
**Time Complexity:** $\mathcal{O}(N^4)$, where $N$ is the number of cows FJ has (in this problem it's 26).
...
Each cow appears exactly twice. Notice that for, say cows A and B to intersect, the pattern that shows up by excluding every other cow must show up as ABAB or BABA. So we can count how many pairs of cows’ crossings **intersect** (interleaved) by taking the entrance and exit data of two cows every time and checking them against the intersection pattern.

**Approach:**

1. Iterate over all pairs of cows (26 letters $\rightarrow$ 26 choose 2).
2. Extract only the crossings of these two cows.
3. Check if the string matches:
   - `ABAB`
   - `BABA`
4. Count each intersecting pair.

**Time Complexity:** $\mathcal{O}(N^2)$, where $N$ is the length of FJ's string.
...
int main() {
	...
	for (int i = 0; i < 26; ++i) {
		for (int j = i + 1; j < 26; ++j) {
			...
			// the two cows to choose
			char cow1 = a[i];
			char cow2 = a[j];

			// making a new string filtering out al other cows other than the two
			// desired cows
			copy_if(crossings.begin(), crossings.end(), back_inserter(result),
			        [cow1, cow2](char c) { return c == cow1 || c == cow2; });

			// concatenating the two cows into the two strings that feature the two
			// cases
			stringstream ss;
			ss << cow1 << cow2 << cow1 << cow2;
			string case1 = ss.str();
			ss.str("");
			ss.clear();
			ss << cow2 << cow1 << cow2 << cow1;
			string case2 = ss.str();
			ss.str("");
			ss.clear();

			if (result == case1 || result == case2) { ++intersection; }
		}
	}
