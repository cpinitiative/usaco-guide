...
Take any two animals, and suppose they share $X$ characteristics with each other.
To determine which animal is which, at least $X + 1$ questions must be asked to
guarantee a differing characteristic, which in turn results in identifying the exact animal.
With this, we can compare every pair of animals and keep track of the max $X + 1$ value
in order to determine the number of questions that need to be asked. By using a
[set](https://usaco.guide/bronze/intro-sets?lang=cpp), we can speed up checking overlapping characteristics.
