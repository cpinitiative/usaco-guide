...
We first read $K$ and $N$ from the input and load all of the session rankings into a two-dimensional array. As the problem statement says, for each pair of cows $(A,B)$, we tally how many sessions cow A finished ahead of cow B. If cow A outperforms cow B in all $K$ sessions, we increment our counter. Once we've examined every ordered pair $(A,B)$, we output the final counter value.
