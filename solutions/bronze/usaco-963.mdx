...
author: Krish Thawani, Hua Zhi Vee, Jonathan Paulson, Ben Dodge, David Li
---
...
for i in range(K): #looping over trials
  for j in range(N): #rankings[i][j] = cow a
    for k in range(j+1,N): #rankings[i][k] = cow b
      better[rankings[i][j]][rankings[i][k]]+=1
