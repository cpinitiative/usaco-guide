...
the subscription fee will be k + 1 as she still needs to subscribe it for one day, and it is easier to calculate the date as we need to add 1 after subtracting two dates to find out the number of dates passed. We start the for loop from 2 to find the value of the array at the current index and previous index. 
