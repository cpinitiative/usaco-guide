...
This question is solved through greedy algorithm. To start with setting variables, as the input varies from 1 to 10^14, it is better to use long long rather than integer types. Variable s is used to store the previous index in the source code.
...
the subscription fee will be k + 1 as she still needs to subscribe it for one day even if the for loop does not operate, and it is easier to calculate the date as we need to add 1 after subtracting two dates to find out the number of dates passed.
...
#include <bits/stdc++.h>
using namespace std;
long long arr[100000];  // long long type should be used as the input can exceed the
                   // range of integer.
int main() {
	long long s = 1;
	long long n = 0;
