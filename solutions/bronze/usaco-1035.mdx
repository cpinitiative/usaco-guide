...
- We need to find the two most optimal places to put cows, to maximize the minimum distance between two occupied stalls $D$.
...
vector<pair<int, int>> get_zeros(string stalls) {
	vector<pair<int, int>>
	    zeros;  // [(start, end)], we can then sort to maximize end-start

	int zeros_start = -1;
	for (int i = 0; i < stalls.size(); i++) {
		if (stalls[i] == '0' &&
		    zeros_start == -1) {  // entered a '0' and not already counting
			// start counting
			zeros_start = i;
		}
		if ((stalls[i] == '1' && zeros_start != -1) ||
		    i == stalls.size() - 1) {  // hit a '1' & we have counted some zeros, or we
