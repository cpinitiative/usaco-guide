...
vector<pair<int, int>> getZeros(string stalls){
    vector<pair<int, int>> zeros; // [(start, end)], we can then sort to maximize end-start

    int zerosStart = -1;
    for (int i = 0; i < stalls.size(); i++){
        if (stalls[i] == '0' && zerosStart == -1){ // entered a '0' and not already counting
            // start counting
            zerosStart = i;
        }
        if ((stalls[i] == '1' && zerosStart != -1) || i == stalls.size()-1){ // hit a '1' & we have counted some zeros, or we have reached the end
            // insert the zero sequence
            zeros.push_back({zerosStart, i});
            zerosStart = -1;
        }
    }
