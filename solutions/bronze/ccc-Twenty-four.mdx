			...
			for (int op1 = 0; op1 < 4; op1++) {
				for (int op2 = 0; op2 < 4; op2++) {
					for (int op3 = 0; op3 < 4; op3++) {
						...
						int second = operation(op2, first, 
							handPermutation.get(2));
						if (second == -INF) {
							continue;
						}

						int third = operation(op3, second, 
	...
	if len(hand_permutation) == 4:
		...
		for op1 in range(4):
			for op2 in range(4):
				for op3 in range(4):
					first = operation(
						op1, hand_permutation[0], hand_permutation[1])
					# If the operation is invalid, continue;
					if first == -INF:
						continue
		...
		for op1 in range(4):
			for op2 in range(4):
				for op3 in range(4):
					...
					second = operation(
						op2, hand_permutation[2], hand_permutation[3])
					if second == -INF:
						continue
