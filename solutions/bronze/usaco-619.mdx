...
Notice that whatever even north-south partion that we place between those two cows, they will always be in different regions.
...
So, extending this logic to all of the cows, we can test each north-south partition between cows where there is at max $N$ possibilities.
For each north-south parition, we can split them into two lists, west and east sorted by their y-coordinates.

We can then loop through all the possible partitions keeping track of how many cows are to the east and west,
