...
Let $X$ be the minimum number of times we must repeat the process for this to happen. To find $X$, we can keep track of the ordering of the cows, and if we ever encounter a previously stored state, we use the number of stored orderings to determine X.

Now we only have to consider the remainder when $K$ is divided by $X$ rather than $K$ itself.
It can be verified by exhaustive search that the maximum possible value of $X$ for $N \leq 100$ is $29640$. Thus, the bounds are small enough so that we can simulate the process $K \bmod X$ times and still pass the problem in time.
