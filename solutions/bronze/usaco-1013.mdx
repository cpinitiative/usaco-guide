...
# create initial order of cows
cows = list(range(1, N + 1))

# simulate two-step process K times
for _ in range(K):
	# reverse sequence of cows between A1 and A2
	i, j = A1 - 1, A2 - 1
	while i < j:
	...
	# reverse sequence of cows between B1 and B2
	i, j = B1 - 1, B2 - 1
	while i < j:
		cows[i], cows[j] = cows[j], cows[i]
		i += 1
