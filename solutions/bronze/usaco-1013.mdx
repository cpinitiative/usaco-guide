...
We notice that after repeating the process a certain number of times, the cows all end up right back where they started. Let $X$ be the minimum number of times we must repeat the process for this to happen.

Now, we just have to consider the remainder when $K$ is divided by $X$ rather than $K$ itself. Since the remainder is always less than N, we can just simulate the process $K \bmod X$ times and still pass the problem in time.
