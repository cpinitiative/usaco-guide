...
title: Swapity Swap
author: Benjamin Qi, David Li, Kevin Sheng, David Guo
---
...
For full credit, we need to do better than just simulating the $K$ processes one after another.

We notice that after repeating the process a certain number of times, the cows all end up right back where they started.
Note that each cow can only be swapped to a finite number of new positions after each process, so it must return to back to its original position for arbitrarily large $N$. This gives us the idea to simulate the swapping process until all cows return to their original positions.

Let $X$ be the minimum number of times we must repeat the process for this to happen. To find $X$, we can keep track of the ordering of the cows, and if we ever encounter a previously stored ordering, we use the number of stored orderings to determine $X$.

Now we only have to consider the remainder when $K$ is divided by $X$ rather than $K$ itself.
It can be verified by exhaustive search that the maximum possible value of $X$ for $N \leq 100$ is $29640$. Thus, the bounds are small enough so that we can just simulate the process $K \bmod X$ times and still pass the problem in time.
