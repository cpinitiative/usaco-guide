...
Thus, we can start at all operations where $l = r$, then gradually expand the operations by subtracting 1 from $l$ and adding 1 to $r$ until the operation is no longer valid, and calculate how many contributions are added by the endpoints each time we expand our operation. This only calculates all odd-length operations, so we do the same starting from $l = r - 1$, to calculate all even-length operations.
...
This can be checked with a few simple if statements. 
