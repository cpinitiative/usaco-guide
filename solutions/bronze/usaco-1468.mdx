...
Consider the array $[4, 2, 3, 3, 6, 3, 3]$. With the first pair of threes, we can form the moos $[4, 3, 3]$ and $[2, 3, 3]$. Then, with the last pair of threes, we can form $[6, 3, 3]$. However, notice that with the last pair, we can also form $[4, 3, 3]$ and $[2, 3, 3]$. This leads to the key observation: we can make all possible moos with the last two occurrences alone.
...
int main() {
	...
	for (int num = 1; num <= n; num++) {
		if ((int)frequency[num].size() >= 2) {
			...
			if ((int)frequency[num].size() >= 3) { ans -= 1; }
		}
	}
