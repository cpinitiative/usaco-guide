...
We need to loop over all possible sets of 3 distinct positions. Then, we can loop through all the infected cows, and store the gene patterns of those three positions at a distinct integer, where we multiply the first, second, and third digit by 16, 4, and 1 respectively, and A represents 0, C represents 1, T represents 2, and G represents 3. 

We then loop through the non-spotted cows, and if any of those combinations of genes are shown in a non-spotted cow, then that set of positions cannot describe a valid set. Otherwise, we can add one to our answer.
