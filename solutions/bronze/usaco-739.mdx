...
author: Brad Ma, Kevin Sheng, Jay Fu, Arnav Gokhale
---
...
We need to loop over all possible sets of 3 distinct positions. Then, we can loop through all the infected cows, and store the gene patterns of those three positions as a integer, where we multiply the first, second, and third digit by 16, 4, and 1 respectively, and A represents 0, C represents 1, T represents 2, and G represents 3.

We then loop through the non-spotted cows, and create the integers the same way as above. If any integer appears in both non-spotted and spotted, then the current set of positions cannot describe a valid set. Otherwise, we can add one to our answer.
...
public class BovineGenomics {
	static final Map<Character, Integer> GENOME_ID = Map.ofEntries(
	    Map.entry('A', 0), Map.entry('T', 1), Map.entry('C', 2), Map.entry('G', 3));
