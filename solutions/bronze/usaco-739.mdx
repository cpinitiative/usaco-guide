...
[Official Analysis (C++)](http://www.usaco.org/current/data/sol_cownomics_silver_open17.html)
...
The way we do this is by interpreting the pattern as a base-4 integer.
If we map A to 0, T to 1, C to 2, and G to 3, the gene pattern CTG would be
the base-4 number $213$ (or $32 + 4 + 3 = 39$ in base 10).

The benefit of this approach is that it allows us to quickly
check if two cows have the same pattern without doing a direct string comparison.

We then loop through the non-spotted cows, and create the integers the same way as above.
