  ...
  for(int i = 0; i < n; i++){
    for(int k: adj[i]){
      ...
      if(colours[k] == colours[i]){
        ...
        // check for if this is change impacts the colourings of the previous pastures
        // since the solution was valid before the current node i and j is not equal to i (since j < i)
        // g must then be equal to k and so we increment k, since k > i > j
        for(int j = 0; j < i; j++){
          for(int g: adj[j]){
