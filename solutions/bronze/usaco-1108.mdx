	...
	for (int i = 0; i < n; i++) {
		...
		for (auto &cow : std::array<std::pair<int, int>, 5>{
			...
			if (cow.first >= 0 && cow.second >= 0) {
				bool is_comfortable = check_comfortable(cow.first, cow.second);

				if (is_comfortable && !comfortable[cow.first][cow.second]) {
					comfortable[cow.first][cow.second] = 1;
					res++;
				} else if (!is_comfortable && comfortable[cow.first][cow.second]) {
					comfortable[cow.first][cow.second] = 0;
					res--;
				}
			}
		}
