...
At each step, check where Farmer John starts and where he's trying to go. If his path doesn't overlap with
Bessie's path, we simply need to add the distance covered by Farmer John to a running total and move him to his starting position.
But if his path somehow overlaps with Bessie's, we only need to add the distance from his current location to Bessi's position to the total.
	...
	while (true) {
		...
		if ((direction == 1 && x <= y && y <= (x + direction_distance)) ||
			(direction == -1 && y <= x && (x - direction_distance) <= y)) {
				total_distance += std::abs(y - x); // distance to bessie from original position
