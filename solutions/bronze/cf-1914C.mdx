...
public class Quests {
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 0; t < testNum; t++) {
            ...
            int[] a = Arrays.stream(read.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
            int[] b = Arrays.stream(read.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
            ...
            for (int z = 1; z < Math.min(n, k); z++) {
                ...
                // compare experience point of completing one more quest or not
                max_score = Math.max(max_score, first_total + a[z] + best_second * (k - z - 1));
                first_total += a[z];
            }
