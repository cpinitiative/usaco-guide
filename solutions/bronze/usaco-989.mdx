...
Let's say in worst case Bessie ran a total of $10^9$ meters and her ending pace is $1 m/s$. In which it will take her $63,245$ seconds.

#### Proof

First, we accelerate her speed to $p$, then decelerate it back to $1$, total distance covered will be:
$$

\sum_{i = 1}^p + \sum_{i = 1}^{p - 1} = \frac{p * (p + 1)}{2} + \frac{p * (p - 1)}{2} = p^2

$$

So, to cover $10^9$ meters:

$$

p^2 = 10^9 \Rightarrow p = \sqrt{10^9} \approx 31,623

$$

Total time $= 31,623(accelerating) + 31,623(decelerating) = 63,245$

For $1000$ queries, it would be sufficient to pass all test cases. However, due to Python's poor constant factor, we have to optimize this solution.
