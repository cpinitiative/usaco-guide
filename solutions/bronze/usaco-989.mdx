...
What's the best strategy that Bessie could use if, hypothetically, there wasn't
a speed limit at the finish line?
...
We'll have Bessie continuously speed up until she's forced to slow down because
of the speed limit. This strategy works because we've observed that when
possible, Bessie should always speed up!
...
for X in Xs:
    ...
    while right != left:
        if right > X:
            remaining_distance = K - ((right + 1) * right / 2 + (right + X - 1) * (right - X) / 2)
            if remaining_distance < 0:
                right = (right - left) // 2 + left
                continue
            time = -(-remaining_distance // right) + right + (right - X)  # Funny way to do ceiling
        else:
            remaining_distance = K - (right + 1) * right / 2
            if remaining_distance < 0:
                right = (right - left) // 2 + left
                continue
            time = -(-remaining_distance // right) + right
        best_time = min(best_time, time)
        left = right
        right = 2 * right
    output.append(int(best_time))

with open(r"race.out", mode="w") as fout:
    for time in output:
        fout.write(str(time) + "\n")
