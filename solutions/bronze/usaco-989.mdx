...
We are asked to calculate the minimum time required for Bessie to run a distance of $K$ meters, while ensuring her final speed does not exceeds $X$ m/s.
To achieve this, we'll have Bessie accelerate as much as possible until she reaches a certain top speed, then decelerate, so that her speed never exceeds
$X$ m/s, and while doing so she also spends exactly 1 second at each speed level during both acceleration and deceleration phases.
...
def fastest_time(dist: int, max_speed: int) -> int:
	speed_up_dist = 0  # Amount of distance where Bessie's speeding up
	slow_down_dist = 0  # and slowing down respectively
	time = 0
...
read = open("race.in", "r").readline
write = open("race.out", "w").write

dist, query_num = map(int, read().split())

for _ in range(query_num):
	max_speed = int(read())
	write(f"{fastest_time(dist, max_speed)}\n")
```
