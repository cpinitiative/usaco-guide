...
Let's consider the positions of our cows as $x_1$, $x_2$, and $x_3$, where $x_1 < x_2 < x_3$.

### Minimum

If the three cows are already all next to each other the answer is $0$.

If two of the cows have a gap of one between then, then the answer must be one, because the last cow will move inside the gap (e.g. 3, 5, and 9, where 9 moves to 4).

However, in all other cases, the minimum is 2.
This is because we can always move $x_1$ to $x_2 + 2$ and $x_3$ to $x_2 + 1$ in two operations.

### Maximum

We will have 2 gaps, $x_1$ to $x_2$ and $x_2$ to $x_3$.

After the first operation, one of these will disappear.
The way to maximize the number of moves it to keep two adjacent cows at the endpoint,
and keep on flipping those two cows until all three cows are grouped in consecutive locations.

We can move one of the endpoints to the other, and keep flipping them, to get the maximum number of moves.
With that, the answer ends up being $max(x_2 - x_1, x_3 - x_2) - 1$.
