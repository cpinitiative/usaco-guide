...
<Spoiler title="Hint 1 (Minimum)">

There are only a few possible answers. How many moves does it take at most to group the cows in consecutive locations?

</Spoiler>

<Spoiler title="Hint 2 (Minimum)">

It takes at most 2 moves.

</Spoiler>

<Spoiler title="Hint 1 (Maximum)">

Each move reduces the gap between the two endpoints by a certain amount. In order to maximize the number of steps, we need to minimize the amount of gaps reduced after each move. 

</Spoiler>

<Spoiler title="Hint 2 (Maximum)">

When there are two consecutive cows, what is the optimal way to move the cow that minimizes the amount of gaps reduced?

</Spoiler>

<Spoiler title="Hint 3 (Maximum)">

Let's say the cows are at location a $<$ b $<$ c where b and c are next to each other, we can move c one position to the left of b. This will only reduce the gap by 1.
</Spoiler>
