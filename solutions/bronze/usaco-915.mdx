...
## Explanation
Lets consider the positions of our cows as $x_1$, $x_2$, and $x_3$, where $x_1 < x_2 < x_3$.

Lets first consider the minimum. Obviously, if the three cows are already all next to each other the answer is 0 (e.g. 1, 2, 3). If two of the cows have a gap of one between then, then the answer must be one, because the last cow will just move inside the gap (e.g. 3, 5, and 9, where 9 moves to 4). However, in all other cases, the minimum is 2. This is because we can always move $x_1$ to $x_2 + 2$ and $x_3$ to $x_2 + 1$ in two operations.

Now lets consider the maximum. We will have 2 gaps, $x_1$ to $x_2$ and $x_2$ to $x_3$. After the first operation, one of these will disappear. The way to maximize the number of moves it to keep two adjacent cows at the endpoint, and keep on flipping those two cows until all three cows are grouped in consecutive locations. We can move one of the endpoints to the other, and keep flipping them, to get the maximum number of moves. So the answer just ends up being $max(x_2 - x_1, x_3 - x_2) - 1$.
...
public class Main {
	public static void main(String[] args) throws IOException {
		...
		int[] cows = new int[3];
		for (int i = 0; i < 3; i++) { cows[i] = sc.nextInt(); }
		Arrays.sort(cows);
		...
		// Max number of moves
		pw.println(Math.max(x2 - x1, x3 - x2) - 1);
...
if x3 == x1 + 2:
	...
	print(1)
	"""
	It can always be solved in two moves by moving x1 -> x3 - 2 and x2 -> x3 - 1.
