...
author: Sathvik Chundru, Ryan Chou, Vikas Thoutam
---
...
int main() {
	...
	int temp;
	if(cow1>cow3) {
		temp = cow3;  cow3 = cow1;  cow1 = temp;
	}
	if(cow2>cow3) {
		temp = cow3;  cow3 = cow2;  cow2 = temp;
	}
	if(cow1>cow2) {
		temp = cow2;  cow2 = cow1;  cow2 = temp;
	}

	// Compute the minimum number of moves, always 0, 1, or 2
	int min;
	if(cow1 + 1 == cow2 && cow2 + 1 == cow3) // In case they are already next to each other
		min = 0;
	else if(cow1 + 2 == cow2 || cow2 + 2 == cow3) // In case two of the cows are one apart, move the other cow in between
		min = 1;
	else // All other cases, move one cow to be in the else if case, then add one to satisfy the else if case
		min = 2;
