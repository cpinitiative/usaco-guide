...
[Official Analysis](https://usaco.org/current/data/sol_prob1_bronze_open23.html)
...
$0, 2, 4, \dots n-1$. If $n$ is even, our solutions are $1, 3, 5, \dots n-1$.
...
Now, let's return to the original problem (what are all possible answers). If I wanted to construct an answer of $2$, is this possible? Let's try with BFFB: It doesn't seem like it! To go from an answer of $1$ (BEBEBB) to an answer of $2$, we have to flip an E to become a B. But doing so actually adds two to our answer (so we can get $3$), not one. So, by starting with BEBEBB (the string we used to get an answer of $1$), we can repeatedly change an E to become a B to add two to our answer. Therefore, we can have an answer of $1, 3, 5, \dots n-1$.
...
- `BF...FE`: If the length is $n$ and $n$ is odd, our min excitement level is $1$ and our max is $n-2$. If $n$ is even, our min is $0$ and our max is $n-2$. Let $x$ be our min and $y$ be our max. We can achieve any excitement level $x, x+2, x+4, \dots y-2, y$.
...
- `F...FB`: If the string length is $n$, we can get any value $0, 1, 2, 3, \dots n-1$.
...
- Only F's: If length $n$, answer is $0, 1, \dots n-1$.
...
Let $x$ be the min and $y$ be the max. We can achieve any value $x, x+2, x+4, \dots y-2, y$.
...
We can add the min / max values from every section together to get the min / max values for the whole string. If we let $x$ be the min for the whole string and $y$ be the max for the whole string, our possible answers are $x, x+2, x+4, \dots, y-2, y$.
