...
public class FEB {
	public static void main(String[] args) {
		...
		while (curIdx < n) {
			...
			while (nextIdx < n && s.charAt(nextIdx) == 'F') {
				nextIdx++;
			}
			if (nextIdx == n) {
				break;
		...
		// Special case: beginning / ending F's
		int numBeginningF = 0;
		while (numBeginningF < n && s.charAt(numBeginningF) == 'F') {
			numBeginningF++;
		}

		int numEndingF = 0;
		while (numEndingF < n && s.charAt(n - 1 - numEndingF) == 'F') {
			numEndingF++;
		}
