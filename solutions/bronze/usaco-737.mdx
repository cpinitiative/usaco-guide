...
For each color, we look left, right, up, and down to determine the limits of that color. This creates a rectangle which must contain that color. Since we are trying to find all posssible starting colors, we can assume each rectangle is as small as possible.
