...
int main() {
    ...
    for (int t = 0; t < test_num; t++) {
        ...
        std::map<long long, vector<long long>> groups;
        for (int i = 0; i < arr.size(); i++) {
            long long id = i - arr[i];
            groups[id].push_back((long long) arr[i]);
        }
        
        long long total = 0;
        for (auto& comp : groups) {
            sort(comp.second.rbegin(), comp.second.rend());
            for (int i = 0; i + 1 < comp.second.size(); i += 2) {
                long long weight = comp.second[i] + comp.second[i + 1];
                if (weight > 0) {
                    total += weight;
                }
            }
	...
	public static void main(String[] args) throws IOException {
		...
		for (int t = 0; t < testNum; t++) {
			...
			for (int i = 0; i < arr.length; i++) {
				...
				groups.get(id).add((long)arr[i]);
			}

			long total = 0;
			for (List<Long> comp : groups.values()) {
				comp.sort(Comparator.reverseOrder());
				for (int i = 0; i + 1 < comp.size(); i += 2) {
					long weight = comp.get(i) + comp.get(i + 1);
