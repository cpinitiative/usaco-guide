...
Let's call a node with only incoming directed edges a "sink". We now claim that there exists a sink $X$ in our tree if and only if all other nodes can reach $X$. Moreover, if $X$ exists, it is the unique sink. Try to see why this is true yourself before continuing.

<Spoiler title = "Proof">

1. If all nodes in the tree can reach some node $X$, then $X$ must be a sink. Moreover, $X$ must be the unique sink, since all other nodes need at least one outgoing directed edge to escape them, so they aren't sinks. $X$ cannot have any outgoing edges, since if there was an edge connecting $X$ to node $Y$, then $Y$ cannot reach $X$.
2. If $X$ is the unique sink, then all nodes in the tree can reach $X$. Suppose some node $Y$ cannot reach $X$. We know that $Y$ has an outgoing edge since it isn't a sink, so let's follow such an edge. Then the node we reach also has an outgoing edge since it isn't a sink, and repeatedly following these edges will inevitably get us to a sink. Since $X$ is the only sink, we must have reached $X$.

</Spoiler>

Therefore, we can keep track of the number of outgoing edges of each node, and if there is exactly one sink $X$, then $X$ is our answer.
