...
author: Melody Yu, Danh Ta Chi Thanh, Jeffrey Meng
---
...
/**
 * @returns whether or not (x, y) is in the rect bounded by bottom
 * left corner (rectBLX, rectBLY) and top right corner (rectTRX, rectTRY)
 */
bool isInRectangle(int x, int y, int rectBLX, int rectBLY, int rectTRX, int rectTRY) {
	return (x >= rectBLX && x <= rectTRX && y >= rectBLY && y <= rectTRY);
}
...
int main() {
	...
	int backBLX, backBLY, backTRX, backTRY;
	cin >> backBLX >> backBLY >> backTRX >> backTRY;
	int frontBLX, frontBLY, frontTRX, frontTRY;
	cin >> frontBLX >> frontBLY >> frontTRX >> frontTRY;

	// Figure out how many corners are covered
	int numCoveredCorners = 0;
	if (isInRectangle(backBLX, backBLY, frontBLX, frontBLY, frontTRX, frontTRY)) {
		numCoveredCorners++;
	}
	if (isInRectangle(backTRX, backBLY, frontBLX, frontBLY, frontTRX, frontTRY)) {
		numCoveredCorners++;
	}
	if (isInRectangle(backTRX, backTRY, frontBLX, frontBLY, frontTRX, frontTRY)) {
		numCoveredCorners++;
	}
	if (isInRectangle(backBLX, backTRY, frontBLX, frontBLY, frontTRX, frontTRY)) {
		numCoveredCorners++;
	}

	if (numCoveredCorners <= 1) {
		/*
		 * if 1 or 0 corners are covered, the
		 * tarp is simply just the area of the rectangle
		 */
		cout << (backTRX - backBLX) * (backTRY - backBLY) << endl;
	} else if (numCoveredCorners == 2) {
		/*
		 * if exactly two corners are covered,
		 * we need to figure out the exposed area.
		 *
		 * calculate the area of the intersection (the covered area),
		 * and subtract it from the total area.
		 */
		int intersectionBLX = max(backBLX, frontBLX);
		int intersectionBLY = max(backBLY, frontBLY);
		int intersectionTRX = min(backTRX, frontTRX);
		int intersectionTRY = min(backTRY, frontTRY);
		int totalAreaOfBackRectangle = (backTRX - backBLX) * (backTRY - backBLY);
		int areaOfIntersection =
		    ((intersectionTRX - intersectionBLX) * (intersectionTRY - intersectionBLY));

		cout << totalAreaOfBackRectangle - areaOfIntersection << endl;
	} else {
		// all 4 corners are covered
		cout << 0 << endl;
	}
}
```
...
public class Billboard {
	public static void main(String[] args) throws IOException {
		...
		int backBLX = Integer.parseInt(st.nextToken());
		int backBLY = Integer.parseInt(st.nextToken());
		int backTRX = Integer.parseInt(st.nextToken());
		int backTRY = Integer.parseInt(st.nextToken());

		st = new StringTokenizer(in.readLine());
		int frontBLX = Integer.parseInt(st.nextToken());
		int frontBLY = Integer.parseInt(st.nextToken());
		int frontTRX = Integer.parseInt(st.nextToken());
		int frontTRY = Integer.parseInt(st.nextToken());
		...
		if (numCoveredCorners <= 1) {
			// if 1 or 0 corners are covered, the tarp is the area of the
			// rectangle
			System.out.println((backTRX - backBLX) * (backTRY - backBLY));
		} else if (numCoveredCorners == 2) {
			/*
			 * if exactly two corners are covered, we need to figure out the
			 * exposed area. calculate the area of the intersection (the covered
			 * area), and subtract it from the total area.
			 */
			int intersectionBLX = Math.max(backBLX, frontBLX);
			int intersectionBLY = Math.max(backBLY, frontBLY);
			int intersectionTRX = Math.min(backTRX, frontTRX);
			int intersectionTRY = Math.min(backTRY, frontTRY);
			int totalAreaOfBackRectangle = (backTRX - backBLX) * (backTRY - backBLY);
			int areaOfIntersection = ((intersectionTRX - intersectionBLX) *
			                          (intersectionTRY - intersectionBLY));

			System.out.println(totalAreaOfBackRectangle - areaOfIntersection);
		} else {
			// all 4 corners are covered
	...
	/**
	 * @returns whether or not (x, y) is in the rect bounded by
	 * bottom left corner (rectBLX, rectBLY) and
	 * top right corner (rectTRX, rectTRY)
	 */
	public static boolean isInRectangle(int x, int y, int rectBLX, int rectBLY,
	                                    int rectTRX, int rectTRY) {
		return (x >= rectBLX && x <= rectTRX && y >= rectBLY && y <= rectTRY);
	}
}
```
...
public class Billboard {
	public static void main(String[] args) throws IOException {
		...
		// if fewer than 2 corners are covered, the whole rectangle must be
		// covered
		if (corner_num < 2) {
...
![](/solutions/bronze/usaco-783/Case_1.1.png)

![](/solutions/bronze/usaco-783/Case_1.2.png)

### Case 2

![](/solutions/bronze/usaco-783/Case_2.png)

### Case 3

![](/solutions/bronze/usaco-783/Case_3.png)

### Case 4

![](/solutions/bronze/usaco-783/Case_4.png)

### Case 5

![](/solutions/bronze/usaco-783/Case_5.png)

### Case 6

![](/solutions/bronze/usaco-783/Case_6.png)
...
![](/solutions/bronze/usaco-783/Corner_Case.png)
