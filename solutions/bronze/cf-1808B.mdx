...
for t in range(test):
	...
	res = 0
	"""
	loop through every column and find the absolute difference value of the column
	first sort the array in decreasing order
	compute the suffix sum of the array
	then the result is difference between
	--> the number of elements after the current element times the current number and
	--> the suffix sum value is subtracted
	"""
	for i in range(m):
	    temp = hashmap[i]
