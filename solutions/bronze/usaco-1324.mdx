...
The intuitive reason for this is that each sentence, not counting conjuctions, has exactly one verb.
...
We also try to use as many conjuctions as possible;
if there's $s$ sentences, we can use a maximum of $\left\lfloor\frac{s}{2}\right\rfloor$ conjuctions,
since each sentence can be joined at most one time.

The rest is some pretty annoying formatting, but it doesn't require any algorithmic tricks.
...
public class MooLanguage {
	public static void main(String[] args) throws IOException {
		...
		for (int t = 0; t < testNum; t++) {
			...
			int maxVerbs = periodNum + Math.min(periodNum, words.get("conjunction").size());
			// Test all combinations of transntive/intransitive verbs
			for (int tv = 0; tv <= words.get("transitive-verb").size(); tv++) {
				for (int itv = 0; itv <= words.get("intransitive-verb").size(); itv++) {
					...
					int conjUsed = Math.min(words.get("conjunction").size(), (tv + itv) / 2);
