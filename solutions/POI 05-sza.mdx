...
The official time limits are very tight and `std::vector` has a bad constant factor, so this approach only gets 100 points if you represent the graph described below without `std::vector`.
