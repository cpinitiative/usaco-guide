...
Similarly, we can make at least half of $S[0 : \pi[|S| - 1]]$'s *suffix* using $T$. We can then combine these two fragments to make $S[0 : \pi[|S| - 1]]$ itself, so therefore $T$ is also a template of $S[0 : \pi[|S| - 1]]$.
