...
Since $S[0 : \pi[|S| - 1]]$ is the longest proper prefix of $S$ that is also a suffix, we know that the only longer string that can be $S$'s template is $S$ itself. Let $T$ be some template of $S$ that isn't $S$ itself.

Next, we claim that we can make at least half of $S[0 : \pi[|S| - 1]]$'s prefix using $T$. If $|T| \geq \frac{\pi[|S| - 1]}{2}$, then we are done. Otherwise, in the process of making $S$ using $T$ from left to right, there must be some point where we transition from (built less than half of $S[0 : \pi[|S| - 1]]$) to (built at least half of but not more than $S[0 : \pi[|S| - 1]]$).

Similarly, we can make at least half of $S[0 : \pi[|S| - 1]]$'s _suffix_ using $T$. We can then combine these two fragments to make $S[0 : \pi[|S| - 1]]$ itself, so therefore $T$ is also a template of $S[0 : \pi[|S| - 1]]$.
...
If $2\pi[i - 1] \geq i$, then $S[0 : i]$ will never be the shortest template of $S$ because we know that $S[0 : \pi[i - 1]]$ is a template of $S[0 : i]$.

This means that we only have $\mathcal O(\log N)$ candidate templates to check! We can then use KMP again on each candidate and check whether the maximum "gap" between matches is sufficiently small.
...
The official time limits are very tight and `std::vector` has a bad constant factor, so this approach only gets 100 points if you represent the graph described below without `std::vector`. For more details, see [this CodeForces blog post](https://codeforces.com/blog/entry/67883).

This is also an interesting example of how something with a less-optimal big-O complexity (i.e. solution 1) runs faster in practice.

</Warning>

First, we compute the prefix function $\pi$ of $S$ using KMP. While doing this, we also construct a tree where node $(i + 1)$'s parent is $\pi[i]$ for each $0 \leq i < N$.

Notice how the subtree of node $i$ is the set of all positions $j$ where $S[0 : i] = S[j - i : j]$.

This means that we can traverse up the tree from node $N$ and keep track of the nodes in the current node's subtree (using a linked list).

To check whether the current node works, we simply check whether the gaps between positions in the subtree are sufficiently small. We can keep track of these gaps by keeping a global maximum that we update each time we erase a node from the linked list.
