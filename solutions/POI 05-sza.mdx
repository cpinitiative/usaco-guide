...
The official time limits are very tight and `std::vector` has a bad constant factor, so this code only scores points. Nevertheless, we think the idea behind this solution is interesting enough to share.
