...
The official time limits are very tight and `std::vector` has a bad constant factor, so this code only scores 60 points. Nevertheless, we think the idea behind this solution is interesting enough to share.

It's also an interesting example of how something with a less-optimal big-O complexity (i.e. solution 1) runs faster in practice.
