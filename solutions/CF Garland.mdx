...
For each query we can keep a sliding window where no more than $a$ letters in the window are not $b$
...
For each query we can use prefix sums to check the number of occurrences of a given letter $a$ and use two pointers to find the maximum Koyomity ending at index $j$.
...
**Note: you do not need to understand this solution at the Silver level**
