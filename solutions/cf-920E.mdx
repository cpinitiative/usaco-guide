...
We can store whether edges are non-existent using an array of hashsets instead of an adjacency list. This way we can find if there is no edge between two nodes in
$\mathcal{O}(1)$ time. Using an adjacency matrix would take too much
memory.

### Optimization 2

Rather than using a standard boolean array to check if a node has not yet been visited, we
can use a set of unvisited nodes. This way we only loop through the nodes that
have not been visited yet and can find the next unvisited node in $\mathcal{O}(\log N)$. We need to use ```upper_bound``` to find
...
void dfs (int x) {
	...
	for (int i = 0; i < n; i++) {
		...
		if (x != i && adj[x].find(i) == adj[x].end()) {
			...
			// the current component can visit group 1 and we dont
