...
You can store whether there is no edge between two nodes using an array of hashsets, instead of a adjacency list. If the input tells you that nodes $a$ and $b$ do not have an edge between them, you would add $a$ to $\texttt{adj}[b]$, and $b$ to $\texttt{adj}[a]$ 
...
Rather than using a standard boolean array to check if a node has not yet been visited, you
can use a set of unvisited nodes. This way you only loop through the nodes that
have not been visited yet and can find the next unvisited node in $\mathcal{O}(\log N)$. You need to use ```upper_bound``` to find
...
Let $N =$ the number of nodes in the graph and $n =$ an abitrary node and $\texttt{degree}(n)=$ the number of neighbooring nodes (number of different nodes $n$ is directly connected to). 
