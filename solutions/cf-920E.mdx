...
You can store whether there is no edge between two nodes using an array of hashsets, instead of a adjacency list. If the input tells you that nodes $a$ and $b$ do node have an edge between them, you would insert node $a$ at $\texttt{array}[b]$, and node $b$ and $\texttt{array}[a]$ 
This way you can find if there is no edge between two nodes in
$\mathcal{O}(1)$ time. Using an adjacency matrix would take too much
memory.
