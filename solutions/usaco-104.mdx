---
id: usaco-104
source: USACO Bronze 2012 January
title: Haybale Stacking
author: Óscar Garries, Brad Ma
---

[Official Analysis](http://www.usaco.org/current/data/sol_stacking.html)

<LanguageSection>
<CPPSection>

## C++ Implementation

```cpp
#include <bits/stdc++.h>

using namespace std;

int main () {
	int n, k;
	cin >> n >> k;
	vector<int> diff(n + 1);
	for (int i = 0; i < k; i++) { // read input and build difference array
		int l, r;
		cin >> l >> r;
		l--; // make zero-indexed
		r--;
		diff[l]++;
		diff[r + 1]--;
	}

	int sol[1000000];
	int tot = 0;
	for (int i = 0; i < n; i++) { // build prefix sum array
		tot += diff[i];
		sol[i] = tot;
	}
	sort (sol, sol + n); // sort to get median
	cout << sol[n / 2] << '\n'; // output the median
}
```

</CPPSection>
	
<JavaSection>

## Java Implementation

```java
import java.util.*;
import java.io.*;

class haybaleStacking {
	public static void main (String[] args) {
		Kattio io = new Kattio();

		int n = io.nextInt();
		int k = io.nextInt();
		int[] differenceArray = new int[n + 1]; 
		// differenceArray[n] is the difference between A[n + 1] and A[n]

		for (int x = 0; x < k; x++) { // read input and build differenceArray
			int a = io.nextInt() - 1; // read and make zero-indexed
			int b = io.nextInt() - 1;

			differenceArray[a]++;
			differenceArray[b + 1]--;
		}

		int[] prefixSum = new int[n + 1];
		int total = 0;

		for (int x = 0; x < n; x++) { // build prefixSum array
			total += differenceArray[x];
			prefixSum[x] = total;
		}

		Arrays.sort(prefixSum);
		io.println(prefixSum[prefixSum.length / 2]);
		io.close();
	}

	// CodeSnip{Kattio}
}
```

</JavaSection>
	
<PythonSection>

## Python Implementation
	
```python
n, k = map(int, input().split())
differences = [0]*(n+1) # make a differences array
for i in range(k):
    start, end = map(int, input().split())
    differences[start-1] += 1 # add one to the start
    differences[end] -= 1 # subtract one from the next number outside of start-end
pref = [0]*n # prefix sums array
s = 0	
for i in range(n): # uses differences and total sum to get where all the haybales are
    pref[i] = differences[i] + s 
    s += differences[i]
pref.sort() # sort array
print(pref[n//2])
```
	
</PythonSection>
	
</LanguageSection>
