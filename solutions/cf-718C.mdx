...
Secondly, we need to utilize the distribute property of matrix multiplication to same-size matricies. That is, if $m_k, k \in [1, n]$ are one-by-two matricies, then the following property is satisfied. 

$$
(m_1 + m_2 + ... + m_n) \cdot \begin{pmatrix} 0 & 1 \\ 1 & 1 \end{pmatrix} = m_1 \cdot \begin{pmatrix} 0 & 1 \\ 1 & 1 \end{pmatrix} + m_2 \cdot \begin{pmatrix} 0 & 1 \\ 1 & 1 \end{pmatrix} + ... + m_n \cdot \begin{pmatrix} 0 & 1 \\ 1 & 1 \end{pmatrix}
$$

In that way, we can utilize lazy propgation on the segment tree. We will store an integer tag in each node denoting the lazy update, which is denotes that we must cycle every node in its subtre by the value of the tag. When propogating, we can simply update the node by binary exponentiating and then multiplying, then storing it in the lazy tag. Querying works as normal on the segment tree. 
...
pll multiply(const pll &a, const Matrix &b) {
	return {(a.FF * b[0][0] + a.SS * b[1][0]) % MOD, 
			(a.FF * b[0][1] + a.SS * b[1][1]) % MOD};
}
...
pll merge(const pll &a, const pll &b) {
	return {(a.FF + b.FF) % MOD, (a.SS + b.SS) % MOD};
}
