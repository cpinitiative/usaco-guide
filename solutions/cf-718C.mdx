...
Secondly, we need to utilize the distributive property of matrix multiplication to same-size matricies. That is, if $m_k$ is a one-by-two matrix for $k \in [1, n]$, then the following property is satisfied:
