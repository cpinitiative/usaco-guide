...
Note that for query operations, we need to find the maximum suffix sum for each path. This can be done by maintaing the maximum suffix sum as well as the actual sum in each segment tree node. 
