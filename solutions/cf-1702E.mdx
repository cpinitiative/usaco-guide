...
First observe that if any number is present on more than 2 dominoes or if a dominoe has the same number repeated twice, then 
it is impossible to create the 2 sets.

We can turn this problem into
a graph problem by drawing edges between dominoes that share numbers. Specifically, let dominoe $i$ have the two numbers 
...
bool dfs(int u, int g, vector<int>& vis, vector<vector<int>>& adj) {
	...
	if (vis[u] == g) return true;
    
    // If the node is already visited with and was marked in the other color
	if (vis[u] == 1 - g) return false;
    
    vis[u] = g; 
	for (auto i : adj[u]) {
        // If there is a problem when dfsing to any of the other nodes we 
...
void solve() {
	...
	for (auto& i : dm) cin >> i.first >> i.second;
    ...
    // Since the graph is not guaranteed to be connected
    // we need to iterate over every value and make sure
    // every value is visited
	for (int i = 0; i < n; i++) {
		if (vis[i]==-1) {
			if (!dfs(i, 0, vis, adj)) {
				cout << "NO\n";
