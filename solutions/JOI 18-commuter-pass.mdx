...
6. The DP transition for `dpU(i) = min(distU[i], dpU(parent_i))` for each parent of `i` (see `dijkstra2` function implementation for more details). `dpV`'s transition is defined similarly.
