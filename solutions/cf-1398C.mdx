...
void solve(){
	int n; cin >> n;
	vector<int> dp(n+1, 0);
	for(int i=1; i<=n; i++){  // put into array
		char c; cin >> c;
		dp[i]=c-'0';
	}
	...
	for(pi p: sumDist){
		...
		// this uses the formula n * (n - 1) / 2, and it gets
            	// the sum of all the positive integers up to and including n
		goodArrays+=f*(f-1)/2;
	}
...
public class GoodSubarrays {
	static long solve (int arrayLength, String stringArray) {
		...
		for (int x = 1; x <= arrayLength; x++) {  // put into array
			array[x] = stringArray.charAt(x - 1) - '0';
		}
		for (int x = 1; x <= arrayLength; x++) {  // build prefix sums
			array[x] += array[x - 1];
		}
		Map<Integer, Long> sumDist = new HashMap<>();
		for (int x = 0; x <= arrayLength; x++) {  // build map
	...
	public static void main (String[] args) {
		...
		int t = io.nextInt();
		for (int x = 0; x < t; x++) {  // each test case
