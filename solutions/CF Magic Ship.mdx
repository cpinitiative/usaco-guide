...
We can binary search on the minimum time it takes to go from the start to the destination. If a time $t$ is possible, then any time greater than it is possible as well, since we can just move in the opposite direction of the wind.
