...
Binary search on the minimum time it will take for all the Packmen to eat all the pellets. In that case, we would have to check whether the Packmen can eat all the pellets in $d$ units of time.
...
There are two main cases we consider for each Packman: one where the Packman first goes to the right and then to the left, and another where the Packman first goes to the _left_ and then to the right. We calculate the subarray of pellets eaten in both of these cases and assign the larger one to the Packman.
