...
We can repeatedly greedily assign the leftmost Packman to the largest possible subarray of pellets that includes the leftmost pellet. Using this strategy, we can check whether all pellets can be eaten when the Packmen are limited to $d$ units of time.
