...
Using centroid decomposition, we can 
 ...
 - update the value at some vertex in $O(\log N)$ time
 - query the sum of the values of all vertices that are distance exactly $D$ away from any vertex in $O(\log N)$ time.
...
What we need for this problem is slightly different since we need the number of vertices $\le D$ away.
