...
Computing this naively would take $O(n^2k)$ time, but we can do much better than that.
...
This means we can use CHT (with a deque) to compute the answer in $O(nk)$ - a significant improvement!
...
This brings the memory used down to $O(n)$, which is good enough to pass.
