...
We can use an ordered set in place of the linked list mentioned in the official solution.
...
int main() {
	...
	sort(begin(tiles_by_depth),end(tiles_by_depth),[&](int x, int y) {
		return f[x] > f[y]; });
	...
	vector<bool> ans(B);
	for (auto a: query) {
		while (tile_index < tiles_by_depth.size() && f[tiles_by_depth[tile_index]] > a[0]) {
