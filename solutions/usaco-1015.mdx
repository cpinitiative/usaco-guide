...
[Official Analysis (C++)](http://www.usaco.org/current/data/sol_triangles_silver_feb20.html)
...
fieldx = [[] for _ in range((MAX * 2) + 1)]
fieldy = [[] for _ in range((MAX * 2) + 1)]
all_points = []

with open('triangles.in') as r:
	n = int(r.readline())
	for _ in range(n):
		x, y = map(int, r.readline().split())
		fieldx[x + MAX].append(y + MAX)
		fieldy[y + MAX].append(x + MAX)
		all_points.append((x + MAX, y + MAX))
...
psumx = [[] for _ in range((MAX * 2) + 1)]
psumy = [[] for _ in range((MAX * 2) + 1)]
