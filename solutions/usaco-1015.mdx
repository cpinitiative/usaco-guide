...
fieldx, fieldy = [[] for _ in range((MAXIMUM * 2) + 1)], [[] for _ in range((MAXIMUM * 2) + 1)]
allcoors = []
...
with open('triangles.in') as r:
	...
	for _ in range(n):
		...
		fieldx[x + MAXIMUM].append(y + MAXIMUM)
		fieldy[y + MAXIMUM].append(x + MAXIMUM)
		allcoors.append((x + MAXIMUM, y + MAXIMUM))
    
for i in range(20001):
	fieldx[i].sort()
	fieldy[i].sort()
    
psumx, psumy = [[] for _ in range((MAXIMUM * 2) + 1)], [[] for _ in range((MAXIMUM * 2) + 1)]
