...
# Alternative Solution (DSU/UF)

Like the DFS solution, we binary search on the answer $x$. $x$ is valid if all $p_i$ are in the same component as $i$, which we can query in $\mathcal{O}(\alpha)$ using a Union Find/Disjoint Set data structure.

Our resulting time complexity is $\mathcal{O}(NlogN)$

<LanguageSection>
<CPPSection>

## C++ Implementation (DSU/UF)
