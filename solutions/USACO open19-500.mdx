...
In retrospect, note that this solution is equivalent to just performing Kruskal by hand when we analyze the edge weight expression and use greedy logic to create the optimal grouping situation. As you can see from Ben's Solution 2, letting Kruskal perform on the graph to find the MST will find the exact same grouping. 
