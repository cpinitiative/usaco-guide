...
int main() {
	...
	for (int x : to_check_x) {
		for (int y : to_check_y) {
			...
			int balance = max({field_amt[0][0], field_amt[0][1], field_amt[1][0], field_amt[1][1]});
			most_balanced = min(most_balanced, balance);
		}
	}
	ofstream("balancing.out") << most_balanced << endl;
}
```
