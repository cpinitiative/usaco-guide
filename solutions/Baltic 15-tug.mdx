---
id: baltic-15-tug
title: Baltic OI 2015 Tug of War
author: Andi Qu
---
...
Let the set of all cycles be $C$ and let the difference in strengths from the nodes we previously deleted be $d$.
...
This means that each cycle contributes a fixed amount to the difference in strengths! Let the absolute value of this amount for the $i$-th cycle be $V_i$.

We now only need to check whether there exist 2 disjoint sets $S$ and $T$ such that $S \cup T = C$ and
...
We can check this using knapsack DP in $O(NK)$ time.

Since we are only checking whether we can obtain some value, we can use a bitset to speed this up.

The final complexity is $O(NK / 64)$, which is fast enough for 100 points.
