...
We can use Dijkstra's to compute the distance from $1$ and $N$ to every vertex. Then our answer is $\text{min}_{A\rightarrow B} dist1[A]+c(A,B)+distN[B]$,

where $c(A,B)$ is the cost to travel from city A to city B after applying the coupon to that flight.
