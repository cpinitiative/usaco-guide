...
Then our answer is $\min\limits_{A\rightarrow B} \texttt{dist1}[A]+c(A,B)+\texttt{distN}[B]$, where
...
$\texttt{dist}[i][0]$ will represent the shortest distance from the start node to node
$i$, without using the discount. $\texttt{dist}[i][1]$ will represent the shortest
