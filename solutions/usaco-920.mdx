...
However, for all $K$ connected component, the grass types can be flipped and still be a valid solution:

![inverted connected component](<usaco-920/inverted_connected_component.png>)

For each connected components, there are $2$ possible ways to arrange them. Thus, if there's $k$ connected components, there's $2^k$ ways to arrange the entire graph.
