...
Now, let's look at one of these, which could look something like this:

![possible connected component](<usaco-920/connected_component.png>)

However, for all $K$ connected component, the grass types can be "inverted":

![inverted connected component](<usaco-920/inverted_connected_component.png>)

So, for each $K$ connected components, there are $2$ possible ways to arrange them, or $2^k$ ways to arrange the entire graph. Since in binary, $2^k$ is $1$ followed by $k$ zeros, we can simply print this instead of manually converting it.
...
int main() {
	...
	if (impossible) {
		...
		/*
		 * if we can construct our solution, then there are 2^cc possible rearrangements,
		 * since we can always invert the types of grass in each connected component.
		 */

		// 2^cc in binary is 1, followed by cc zeros.
		cout << 1;
		for (int i = 0; i < cc; i++) {
			cout << 0;
		}
