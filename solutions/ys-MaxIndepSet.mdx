...
The complement or inverse of a graph G is a graph H on the same vertices
...
one fills in all the missing edges required to form a complete graph,
and removes all the edges that were previously there.
...
void mcp(int x, int dt, vector<ll> &f, vector<ll> a) {
...
void print_mask(ll ans) {
	...
	while (ans) {
		printf("%d ", __builtin_ctzll(ans)), ans -= ans & (-ans);
	}
}
...
int main() {
	...
	/*
	 * 1.use Bitmask to represent complement graph
