...
and remove all the edges that were previously there.
...
It's not a good idea to store just the number of points in the answer graph because it'll be hard to transfer.
...
If the two parts are $S1$ and $S2$, loop through **every subgraph** of $S1$,

Then, find a subgraph of $S2$ such that it has direct edges with **all of the points** in the subgraph of $S1$ that we're looping through.
