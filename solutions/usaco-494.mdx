...
Almost every time the bounds are ~$20$, it suggests bitmask DP.

Let's go through the information we need to implement it!

#### States

Let's use a bitmask ($mask$) to keep track of the cows we're using.
...
$\texttt{dp}[mask] =\{$current height, maximum safety factor achievable$\}$
...
$\texttt{dp}[mask] = \max_{j \in mask}(\texttt{dp}[mask],\hspace{0.1cm}\min(\texttt{dp}[mask \char`\\ j] - weight_j,\hspace{0.1cm}strength_j))$

Such that $j$ is the index of the $j$th cow and $weight_j,\hspace{0.1cm}strength_j$ represents the weight and strength of cow $j$ respectively.
