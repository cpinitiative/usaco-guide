...
In this problem, we're asked the minimum number of coins of distinct weights needed to achieve some weight, $x$. You can read about the solution to this classical problem in [CPH Chapter 7](https://usaco-guide.vercel.app/CPH.pdf) under "Coin Problem".
...
\[\texttt{dp[w]} = \min_{i=1}^n{(\texttt{dp[w]} - \texttt{coins[i]})}\]
