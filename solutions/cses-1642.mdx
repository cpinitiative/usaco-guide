...
values that we can achieve, and the two numbers that add up to
that value using a map. Then, the problem is reduced to finding two numbers $a$
and $b$ that sum up to a mapped value such that no indice of $a, b, c, d$ is repeated.
