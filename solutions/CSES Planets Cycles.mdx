...
int main() 
{
	...
	for (int i = 0; i < n; i++)
	{
		if (!visited[i])
		{
			...
			while (!cycle.empty())
			{
				//loop started, don't need to decrement anymore
				if (cycle.front() == cycle.back())
				{
					decrement = 0;
				}
				cyclelength[cycle.front()] = steps;
				steps -= decrement;
				cycle.pop();
			}
		}
