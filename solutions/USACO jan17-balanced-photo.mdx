...
The official solution uses a [binary indexed tree](https://usaco.guide/gold/PURS/#binary-indexed-tree).

An alternative solution uses an order statistic tree.(C++ only)
...
However, `std::set` is **NOT** a random access data-structure, which means that the time complexity of finding all cows higher than the current cow is $O(N)$.(e.g using `distance()`)
