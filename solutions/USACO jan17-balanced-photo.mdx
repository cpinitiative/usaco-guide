...
To lower the time complexity, we can use an [order statistic tree](/gold/PURS/#order-statistic-tree) to find the number of cows that are higher than each cow in $\mathcal O(\log N)$ time.
