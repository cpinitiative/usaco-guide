...
We aim to quickly determine, for each cow, the number of _taller_ cows to its left and right.
...
We can use [order statistic trees](/gold/PURS/#order-statistic-tree) to achieve this in $\mathcal O(N \log N)$ time.

<Warning>

We may **not** use [`std::set`](http://www.cplusplus.com/reference/set/set/) because it does not support indexing. Additionally, since BBSTs do _not_ support random access, using operations such as [`std::distance`](http://www.cplusplus.com/reference/iterator/distance/) to index takes linear time.
