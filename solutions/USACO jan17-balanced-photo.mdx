...
We may **not** use [`std::set`](http://www.cplusplus.com/reference/set/set/) because it is a random access data-structure. The time complexity of counting the number of heights taller than the current cow is $\mathcal O(N)$ (using [`set::lower_bound`](http://www.cplusplus.com/reference/set/set/lower_bound/) and [`std::distance`](http://www.cplusplus.com/reference/iterator/distance/)).
