...
Both $\mathcal{O}(N^2)$ and $\mathcal{O}(N\max{a[i]})$ algorithms are not hard
to come up with, but the latter one is more enlightening.
...
and query is unbalanced.

Try to consider the both of them and try to "combine" their ideas.

</Spoiler>

<Spoiler title="Explanation">

**For $\mathcal{O}(N^2)$ DP**:
...
For convience, we'll call the red part 0-part, the black 1-part.
...
For each state, we preprocess the 1-part, and predict the 0-part of next
element.
