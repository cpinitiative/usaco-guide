...
other for preprocessing, this is the key to reduce time complexity.
...
And we want some number AND $\texttt{cur} = 9$.

When we consider $\texttt{pre}$, we predict what the next element is, but to
avoid high time complexity, we'll only calculate 10 bits. Here, we'll store in
the DP array that there's an element ($\texttt{pre}$), black part is $0011110101$
and when red part of next element is $\color{red}0110110101$, it has a
contribution to $\texttt{pre}\ \&\ \texttt{cur} = 5$, only for red parts.

Now when we consider $\texttt{cur}$, to calculate answer where sequence ends
with $\texttt{cur}$, use results we preprocessed before, enumerate black part of
last element. When we get to $0011110101$, the black part has contribution 4,
thus we get red part contribution is 5. Then we can transfer from
$\texttt{pre}$, and its red part has a contribution 5 with
$\color{red}0110110101$.

Details of implementation are given below.
