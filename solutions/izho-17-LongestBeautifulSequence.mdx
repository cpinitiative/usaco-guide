...
Both $\mathcal{O}(N^2)$ and $\mathcal{O}(N\max{a[i]})$ algorithms are not hard to
...
The other is to traverse all the possible outcomes of the next number, (kind
of predicting next answers) $\mathcal{O}(1)$ to query current answer in DP array
and $\mathcal{O}(\max{a[i]})$ to update, but needs a bit more memory.
...
Let $\texttt{dp}[i][j]$ be the length of longest beautiful sequence of the first $i$
elements that ends with $j$,
...
Let $\texttt{dp}[i][j][q]$ be the length of longest beautiful sequence of the first
$i$ elements which satisfies $\text{popcount}(\texttt{EndElement}\&j)=q$,
...
$$
\texttt{dp}[i][j][q]=\texttt{dp}[i-1][j][q],q\not=\text{popcount}(a[i]\&j)
$$
...
Let $\texttt{ans}[i]$ be the length of longest beautiful sequence that ends with
$a[i]$, and $\texttt{dp}[p][i][j][q]$ be the length of longest beautiful
sequence of the first $p$ elements which satisfies $i = \text{b1}(\texttt{EndElement})$,
...
$$
\texttt{ans}[i] = \max_{\texttt{pre1} < 2^{10}} \{ \texttt{dp}[i-1][\texttt{pre1}][\texttt{cur0}][k[i] - \text{popcount}(\texttt{pre1} \& \texttt{cur1})] + 1 \}
$$

$$
\texttt{dp}[i][\texttt{cur1}][\texttt{nex0}][\text{popcount}(\texttt{nex0} \& \texttt{cur0})] = \max \{ \texttt{dp}[i-1][\texttt{cur1}][\texttt{nex0}][\text{popcount}(\texttt{nex0} \& \texttt{cur0})] , \texttt{ans}[i] \}
$$

and

$$
\texttt{dp}[p][i][j][q] = \texttt{dp}[p-1][i][j][q]
$$

for $\texttt{dp}[p][i][j][q]$ which is not updated above.
