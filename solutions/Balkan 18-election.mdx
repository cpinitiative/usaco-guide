...
Finding $w.A$ is a bit more tricky though. We will now prove that the lower bound on $w.A$ is

$$
\max(\max(u.A + v.S, u.S + v.A), u.L + v.R)
$$

and that it can be attained.
...
Consider the greedy strategy: iterate from left to right and change `T`s to satisfy the increasing condition, then do the same from right to left. Then equality holds when we set $i$ equal to one less than the position of the leftmost `T` removed when doing the right to left iteration.

Therefore, this is a lower bound and it can be attained.

The final complexity of this algorithm is $O((N + Q) \log N)$.
