...
Do you think you have an $\mathcal{O}(N^3)$ solution that doesn't involve looping through all triples of nodes? If so, is it really $\mathcal{O}(N^3)$ or is it actually amortized $\mathcal{O}(N^2)$?
...
We can loop through each **valid** pair $(i, d)$ in amortized $\mathcal{O}(N)$ time, since each pair corresponds to at least one node in the tree.
...
Since we only care about $\texttt{after}_i[d]$ for **valid** pairs $(i, d)$, we can just keep a global array storing the maximum values of $\texttt{after}_i[d]$ so far (with all elements as 0 initially). This allows us to compute all interesting $\texttt{after}_i[d]$ in $\mathcal{O}(N)$ time!

Alternatively, we can sort the children of $r$ by their depths and then compute $\texttt{after}_i[d]$ directly, which is still fast enough to pass.

The final complexity of this code is $\mathcal{O}(N^2)$ time with $\mathcal{O}(N)$ memory.
