...
graphs (DAG) with vertex set $S$.

Since we're constructing a DAG, we can transfer the state by adding new points
like 0-out-degree points (or 0-in-degree points is OK).
...
Let $\texttt{dp}[S]$ be the number of DAG configs on the subgraph induced by
vertex set $S$.
...
$$
\begin{aligned}
\texttt{dp}[S] &= \sum_{\emptyset\neq P\subseteq S, P\text{ independent}}(\#\text{ configs where  }P=\text{the set of vertices with out-degree 0})\\
&= \sum_{\emptyset\neq P\subseteq S, P\text{ independent}}(-1)^{|P|+1}\cdot (\#\text{ configs where  }P\subseteq\text{the set of vertices with out-degree 0}) \\
