...
$$
A_S = \bigcup_{\varnothing\not=P\subseteq S,P \text{ is an indep set}}A_{S/P} \text{ with edges of points in }S\text{ to }S/P
$$

It's obvious we cannot add all sizes of $A_{S/P}$ directly, because when we
assign $S$ to be 0-out-degree points, there might be 0-out-degree points in
$A_{S/P}$ previously which might be 0-out-degree in $A_S$ still (no edges to
points in $S$ from them ).

So fix $S$, let $B_P$ (empty if $P$ doesn't satisfy conditions above) to be
$A_{S/P} \text{ with edges of points in }S\text{ to }S/P$, so $|B_P|=|A_{S/P}|$.
...
$$
|A_S| = \sum_{\varnothing\not=P\subseteq S,P \text{ is an indep set}}(−1)^{|P|+1}|A_{S/P}|
$$

DP equation form:

$$
dp[S]=\sum_{\varnothing\not=P\subseteq S,\texttt{indepset}[P]=\texttt{true}}(−1)^{|P|+1}dp[S/P]
$$
