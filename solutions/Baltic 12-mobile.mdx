...
We can solve this in $O(N \log N)$ time by sorting the segments by their left endpoints and then merging each segment into the union if it intersects the union of the previous segments.

Unfortunately, this is just a bit too slow ($O(N \log N \log L)$ in total) and only scores 50 points.
...
This gets rid of the $\log N$ factor in our original complexity and so the final complexity is $O(N \log L)$, which is good enough for 100 points.

(There is also an $O(N)$ solution - try to come up with it yourself!)
