...
int number_of_wins(vector<int> &bessie, vector<int> &elsie, bool smaller_card_wins)
{
	/*
...
int main()
{
	...
	ans += number_of_wins(bessie_first, elsie_first, false);

	// In second half of game person with card of smaller value win
	ans += number_of_wins(bessie_second, elsie_second, true);
