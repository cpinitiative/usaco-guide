...
int numberOfWins(vector<int> &bessie, vector<int> &elsie, bool smallerCardWins)
{
	...
	** But what is person with smaller value card wins
	...
	*/
	...
	for (int i = 0; i < n; i++)
	{
		if (bessie[bpos] > elsie[i]) // bessie wins
		{
			wins++;
			bpos++;
		}
		// else if bessie cannot win with current card she will just use her weakest card 
	}
...
int main()
{
	...
	// We will store first and second half of cards separately
