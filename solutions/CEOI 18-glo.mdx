...
that it is optimal to simply subtract $x$ from the interval $[1, r]$ instead of
some other $d < x$.
...
Simply take the LIS of the array. Any $\mathcal{O}(n \log n)$ algorithm to find
the LIS will pass.

# General Solution

For each $i$, let $L_i$ be the length of the longest increasing subsequence
ending at $i$ and $R_i$ be the length of the longest increasing subsequence
starting at $i$ after $[1, i]$ is decremented. We can compute each $R_i$ by
storing the length of the longest decreasing subsequence for each prefix of the
...
In the implementation $L_i$ is $dp_i$ and $R_i-1$ is the variable $j$ on line
$27$.
