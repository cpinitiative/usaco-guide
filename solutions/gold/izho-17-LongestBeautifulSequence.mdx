...
In order to reconstruct a solution, we need to store the ending index of the optimal LBS in $\texttt{dp}[i][x]$ in addition to its length. In our implementation, our `State` object holds an `end` property in addition to a `len` property. In our example below, we represent each state as `{len, end}`.
</Info>
...
- **Base case:** $\texttt{dp}[a_0 = 1] = \{1, 0\}$.
	- all other DP values are initially 0
	- $\texttt{prv}[i] = i$
- $\texttt{dp}[a_1 = 2] = \{2, 1\}$.
	- we loop over all states $\texttt{dp}[x]$ where $x \leq M$ and $x$ is in $\texttt{trans}_{k_i}(a_i)$. The values of $x$ that satisfy these conditions are $0$, $1$, and $4$.
	- Out of these $x$'s, $\texttt{dp}[x = 1]$ has the greatest value of `len`, so:
		- $\texttt{dp}[a_1]. \texttt{len} = \texttt{dp}[x].\texttt{len} + 1 = 2$.
		- $\texttt{dp}[a_1].\texttt{end} = i = 1$.
		- $\texttt{prv}[i] = \texttt{dp}[x].\texttt{end} = 0$.
- $\texttt{dp}[a_2 = 3] = \{3, 2\}$.
	- here, $x \in \{1, 2\}$ and $\texttt{dp}[x = 2]$ has the greater value, so:
		- $\texttt{dp}[a_2]. \texttt{len} = \texttt{dp}[x].\texttt{len} + 1 = 3$.
		- $\texttt{dp}[a_2].\texttt{end} = i = 2$.
		- $\texttt{prv}[i] = \texttt{dp}[x].\texttt{end} = 1$.
- $\texttt{dp}[a_3 = 4] = \{4, 3\}$.
	- here, $x \in \{0, 1, 2, 3\}$ and $\texttt{dp}[x = 3]$ has the greatest value so:
		- $\texttt{dp}[a_3]. \texttt{len} = \texttt{dp}[x].\texttt{len} + 1 = 4$.
		- $\texttt{dp}[a_3].\texttt{end} = i = 3$.
		- $\texttt{prv}[i] = \texttt{dp}[x].\texttt{end} = 2$.
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < (1 << B); j++) {
			if (bc[j][a[i]] == k[i] && dp[j].len + 1 > dp[a[i]].len) {
				dp[a[i]].len = dp[j].len + 1;
				dp[a[i]].end = i;
				prv[i] = dp[j].end;
			}
		}
...
However, each state $\texttt{dp2}[i][k]$ now encapsulates $|\texttt{trans}_k(i)| \leq M$ of our original $\texttt{dp}$ states. Similarly, an LBS with fixed final value $a_i$ **affects $M$  $\texttt{dp2}$ states** compared to one $\texttt{dp}$ state: namely, all states $\texttt{dp2}[x][k]$ where $x \in [0, M)$ and $k = \texttt{bc}[a_i][x]$.
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		if (dp2[a[i]][k[i]].end != -1) {
			prv[i] = dp2[a[i]][k[i]].end;
		}
		for (int j = 0; j < (1 << B); j++) {
			State& affected_state = dp2[j][bc[a[i]][j]];
			if (len > affected_state.len) {
				affected_state = State{len, i};
			}
		}
		if (len > ans) {
			ans = len;
...
- $\texttt{dp}$ (only one possible value of $x$) $\rightarrow \mathcal{O}(M)$ transition, $\mathcal{O}(1)$ affected states
...
Concretely, let $l_x = \texttt{l}(x, b)$ be the number represented by the $b$ leftmost bits of $x$ and $r_x = \texttt{r}(x, b)$ be the number represented by the $\texttt{bit\_count}(x) - b$ rightmost bits. In our new DP state $\texttt{dp3}[i][j][k]$, we will:
- fix $l_x = i$ (this is the $\texttt{dp}$ part of our new state)
...
Let's say we're currently processing $a_i$. If we have $c$ such that $c \in \texttt{trans}_{k_i}(a_i)$, then $\texttt{bc}[c][a_i] = k_i$. By considering the left and right sides of $c$ and $a_i$ separately, we can rewrite $\texttt{bc}[c][a_i]$ as $\texttt{bc}[l_c][l_{a_i}] + \texttt{bc}[r_c][r_{a_i}] = k_i$.
___
...
$1 + 0 = \boxed1$.
___
Since our DP state $\texttt{dp3}[x][y][k]$ fixes $l_c = x$, we should rewrite this equation to solve for $r_c$: $\texttt{bc}[r_c][r_{a_i}] = k_i - \texttt{bc}[l_c][l_{a_i}]$. Note this restriction matches our $\texttt{dp3}$ restriction of $\texttt{bc}[r_c][j] = k$, so we have $y = r_{a_i}$ and $k = k_i - \texttt{bc}[l_c][l_{a_i}] = k_i - \texttt{bc}[x][l_{a_i}]$. As for $x = l_c = \texttt{l}(c, b)$, we simply enumerate all $2^b$ possibilities; therefore, $i \in [0, 2^b)$.

**In summary, the transitions to $a_i$ are all states $\texttt{dp3}[x][r_{a_i}][k_i - \texttt{bc}[x][l_{a_i}]]$ where $x \in [0, 2^b)$**.
...
Starting with the obvious, $x = l_{a_i}$. We also have the restriction that $r_{a_i} \in \texttt{trans}_k(y) \Rightarrow k = \texttt{bc}[r_{a_i}][y]$. Because $r_{a_i}$ is a constant, $k$ depends solely on $y$.

**In summary, the states affected by $a_i$ are all states $\texttt{dp}3[l_{a_i}][y][\texttt{bc}[r_{a_i}][y]]$**.

$y$ has to have the same amount of bits as $r_{a_i} = \texttt{r}(a_i, b)$ which has $\log_2{M} - b$ bits. Therefore, $y$ has $2^{\log_2M - b} = \boxed{\frac{M}{2^b}}$ possibilities.
...
- $\texttt{dp}[i][j][k]$: what was previously referred to as $\texttt{dp3}$
