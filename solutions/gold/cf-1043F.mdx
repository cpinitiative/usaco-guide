...
Computing $\texttt{gcd\_ways}[i]$ from $\texttt{gcd\_ways}[i-1]$ seems to be nigh-imossible.
...
We initially count the raw number of size-$i$ subsets with the binomial, but
