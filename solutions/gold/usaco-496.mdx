...
This problem asks for the minimum number of lanes required such that cows in the same lane never catch up to one another.
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		if (upper_bound(min_ends.begin(), min_ends.end(), end) == min_ends.end()) {
			min_ends.push_back(end);
		} else {
			*upper_bound(min_ends.begin(), min_ends.end(), end) = end;
		}
	}
