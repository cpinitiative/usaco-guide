...
only $\mathcal{O}(n)$ endpoints to consider. Considering we want to find the maximum
...
With this transformation, we can directly apply our max-subarray algorithm of choice to get our answer.

Directly implementing the approach above is enough to pass the problem, and is
...
With this, we can calculate the best contribution as being the best value of
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		if (a == c) {
			...
			// like Kadanes, we 'reset' here or we add on
