...
then $a - b$ must be equal to $d - c$. How can this help us perform MITM on the 
array? 
...
With the information given in the hints, we can do MITM and consider every possible 
...
left and right sides. In a valid answer, the adjacent attitudes from the left side 
must be the opposite of the adjacent differences from the right side. 
With this observation, we can find the best pairing for each possible
result from the left side in $\mathcal{O}(\log{N})$.

## Implementation

**Time Complexity:** $\mathcal{O}(3^{\left \lceil{\frac{N}{2}}\right \rceil}
\log{3^{\left \lceil{\frac{N}{2}}\right \rceil}})$
...
int main() {
	...
	const auto calc_subarray = [&](int l, int r) -> vector<array<int, 4>> {
		...
		for (int i = 0; i < max_mask; i++) {
			...
			res[i] = {subtracted[1] - subtracted[0], subtracted[2] - subtracted[1], subtracted[0], i};
		}
		return res;
	};
	...
	const int INF = 1e9;
	/** @return final attitude toward the hero, or -INF if they aren't all the same */
	const auto calc_attitude = [](const array<int, 4> &x, const array<int, 4> &y) -> int {
		if (x[0] != -y[0] || x[1] != -y[1]) {
			return -INF;
		}
		return x[2] + y[2];
	...
	for (auto &i : left) {
		...
		auto it = lower_bound(begin(right), end(right), 
													array<int, 4>{-diff_1, -diff_2, INF, 0});
