...
of the occurences of the parent region as a supervisor of the region of our
...
int main() {
	...
	for (int i = 0; i < q; i++) {
		...
		if ((int)comp[e1].size() >= BLOCK) {
			...
			int total = 0;
			for (int u : comp[e1]) {
				total += lower_bound(begin(comp[e2]), end(comp[e2]),
				                   tout[tin_node[u]]) -
				       lower_bound(begin(comp[e2]), end(comp[e2]),
				                   tin[tin_node[u]]);
			}
			cout << total << '\n';
		}
