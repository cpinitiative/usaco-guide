...
To count the $\text{number of previous segments that don't intersect} segment_k$, we use an indexed set that stores the right endpoints of previous segments. We can use its `order_of_key` function to determine the number of previous segments that end before our left endpoint.
