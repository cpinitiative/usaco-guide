...
We process the segments one-by-one. Let the complexity sum of the first $k$ segments be $c_k$. We'll use $c_k$ to calculate $c_{k+1}$ in a process similar to dynamic programming. Our answer would then be $c_N$.
