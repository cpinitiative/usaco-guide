...
Notice the constraints on the input, $\mathcal{O}(N \cdot K)$ is feasible. Meaning, for each of the $N$ cows, we need to calculate the current sum of skill level for them $K$ times.
