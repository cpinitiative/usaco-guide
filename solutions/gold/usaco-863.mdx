...
Note on the constraints of the input, $\mathcal{O}(NK)$ is feasible.

</Spoiler>

<Spoiler title="Hint 2">

We notice that we just need to find the best possible sums for the first $i$ cows as we iterate $i$ from $1$ to $N$.

</Spoiler>

<Spoiler title="Hint 3">

This can be solved with dynamic programming. Keeping a DP array, where ith element represents the best possible sums from 1 to ith cow.
