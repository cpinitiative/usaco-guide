...
Notice the constraints on the input, $\mathcal{O}(N \cdot K)$ is feasible. Meaning, for each of the $N$ cows, we need to calculate something for them $K$ times.

</Spoiler>

<Spoiler title="Hint 2">

We notice that we just need to find the best possible sums for the first $i$ cows as we iterate $i$ from $1$ to $N$. Notice this nature of computing solutions fits with the idea of dynamic programming.
