...
public class MoortalCowmbat {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < M; i++) {
			for (int j = 0; j < M; j++) { cost[i][j] = io.nextInt(); }
		}
		...
		for (int idx = K - 1; (idx < 2 * K - 1) && (idx < N); idx++) {
			...
			// Find a letter 'j' with minimum cost to change all the letters to.
			ans[idx] = dp[idx][0];
			for (int j = 1; j < M; j++) { ans[idx] = Math.min(ans[idx], dp[idx][j]); }
		}
	...
	// count(i, j, k) is the number of appearances of letter k between index i
	// and j.
