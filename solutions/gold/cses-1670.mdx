...
<Spoiler title="Hint">
Think of all $9!$ possible configurations of the board as the nodes, and our moves
as the edges that connect them. We then perform BFS, starting at our intial given
configuration.
</Spoiler>

The difficulty of this problem is with how we choose to implement the BFS. We present
two solutions with the first being faster and the second being easier to write (note that the second solution may TLE)
TLE).
...
<Warning>
While this solution is much easier to implement, it is extremely difficult to write an implementation that does not TLE in Java or Python. It may also even TLE in C++ if written inefficiently.
</Warning>
## Explanation
Since there are only $9!$ states the grid can be in, we can use BFS to find the
number of moves needed to reach the $\texttt{target}$ from the starting grid. We
...
We then perform BFS normally but with a few notable changes. First, since we can't have
strings be the indices of an array, we store the "depth" of each string (number of moves required to reach a board configuration) inside our queue.

In addition, we store the strings (board configurations) we have already seen inside an **unordered_set**, as using normal (ordered) sets will TLE.
<Info title="Significance of using unordered set">
As we will learn in the [hashing module](/gold/hashmaps), unordered set does searches and insertions in constant time despite a large constant factor while a normal set does this in $\mathcal{O}(\log n)$. Since we are dealing with up to $9!$ terms, it is necessary to use unordered set.
</Info>
...
const int fact[9] = {1, 1, 2, 6, 24, 120, 720, 5040, 40320}, fact9 = 362880;
unordered_set<string> visited;

// Defined globally to be used in processwap
queue<pair<string, int>> q;
int moves;
string curboard;

// Processing swapping the numbers in x, y positions
void processwap(int x, int y) {
