...
As we will learn in the [hashing module](/gold/hashmaps), unordered set does searching and inserting in $\mathcal{O}(1)$ albiet with a large constant factor. A normal set does this in $\mathcal{O}(\log n)$, so since we are dealing with up to $9!$ terms, it is necessary to use unordered set.
</Info>
