...
We then perform BFS normally but with a few notable changes. Since we cannot have
our strings be the indices of an array, we store the "depth" of each string (number of moves required to reach a board configuration) inside our queue.
