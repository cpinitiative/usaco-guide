...
Rather than looking at the problem as a 2d array. Try to represent the problem in graph terms.

</Spoiler>

<Spoiler title="Hint 2">

We can turn each $(n+1)(m+1)$ into a node, and the edge weight between the four surrounding nodes their shared side length.
...
int main() {
	...
	sort(edges.begin(), edges.end(), [](const Edge &t, const Edge &y) {
		return t.cost < y.cost;
	});

	long long ans = 0;
	for (Edge &i : edges) {
		if (dsu.unite(i.i, i.j)) {
			ans += i.cost;
		}
	}
	cout << ans << endl;
