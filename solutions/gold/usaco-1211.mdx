...
We can figure out this problem require the [Kruskal's MST algorithm](https://usaco.guide/gold/mst#kruskals) in some way. We know this because it asks to construct a minimum spanning tree among the cows, and a minimum spanning tree uses the shortest edges to connect all nodes.

</Spoiler>

<Spoiler title="Hint 2">

We soon find out that to pick out the best edges for the minimum spanning tree, we first have to establish a fully connected graph containg $N^2$ edges. 

However, there exists a workaround that can allow us to establish less edges at beginning while ensuring all the edges that should belong to the MST are not filtered out. 

Maybe we can somehow exploit the y constraint $0 \le y_i \le 10$ given to each cow?

</Spoiler>

<Spoiler title="Hint 3">

Given the coordinates of three cows. Can we somehow use the triangle inequality theorem to our advantage to filter out an edge?
