...
![el arbol](solutions/gold/usaco-1019/greph.png)
...
It can be proven the maximum number of divisors for an integer up to the constraints of $N$ will be small enough for us to run an linear time algorithm for each divisor (The upper bound is around $N^{o(1)}$, using the [little o notation](https://en.wikipedia.org/wiki/Big_O_notation#Little-o_notation), with the exact formula [here](https://math.stackexchange.com/questions/1053062/upper-limit-for-the-divisor-function/1053070#1053070). Additionally, a table for the upper bound of the number of divisors can be found [here](https://wwwhomes.uni-bielefeld.de/achim/highly.txt)).
