...
int main() {
	cin >> n >> x;
	vector<int> s(n);
	// Notice we only need to store 2 rows!
	vector<vector<ll>> dp1(n + 1, vector<ll>(x + K + 1)); // the subarray we're currently at
	vector<vector<ll>> dp2(n + 1, vector<ll>(x + K + 1)); // the next subarray (we'll fill this up using dp1)
