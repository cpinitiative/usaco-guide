...
Note that a cow can move along exactly those edges whose elevation difference is at most $D$. So the cells reachable from a starting point are the cells in its connected component in this graph.

If we add edges in increasing order of weight, then components grow exactly in the order their required difficulty increases. So, the first time a connected component reaches size $T$, every starting point inside it has found its minimum valid difficulty.
...
waypoints = [i*M + j for i in range(N)  for j in range(M) 
				if waypointGraph[i][j] == 1]
...
for a, b, d in edges:
    ...
    if sizes[root] >= T:
        total += counts[root] * d
        counts[root] = 0   # those starting points have now been answered
