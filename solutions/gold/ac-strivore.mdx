...
The answer doesn't depend on the content of the string.

</Spoiler>

<Spoiler title="Hint 2">

Say we fix the positions of the string's characters relative to the inserted ones.
...
// adapted from https://usaco.guide/gold/modular?lang=cpp#solution---exponentiation
...
int main() {
	...
	for (int i = 1; i <= char_num; i++) {
		place_num.push_back(
			place_num.back() * (i + str.size() - 1) % MOD * mod_inv(i) % MOD
		);
	}
...
public class Main {
	...
	// adapted from https://usaco.guide/gold/modular?lang=java#solution---exponentiation
