...
struct Pasture {
	...
	int index;
	bool had_hay;
	bool operator>(const Pasture &other) const { return cost > other.cost; }
};
...
int main() {
	...
	// store yumminess for each haybale
	vector<int> hay(n, 0);
	for (int i = 0; i < k; i++) {
		int index, yumminess;
		cin >> index >> yumminess;
	...
	// Priority Queue stores tuple of cost to reach a node, the node index, and whether
	// cow had hay or not
	priority_queue<Pasture, vector<Pasture>, greater<Pasture>> pq;

	/*
	 * dist[i][0] = shortest distance from cow i to barn without stopping for any
	 * haybale dist[i][1] = shortest distance from cow i to barn with stopping for one
	 * haybale
	 */
