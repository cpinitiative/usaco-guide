...
Consider a range $A[i\dots j]$. Since elements need to be adjacent to be merged and each new element will originate from a series of merges on a subarray, we can think about the subarrays of $A[i\dots j]$.
