...
int main() {
	...
	// best_path[r][c] keeps track of which cells can lead to the best lexicographical result
	...
	for (int i = 0; i < path_len - 1; i++) {
		...
		// Pass 1: Look at all neighbors of cells currently marked as "best" to find the smallest character available for the next step.
		...
		// Pass 2: Now that we know the best character, we mark all cells on the next diagonal (i + 1) that possess this character and are reachable.
		for (int r = 0; r <= min(n - 1, i); r++) {
			int c = i - r;
			if (c >= n || !best_path[r][c]) continue;
