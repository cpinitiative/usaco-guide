...
int main() {
	...
	// best_path[r][c] keeps track of which cells can lead to the
	// best lexicographical result
	...
	for (int i = 0; i < path_len - 1; i++) {
		...
		// Pass 1: Look at all neighbors of cells currently marked as "best"
		...
		// Pass 2: Now that we know the best character, we mark all cells on the
		...
		for (int r = 0; r <= min(n - 1, i); r++) {
			...
			if (r + 1 < n && grid[r + 1][c] == best) { 
				best_path[r + 1][c] = true; 
			}
			if (c + 1 < n && grid[r][c + 1] == best) { 
				best_path[r][c + 1] = true; 
			}
		}
	}
