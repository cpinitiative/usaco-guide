...
int main() {
	...
	vector<vector<int>> lst(n + 2, vector<int>(n + 2)),
	    lst2(n + 2, vector<int>(n + 2));
	vector<vector<bool>> pus(n + 2, vector<bool>(n + 2));
	vector<int> d(n + 2);
	...
	for (char i = 'z'; i >= 'a'; --i) {
		for (int R = 1; R <= n; ++R) {
			...
			for (int S = 1; S <= n; ++S) {
				...
				while (ss && lst[R - 1][S] >= lst[R][d[ss]]) { --ss; }
				if (mat[R][S] == i) {
					ans += ss;
					if (ss && d[1] == S) { --ans; }
				}
				while (ss && lst[R][S] >= lst[R][d[ss]]) { --ss; }
				if (lst[R][S] <= (R - 1) && lst[R][S] != 0) { d[++ss] = S; }
			}
		}
		for (int R = n; R >= 1; --R) {
			ss = 0;
