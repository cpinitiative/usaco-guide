...
The complexity of this approach is still $\mathcal{O}(n^2)$ for each letter, which gives us an overall complexity of $\mathcal{O}(\sigma \cdot n^2)$, where $sigma = 26$ is the length of the alphabet.
