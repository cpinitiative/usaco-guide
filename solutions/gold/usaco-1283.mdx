...
### Case 2: $A_i > A_{i + 1}$
Let's say $A$ is now $[4, 6, 4]$. Let's take a valid path for $A_1...A_2$ (red) and transform it into one for $A_0...A_2$:
...
// n choose k = n!/[(n - k)!k!]
ll C(ll n, ll k) { return F[n] * I[n - k] % M * I[k] % M; }
signed main() {
	cin.tie(0)->sync_with_stdio(0);
	F[0] = I[0] = 1;
	// F[i] = i!, I[i] = inverse of i!
	for (int i = 1; i < A; i++) {
		F[i] = F[i - 1] * i % M;
