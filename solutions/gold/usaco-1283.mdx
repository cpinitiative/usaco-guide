...
If we construct a path that satisfies constraints $A_{i + 1}\dots A_n$, how can we get a path that satisfies $A_i\dots A_n$?

</Spoiler>

<Spoiler title="Solution">

Consider recursively constructing a solution. If we construct a path that satisfies constraints $A_{i + 1}\dots A_n$, how can we get a path that satisfies $A_i\dots A_n$? And if we know $C_{i + 1}$, the number of paths that satisfy $A_{i + 1}\dots A_n$, how do we get $C_i$?

### Case 1: $A_i \leq A_{i + 1}$

Let's say $A = [4, 6, 2]$. If we take a path that satisfies $A_1\dots A_2$ (*red*), how can we extend it to a path that satisfies $A_0\dots A_2$?
...
To generalize, assuming that our constructed path for $A_{i + 1}\dots A_n$ minimizes turns, this also means any path we construct in the above way for $A_i\dots A_n$ will minimize turns as well. For $C_i$'s relationship to $C_{i + 1}$, note that in the above example, we have 2 "units" (moves 1, 2 and moves 3, 4) of which we need to pick 1: we already have 1 unit, the two blues, and we need $B_i = \frac{A_i}{2}$ units total. Thus in general, we will have to pick $B_i - 1$ units from $B_{i + 1} - 1$ units, which gives us that $\boxed{C_i = {B_{i + 1} - 1 \choose B_i - 1} * C_{i + 1}}$.

### Case 2: $A_i > A_{i + 1}$

Let's say $A$ is now $[6, 4, 6]$. Let's take a valid path for $A_1\dots A_2$ (red) and transform it into one for $A_0\dots A_2$:
