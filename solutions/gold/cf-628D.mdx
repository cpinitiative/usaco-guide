...
2. Let $\texttt{count\_magic}(x)$ define all the magic numbers $\leq x$. The frequency of magic numbers in the range $[a, b]$ is equivalent to $\texttt{count\_magic}(b) - \texttt{count\_magic}(a - 1)$.
