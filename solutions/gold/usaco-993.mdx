...
int main() {
	...
	dp[0][0] =
	    0;  // Base case: Start at city 0 (originally city 1) on day 0 with 0 moonies
...
public class Main {
	public static void main(String[] args) throws IOException {
		...
		final int MAX_DAYS = 1000;  // Maximum number of days Bessie can travel

		// dp[t][i]: max moonies at city i on day t
		int[][] dp = new int[MAX_DAYS][n];
		for (int[] row : dp) Arrays.fill(row, -1);
		dp[0][0] = 0;  // Base case: Start at city 0 (originally city 1) on day 0 with 0
		               // moonies
