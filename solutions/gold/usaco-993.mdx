...
int main() {
	...
	// DP table: dp[t][i] represents the maximum moonies Bessie can have at city i on
	// day t
	...
	for (int t = 0; t <= MAX_T; t++) {
		...
		// After processing all cities for day t, calculate the profit if Bessie returns
		// to city 1
...
public class TimeIsMooney {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 1; i <= n; i++) { moonies[i] = Integer.parseInt(st.nextToken()); }
		...
		// DP table: dp[t][i] represents the maximum moonies Bessie can have at city i
		// on day t
		long[][] dp = new long[MAX_T + 1][n + 1];
		for (int t = 0; t <= MAX_T; t++) { Arrays.fill(dp[t], -1); }
		dp[0][1] = 0;  // Base case: Start at city 1 on day 0 with 0 moonies
		...
		for (int t = 0; t < MAX_T; t++) {
			for (int i = 1; i <= n; i++) {
				...
				for (int neighbor : adj.get(i)) {
					if (t + 1 <= MAX_T) {
						// Update dp[t + 1][neighbor] by considering a transition from
						// an adjacent city on the previous day
						dp[t + 1][neighbor] =
						    Math.max(dp[t + 1][neighbor], dp[t][i] + moonies[neighbor]);
					}
				}
			}

			// After processing all cities for day t, calculate the profit if Bessie
			// returns to city 1
	...
	for i in range(1, n + 1):
		...
		for neighbor in adj[i]:
			if t + 1 <= MAX_DAYS:
				...
				dp[t + 1][neighbor] = max(
					dp[t + 1][neighbor], dp[t][i] + moonies[neighbor]
				)
