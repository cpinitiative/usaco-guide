...
We can solve the problem by simulating each test case and recalculating the summations for each test case in $\mathcal{O}(TK)$. However, this can easily be sped up to $\mathcal{O}(T^2 + K)$ by pre-calculating $p_i$ using Pascal's identity and by precalculating the prefix sums of both $p_i$ and $p_i i$ so we can get the values of the summations in $\mathcal{O}(1)$ instead of $\mathcal{O}(T)$. This gives us the first $9$ test cases but is not good enough.

In order to fully solve the question, we need to remove the $\mathcal{O}(K)$ factor. We can do this if we are able to process multiple values of $E_x$ in a single step.
...
$E_x = a \cdot (a \cdot \dots (a \cdot E_{x - k} + b) \dots + b) + b$

Simplifying gives us a geometric sequence.

$E_x = a^k \cdot E_{x - k} + a^{k - 1}b + a^{k - 2} + \dots + b$
...
Since there are at most $T$ values of $E_x$, $a$ and $b$ can change at most $T$ times so the resulting time complexity will be $\mathcal{O}(T^2 + T \log K)$.
