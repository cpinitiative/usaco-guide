...
author: Benjamin Qi, Óscar Garries, Tirth Gohil
---
...
<JavaSection>
We are using Binary Indexed Tree Approach for Java as Segment Tree will result in TLE.
```java
import java.io.*;
import java.util.*;
 
// Utilizing the Fact that Total Unique Number of Salaries Including the Queries and Input Array is no more than n (2e5) + 2 * q (2e5) which is 6*10^5.

// So we don't have to worry about salary in range 1e9, we will compress it.

public class Main { static Reader fr = new Reader(); static PrintWriter out =
