...
struct Fenwick{
    ...
    void add(int x, int val){
        for(;x<n;x=x|(x+1))mn[x]+=val;
    }
    int sumt(int r){
        int s = 0;
        for(;r>=0;r=(r&(r+1))-1)s+=mn[r];
        return s;
    }
    int sum(int a, int b){return sumt(b)-sumt(a-1);}
};
...
signed main(){
    ...
    vector<int> allap; //all coordinates to be compressed
    map<int, int> compr;
    vector<int> p(n+1);
    for(int i = 1;i<=n;i++){
        cin>>p[i];
    ...
    for(int i = 0;i<q;i++){
        char c;cin>>c;
        int a, b;cin>>a>>b;
        allap.push_back(a);allap.push_back(b);
        queries.push_back({c, {a, b}});
    }

    sort(allap.begin(), allap.end());
    int nn=allap.size()+1;
    Fenwick fenw(nn);

    for(int i = 0;i<nn-1;i++)compr[allap[i]]=i;
    for(int i = 1;i<=n;i++){
        p[i]=compr[p[i]];
        fenw.add(p[i], 1);
    }
    for(auto thing:queries){
        if(thing.first=='!'){
            int ts=compr[thing.second.second];
            fenw.add(p[thing.second.first], -1);
            fenw.add(ts, 1);
            p[thing.second.first]=ts;
        }
        else{
            cout << fenw.sum(compr[thing.second.first], compr[thing.second.second]) << '\n';
        }
    }
}
```
