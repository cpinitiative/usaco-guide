...
int main() {
	...
	cin.tie(nullptr);
	int N;
	string cur, prev;  // we make sure each pair of consecutive strings are in
	                   // the right order

	cin >> N >> prev;  // no previous string to compare the first string to, so
	                   // read directly into prev
	for (int i = 1; i < N; i++) {
		cin >> cur;

		int leng =
		    min(prev.length(), cur.length());  // maximum length to compare
		int j;
		for (j = 0; j < leng; j++) {
			if (prev[j] != cur[j]) {
