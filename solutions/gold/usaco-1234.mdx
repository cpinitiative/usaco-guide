...
bool zero[N][N];
// number of nonzero expressions in I(i, j)
ll dp[N][N];
...
signed main() {
    ...
    while (t--) {
        ...
        dp[0][0] = 0;
        // |I(i, 0)| = |I(0, i)| = 1 -> # of nonzero expressions = 1 - # of zero expressions
        for (int i = 1; i <= n; i++) {
            dp[0][i] = !zero[0][i];
            dp[i][0] = !zero[i][0];
        }
