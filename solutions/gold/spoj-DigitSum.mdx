...
source: SPOJ
title: Digit Sum
authors: Chongtian Ma, Denys Zhyrnov
---
...
**Time Complexity:** $\mathcal{O}(n^2)$ for each query, where $n$ is the length of the number.
...
Define $\mathtt{dp[i][b]}$ to be the sum of digits and the amount of different numbers we can make in suffix starting from position $i$, $b$ is the same freedom value as in the solution above.
...
array<ll, 2> calc(int pos, bool is_free) {
	...
	for (int d = 0; d < 10; d++) {
		...
		// Add contribution of the current digit and total sum of digits after
		// current
