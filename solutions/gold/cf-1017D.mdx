...
We can first precompute the wu values for each pair of strings. Then, we can prefix sum up the number of strings that gives each wu value of each string and utilize binary search. Note that in the implementation, 
...
#include <bits/stdc++.h>
using namespace std;
const int INF = 1e9;  // arbitrarily large integer
...
int main(){
	 ...
	 */
	vector<vector<pair<int, int>>> pref_wu(1 << n, vector<pair<int, int>>(1 << n));
	for (int i = 0; i < (1 << n); i++) {
		for (int j = 0; j < (1 << n); j++) {
			pref_wu[i][j] = {wu.back() - wu[i ^ j], str_cnt[j]};
		}
	...
	for (int query = 0; query < q; query++) {
		...
		// binary search for the least element with count greater than k
		auto it = upper_bound(pref_wu[val].begin(), pref_wu[val].end(), make_pair(k, INF));
