...
Now, we can phrase the problem as:
...
To compute the distances from each node, we'll calculate "inside" and "outside" distances for each node.
...
We can solve this with a simple [DFS](/silver/graph-traversal) through the tree, since the inside value of the parent is just the inside value of the child (plus a slash).
</Spoiler>

<Spoiler title="Calculating outside distances">
Surprisingly, we can also solve this with a DFS!

Note that if we have calculate the outside value for a node's parent, we have all the information we need to calculate itself as well.
