...
The total length for a specific permutation is:  $$|S_{p_1}| + \sum_{i=2}^{k} (|S_{p_i}| - \texttt{overlap}(S_{p_{i-1}}, S_{p_i}))$$
...
int getMergeLength(HashedString &a, HashedString &b) {
	...
	for (int k = 0; k < b.size; ++k) {
		if (r < 0) { break; }
		// Compare the hash of the suffix of a and the prefix of b with length
		// k+1
...
int main() {
	...
	sort(v.begin(), v.end(),
	     [](HashedString a, HashedString b) { return a.size < b.size; });
