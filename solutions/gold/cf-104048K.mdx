...
Let $overlap(A, B)$ be the length of the longest suffix of $A$ that is equal to a prefix of $B$. The cost (additional characters needed) to append $B$ after $A$ is $|B| - overlap(A, B)$.

We can precompute these overlap costs for every pair of strings $(i, j)$. Using rolling hashes, we can check equality of suffixes and prefixes in $O(1)$ after $O(L)$ preprocessing.
...
$$\lvert S_{p_1}\rvert + \sum_{i=2}^{k}\Bigl(\lvert S_{p_i}\rvert - \operatorname{overlap}(S_{p_{i-1}}, S_{p_i})\Bigr)$$
...
1.  **Preprocessing Hashes:** $O(N \cdot L)$, where $L$ is the maximum length of a string.
2.  **Filtering Substrings:** We compare every pair. $O(N^2 \cdot L)$.
3.  **Overlap Calculation:** We compute overlaps for all pairs. $O(N^2 \cdot L)$.
4.  **Permutations:** There are $N!$ permutations. For each, we sum the costs in $O(N)$. Total: $O(N! \cdot N)$.

The overall time complexity is $O(N^2 L + N! \cdot N)$, which fits well within time limits for small $N$.
...
// Calculates the length added by appending b to a
// (Size of b minus the longest suffix of a matching a prefix of b)
int getMergeLength(HashedString &a, HashedString &b){
    ...
    for (int k = 0; k < b.size; ++k) {
        ...
        if (a.get_hash(r, a.size - 1) == b.get_hash(0, k)){
            mx = k + 1;
        }
        r--;
    }
...
int main(){
    ...
    sort(v.begin(), v.end(), [](HashedString a, HashedString b){ return a.size < b.size;});
    ...
    vector merge_length(n, vector<int>(n));
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            if (i == j) continue;
