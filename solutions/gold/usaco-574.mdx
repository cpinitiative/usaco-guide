...
int main() {
    ...
    for (int i = 0; i <= max_fullness; i++) {
        if (dp[i].first) {
            if (i + orange <= max_fullness) {
                dp[i + orange].first = true;
            }
            if (i + lemon <= max_fullness) {
                dp[i + lemon].first = true;
            }
            dp[i / 2].second = true;
        }
    }
    ...
    for (int i = 0; i <= max_fullness; i++) {
        if (dp[i].second) {
            if (i + orange <= max_fullness) {
                dp[i + orange].second = true;
            }
            if (i + lemon <= max_fullness) {
                dp[i + lemon].second = true;
            }
        }
    }
...
## Alternative Explanation - BFS/DFS
...
int main() {
    ...
    while (!q.empty()) {
        ...
        q.pop();
        if (water) {
            if (fullness + orange <= max_fullness && !visited[fullness + orange].second) {
            ...
            if (fullness + orange <= max_fullness && !visited[fullness + orange].first) {
...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        while (!queue.isEmpty()) {
            ...
            int water = curr[1];
            if (water == 1) {
                if (fullness + orange <= maxFullness && !visited[fullness + orange][1]) {
                ...
                if (fullness + orange <= maxFullness && !visited[fullness + orange][0]) {
...
int max_fullness, orange, lemon;
vector<pair<bool, bool>> visited;  // visited[i][0] is water unused; visited[i][1] is water used

void dfs(int fullness, bool water) {
    if (water) {
        if (visited[fullness].second) {
            return;
        }
        visited[fullness].second = true;
        if (fullness + orange <= max_fullness) {
            dfs(fullness + orange, true);
        }
        if (fullness + lemon <= max_fullness) {
            dfs(fullness + lemon, true);
        }
    } else {
        if (visited[fullness].first) {
            return;
        }
        visited[fullness].first = true;
        if (fullness + orange <= max_fullness) {
            dfs(fullness + orange, false);
        }
        if (fullness + lemon <= max_fullness) {
            dfs(fullness + lemon, false);
        }
        dfs(fullness / 2, true);
    }
}
