...
This means that our answer is $\operatorname{lcm}(s_1, s_2, \dots, s_n)$, as the LCM
...
Note that using the formula $\operatorname{lcm}(a, b) = \frac{a \cdot b}{\gcd(a, b)}$ and
...
$$
\operatorname{lcm}(a, b) = \prod_{p \mid ab} p^{\max(\text{power of } p \text{ in } a, \text{ power of } p \text{ in } b)}.
$$
