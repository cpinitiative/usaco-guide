...
each node has an in-degree and out-degree of one, meaning that our graph is composed
of disjoint cycles. To understand why, observe that a node connected to a cycle without being part
...
after a certain amount of time. Specifically, if $s_i$ denotes the size of the cycle
...
the value modulo $10^9 + 7$, and the LCM can grow far beyond that.
Note that using the formula $\text{lcm}(a, b) = \frac{a \cdot b}{\gcd(a, b)}$ and
iteratively calculating the LCM does not work, as $a$ and $b$ being under modulo means
the modular inverse of the GCD will not be calculated correctly.
...
each $s_i$, and combining it with the LCM of all previous $i$ using the formulation above. We store prime factors in a map and use trivial division to calculate our prime factorization.
