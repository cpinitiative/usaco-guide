...
The intended solution has nothing to do with bitsets.
...
vector<long long> naive_search(vector<int> primes) {
	...
	function<void(long long, int)> dfs;
	dfs = [&](long long prod, int at) {
		ret.push_back(prod);
		for (int i = at; i < primes.size(); i++) {
			// use division in comparison to prevent overflow
			if (MAX_ANSWER / primes[i] < prod) {
				break;
			}
			dfs(prod * primes[i], i);
		}
