...
Root the tree. For an edge from $u$ -> $v$, where $u$ is 
...
// BeginCodeSnip{Binary Indexed Tree (Click to expand)}
...
class Tree {
  ...
  public:
    Tree(int n, vector<array<int, 3>> &edges) 
        : n(n), log2dist(1 + (int)log2(n)), bit(n + 3), 
          tin(n), tout(n), edges(edges), adj(n) {
