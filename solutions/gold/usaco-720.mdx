...
Observe that two cows $i$ and $j$ will intersect cross if
...
$\texttt{posB}[\texttt{A}[i]]$. Then, we can sweep from through array
$\texttt{A}$ starting from $1$, erase the current $\texttt{posB}[\texttt{A}[i]]$
and query for the number of positions less than $\texttt{posB}[\texttt{A}[i]]$
to obtain the number of cows that cross with cow $\texttt{posA}[i]$.

To handle cyclic shifts, note we only care about moving the first cow to the
last position. All other crossings will hold constant. Consider that shifting
