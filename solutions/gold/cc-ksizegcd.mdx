...
$\texttt{max\_size}_{i, \gcd} =$ the maximum size of the subarray ending at $i$ with a GCD of $\gcd$.

Then, we can perform updates in $\mathcal{O}(\log N)$ time by extending subarrays that ended at index $i - 1$ to index $i$.

The answer for length $\texttt{len}$ will be the maximum $\gcd$ for which $\texttt{max\_size}_{i, \gcd} = \texttt{len}$
