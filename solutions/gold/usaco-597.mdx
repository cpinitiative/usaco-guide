...
public class Main {
    ...
    static boolean valid(int pos, int idx, int power, int dir) {
        ...
        if (idx <= 0 && dir == 0) {
            return (idx < 0 || pos - power <= haybales[idx]);
        }
        ...
        if (dir == 0) {  // left
            if (pos - power <= haybales[0]) {
                return true;
            }

            // go as left as possible
            int next_idx = idx;
            while (next_idx >= 0 && pos - power <= haybales[next_idx]) {
                next_idx--;
            }
            ...
            if (pos + power >= haybales[n - 1]) {
                return true;
            }

            // go as right as possible
            int next_idx = idx;
            while (next_idx < n && haybales[next_idx] <= pos + power) {
                next_idx++;
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        haybales = new int[n];
        for (int i = 0; i < n; i++) {
            haybales[i] = Integer.parseInt(br.readLine()) * 2;  // double each position to eliminate decimals
        }
        ...
        while (l <= r) {
            ...
            if (idx < 0) {
                ...
                while (idx < n && haybales[idx] == pos_l) {
                    idx++;
                }
            }
...
with open("angry.in", "r") as f:
    ...
    hay = [int(f.readline().strip()) * 2 for _ in range(n)]  # double positions to avoid decimals
hay.sort()
