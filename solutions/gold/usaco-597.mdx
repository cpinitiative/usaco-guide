	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < n; i++) {
			haybales[i] = Integer.parseInt(br.readLine()) *
			              2;  // double each position to eliminate decimals
		}
    ...
    while j < n and r <= mid:
        if hay[i] + r >= hay[j]:
            ...
            if j - i == 1:
                ...
                # this is because the anchor is the haybale right before it, so we cannot start a new chain
