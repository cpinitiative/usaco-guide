...
Process the blocks from largest to smallest so we can incrementally fill the even positions starting from the bottom and working out way up to the top and also fill in the odd positions with whatever blocks we don't use in the even positions.
