...
To understand why this works, imagine we only have three flavors of cows, and some cows share more than one flavors with each other. If we were to only
...
int main() {
	...
	cout << ((long long)n * (n - 1) / 2) - compatible << endl;
}
```
