...
template <int SZ> struct LCA {
	...
	void dfs(int u, int prev) {
		...
		last_seen_node[T[u]].push_back({cnt, curr_node[T[u]].size() ? curr_node[T[u]].back() : 0});
	}
	...
	int last(int a, int c) {
		auto it = upper_bound(begin(last_seen_node[c]), end(last_seen_node[c]), make_pair(label[a], MOD));
