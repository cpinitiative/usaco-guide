...
template <int SZ> struct LCA {
	...
	vector<int> adj[SZ], cur[SZ];
	vector<pair<int, int>> CUR[SZ];
	...
	void dfs(int u, int prev) {
		...
		cur[T[u]].pop_back();
		CUR[T[u]].push_back({cnt, cur[T[u]].size() ? cur[T[u]].back() : 0});
	}
	...
	int last(int a, int c) {
		auto it = upper_bound(begin(CUR[c]), end(CUR[c]), make_pair(label[a], MOD));
...
template <int SZ> struct HLD {
	...
	int closest_anc(int x,
	                int col) {  // depth of closest ancestor with desired color
		while (x) {
	...
	bool exists(int x, int y, int col) { return closest_anc(x, col) >= depth[y]; }
};
