...
int main() {
	...
	dist[0] = 0;
	// tracks which (dir, coord) pairs have already been fully processed
	...
	while (!q.empty()) {
		...
		if (processed.count({dir, coord})) { continue; }
		processed.insert({dir, coord});
...
# tracks which (direction, coord) pairs have already been fully processed
processed: set = set()
...
while queue:
	...
	if curr_direction:  # is horizontal
		if (True, curr_fp.y) not in processed:
			processed.add((True, curr_fp.y))
			for fp_i in y_lines[curr_fp.y]:
				if dist[fp_i] == -1:
					queue.appendleft((fp_i, not curr_direction))
					dist[fp_i] = dist[curr_idx] + 1
	else:  # is vertical
		if (False, curr_fp.x) not in processed:
			processed.add((False, curr_fp.x))
