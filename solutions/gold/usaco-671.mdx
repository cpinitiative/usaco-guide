...
<Warning>
The video code is worst-case $\mathcal{O}(N^2)$, not $\mathcal{O}(N)$. Each BFS
pop can rescan an entire $x$- or $y$-coordinate group, so a group containing
$\mathcal{O}(N)$ points may be scanned from $\mathcal{O}(N)$ queue states. To
make the solution linear, mark or erase each coordinate group after processing
it once.
</Warning>
