...
Since the last swipe that cleared that was a palindrome, we can take on $i$
		...
		for (int len = 2; len <= gemNum; len++) {
			for (int start = 0; start <= gemNum - len; start++) {
				...
				if (gems[start] == gems[start + 1]) {
					swipes[start][end] = Math.min(swipes[start][end], swipes[start + 2][end] + 1);
				}
			}
		}
