...
[Official Editorial (C++)](https://codeforces.com/blog/entry/22256)
...
int main() {
	 ...
	 */
	vector<vector<int>> swipes(n + 1, vector<int>(n, 0));

	for (int i = 0; i < n; ++i) swipes[i][i] = 1;
		...
		for (int start = 0; start <= n - len; ++start) {
			...
			for (int match = start + 2; match <= end; ++match) {
				if (gems[start] == gems[match]) {
					...
					swipes[start][end] =
					    min(swipes[start][end],
					        swipes[start + 1][match - 1] + swipes[match + 1][end]);
				}
			}
			...
			if (gems[start] == gems[start + 1]) {
				swipes[start][end] =
				    min(swipes[start][end], swipes[start + 2][end] + 1);
			}
		}
	}
