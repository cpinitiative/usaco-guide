...
- Multiply the current number by 10. At this time, the sum of each digit does not change.

- Add 1 to the current number. At this time, the sum of each digit increases
...
can be equated with modK.

Therefore, we only need to find the shortest path from 1 to 0 on a graph of K
vertices, where each vertex is equated with modK, and this can be found in O(K)
time using 01BFS.
...
In reference to this problem here if we go from 9 to 10 we dont need to worry as vertex 10 will already be visited thus we can skip this case

Note that 01BFS is a breadth-first search algorithm that prepares a deque
