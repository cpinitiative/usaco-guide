...
Note that during the contest, the problem was modified due to a bug in the solution. Because the original problem was unsolvable with the given constraints, the problem was modified, but the solution to the modified problem was not posted there.
...
Looping through sequence `a`, we can try to add to each subsequence in `dp`. We can track the current LCIS ending at elements of `b` that are smaller than `a[i]` (I called this `current`), and store its index to extend it if `a[i]` matches a later element (I called this `last`).
