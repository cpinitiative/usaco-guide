...
arr_num = [0]  # index 0 won't be used
pref_sums = [0]
for i in range(1, MAX_VAL + 1):
	if i % 2 == 0:
		arr_num.append((two_pows[i - 1] - two_pows[i // 2]) % MOD)
	else:
		arr_num.append((two_pows[i - 1] - two_pows[(i - 1) // 2]) % MOD)
	pref_sums.append((pref_sums[-1] + arr_num[-1]) % MOD)
