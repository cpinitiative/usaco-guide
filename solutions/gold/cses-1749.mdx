...
array after each removal, which costs $\mathcal{O}(n)$ per query, which
...
All three solutions below process each query in $\mathcal{O}(\log n)$ or $\mathcal{O}(\log^2 n)$, giving $\mathcal{O}(n \log n)$ or $\mathcal{O}(n\log^2 n)$ overall.
...
also access the original index at a position in $\mathcal{O}(\log n)$ time using `find_by_order`.
...
`prefix_sum(index) == k`. Each step of the binary search does a BIT query, giving $\mathcal{O}(\log^2 n)$ per operation.
...
This finds the answer in $\mathcal{O}(\log n)$ by descending exactly one root-to-leaf path.
