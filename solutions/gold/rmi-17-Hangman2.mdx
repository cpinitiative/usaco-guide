...
Consider a $\mathcal{O}(NKmin(N, K))$ solution. What is $min(n, k)$ bounded by?
...
BeginCodeSnip{String Hashing Template}
class HashedString {
  private:
...
int main() {
    ...
    for (int t = 0; t < tests; t++) {
        ...
        if (n < k) {
            ...
            for (int i = 0; i < n; i++) {
                hashed[i] = HashedString(s[i]);
            }

            for (int i = 0; i < k; i++) {
                for (int j = i + 1; j < k; j++) {
                    vector<array<long long, 4>> intervals(n);
                    for (int x = 0; x < n; x++) {
                    ...
                    for (int x = 1; x < n; x++) {
                        ...
                        if (works) {
                            res[intervals[x - 1][3]] = res[intervals[x][3]] = true;
                        }
                    }
                }
            }
