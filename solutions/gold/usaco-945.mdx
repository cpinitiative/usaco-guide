...
We notice that we just need to find the minimum waste for the first $i$ snakes with only $j$ switches as we iterate $i$ and $j$ from $1$ to $N$ and $0$ to $K$. Notice this nature of computing solutions fits with the idea of dynamic programming.
