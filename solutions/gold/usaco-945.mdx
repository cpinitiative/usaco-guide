...
Note on the constraints of the input, setting $K$ upperbound as $N$, $\mathcal{O}(N^3)$ is feasible.
...
This can be solved with dynamic programming. Keeping a 2d DP array, where $(j,i)$th element represents the minimum waste from $1$ to $i$th snake at $j$ switches.
