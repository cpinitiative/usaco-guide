...
A slow way of calculating our answer would be to iterate on our left endpoint, and count 
...
amortizes to $\mathcal{O}(n)$. To understand why, consider the following facts:
...
the number of corresponding right endpoints. For each $i$, we calculate the location of the nearest larger element to the left and right. 
...
ll conta(int n, std::vector<int> a) {
	...
	const auto nearest_bigger = [&](std::vector<int> &stk, int idx) -> int {
		while (stk.size() > 1 && a[stk.back()] < a[idx]) {
			stk.pop_back();
		}
	...
	for (int i = 0; i < n; i++) {
		...
		if (big_r[idx] <= rb) {
			res += 1ll * (idx - lb + 1) * (rb - big_r[idx] + 1);
		}
	}
