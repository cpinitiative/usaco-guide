...
If we frame the problem as traversing outward from the rope at $m$, this condition becomes reversed. To traverse from the rope at $m$ to another rope, the jumps we take must have non-decreasing lengths.

Given the condition for a valid path, there is no case where we would want to reach position $m$ with more initial rope than required. Thus, we can model this problem as a single-source shortest paths problem, expanding outward from the rope at $m$ to calculate the required rope lengths. Naively computing distances takes $\mathcal{O}(N^2 \log{N})$ time.
...
Consider calculating $\text{dist}_l[i]$, with the logic for calculating $\text{dist}_r[i]$ being similar. Observe that:
...
Without loss of generality, assume that $p[i] \leq d$. Then, we can rewrite the condition for
