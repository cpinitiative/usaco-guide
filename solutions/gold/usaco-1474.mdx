...
To compute the answer, we can store a variable $ans$ (initially equal to $0$). Whenever two components of size $x$ and $y$ are linked, add $\binom{x+y}{2}-\binom{x}{2}-\binom{y}{2}$ to $ans$. Whenever the size of a component is changed from $x$ to $x+1$, add $\binom{x+1}{2}-\binom{x}{2}=x$ to $ans$.
