...
* The largest element should be in the "peak" of the pyramid, which may not be in the center of the array.
...
We greedily choose the minimum of these two for each element.
...
For any element $x$, consider the set of all elements larger than it, denoted as $S$.
In a valid pyramid structure, the values increase to a peak and then decrease. This implies that the set of largest elements $S$ must form a *contiguous block* in the center of the array, and $x$ must be placed outside this block (either to the immediate left or immediate right).
...
* Left Cost: The number of swaps needed to move $x$ to the left of $S$ is exactly the count of elements in $S$ that are currently at indices *less than* $x$'s index.
* Right Cost: The number of swaps needed to move $x$ to the right of $S$ is exactly the count of elements in $S$ that are currently at indices *greater than* $x$'s index.
...
This reduces to dynamically counting the number of inversions. We model the array as a binary sequence where a *1* represents an already processed (larger) element.
* $L$ is the number of already processed (larger) elements located to the left of the current index.
* $R$ is the number of already processed (larger) elements located to the right of the current index.
* **Query:** To find $L$, we need the sum of values in the range $[0, i-1]$. Since we have processed $k$ elements so far, we can derive $R$ as $k - L$. This is a **Range Sum Query**.
* **Update:** To mark index $i$ as processed, we change the value at $i$ from $0$ to $1$. This is a **Point Update**

Since we need both operations to be efficient, we use a **Binary Indexed Tree** or similar PURS data structure.
