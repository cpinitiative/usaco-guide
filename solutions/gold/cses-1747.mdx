...
* The largest element $N$ must eventually end up at the "peak" of the pyramid. However, relative to the *other* elements, its final position doesn't strictly matter yet—it just splits the array into a left side and a right side.
...
This is a dynamic counting problem on indices. We can model the state of the array as a sequence of $0$s and $1$s, where a $1$ at index $j$ means the element originally at $j$ has been processed.
...
Since we need both operations to be efficient, a **Binary Indexed Tree (Fenwick Tree)** or a **Segment Tree** is ideal. Both support these operations in $O(\log N)$ time.
...
// Standard Fenwick Tree implementation for Point Update Range Sum
struct FenwickTree {
	int size;
	vector<int> tree;
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		elements[i].second = i + 1; // 1-based indexing for BIT
	}
