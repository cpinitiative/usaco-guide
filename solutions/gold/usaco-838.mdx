...
How do we systematically figure out if some observations are contradicting each other? Try convert them into a directed graph, where you construct an edge from cow node $a$ to cow node $b$ if $a$ is placed infront of $b$ in any of the observations. 
