...
Notice that we can dissect the problem into two parts. The one that deals with sorting out the cows given first $X$ observations, and the one that deals with finding $X$ itself.

</Spoiler>

<Spoiler title="Hint 2">

Assuming we have a device that can tell if the first $X$ observations don't contradict each other. We can binary search the value $X$ in $\mathcal{O}(\log M)$ time.

</Spoiler>

<Spoiler title="Hint 3">

A way to determine if a list of observations don't contradict each other is to convert this into a directed graph problem. Where you construct an edge from cow node $a$ to cow node $b$ if $a$ is placed infront of $b$ in any of the observations. 

</Spoiler>

<Spoiler title="Hint 4">

We observe that to satisfy the strict hierachy of one cow must be ahead of another, the graph constructed must a direct-acylic-graph, a directed graph without cycles!
