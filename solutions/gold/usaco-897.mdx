...
/** Modular exponentiation (from the module) */
...
Due to Python's constant factor, the below solution TLEs on few test cases.
...
read = open("poetry.in", "r").readline
write = open("poetry.out", "w").write
...
rhyme = [ord(read().strip()) - ord('A') for _ in range(m)]
