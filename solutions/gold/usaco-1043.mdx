...
Thus, it'll take $\operatorname{lcm}(\texttt{len}(0), \texttt{len}(1), \dots, \texttt{len}(N - 1))$ to reach their
...
For any remaining cows, we can just cycle them back to themselves. This is because a self-loop is equivalent to removing that cycle from the graph ($\operatorname{lcm}(1, x) = x$),
...
[Official Editorial (C++)](https://usaco.org/current/data/sol_exercise_gold_open20.html)
