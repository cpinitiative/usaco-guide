...
int main(){
	...
	vector<int> sizes(N,0), heavy_index(N, -1), path_index(N, -1)
	, depth(N, 0), parents(N, -1), ans(N, 0), cur_ans(N, 0);
	...
	for(auto &p:cow_proficiency){
		if(heavy_index[p.second] != -1){
			ans[p.second] = segtrees[heavy_index[p.second]].
			range_sum(0, path_index[p.second] + 1);
		}
		...
		while(p.second >= 0){
			if(heavy_index[p.second] != -1){
				...
				if(path_index[p.second] + 1 < (int)heavy_paths[heavy_index[p.second]].size()){
					current_value = segtrees[heavy_index[p.second]].
					range_sum(path_index[p.second] + 1, path_index[p.second] + 2);
					segtrees[heavy_index[p.second]].set(path_index[p.second] + 1, --current_value);
				}
