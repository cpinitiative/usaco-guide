...
class segTree{
	...
	public:
		...
		int query(int l, int r){
			int ans = 0;
			for(l += (int)segtree.size() >> 1, r += ((int)segtree.size() >> 1) + 1; l < r; l >>= 1, r >>= 1){
...
int main(){
	...
	for(auto &p:cow_proficiency){
		if(heavy_index[p.second] != -1){
			ans[p.second] = segtrees[heavy_index[p.second]].query(0, path_index[p.second]);
		}
		...
		while(p.second >= 0){
			if(heavy_index[p.second] != -1){
				...
				if(path_index[p.second] + 1 < (int)heavy_paths[heavy_index[p.second]].size()){
					segtrees[heavy_index[p.second]].update(path_index[p.second] + 1, -1);
				}
