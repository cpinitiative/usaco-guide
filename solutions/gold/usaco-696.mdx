...
using namespace std;
const int mx = 1e5+1;
int n, t = 0, out[mx], in[mx], bit[mx], ans[mx];
vector<int> adj[mx];
...
void dfs(int cur, int prev) {
    in[cur] = t;
    for (int nxt: adj[cur]) if (nxt != prev) dfs(nxt, cur);
    out[cur] = t++;
}
...
int main() {
    ...
    for (int i=0; i<n;) {
        ...
        while (r < n && a[i].first == a[r].first) {
            ans[a[r].second] = sum(out[a[r].second]) - sum(in[a[r].second] - 1);
            r++;
        }

        for (int j=i; j<r; j++) update(out[a[j].second], 1);
