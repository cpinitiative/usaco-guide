...
Note that we can compute the portion with $x$ as a factor independently from the constant in the equation. Thus, we can support $\mathcal{O}(\log n)$ removals by calculating modular inverses, and $\mathcal{O}(1)$ additions and queries.
