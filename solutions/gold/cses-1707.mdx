...
public class GraphGirth {
	...
	static int bfs(int start, List<List<Integer>> adj, int currMin) {
		Arrays.fill(dist, -1);
		...
		while (!q.isEmpty()) {
			...
			for (int next : adj.get(curr)) {
				if (dist[next] == -1) {
					...
					dist[next] = dist[curr] + 1;
					q.add(next);
				} else if (parent[curr] != next) { // if the next cell is not the assigned parent, we have a cycle
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < n; i++) { // suppose the current node is part of the shortest cycle
