...
source: CSES
title: Graph Girth
author: Benjamin Qi, Andi Qu, David Zhou, Chirag Jain
---
...
int main()
{
    ...
    // Run BFS from every node (O(N^2 + NM) = O(N^2) in dense graph)
    for (int start = 1; start <= n; ++start)
    {
        vector<int> dist(n + 1, INF);  // Distance from 'start' node
        vector<int> parent(n + 1, -1); // Track parent to avoid trivial cycle
        queue<int> q;
        ...
        while (!q.empty())
        {
            ...
            for (int neighbor : adj[x])
            {
                ...
                else if (neighbor != parent[x])
                {
                    // Found a cycle: length = dist[x] + dist[neighbor] + 1
                    answer = min(answer, dist[x] + dist[neighbor] + 1);
                }
            }
        }
    }

    // Output -1 if no cycle found, otherwise the shortest cycle length
