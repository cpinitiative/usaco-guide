...
We can BFS from the starting point of A and check if we can reach B. If we reach B, we must have done it in the shortest path possible because BFS visits the nearest cell first.

Note that DFS is incorrect as it effectively tries to perform a brute force simulation of every single possible path from A to B.
