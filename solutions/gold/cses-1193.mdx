...
vis = [[False for _ in range(1000)] for _ in range(1000)]

# previousStep stores the previous direction that we moved in to arrive at this cell
previousStep = [[-1 for _ in range(1000)] for _ in range(1000)]
...
if vis[end[0]][end[1]]:
    ...
    steps.reverse()
    print(len(steps))
    print(''.join(stepDir[step] for step in steps))
else:
    print("NO")
```
