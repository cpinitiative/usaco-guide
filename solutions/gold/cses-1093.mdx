...
Since the two sets have to have an equal sum, it's evident that both of them have to have half of  $1, 2, \dots, N$. So, if $\frac{N(N+1)}{2}$, or the sum of the first $N$ positive integers isn't even, then a solution isn't possible. For each subset, we want its sum to be half of this sum, or $\frac{N(N+1)}{4}$
...
If $\texttt{dp}[i][j]$ = the number of ways to make sum $j$ with the numbers from $1\dots i$, then our transitions will be $\texttt{dp}[i][j] = \texttt{dp}[i - 1][j] + \texttt{dp}[i - 1][j - i]$, if we don't include the current element, or we do.
