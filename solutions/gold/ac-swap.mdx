...
the number of ways to create a string with $K$ `K`s, $E$ `E`s, and
$Y$ `Y`s, given that we've performed $\texttt{cost}$ swaps.
...
$$
dp[\texttt{cost} + \texttt{between}, K + 1, E, Y] \mathrel{+}= dp[\texttt{cost},K,E,Y]
$$
...
**Time Complexity:** $\mathcal{O}(S^3)$<Asterisk>Note that there is an additional constant
factor of $465$ from the $\texttt{cost}$ state in our DP</Asterisk>
...
int main() {
	...
	// stores the number of occurences of a letter on a prefix
	...
	vector<vector<vector<vector<long long>>>> dp(
	    MAX_SWAPS, vector<vector<vector<long long>>>(
	                   count[0] + 1,
			...
			for (int ys = 0; ys <= count[2]; ys++) {
				for (int cur = 0; cur < MAX_SWAPS; cur++) {
					...
					if (es < count[1]) {
						int next = pos[1][es];
						// swap out letters in the way
						int cost = max(0, pref[next][0] - ks) +
						           max(0, pref[next][2] - ys);

						if (cur + cost < MAX_SWAPS) {
							dp[cur + cost][ks][es + 1][ys] +=
							    dp[cur][ks][es][ys];
						}
					}
					...
					if (ys < count[2]) {
						...
						// swap out letters in the way
						int cost = max(0, pref[next][0] - ks) +
						           max(0, pref[next][1] - es);

						if (cur + cost < MAX_SWAPS) {
							dp[cur + cost][ks][es][ys + 1] +=
							    dp[cur][ks][es][ys];
						}
					}

					// all letters have been used + don't exceed threshold for
					// swaps
