...
We can sort the queries from greatest to least weight to process them offline. We can also sort the edges based on the same order. At each query, we can add every edge with a weight greater than or equal to the current query's weight.
