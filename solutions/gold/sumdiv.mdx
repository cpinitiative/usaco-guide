...
int main() {
	...
	if (b == 0) {
		...
		if (a > 1 && ((a - 1) % MOD) != 0) {
			...
			// if a - 1 is divisible by 10^9 + 7, than d - 1 doesn't have inverse.
			// a is a prime of the form k * (10^9 + 7) + 1,
			// by summing the values a^i modulo (10^9 + 7), where i is 0 <= i <= b.
			// the result is (b + 1), so we can just multiply by (b + 1).
			ans = (ans * ((b + 1) % MOD)) % MOD;
		}
