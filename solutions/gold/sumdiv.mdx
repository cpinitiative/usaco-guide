...
$d_1^{B*p_1} \cdot d_2^{B*p_2} \cdot ... \cdot d_k^{B*p_k}$ and we can compute
the sum of divisors using the above formula,
$S = \prod_{i=1}^{k}\frac{d_i^{B*p_i+1} - 1}{d_i -1}$.
...
$\frac{d_i^{B*p_i+1} - 1}{d_i -1} = 1+d_i + d_i^2 + ... + d_i^{B*p_i}$.

which is equal to $(B+1)\%(10^9+7)$, whenever, $d_i\%(10^9+7)=0$
...
**Time Complexity:** $\mathcal{O}(\sqrt{A}*logB)$
...
void ext(ll a, ll b, ll &x, ll &y){ // Extended euclidean gcd algorithm to find modular inverse
	if (b==0){
		x = 1, y = 0;
	} else {
		ext(b, a%b, x, y);
...
ll inv(ll a, ll m){  // finds multiplicative inverse modulo m given, gcd(a,m)=1
	ll x = 0, y = 0;
	ext(a, m, x, y);
	return (x+m)%m;
}
	...
	if (b==0){
		...
		for (ll i = 2; i*i<=a; i++){
			if (a%i==0){
				...
				// maximum power of i which divides a
				ll co = 0;
				while (a%i==0){
					co++;
					a/=i;
				...
				ll dk_1 = (pw(i, (co * (b % (mod - 1)) + 1)%(mod - 1), mod) - 1 + mod) % mod;
				ans = (ans * ((d_1 * dk_1) % mod)) % mod;
			}
		}
		if (a > 1 && ((a-1) % mod) != 0){
			// if a is greater than 1 and is coprime to 10^9+7
			// finds the value of d-1 and (d^k - 1) modulo (10^9+7)
			ll d_1 = inv((a - 1 + mod) % mod, mod);
			ll dk_1 = (pw(a, (b + 1) % (mod - 1), mod) - 1 + mod) % mod;
			ans = (ans * ((d_1 * dk_1) % mod)) % mod;
		} else if (a > 1){
			// if a is not coprime to 10^9+7 , we can just multiply by (b+1)
			ans = (ans * ((b + 1) % mod))%mod;
		}
		fout<<ans<<'\n';
	}
