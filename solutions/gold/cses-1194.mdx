...
# needed later to reconstruct the escape path by backtracking
direction = [[-1] * m for _ in range(n)]

# movement deltas and their letter labels
dir_r = [1, 0, -1, 0]
dir_c = [0, 1, 0, -1]
convert = "DRUL"  # Down, Right, Up, Left

# best border cell the person can reach before any monster
closest_dist = float("inf")
closest_r, closest_c = -1, -1


def bfs(q, dist):
	# multi-source BFS, fills dist with shortest distances from all sources in q
	while q:
		r, c = q.popleft()
		for i in range(4):
			next_r, next_c = r + dir_r[i], c + dir_c[i]
			if (
				0 <= next_r < n
				and 0 <= next_c < m
				and maze[next_r][next_c] != "#"  # skip walls
				and dist[next_r][next_c] > dist[r][c] + 1  # only update if shorter
			):
				dist[next_r][next_c] = dist[r][c] + 1
				direction[next_r][next_c] = i  # record move for path reconstruction
				q.append((next_r, next_c))


def check_valid(r, c):
	# a border cell is a valid escape if the person arrives strictly before any monster
	global closest_dist, closest_r, closest_c
	if (
		maze[r][c] != "#"
		and dist_person[r][c] < dist_monster[r][c]  # person wins the race
		and dist_person[r][c] < closest_dist  # keep track of shortest escape
	):
		closest_dist = dist_person[r][c]
		closest_r, closest_c = r, c
...
for i in range(n):
	for j in range(m):
		if maze[i][j] == "A":
			...
			dist_monster[i][j] = 0  # each monster is its own BFS source
			q.append((i, j))

# flood fill from all monsters at once
bfs(q, dist_monster)

# flood fill from the person
# direction grid is only written here, so monster BFS above intentionally skips it
q = deque([(person_r, person_c)])
bfs(q, dist_person)

# check every border cell for a valid escape
for i in range(n):
	check_valid(i, 0)
	check_valid(i, m - 1)
for j in range(m):
...
if closest_dist == float("inf"):
	...
	# backtrack from escape cell to start using the direction grid, then reverse
	res = []
	r, c = closest_r, closest_c
	while (r, c) != (person_r, person_c):
		idx = direction[r][c]
