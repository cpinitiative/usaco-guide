...
void check_valid(int r, int c) {
    if (maze[r][c] != '#' &&
        dist_person[r][c] < dist_monster[r][c] &&
...
public class Monsters {
    static int n, m;
    static String[] maze;
    static int[][] distMonster, distPerson, direction; // direction is for path reconstruction
    ...
    public static void checkValid(int r, int c) {
        if (maze[r].charAt(c) != '#' &&
            distPerson[r][c] < distMonster[r][c] &&
...
Note that the Python solution is compiled with PyPy to avoid TLE.
```py
# compile with PyPy for AC; otherwise will TLE
...
def check_valid(r, c):
    ...
    if (maze[r][c] != '#' and
        dist_person[r][c] < dist_monster[r][c] and
        dist_person[r][c] < closest_dist):
        closest_dist = dist_person[r][c]
