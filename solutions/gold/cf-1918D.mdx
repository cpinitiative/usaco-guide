...
$$
dp[i]=\min_{x=j, \cdots, i-1} dp[x]+a_i
$$
...
// BeginCodeSnip{Min Segment Tree (from PURS module)}
template <class T>
...
int main() {
	...
	for (int t = 0; t < test_num; t++) {
		...
		for (int& i : arr) {
			std::cin >> i;
			pref_sum.push_back(pref_sum.back() + i);
		}
		...
		/*
		 * @return the lowest index i such that the sum of elements
		 * in the range (i, at) is at most max_sum.
		 * Note that -1 is a valid return value
		 */
		auto lowest_start = [&](int at, long long max_sum) {
			int lo = -1;
			int hi = at - 1;
			int valid = -1;
			while (lo <= hi) {
				int mid = (lo + hi) / 2;
				if (pref_sum[at] - pref_sum[mid + 1] <= max_sum) {
					hi = mid - 1;
					valid = mid;
				} else {
					lo = mid + 1;
		...
		while (lo <= hi) {
			...
			for (int i = 0; i < len; i++) {
				int from = lowest_start(i, mid);
				long long best = from == -1 ? 0 : INT64_MAX;
				// calculate the best starting position to block from
				best = std::min(best, min_cost.range_min(std::max(from, 0), i));
