...
Ignore the fact that we need to print # inversions for each $j = 0, 1, \dots , N$. How can we compute the # inversions just for $j = N$ in $\mathcal{O}(N \log N)$ time?
</Spoiler>

<Spoiler title="Hint 2">
Notice that the inversions is non increasing as $j$ decreases. What information should we gather now, so that we can reduce our answer from $j = N$ to $j = N - 1$?
</Spoiler>

<Spoiler title="Hint 3">
The number of total inversions can be broken down to sum of inversions of each element in the hairline list. How can we precompute the # of inversions each element causes so that as we transition out answer from $j = N$ to $j = 0$, each step takes $\mathcal{O}(1)$ time?
</Spoiler>
...
// The C++ Solution uses Segement Tree instead BIT,
...
// Increase the element at k by x in a segment tree
