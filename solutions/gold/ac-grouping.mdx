...
The goal is to partition a set of $N$ rabbits into any number of disjoint groups to maximize the total compatibility score.

### The Core Idea: Subsets of Subsets

Instead of deciding which group each rabbit belongs to one by one, we think about the problem in terms of **masks**.
...
For every mask $m$, we calculate `sc[m]`, which is the sum of $a_{i,j}$ for all pairs $(i,j)$ such that both &i& and &j& are in the mask.
...
To compute $dp[m]$, we iterate through all possible submasks $s$ of $m$. We treat $s$ as **one individual group** and combine its precalculated score with the optimal solution for the rest of the mask ($m \setminus s$).

$$
dp[m] = \max_{s \subseteq m} \{ dp[m \setminus s] + sc[s] \}
$$

#### Efficiency Note: Iterating Over Submasks

The code uses a specific bitwise trick to iterate over submasks:
`for(int s = m; s; s = (s - 1) & m)`
This loop visits every submask of $m$ exactly once. While a naive approach (iterating all  $2^N$ masks for every mask) would be $O(4^N)$, this specific submask iteration reduces the total complexity to $O(3^N)$.
...
The precalculation takes $\mathcal{O}(N^2 \cdot 2^N)$, and the DP transitions take $\mathcal{O}(3^N)$. For $N=16$, $3^{16} \approx 4.3 \times 10^7$ which comfortably fits within a 1-second time limit.
...
// Use long long as scores can exceed the range of a 32-bit integer
