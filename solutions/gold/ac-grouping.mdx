...
$$
dp[i] = \max_{j \subseteq i} \{ dp[j] + dp[i \oplus j] \}
$$

For an efficient way to iterate over submasks, we use the pattern `for (int j = i; j; j = (j - 1) & i)`. You can read more about this technique in the [Merging Subsets section](/gold/dp-bitmasks#merging-subsets) of the module.
