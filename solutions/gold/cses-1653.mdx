...
We use a DP array where each element corresponds to a bitmask representing which people have already been transported. Each entry in the DP array stores a pair that represents the state of the rides, including the number of rides taken and the weight of the current ride. The bitmask allows us to efficiently represent subsets of people, with the $i$-th bit indicating whether the $i$-th person has been transported.

For each bitmask $mask$, we iterate over all people to determine which person could be the last one added to the subset. By removing the $i$-th person from the current subset $(mask \oplus (1 \ll i))$, we calculate the previous state. If adding the $i$-th person's weight to the current ride does not exceed the weight limit, we update the total weight. Otherwise, we start a new ride, incrementing the number of rides, and set the total weight to that person's weight. At every step, we update $dp[mask]$ only if the new state is better (fewer rides or a lighter last ride).

Finally, when all people are transported $(mask = (1 \ll people) - 1)$, the result is stored in $dp{[(1 \ll people) - 1]}$, which we output as the final answer.
