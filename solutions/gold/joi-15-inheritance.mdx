...
At first glance, it seems Kruskal's would be more appropriate for this problem, so let's try that.
...
runtime of about $\mathcal{O}(MK \alpha(N))$, which is too slow given the bounds.
...
For this to hold, each DSU's connected components must be obtainable from the one after by adding some edges (zero edges is fine too).
...
Using binary search reduces $\mathcal{O}(MK \alpha(N))$ to $\mathcal{O}(M \log K \alpha(N))$, which runs in time.

## Implementation

**Time Complexity:** $\mathcal{O}(NK + M \log K \alpha(N))$
