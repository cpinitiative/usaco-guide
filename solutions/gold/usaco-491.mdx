...
We can BFS from the three points of Bessie, Elsie, and the Barn. By doing so, we determine the distance from these three points to every other point. After, we can loop over all the cells and test it as a meeting location. We determine the minimum energy spent by doing so.
