...
that we always add edges that cost as less as possible. Let the current element
be $a[k]$. There is an edge between $k$ and $k - 1$ if $a[k]$ divides $a[k -
1]$. In that case, we can go further to $a[k - 2]$ and so on until $a[k]$ no
longer divides $a[k - i]$, or when the two elements are connected already. We
apply the same operation in the other direction ($k + 1$, ...). 
