...
// returns all restrictions if valid
...
vector<pair<int, int>> check(int k) {
	...
	for (int i = n; i > 0; i--) {
		...
		if (p[i] && (lr[i].first > R[p[i]] || lr[i].second < L[p[i]])) {
			return {};
		}
		// propagate restrictions to parent
		lr[p[i]] = { max(l[p[i]], l[i]), min(r[p[i]], r[i]) }
	}
...
// for a more detailed explanation of check and solve see above
vector<pair<int, int>> check(int k) {
	vector<pair<int, int>> lr(n + 1);
	for (int i = 1; i <= n; i++) { lr[i] = {L[i] - k, R[i] + k}; }
