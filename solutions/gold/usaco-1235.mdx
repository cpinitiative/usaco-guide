...
Thus, we can build our solution top-down: starting at node 1, we can pick any value satisfying all restrictions found from the `ck` function (in the solution, we simply pick the left endpoint of the range of values for simplicity). Then, if this value of $x$, all descendants are restricted by $[x - k, x + k]$ (in addition to their own restrictions from `ck`).
...
```cpp
void sl(int k) {
	// we have to call ck again to reconstruct l[i] and r[i]
	assert(ck(k));
	// we repurpose l and r to be restrictions
	// on i's descendants rather than ancestors
	// (loop starts from 0 b/c of p[1] = 0)
	for (int i = 0; i <= n; i++) {
		L[p[i]] = max(L[p[i]], l[i]);
		R[p[i]] = min(R[p[i]], r[i]);
		l[i] = 0, r[i] = K;
