...
corner of our chamber. The sliding window approach is as follows: when iterating 
over each row, we maintain an array of monotonic deques for
each column that allows us to find the best chamber for each column. Then,
we keep another monotonic deque that keeps track of the best possible result
from our sliding window of columns.
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m; j++) {
            cin >> grid_sum[i][j];
            grid_sum[i][j] += grid_sum[i - 1][j] + grid_sum[i][j - 1] 
                            - grid_sum[i - 1][j - 1];
        }
    }
    ...
    for (int i = 2; i + d - 1 < m; i++) {
        for (int j = 2; j + c - 1 < a; j++) {
            while (!col_best[i].empty() && c_sum(j, i) <= c_sum(col_best[i].front(), i)) {
    ...
    for (int i = 1; i + a - 1 <= n; i++) {
        ...
        for (int j = 2; j + d - 1 < b; j++) {
            while (!rmq.empty() && c_sum(col_best[j].front(), j) <= c_sum(rmq.back()[0], rmq.back()[1])) {
                rmq.pop_back();
            }
        ...
        for (int j = 1; j + b - 1 <= m; j++) {
            ...
            int nxt = j + b - d;
            if (nxt + d - 1 >= m) { break; }
            while (!rmq.empty() && c_sum(col_best[nxt].front(), nxt) <= c_sum(rmq.back()[0], rmq.back()[1])) {
                rmq.pop_back();
            }
        ...
        for (int j = 2; j + d - 1 < m; j++) {
            ...
            int nxt = i + a - c;
            while (!col_best[j].empty() && c_sum(nxt, j) <= c_sum(col_best[j].back(), j)) {
                col_best[j].pop_back();
            }
