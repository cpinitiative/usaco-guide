...
For Python, we use a ```min_heap```, ```max_heap```, and ```dictionary``` for counts. The ```min_heap``` and ```max_heap``` are for access, while the counts ```dictionary``` determines removal.
