...
In case of splitting the cows into $K$ groups, we simply remove the $K - 1$
...
ll calc_length(ll a, ll b) {
	a++, b++;
	return (a * factor1 + b * factor2) % M;
}

/**
 * Prim's Algorithm for dense graph to build the MST by scanning
 * @return the edges used to construct the MST
 */
	...
	for (int i = 0; i < N; i++) {
		...
		for (int j = 0; j < N; j++) {
			if (!visited[j]) {
				dist[j] = min(
					dist[j], 
					calc_length(min(min_dist_node, j), max(min_dist_node, j))
				);
			}
		}
	}
...
sort all edges in linear time, we might be able to solve it also with Kruskal's
Algorithm as well.
...
/**
 * @return the number of miles cow a and b are willing to walk to see each other
 */
ll calc_length(ll a, ll b) { return (a * factor1 + b * factor2) % M; }
...
int main() {
	...
	for (int i = 0; i < N * (N - 1) / 2; i++) {
		auto &[a, b] = edges[i];
		if (dsu.unite(a, b)) {
			edge_lens.push_back(calc_length(a, b));
		}
	}
