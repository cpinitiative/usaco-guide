...
We keep a running variable $\texttt{r\_{start}}$ which keeps track of the earliest row we can start the fort at and iterate through all possible row endings ($\texttt{r\_{end}}$ in the code). Note that if a swampy area resides in one of the columns themselves, no fort can go through that row, so we must check for that and update $\texttt{r\_{start}}$ accordingly.
