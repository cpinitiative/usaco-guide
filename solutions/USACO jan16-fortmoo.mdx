...
We will use an adaption of 2D Kadane's algorithm to solve this problem.

Iterate through all possible pairs of columns. For each pair, we'll solve the problem with the restriction that the fort must be aligned along these two columns. For each row between these two columns, we will use prefix sums to check if the area between them is clear.

We keep a running variable `r_start` which keeps track of the earliest row we can start the fort at and iterate through all possible row endings (`r_end` in the code). Note that if a swampy area resides in one of the columns themselves, no fort can go through that row, so we must check for that and update `r_start` accordingly.
