...
ll ans;
int N, dp[maxX];  // number of elements, array to keep track of counts for each
                  // divisor
bool b[maxX];
vector<int> primes;
...
void init() {
	...
	for (int i = 2; i * i < maxX; i++)
		if (b[i])
			for (int j = i * i; j < maxX; j += i) b[j] = false;
...
void compute(int x) {
	...
	for (int mask = 0; mask < (1 << K); mask++) {
		...
		// Generate the product of the prime factors according to the current
		// mask
		for (int i = 0; i < K; i++)
			if (mask & (1 << i)) mu *= pf[i];
