...
The following code will also AC because USACO has poo poo test cases, but it is technically $\mathcal{O}(N^2 \log N)$. It exploits the fact that we can break early once we found an index to remove and loop linearly through all the children from high to low. It can be hacked with a suitable graph where one node has a large number of children.
...
bool dfs(int node, int p, int k) {
	...
	// linearily search instead of binary searching
