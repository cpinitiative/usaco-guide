...
In our counting approach, each row eventually assigns one column for `A` and one column for `B`.

A **violation** occurs when a row’s assignments are inconsistent with a valid completion. This can happen in two ways:

- The assigned `A` and `B` columns coincide (both placed in the same cell), or
- A forced `A` or `B` position in the original grid is not respected.

Thus, a violation is any row-level conflict that prevents the assignment from being valid.

The Inclusion–Exclusion step counts configurations where certain rows are allowed to violate this condition, and then corrects the overcounting using alternating signs.
...
The direct implementation evaluates all triples `(i, j, k)`, which gives
$\mathcal{O}(N^3)$ time complexity.

To improve this, fix a value of `i`. After fixing `i`, the remaining part of the formula becomes:
...
- The `j` terms depend only on `j`.
- The `k` terms depend only on `k`.
- There is no interaction between `j` and `k`.

So the double sum factorizes into:

( sum over j ) * ( sum over k )

Each of these can be computed in $\mathcal{O}(N)$.

Since `i` runs at most $N$ times, the total complexity becomes:

$\mathcal{O}(N^2)$

The key idea is that once `i` is fixed, the remaining inclusion–exclusion contributions separate into two independent sums.
