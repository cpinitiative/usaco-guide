...
int main() {
    ...
    // Compute the modular inverse of the largest factorial then do it for all factorials in reverse order
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (colA[i] == -1 && colB[i] != -1 && !hasA[colB[i]]) {
            config_count[1]++;
        }
        if (colA[i] != -1 && colB[i] == -1 && !hasB[colA[i]]) {
            config_count[2]++;
        }
    }
