...
A **violation** occurs when a row is assigned to a column that contradicts one of its fixed constraints.

- If a row contains an `A` at column `c`, it must be assigned to column `c`. Assigning it elsewhere is an **A-violation**.
- If a row contains a `B` at column `c`, it must be assigned to column `c`. Assigning it elsewhere is a **B-violation**.

So, a violation simply means breaking a forced `A` or `B` position in the permutation.
