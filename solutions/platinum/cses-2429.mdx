...
Overcounting happens when there is a violation.

---
### What is a Violation?

In our construction, each row is assigned one column for `A` and one column for `B`.

A **violation** occurs if these two assignments coincide — that is, both `A` and `B` end up in the same cell of a row. Such a configuration is invalid.

The Principle of Inclusion–Exclusion counts configurations where certain rows are allowed to violate this condition, and then corrects the overcounting using alternating signs.
...
- `C0`: Number of rows containing neither `A` nor `B`
- `C1`: Number of rows that contain a `B` in column `c`, where column `c` contains no `A`
- `C2`: Number of rows that contain an `A` in column `c`, where column `c` contains no `B`
- `C3`: Number of columns containing neither `A` nor `B`
- `C4`: Number of columns without `A`
- `C5`: Number of columns without `B`
