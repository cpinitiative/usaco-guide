...
swaps we need to make in order to sort an array in non-decreasing order. 
How can we use this idea to find the minimum value of $k$ needed to sort 
the array?
...
will need to swap them when sorting the array. Thus, the answer is the maximum 
...
indice $i$ there exists a $j$ where $i < j$ and $w_i \leq w_j$, then we can't 
increase the value of any inversion pairs where the second value's index is 
greater than $j$. Thus, we only need to worry about updating the inversion pair 
values on the range $[i + 1, j - 1]$. To find said value of $j$, we utilize a 
monotonic stack.

The most straightforward way to solve the problem from here is to use a lazy 
segment tree to keep track of the best inversion pair value for each indice 
...
Note that a more elegant (although less intuitive) Binary Indexed Tree solution 
