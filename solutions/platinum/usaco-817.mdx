...
class Tree {
  ...
  public:
	Tree(int _q)
	    : q(_q), log2dist(1 + (int)log2(q)), up(log2dist, vector<int>(q)),
	...
	void add(int par) {
		++cur;
		if (par == -2) {
			diameter[cur] = {cur, cur};
			for (int i = 0; i < log2dist; i++) { up[i][cur] = cur; }
			root[cur] = cur;
		} else {
			up[0][cur] = par;
			...
			root[cur] = root[par];
			dep[cur] = dep[par] + 1;
			const auto [a, b] = diameter[root[cur]];
			int dist_a = dist(cur, a);
			int dist_b = dist(cur, b);
			int cur_dist = dist(a, b);
			if (dist_a > cur_dist) {
				diameter[root[cur]] = {a, cur};
			} else if (dist_b > cur_dist) {
				diameter[root[cur]] = {b, cur};
			}
		}
	}
