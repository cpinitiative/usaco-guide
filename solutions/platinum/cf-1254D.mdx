...
We can solve this problem using Heavy-Light Decomposition (HLD) combined with a Fenwick Tree (BIT). The tree is first decomposed into heavy and light edges, and then each node's size and depth are calculated. When updating a node, contributions are added using the BIT, ensuring $\mathcal{O} (\log N)$ complexity per update. To compute the expected value during queries, we aggregate contributions from light vertices and iterate over the heavy vertices.
