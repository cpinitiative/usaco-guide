...
This problem can be solved with a standard sparse segment tree. We keep track 
...
class SparseSegtree {
  private:
    ...
    inline array<int, 2> comb(const array<int, 2> &a, const array<int, 2> &b) {
  ...
  public:
    ...
    inline void range_set(int ql, int qr, int val) {
        range_set(root, 0, n - 1, ql, qr, val);
    }

    inline int walk_right(int mx) {
        return walk_right(root, 0, n - 1, {0, 0}, mx);
    }
};
