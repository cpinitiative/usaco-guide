...
class LazySegtree {
  private:
    ...
    /** pushes down the lazy update on node v to node v's children */
  ...
  public:
    LazySegtree(const vector<int> &arr) 
        : sz((int) arr.size() - 1), t(4 * sz, ID), 
          lz(4 * sz, LZ_ID), vals(arr) {
        build(1, 0, sz - 1);
    }
    ...
    int walk_right(int mx) {
        return walk_right(1, 0, sz - 1, ID, mx);
    }

    /** @return largest prefix sum in the whole array */
    int best_pf() { return t[1][0]; }
};
...
int main() {
    ...
    for (int i = 0; i < (int) tests.size(); i++) {
        const vector<int> &cur = tests[i];
        if (cur[0] == 0) {
            st.range_set(id(cur[1]), id(cur[2] + 1) - 1, cur[3]);
        } else {
            cout << (st.best_pf() <= cur[1] ? n : st.walk_right(cur[1])) << "\n";
        }
    }
}
