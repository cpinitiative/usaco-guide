...
Each node in the segment tree will store the sum of values in its range. Additionally, we need to maintain "lazy tags" to represent pending updates. Since the update is a linear function (affine transformation), our lazy tag will be a pair $(b, c)$ representing the operation $f(x) = b \cdot x + c$.
...
void apply_tag(int node, int l, int r, ll b, ll c) {
	...
	// (b_final, c_final) = (b_new * old_b) * x + (b_new * old_c + c_new)
...
ll query(int node, int l, int r, int ql, int qr) {
	...
	push(node, l, r);
	int mid = (l + r) / 2;
	return (query(2 * node, l, mid, ql, qr) + query(2 * node + 1, mid, r, ql, qr)) %
	       MOD;
}
