...
We can encode a number's digits using bitmasking. Maintain a frequency array of the bitmasks;
afterward compare the masks between them to check if they have bits in common i.e. common digits.
	...
	while (t--) {
		...
		for (int mask = 0; mask < (1 << 10); mask++) {
			...
			for (int mask2 = mask + 1; mask2 < (1 << 10); mask2++) {
				...
				if (mask & mask2) { ans += 1LL * freq[mask] * freq[mask2]; }
			}
		}
		cout << ans << '\n';
	}
