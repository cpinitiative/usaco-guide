...
However, directly calculating binary lifts would take up too much memory. From here, there are two ways to proceed:
...
you start from will end up being inside a cycle. Thus, we can calculate our binary lifts like normal,
...
int main() {
     ...
     * size k + 1 to calculate the next stone
     * the frog will jump to.
    */
    int l = 0, r = k;
    for (int i = 0; i < n; i++) {
        while (r + 1 < n && rocks[r + 1] - rocks[i] < rocks[i] - rocks[l]) {
            ++l, ++r;
        }
        next_stone[i] = (rocks[i] - rocks[l] >= rocks[r] - rocks[i]) ? l : r;
    }
    const int LOG = 64 - __builtin_clzll(m);
    vector<int> res(n);
    ...
    */
    for (int i = 0; i < LOG; i++) {
        if ((m >> i) & 1) {
            for (int j = 0; j < n; j++) {
                res[j] = next_stone[res[j]];
            }
        }
        vector<int> nxt_t(n);
        for (int j = 0; j < n; j++) {
            nxt_t[j] = next_stone[next_stone[j]];
        }
        next_stone = nxt_t;
    }
    for (int i = 0; i < n; i++) {
...
int main() {
    ...
    */
    ...
    /**
     * Calculate the size of the cycles formed.
    */
