...
$1.$ **Splitting the characters:** Ways to divide `f` identical items into `k` non-empty segments (Stars and Bars Problem):

$$
\binom{f - 1}{k - 1}
$$

$2.$ **Merging with existing segments:** Ways to interleave these `k` new segments among the `j` segments already processed:
