...
public class PlanetQueries2 {
	...
	public static void main(String[] args) {
		...
		q = x.nextInt();
		arr = new int[n + 1];
		for (int i = 1; i <= n; i++) { arr[i] = x.nextInt(); }
		binaryLifting = new int[n + 1][20];
		len = new int[n + 1];
		/* 
		 * in preprocess, doing dfs, for each node, to ensure to covering
	...
	static void preprocess() {
		for (int i = 1; i <= n; i++) { binaryLifting[i][0] = arr[i]; }
		/*
	...
	static void dfs(int node, boolean visited[]) {
		...
		/*
		 * head recursion is being used here, in order to start counting the height of the graph, from the 
		 * left nodes of the tree (if one exists...)
		 */
	...
	static void query() {
		...
		for (int i = 0; i < q; i++) {
			...
			/*
			 * in any of the edge cases, for example, (u,v) u being in cycle and v is in tree
		 	 * u will never be able to "jump" to v, and this is precisely being checked in the above if conditions.
			 */
		}
	}
