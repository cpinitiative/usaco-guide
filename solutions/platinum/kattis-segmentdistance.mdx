...
Now if $P_2=P_3$, then the line segments are intersecting. So the shortest distance is 0. (Trivial case)

Otherwise keep repeating the process till you get to a point where $P_n$, $P_{n+1}$, $P_{n+2}$, $P_{n+3}$ where $P_n = P_{n+2}$ and $P_{n+1} = P_{n+3}$. This means that we have reached the last loop. Here we can observe that **always** at least one of $P_n$ or $P_{n+1}$ is an endpoint of a line segment.

Edge Case: This may NOT be true when the line segments are parallel. In that case, the shortest distance can be obtained from a set of infinite points which contains at least one of the endpoints.
