...
[Official Analysis](https://usaco.org/current/data/sol_haybales_platinum_dec15.html)
...
struct LazySegtree {
	...
	array<ll, 2> qry(int v, int tl, int tr, int ql, int qr) {
		if (qr < tl || ql > tr) return ID;
		if (ql <= tl && tr <= qr) { return t[v]; }
		pushdown(v, tl, tr);
		int m = (tl + tr) / 2;
		return join(qry(2 * v, tl, m, ql, qr),
		            qry(2 * v + 1, m + 1, tr, ql, qr));
	}

	/** @return {sum, minimum value} of haybales in [ql, qr] */
	array<ll, 2> qry(int ql, int qr) { return qry(1, 0, sz - 1, ql, qr); }

	void upd(int v, int tl, int tr, int ql, int qr, int amt) {
		...
		if (ql <= tl && tr <= qr) {
			...
			pushdown(v, tl, tr);
			int m = (tl + tr) / 2;
			upd(2 * v, tl, m, ql, qr, amt);
			upd(2 * v + 1, m + 1, tr, ql, qr, amt);
			t[v] = join(t[2 * v], t[2 * v + 1]);
		}
	}

	/** adds to every haybale in the range [ql, qr] */
	void upd(int ql, int qr, int amt) { upd(1, 0, sz - 1, ql, qr, amt); }
};
