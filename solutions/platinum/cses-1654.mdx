...
This results in **$O(n^2)$** time, which is too slow for large $n$.

Lets understand the relations:

- $x \mid y = x$  →  $y$ is a **submask** of $x$
- $x \& y = x$  →  $y$ is a **supermask** of $x$
- $x \& y \ne 0$  →  total minus masks **disjoint** with $x$
...
- $dp[\text{mask}]$ → number of submasks of $mask$
- $kp[\text{mask}]$ → number of supermasks of $mask$

This preprocessing runs in **$O(20 \cdot 2^{20})$**.
