...
[Official Analysis (Java)](https://usaco.org/current/data/sol_disrupt_platinum_open18.html)
...
We can optimize this by using small-to-large merging when merging the sets of child nodes. Each insertion moves an element to a set at least twice as large as before, so each element is inserted at most $\mathcal{O}(\log M)$ times, meaning that the time complexity of the DFS is $\mathcal{O}(N + M \log^2 M)$.
