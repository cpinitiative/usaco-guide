...
## Implementation
The code below runs in $O(N \log N + M \log^2 M)$. Time complexity of similar solutions may vary based on how LCA is implemented.
...
struct LCA {
	...
	LCA(Tree& adj, int root = 0): n(adj.size()), k(bit_width(2*n-1)), root(root), first(n), depth(n), parent(n), st(k) {
		tour.reserve(2*n-1);
		dfs(adj, root, -1);
