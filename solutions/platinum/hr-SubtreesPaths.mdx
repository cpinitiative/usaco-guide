...
The explanation is similar to the one mentioned [here](https://usaco.guide/plat/hld?lang=cpp#template).
...
struct LST {
	...
	int query(int n, int b, int e, int i, int j) {
		...
		int mid = (b + e) >> 1;
		return combine(query(2 * n, b, mid, i, j), query(2 * n + 1, mid + 1, e, i, j));
	}
} lseg;
// EndCodeSnip
