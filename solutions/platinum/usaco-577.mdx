...
Modifying the algorithm for "lower mode" is left as an exercise to the reader.
...
struct segtree{
	...
	item merge(item a, item b){
		...
		return {a.ans + b.ans + take, a.bessie + b.bessie - take, a.elsie + b.elsie - take};
	}
	...
	int query(){
		...
		for(int l = n, r = 2 * n; l < r; l /= 2, r /= 2){
			if(l % 2 == 1){
				left_item = merge(left_item, seg[l++]);
			}
			if(r % 2 == 1){
				right_item = merge(seg[--r], right_item);
			}
		}
...
int main(){
	...
	const segtree::item BESSIE_CARD = {0, 1, 0}, ELSIE_CARD = {0, 0, 1}, RESET = {0, 0, 0};
    ...
    // sort to remove smallest cards first for "lower mode"
