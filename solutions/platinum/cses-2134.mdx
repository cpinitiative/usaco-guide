...
public class PathQueriesHLD {
	...
	static void modify(int pos, int val, int node, int left, int right) {
		...
		if (pos <= mid) modify(pos, val, node * 2, left, mid);
		else modify(pos, val, node * 2 + 1, mid + 1, right);
	...
	static void decompose(int curr, int parent, int head) {
		...
		for (int child : graph[curr]) {
			if (child == parent || child == heavy) continue;
	...
	static int findLCA(int u, int v) {
		...
		for (int level = LOG - 1; level >= 0; level--) {
			if (depth[u] - (1 << level) >= depth[v]) u = ancestor[u][level];
		}
		for (int level = LOG - 1; level >= 0; level--) {
	...
	static int queryPath(int child, int parent) {
		...
		while (child != parent) {
			if (chain[child] == child) {
				...
				result = Math.max(result, getMax(position[chain[child]],
				                                 position[child], 1, 1, nodes));
				child = ancestor[chain[child]][0];
			} else {
				result = Math.max(
				    result, getMax(position[parent] + 1, position[child], 1, 1, nodes));
				break;
			}
		}
		return result;
	...
	static class FastScanner {
		...
		int nextByte() {
			if (total == -1) throw new InputMismatchException();
			if (current >= total) {
				current = 0;
				try {
		...
		int nextInt() {
			int c = nextByte();
			while (isWhitespace(c)) c = nextByte();
			int sign = 1;
			if (c == '-') {
				sign = -1;
				c = nextByte();
	...
	static class FastPrinter {
		...
		public FastPrinter() { output = new StringBuilder(); }

		public void write(int num) { output.append(num); }

		public void write(char ch) { output.append(ch); }

		public void write(String str) { output.append(str); }

		public void flush() { System.out.print(output.toString()); }
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= nodes; i++) values[i] = sc.nextInt();
		...
		while (queries-- > 0) {
			...
			if (type == 1) {
				...
				int lca = findLCA(u, v);
				int res = Math.max(Math.max(queryPath(u, lca), queryPath(v, lca)),
				                   values[lca]);
				printer.write(res);
				printer.write(' ');
			}
