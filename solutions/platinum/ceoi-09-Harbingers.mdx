...
void dfs(int u = 1, int p = 0, ll d = 0) {
	...
	if (u == 1) {
		...
		min_time[u] = line_min(V[u]) + d * V[u] +
		              S[u];  // get dp value by querying convex hull
		Line l(
		    -d,
		    min_time[u]);    // construct new line that might be added into hull
		prev_max = stk_max;  // store previous hull size
		prev_pos = stk_max =
		    remove_pos(l);  // update hull size to include new line
		prev_line =
		    stk[stk_max];    // store previous line at replacement position
