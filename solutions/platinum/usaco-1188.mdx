...
Note that we can insert an element into the prefix by swapping it down to the left until it reaches position $i$ where $|a[i] - a[i - 1]| > k$ because its impossible to keep swapping it. However, if $a[i] > a[i + 1]$, it is more optimal to swap it back to the right until it reaches a point where $a[i] < a[i + 1]$.
