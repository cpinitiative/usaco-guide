...
For example: If 2 and 3 want the same thing and 3 and 4 want the same thing it means that 2, 3 and 4 want the same thing. After we determine the components we need to find out every combination of them. To do that we use a knapsack DP (you can find a good tutorial [here](/gold/knapsack)). But it would take $\mathcal{O}(N ^ 2)$ which will cause TLE. In order to speed it up we use a bitset as a knapsack. The final complexity is $\mathcal{O}(N\logN)$.
