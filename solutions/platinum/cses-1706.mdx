...
For example: If 2 and 3 want the same thing and 3 and 4 want the same thing it means that 2, 3 and 4 want the same thing. After we determine the components we need to find out every combination of them. To do that we use a knapsack DP (you can find a good tutorial [here](/gold/knapsack)). But it would take $\mathcal{O}(N ^ 2)$ which will cause TLE. In order to speed it up we use a bitset as a knapsack. The final complexity is $\mathcal{O}(N\log^2N)$.
...
class DSU {  // Disjoint Set Union
  ...
  public:
	...
	int find_size(int x) { return sz[find_root(x)]; }
	int find_root(int x) {
		if (t[x] == x) return x;
		t[x] = find_root(t[x]);
		return t[x];
...
int main() {
	...
	vector<int> sizes;
	for (int i = 1; i <= n; i++) {
		if (ds.find_root(i) != i) continue;
		sizes.push_back(ds.find_size(i));
	}
