...
(Official Analysis)(https://github.com/mostafa-saad/MyCompetitiveProgramming/blob/master/Olympiad/POI/official/2011/editorial/rot.pdf)
...
int main() {
	...
	function<void(int)> dfs = [&](int idx) {
		...
		if (val == 0) {
			...
			inversions[idx] =
			    inversions[idx_l] + inversions[idx_r] + min(way_1, way_2);
		} else if (val > 0) {
			sets[idx].insert(val);
		}
	};
