...
For each edge of the convex polygon, this condition can be represented as a **half-plane**: the set of points lying on the inward side of the edge, shifted inward by distance `d`.

The intersection of all these half-planes represents all points whose distance from every polygon edge is at least `d`.  
If this intersection is non-empty, then such a point exists; otherwise, it does not.

Checking whether this intersection exists can be done using the standard **Half-Plane Intersection** algorithm.


---
### Solution

The answer is a real value, so we use **binary search** on the maximum possible distance `d`.

For a fixed `d`, we check whether there exists a point inside the polygon whose distance from every edge is at least `d`.  
This feasibility check is done by shifting each polygon edge inward by distance `d`, forming a set of half-planes, and verifying whether their intersection is non-empty.

If the intersection exists, then `d` is feasible and we try a larger value.  
Otherwise, we reduce `d`.

Repeating this process for sufficient iterations yields the required precision for the maximum distance.
