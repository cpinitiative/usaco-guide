...
range can take on. For example, a range update $$add(x)$$ restricts the values 
on the range to being $\geq x$, and a range update $remove(x)$ restricts the 
...
the final value remains. Recall how each $add$ and $remove$ update was essentially
imposing an upper and lower bound on the values. Thus, restricting the values on 
the range to being on the interval $[a, b]$ is essentially the same as performing 
two updates: $add(a)$ and $remove(b)$. As a result, the final result for an 
...
class Segtree {
  ...
  public:
    ...
    void update(int ql, int qr, const Update &x) {
        update(1, 0, sz - 1, ql, qr, x);
    }
...
void buildWall(int n, int k, int op[], int left[], int right[], int height[], int final_height[]) {
    ...
    for (int i = 0; i < k; i++) {
        if (op[i] == 1) {
            ...
            st.update(left[i], right[i], {-INF, height[i]});
        }
    }
    for (int i = 0; i < n; i++) {
        final_height[i] = st.get(i).min_val;
    }
}
```
