...
- Case 2: there must be exactly one more cup of hot water poured than cold water poured. Thus, there will be $c + 1$ cups of hot water and $c$ cups of cold water for some $c \geq 0$.
- The temperature is l+(h-l)(c+1)/(2c+1) when c is odd. This can be proven to be a monotonically decreasing function. Since it's monotonic, we can binary search to approximate the best value for $c$, and test the odd integers aroudn it

Binary search on the maximum number of odd cups $c$ which gives a temperature of at least $t$.
