...
int num_of_integers, exponent;

/*
 * We can define a prime factorization pair as the following:
 * (a, b) represents a prime number (a) to the power of (b). However, 
 *  to help us later, we can write (a, b) as (a, b mod k).
 */
...
vector<pi> find_factor_inverse(vector<pi> original){
	...
	vector<pi> inverse(num_of_integers);
...
int main(){
	...
	vector<int> arr(num_of_integers);
