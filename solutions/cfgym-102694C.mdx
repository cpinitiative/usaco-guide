...
The sloth will always try to move upwards from the start node $(st)$ to the least common ancestor of the start and end nodes $(lca)$ and then move
downwards from the least common ancestor towards the end node $(end)$.

Let $dist(i, j)$ be the distance from node i to node j.

So if the sloth has energy amount $e$, there are 3 separate cases:

1. $e ≥ dist(st, end)$. In this case the result will be the sloth reaching the end.

2. $e < dist(st, end)$ and $e < dist(st, lca)$ in which the result will be the sloth reaching the $e$th parent of $st$.

3. $e < dist(st, end)$ and $e >= dist(st, lca)$ in which the result will be the sloth reaching the $(e - dist(st, lca))$th parent of $end$.

So first, we can can run DFS once to find the depth of every node.

Then we can create a matrix $anc$ to store the ancestors of each node,
where $anc[i][j]$ is the $2^j$th ancestor of node $i$.
...
public class slothNaptime {
	...
	public static final int LOGN = 20;
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 1; i <= Q; i++) {
			...
			if (energy <= path1) {
				...
				System.out.println(jump(end, path2 - (energy - path1)));
			} else {
				// reaches the end.
