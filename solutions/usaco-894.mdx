---
id: usaco-894
source: USACO Silver 2019 January
title: Grass Planting
author: Neo Wang, Aadit Ambadkar, Sandeep Wadhwa, Kevin Sheng
---

[Official Analysis (C++)](http://www.usaco.org/current/data/sol_planting_silver_jan19.html)

## Solution 1 - Intended Solution

**Time Complexity:** $\mathcal{O}(N)$

Let $\texttt{deg}[i]$ represent the degree of node $i$: the number of paths
connecting the node. Then, considering that node, and that node alone, you would
need $\texttt{deg}[i]+1$ grass types. This is because you would need
$\texttt{deg}[i]$ for each of the adjacent nodes, and $1$ for the node itself.
It can be shown that the tree can also be colored accordingly in
$\max(\texttt{deg}[i])+1$ colors.

<LanguageSection>
<CPPSection>

```cpp
#include <bits/stdc++.h>
using namespace std;

const int MAX_N = 10000;

// FastIO: see General -> Fast Input and Output
void setIO(string name = ""){
	ios_base::sync_with_stdio(0);
	cin.tie(0);
	if (!name.empty()) {
		freopen((name + ".in").c_str(), "r", stdin);
		freopen((name + ".out").c_str(), "w", stdout);
	}
}

int main() {
	setIO("planting");

	int field_num; 
	cin >> field_num;

	vector<int> deg(field_num + 1);  // 1-indexed
	for (int p = 0; p < field_num - 1; p++) {
		int field1, field2;
		cin >> field1 >> field2;

		deg[field1]++;
		deg[field2]++;
	}

	int max_deg = 0;
	for (int f = 1; f <= field_num; f++) {
		max_deg = max(max_deg, deg[f]);
	}

	cout << max_deg + 1 << endl;
}
```

</CPPSection>
<JavaSection>

```java
import java.io.*;
import java.util.StringTokenizer;

public class Planting {
	public static void main(String[] args) throws IOException {
		Kattio io = new Kattio("planting");

		int fieldNum = io.nextInt();
		int[] deg = new int[fieldNum + 1];  // 1-indexed
		for (int p = 0; p < fieldNum - 1; p++) {
			int field1 = io.nextInt();
			int field2 = io.nextInt();
			deg[field1]++;
			deg[field2]++;
		}

		int maxDeg = 0;
		for(int f = 1; f <= fieldNum; f++) {
			maxDeg = Math.max(maxDeg, deg[f]);
		}

		io.println(maxDeg + 1);
		io.close();
	}

	//CodeSnip{Kattio}
}
```

</JavaSection>
<PySection>

```py
with open('planting.in') as read:
	field_num = int(read.readline())
	deg = [0 for _ in range(field_num + 1)]  # 1-indexed
	for _ in range(field_num - 1):
		field1, field2 = [int(i) for i in read.readline().split()]
		deg[field1] += 1
		deg[field2] += 1

max_deg = max(deg)
print(max_deg + 1, file=open('planting.out', 'w'))
```

</PySection>
</LanguageSection>

## Solution 2 - DFS

**Time Complexity**: $\mathcal{O}(N)$

This can also be solved using [DFS](https://usaco.guide/silver/dfs).
Although it is less elegant, it gets the job done.

<LanguageSection>
<CPPSection>

```cpp
#include <bits/stdc++.h>
using namespace std;

const int MAX_N = 10000;

// FastIO: see General -> Fast Input and Output
void setIO(string name = ""){
	ios_base::sync_with_stdio(0);
	cin.tie(0);
	if (!name.empty()) {
		freopen((name + ".in").c_str(), "r", stdin);
		freopen((name + ".out").c_str(), "w", stdout);
	}
}

vector<int> needed_types;
vector<vector<int>> neighbors;

void process_fields(int at, int prev) {
	int type_num = 1;
	for (int n : neighbors[at]) {
		if (n == prev) {
			continue;
		}
		while (type_num == needed_types[at] || type_num == needed_types[prev]) {
			type_num++;
		}
		needed_types[n] = type_num;
		process_fields(n, at);
		type_num++;
	}
}

int main() {
	setIO("planting");

	int field_num; 
	cin >> field_num;

	needed_types = vector<int>(field_num);
	neighbors = vector<vector<int>>(field_num);
	for (int f = 0; f < field_num - 1; f++) {
		int field1, field2;
		cin >> field1 >> field2;
		neighbors[--field1].push_back(--field2);
		neighbors[field2].push_back(field1);
	}

	needed_types[0] = 1;
	process_fields(0, 0);
	int min_types = 0;
	for (int t : needed_types) {
		min_types = max(min_types, t);
	}
	
	cout << min_types << endl;
}
```

</CPPSection>
<JavaSection>

```java
import java.io.*;
import java.util.*;

public class Planting {
	static int[] neededTypes;
	static List<Integer>[] neighbors;

	public static void main(String[] args) throws IOException {
		Kattio io = new Kattio("planting");
		
		int fieldNum = io.nextInt();

		neighbors = new List[fieldNum];
		for (int f = 0; f < fieldNum; f++) {
			neighbors[f] = new ArrayList<>();
		}
		for (int i = 0; i < fieldNum - 1; i++) {
			int field1 = io.nextInt();
			int field2 = io.nextInt();
			neighbors[--field1].add(--field2);
			neighbors[field2].add(field1);
		}

		neededTypes = new int[fieldNum];
		neededTypes[0] = 1;

		processFields(0, 0);
		
		int minTypes = 0;
		for (int t : neededTypes) {
			minTypes = Math.max(minTypes, t);
		}

		io.println(minTypes);
		io.close();
	}

	static void processFields(int at, int prev) {
		int typeNum = 1;
		for (int n : neighbors[at]) {
			if (n == prev) {
				continue;
			}
			while (typeNum == neededTypes[at] || typeNum == neededTypes[prev]) {
				typeNum++;
			}
			neededTypes[n] = typeNum;
			processFields(n, at);
			typeNum++;
		}
	}

	//CodeSnip{Kattio}
}
```

</JavaSection>
<PySection>

Implementing recursion as normal results in stack overflow for the last test case
($10^5$ fields connected together in a single chain). To remedy this, we use our own
stack.

```py
with open('planting.in') as read:
	field_num = int(read.readline())
	neighbors = [[] for _ in range(field_num)]
	for _ in range(field_num - 1):
		field1, field2 = [int(i) - 1 for i in read.readline().split()]
		neighbors[field1].append(field2)
		neighbors[field2].append(field1)

needed_types = [0 for _ in range(field_num)]
needed_types[0] = 1

stack = [(0, 0)]
while stack:
	at, prev = stack.pop()
	type_num = 1
	for n in neighbors[at]:
		if n == prev:
			continue

		while type_num in [needed_types[at], needed_types[prev]]:
			type_num += 1

		needed_types[n] = type_num
		stack.append((n, at))
		type_num += 1

print(max(needed_types), file=open('planting.out', 'w'))
```

</PySection>
</LanguageSection>
