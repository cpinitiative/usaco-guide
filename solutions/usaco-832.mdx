...
1. The first of these cases is when cow one is fixed. In this case, the solution is simple, as we just output its fixed position. 
2. In the second case, we must consider is if cow one is in the given hierarchy of cows. In this case, we want to place every cow in the hierarchy as early as possible. 
3. The last case is when cow one is neither fixed nor in the hierarchy. In this case, we would want to place every cow in the hierarchy as late as possible, allowing for cow one to get the earliest position possible.
