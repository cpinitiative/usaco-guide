	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			...
			for (int ii = 0; ii < schedule[i].size(); ii++) {
				next_show[i][j].push_back(find_show_time(schedule[i][ii] + length[i], j));
			}
		}
	}
		...
		for (int j = 0; j < N; j++) {
			...
			if (x != -1) {
				...
				if (dp[subset].first == -1 || 
					schedule[dp[subset].first][dp[subset].second] + length[dp[subset].first] < 
					schedule[j][x] + length[j]) {
					dp[subset] = {j, x};
				}
				if (schedule[dp[subset].first][dp[subset].second] + length[dp[subset].first] >= L) {
