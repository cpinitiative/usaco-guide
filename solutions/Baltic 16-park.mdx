...
Let's call the circle you are moving around between exits $x$. Given two other circles, $a$ and $b$, observe that it's optimal to move the center of $x$ through the midpoint of the line segment connecting the centers of $a$ and $b$. Using this observation, we claim that $x$ can move between two circles $a$ and $b$ as long as $dist(a, b) \leq x_r$, where $x_r$ denotes the radius of $x$ and $dist(a, b)$ denotes the distance between the borders of $a$ and $b$. Also, let's drop horizontal and vertical line segments from each border to each circle.

Finally, observe that you can go from exit $e_1$ to exit $e_2$ as long as there isn't some chain of line segments with weight $\leq x_r$ that completely blocks off the path between exits. For example, you can go from the top-right exit to the bottom-left exist as long as there isn't a chain of line segments from the top border to the bottom, top to right, bottom to left, or left to right. 

## Creating a Graph

Now, you have some circles with weights between each other. Since $n$ is small, let's draw edges between every pair of circles. Additionally, let's draw an edge between each circle and the four borders of the park, creating $n ^ 2 + 4n$ edges overall. Additionally, let's add four nodes (on top of the circles) for the four borders of the park.

The problem of checking whether we can go between two exits now becomes checking, for some $r$, whether edges with weight $\leq r$ connect certain borders which involves some casework. 

## Offline Queries and DSU

To efficently answer queries, let's process them in order of increasing $x_r$ and store a DSU. Now, we can add edges one by one as long as their weight $\leq x_r$, and then check connectivity between the border nodes.
