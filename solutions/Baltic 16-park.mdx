...
Let's call the circle you are moving around between exits $x$. We claim that $x$ can move between two circles $a$ and $b$ as long as $dist(a, b) - a_r - b_r \leq x_r$, where $x_r$ denotes the radius of $x$, $a_r$ and $b_r$ denote the radius of $a$ and $b$ and $dist(a, b)$ denotes the distance between the centers of $a$ and $b$. Also, let's add horizontal and vertical line segments from each border of the park to each circle with the distance between them as well.
