...
bool check(long long dist) {
    long long currPoint = intervals[0].first; //starting off at earliest possible point
    long long currInterval = 0; //what patch we are currently on
    long long placed = 0; //how many cows have we placed in total
    while (placed < n) {
