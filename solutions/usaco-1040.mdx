...
We can make a $\mathcal{O}(N \log N)$ algorithm by sorting every particle in ascending order, first by its $x$ spin, and then by its $y$ spin, resulting in a list $p_1,  p_2.. p_{N-1},  p_{N}$.
