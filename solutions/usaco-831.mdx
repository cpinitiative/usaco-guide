...
public class TeamTicTacToe {
	...
	static boolean teamWins(char ch1, char ch2) {
		// Check diagonals with a single cow
...
# Does 1 cow win?
def cow_wins(game: [str], cow: str) -> bool:
    # Check diagonals
...
# Does a team win?
def team_wins(game: [str], cow1: str, cow2: str) -> bool:
    # Test if a team wins based on 3 characters in a row, column, or diagonal
...
def cow_contrib(game: [str], pts: [(int, int)]) -> {str}:
    contained = set()
    for pt in pts:
...
# A function that adds the amount of cows who contributed to the winners count
