...
bool check_single_cow_winner(char ch, vector<string> &board) {
	for (int i = 0; i < 3; i++) {
		// check rows
...
int individual_wins(vector<string> &board) {
...
bool check_double_cow_winners(char ch1, char ch2, vector<string> &board) {
	for (int i = 0; i < 3; i++) {
		// check rows
...
int team_wins(vector<string> &board) {
