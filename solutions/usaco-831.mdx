...
for letter in alphabet:
	# Check diagonals
	if board[0][0] == letter and board[1][1] == letter \
			and board[2][2] == letter:
		solo_wins += 1
		continue
	# We need the continue for the following conditionals
	# because we don't want to double count a team win
	if board[0][2] == letter and board[1][1] == letter \
	...
	for i in range(3):
		...
		if board[i][0] == letter and board[i][1] == letter \
				and board[i][2] == letter:
			solo_wins += 1
			break
		if board[0][i] == letter and board[1][i] == letter \
				and board[2][i] == letter:
			solo_wins += 1
			break
...
def check_team_win(ch1, ch2, a, b, c):
	# If none of the three characters are ch1 or ch2,
...
for l1_index in range(26):
	for l2_index in range(l1_index + 1, 26):
		# Check diagonals
		if check_team_win(alphabet[l1_index], alphabet[l2_index], board[0][0],
						  board[1][1], board[2][2]):
			team_wins += 1
			continue
		if check_team_win(alphabet[l1_index], alphabet[l2_index], board[0][2],
						  board[1][1], board[2][0]):
			team_wins += 1
			continue
		# Check rows and columns
		for i in range(3):
			if check_team_win(alphabet[l1_index], alphabet[l2_index],
...
for thing in board_set_list:
	e = None
	thing = list(thing)
	if len(thing) == 1: # solo wins
		if thing[0] not in winners:
			solo_wins += 1
			winners.add(thing[0])

	elif len(thing) == 2:
		if (thing[0], thing[1]) not in winners: # Team wins
			team_wins += 1
