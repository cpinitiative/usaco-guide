...
bool checkSingleCowWinner(char ch, string s[]) {
  for (int i = 0; i < 3; i++) {
    //check rows
    if (s[i][0] == ch && s[i][0] == s[i][1] && s[i][0] == s[i][2]) 
      return true;
    //check columns
    if (s[0][i] == ch && s[0][i] == s[1][i] && s[0][i] == s[2][i]) 
      return true;
  }
  //check diagonals
  if (s[0][0] == ch && s[0][0] == s[1][1] && s[0][0] == s[2][2]) 
    return true;
  if (s[0][2] == ch && s[0][2] == s[1][1] && s[0][2] == s[2][0]) 
    return true;

  return false;
}
//determine the number of individual cow wins
int individualWins(string s[]) {
  int wins = 0;
  for (char ch = 'A'; ch <= 'Z'; ch++) {
    wins += checkSingleCowWinner(ch, s);
  }
  return wins;
}
//check if the cows ch1 and ch2 are strictly present in the row/column/diagonal under review.
bool checkIfWinners(char ch1, char ch2, char x, char y, char z) {
  if (x == y && x != z) {
    if (x == ch1 && z == ch2) 
      return true;
    if (x == ch2 && z == ch1) 
...
bool checkDoubleCowsWinners(char ch1, char ch2, string s[]) {
  int wins = 0;
  for (int i = 0; i < 3; i++) {
    // check rows
    if (checkIfWinners(ch1, ch2, s[i][0], s[i][1], s[i][2])) 
      return true;
    //check columns
    if (checkIfWinners(ch1, ch2, s[0][i], s[1][i], s[2][i])) 
      return true;
  }
  //check diagonals
  if (checkIfWinners(ch1, ch2, s[0][0], s[1][1], s[2][2])) 
    return true;
  if (checkIfWinners(ch1, ch2, s[0][2], s[1][1], s[2][0])) 
    return true;
  return false;
}
//determine the number of team wins
int teamWins(string s[]) {
  int wins = 0;
  for (char ch1 = 'A'; ch1 <= 'Z'; ch1++) {
    for (char ch2 = ch1 + 1; ch2 <= 'Z'; ch2++) {
      wins += checkDoubleCowsWinners(ch1, ch2, s);
    }
  }
  return wins;
}
int main() {
  ...
  int individualCows = individualWins(s);
  int teamCows = teamWins(s);

  cout << individualCows << endl;
  cout << teamCows << endl;
}
```
...
def cow_wins(game: List[str], cow: str) -> bool:
	# Check diagonals
	if (game[0][0] == game[1][1] == game[2][2] == cow
			or game[0][2] == game[1][1] == game[2][0] == cow):
		return True

	# Check rows and columns
	for i in range(3):
		if (game[0][i] == game[1][i] == game[2][i] == cow
...
def team_wins(game: List[str], cow1: str, cow2: str) -> bool:
	...
	if (check3(game[0][0], game[1][1], game[2][2])
			or check3(game[0][2], game[1][1], game[2][0])):
		return True

	# And also check rows and column
	for i in range(3):
		if (check3(game[0][i], game[1][i], game[2][i])
				or check3(game[i][0], game[i][1], game[i][2])):
			return True
	return False
