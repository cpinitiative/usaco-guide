...
#include <bits/stdc++.h>
using namespace std;
bool individualWinsUtil(char ch, string s[]) {
  for (int i = 0; i < 3; i++) {
    //check rows
...
int individualWins(string s[]) {
  ...
  for (char ch = 'A'; ch <= 'Z'; ch++) {
    wins += individualWinsUtil(ch, s);
  }
  return wins;
}
bool teamWinsUtil2(char ch1, char ch2, char x, char y, char z) {
  if (x == y && x != z) {
    if (x == ch1 && z == ch2) 
      return true;
    if (x == ch2 && z == ch1) 
...
bool teamWinsUtil(char ch1, char ch2, string s[]) {
  int wins = 0;
  for (int i = 0; i < 3; i++) {
    // check rows
    if (teamWinsUtil2(ch1, ch2, s[i][0], s[i][1], s[i][2])) 
      return true;
    //check columns
    if (teamWinsUtil2(ch1, ch2, s[0][i], s[1][i], s[2][i])) 
      return true;
  }
  //check diagonals
  if (teamWinsUtil2(ch1, ch2, s[0][0], s[1][1], s[2][2])) 
    return true;
  if (teamWinsUtil2(ch1, ch2, s[0][2], s[1][1], s[2][0])) 
    return true;
  return false;
}
