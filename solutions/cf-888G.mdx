...
Firstly, note that because there is essentially an edge between any pair of nodes, the value of the nodes themselves can be considered unordered. Thus, the answer will not change if we sort the elements. 
...
After these two steps, we have obtained a sorted array with no duplicates whos answer is the same as the original problem.
...
Note that this implies that it is always better to connect $l_2$ to $l_1$; however, this configuration may not be optimal. The proof is analguous if instead $r_2$ is disconnected. $\blacksquare$
