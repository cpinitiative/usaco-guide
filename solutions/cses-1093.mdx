...
Since the two sets have to have an equal sum, it's evident that both of them have to have half of  $1, 2, \ldots, N$. So, if $\frac{N(N+1)}{2}$, or the sum of the first $N$ positive integers isn't even, then a solution isn't possible. For each subset, we want it's sum to be half of this sum, or $\frac{N(N+1)}{4}$
...
int main() {
	...
	vector<vector<int>> dp(n, vector<int>(sum_elem + 1, 0));
