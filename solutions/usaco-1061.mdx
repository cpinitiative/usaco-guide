...
public class StuckInARut {
	...
	public static void main(String[] args) throws IOException {
		...
		ArrayList<ArrayList<Integer>> east = new ArrayList<~>();
		ArrayList<ArrayList<Integer>> north = new ArrayList<~>();
		...
		ArrayList<ArrayList<Integer>> meetTime = new ArrayList<~>();
		...
		Collections.checkedSortedMap(meetTime,  new Comparator<ArrayList<Integer>>() {
			public int compare(ArrayList<Integer> s1, ArrayList<Integer> s2) {
				return Integer.compare(s1.get(0), s2.get(0));
			}});
		
		ArrayList<Integer> ans = new ArrayList<~>(Collections.nCopies(N,  myINF));
	...
	// https://usaco.guide/general/input-output?lang=java#method-3---io-template
