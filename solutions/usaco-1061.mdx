...
Now we can sort cows by their static coordinates, which ensures that earlier collisions will execute first, and simulate those collisions.
        ...
        for (int[] northCow :
            ...
            for (int[] eastCow :
                ...
                if (northCow[0] > eastCow[0] && northCow[1] < eastCow[1]) {
                    ...
                    // If the east cow gets there before, we can save the
                    // north cow's position.
                    if (northDistance > eastDistance && stoppedLocations[eastCow[2]] == -1) {
