...
[Official Analysis](https://img.atcoder.jp/abc169/editorial.pdf#page=12](https://codeforces.com/blog/entry/55701)
...
1 3 2 4
In this permutation, the first element is still in this same place.
...
We can add the number of valid permutations times the number of ways to choose a permutation of that length.
...
int main() {
  ...
  ll ans = 0;
  for (int i = 0; i <= k; i++) {
    int a[i]; for (int j = 0; j < i; j++) a[j] = j;
    int amt = 0;
    do {
      bool valid = 1;
      for (int j = 0; j < i; j++)
