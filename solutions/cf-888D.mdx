...
int main() {
  ...
  ll c[n + 1][k + 1]; fill_n(&c[0][0], (n + 1) * (k + 1), 0);
  ...
  for (int i = 0; i <= k; i++) {
    ...
    // Calculate number of valid permutations result in all positions shuffled.
    ...
    // Add the number of valid permutations of i elements multiplied by the number of ways to choose i elements.
    ans += c[n][i] * amt;
  }
