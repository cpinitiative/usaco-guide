...
Perform [BFS](https://usaco.guide/silver/graph-traversal?lang=cpp) starting from any node and calculate the distance (level) from the root to each node. Every node will have a parity of even or odd. 
...
The categories are disjoint and exhaustive, so one group must have $\leq n/2$ nodes. We can output the smaller of the two sets to satisfy the problem's requirements.
