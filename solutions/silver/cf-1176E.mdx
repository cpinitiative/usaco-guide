...
Another interpretation is that the BFS traversal constructs a bipartite spanning tree, whose level-based partition guarantees that one side has size at most $\left \lfloor \frac{n}{2} \right \rfloor$, yielding a valid set of nodes to select.
