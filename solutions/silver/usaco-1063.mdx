...
simply keep one prefix sum. The amount of cows above and below a y-coordinate can then be calculated
as a rectangle with the current y-coordinate and either extrema being the endpoints.
...
long long rectSum(vector<vector<long long>> &grid, int r, int R, int c, int C) {
...
int main() {
	...
	for (auto &e : nums) { cin >> e.first >> e.second; }
