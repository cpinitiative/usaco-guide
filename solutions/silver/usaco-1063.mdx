...
bool sort_by_x(const pair<int, int> &a, const pair<int, int> &b) { return a.first < b.first; }
bool sort_by_y(const pair<int, int> &a, const pair<int, int> &b) { return a.second < b.second; }

int rectSum(const vector<vector<long long>> &grid, int r1, int r2, int c1, int c2) {
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		for (int j = i + 1; j < n; j++) {
			...
			int above = rectSum(grid, 1, r1 - 1, c1, c2);
			int below = rectSum(grid, r2 + 1, n, c1, c2);
