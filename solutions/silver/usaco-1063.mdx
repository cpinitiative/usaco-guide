...
![](/solutions/silver/usaco-1063/initial.png)
...
![](/solutions/silver/usaco-1063/countercase.png)

Drawing out the boxes created by this observation, we now have the following boxes:

![](/solutions/silver/usaco-1063/corners.png)
...
![](/solutions/silver/usaco-1063/new.png)
...
## Explanation
Instead of keeping two prefix sums for the number of cows above and below a y-coordinate, we can
simply keep one prefix sum: a 2D prefix sum directly summing the coordinate compressed grid.

The main difference between this and the solution above is that this uses rectangular
sums to find the points above and below a y-coordinate, while the one above keeps track
of the number of points above and below a y-coordinate.
...
bool sortByX(pair<int, int> a, pair<int, int> b) { return a.first < b.first; }
bool sortByY(pair<int, int> a, pair<int, int> b) { return a.second < b.second; }

int rectSum(vector<vector<long long>> &grid, int r1, int r2, int c1, int c2) {
...
int main() {
	...
	// compress X
	sort(nums.begin(), nums.end(), sortByX);
	for (int i = 0; i < nums.size(); i++) { nums[i].first = i + 1; }

	// compress Y
	sort(nums.begin(), nums.end(), sortByY);
	for (int i = 0; i < nums.size(); i++) { nums[i].second = i + 1; }

	// add points to grid and compute prefix
	...
	long long res = 0;
	// choose two points, count rectangles
	sort(nums.begin(), nums.end(), sortByX);
	for (int i = 0; i < n; i++) {
		for (int j = i + 1; j < n; j++) {
			int r = nums[i].first, R = nums[j].first;
			int c = min(nums[i].second, nums[j].second);
			int C = max(nums[i].second, nums[j].second);

			int above = rectSum(grid, 1, r - 1, c, C);
			int below = rectSum(grid, R + 1, n, c, C);

			res += (above + 1) * (below + 1);
		}
	}

	// add results for single or no cow
	cout << res + n + 1 << "\n";
}
```
