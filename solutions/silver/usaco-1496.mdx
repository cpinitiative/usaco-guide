...
int main() {
	...
	while (t--) {
		...
		// since time complexity is logarithmic, 100 iterations
		// is enough for the process to conclude
		for (int i = 0; i < 100; i++) {
			if ((a == c && b == d) || c < a || d < b) break;

			long long ops;
			if (c >= d) {
				// backtrack from multiple type 1 operations
				ops = min((c - a) / d, c / d);
				c -= ops * d;
			} else {
				// backtrack from multiple type 2 operations
				ops = min((d - b) / c, d / c);
				d -= ops * c;
			}
