...
We need to use a 3D grid to correctly track visited states. The DFS may visit the same $(i, j)$ state earlier in its processing but through a greater number of operations. To properly track visited cells, we need to make sure the visited grid has a record for how many operations it took to reach each state every time.
