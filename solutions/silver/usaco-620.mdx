...
def main():
	...
	res = (
		m  # initially both pails are empty, so there is a difference of m-0, which is m
	)
...
We can perform a DFS starting from the base state of $(0, 0)$. We perform each operation to find the states $(i, j)$ where pail $X$ has $i$ and pail $Y$ has $j$ units of milk.

We need to use a 3D grid to correctly track visited states. The DFS may visit the same state earlier in its processing but in a greater number of operations. To properly track visited cells, we need to make sure the visited grid has a record for how many operations it took to reach each state.

Simulating the all six different operations every time is enough to pass the constraints.
