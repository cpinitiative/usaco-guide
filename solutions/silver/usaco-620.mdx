...
Note that state with $l$ being a specific value (herein referred to as $l_0$) can only transition to a state where $l$ is $l_0+1$, so we compute $\texttt{f}$ in order of $l$.
