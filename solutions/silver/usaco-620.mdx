...
int main() {
    ...
    int res = m;  // initially both pails are empty, so there is a difference of m-0, which is m
...
def main():
    ...
    res = m  # initially both pails are empty, so there is a difference of m-0, which is m
