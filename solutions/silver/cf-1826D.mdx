...
Notice how we can rearrange the given equation:
$$
b_{i1} + b_{i2} + b_{i3} - (r - l) = b_{i1} + b_{i2} + b_{i3} + (l - r).
$$
The given equation demonstrates how as the distance between the left and right boundaries increase, 
the value of the equation decreases. Thus, in order to maximize the value, it is best to set 
two of the sights at the boundaries. Then, the distance between the boundaries is decreased, meaning the
value of the equation is increased. Knowing this, we can create a new equation reflecting that two
of the sights are at the boundaries.
$$
b_{i1} + b_{l} + b_{r} + (l - r) = b_{i1} + (b_{l} + l) + (b_{r} - r).
$$
This simplifies the problem into finding a middle sight and adding it to the maximum from the left and right.
