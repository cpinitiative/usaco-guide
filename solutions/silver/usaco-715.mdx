...
Now we neeed to find a subarray of length $K$ with the least number of $1$s (i.e., the least sum).
...
public class MaxCross {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < b; i++) {
			signals[Integer.parseInt(br.readLine()) - 1]++;
		}

		// prefix sums precomputation
		int[] prefSum = new int[n + 1];
		for (int i = 0; i < n; i++) {
			prefSum[i + 1] = signals[i] + prefSum[i];
		}
