...
To check if a given minimal wormhole width $x$ is valid, we use BFS to find all positions
that can reach each other by traversing wormholes that are at most width $x$.
Then we look at each cow's initial position along with the position it's supposed to be in.
If the two are in the same component, we're golden.
