...
Once all connected components are found, we check if each cow $i$ is in the same connected component as $p_i$. If one cow is in the wrong place, then it is not possible to sort the cows with a wormhole of at least width $x$.
