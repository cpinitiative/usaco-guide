...
This is fast enough to pass; however, there is a way to get rid of the this bottleneck:
...
however, we can preprocess $\mathrm{C_{sum}}[i] = \sum_{j=i}^{k}\text{C}[i]\text{C}[j]$ in $\mathcal{O}(k)$ using postfix sums, and then compute
$\sum_{i=1}^{k}\mathrm{C_{sum}}[i+1]$ in $\mathcal{O}(k)$ time as well.
