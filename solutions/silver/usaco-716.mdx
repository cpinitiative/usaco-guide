...
Let's consider an arbitrary cow. We can determine which squares that cow can reach by performing either BFS or DFS, without crossing any roads. Then, we can determine which cows cannot be visited without crossing any roads by checking if that cow's square is reachable.
