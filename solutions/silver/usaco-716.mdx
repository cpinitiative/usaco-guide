...
Finally, to determine all such pairs of cows, we can perform BFS or DFS for each cow and compute the number of unreachable cows.
For each pair of connected components $A$ and $B$, any cow from $A$ must cross a road with any cow from $B$; thus, we compute the number of unreachable cows by enumerating all pairs of connected components, and summing the pairwise products of the number of cows in each connected component, taking care only to count each pair once.
...
void bfs(int sx, int sy) {
	...
	// If visited already, nothing will be pushed into q, so nothing will run.
	if (!vis[sx][sy]) {
		vis[sx][sy] = true;
	...
	while (!q.empty()) {
		...
		// Enumerate the possible directions for next step
		for (int d = 0; d < 4; d++) {
...
int main() {
	...
	// Boundary conditions:  Cannot go beyond the grid.
	for (int j = 1; j <= n; j++) {
	...
	for (int l = 1; l <= r; l++) {
		...
		// Get the d-values for the directions of travel that the road prevents
		// (x, y) to (x1, y1)
		...
		// (x1, y1) to (x, y)
		current_dx = x - x1, current_dy = y - y1;
		d = 0;
		for (; d < 4; d++)
			if (current_dx == dx[d] && current_dy == dy[d]) break;
		assert(d < 4);
	...
	// Compute amount of cows in each square
	for (int i = 1; i <= k; i++) {
		int x, y;
		fin >> x >> y;
		cows[x][y]++;
	...
	// BFS at each point to build sz[i][j], the number of cows for the
	// connected component starting at (i, j).  If the point is already
	// visited, sz[i][j] remains 0.
	for (int i = 1; i <= n; i++)
		for (int j = 1; j <= n; j++) bfs(i, j);

	// Run over pairs of connected components A = (i, j) and B = (i1, j1);
	// extra points that do not "start" a connected component will still be
	// enumerated, but will not contribute to the answer as their sz[][] is 0.
	int ans = 0;
	for (int i = 1; i <= n; i++)
		for (int j = 1; j <= n; j++)
	...
	// Divide by 2, because we counted each pair and its reverse in the summation above
	fout << ans / 2 << endl;
	return 0;
}
```
