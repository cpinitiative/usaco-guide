...
Naively implementing this solution takes $\mathcal{O}(N^4)$ time, as we are computing pairwise products with up to $\mathcal{O}(N^2)$ components. This can be optimized to $\mathcal{O}(N^2)$, as described below:

<Spoiler title="Optimizing Pairwise Product Sums">

Let the 0-indexed array containing the number of cows in each connected component be $\mathrm{C}$.
Naively, we would implement the pairwise product sum as follows:
...
for (int i = 0; i < C.size(); i++) {
	for (int j = 0; j < i; j++) { distant_pairs += C[i] * C[j]; }
}
```
...
Using the distributive property, we may factor out $\mathrm C[i]$ from the inner loop, and do the rest using a temporary variable representing the prefix sum, like this:
...
This will make the time complexity linear to the number of connected components, making our final time complexity $\mathcal O(N^2)$.
...
Note that using `set` here adds an extra $\log R$ factor to the time complexity.
