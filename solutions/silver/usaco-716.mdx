...
int main() {
	...
	// Initialize all dircetions as true first, except boundary conditions handled
	// later.
	memset(can_visit, 1, sizeof can_visit);
	// Boundary conditions: Can't go up (d=2) on first row, or go down (d=0) on last
	// row.
	for (int j = 1; j <= n; j++) {
	...
	for (int l = 1; l <= r; l++) {
		...
		int d = 0;
		for (; d < 4; d++)
			if (current_dx == dx[d] && current_dy == dy[d]) break;
		...
		for (int j = 1; j <= n; j++)
			for (int i1 = 1; i1 <= n; i1++)
				for (int j1 = 1; j1 <= n; j1++)
					if (i != i1 || j != j1) ans += sz[i][j] * sz[i1][j1];
	fout << ans / 2 << endl;
	return 0;
}
```
