...
Let's view the grid as a graph, where each cell is a vertex, and each border between two grids that does not contain a road is an edge.
...
We perform floodfill to find each connected component and the number of cows within it.
...
Naively implementing this solution takes $\mathcal{O}(N^4)$ time, as we are computing pairwise products with up to $\mathcal{O}(N^2)$ components; this can be optimized to $\mathcal{O}(N^2)$:

<Spoiler title="Optimizing Pairwise Product Sums">
...
Naïvely, we would compute $\sum_{i=1}^{k}\sum_{j=1}^{i-1}\mathrm{C}[i]\mathrm{C}[j]$ for our answer;
however, we can precompute $\mathrm{C_{sum}}[i] = \sum_{j=1}^{i}\mathrm{C}[j]$ in $\mathcal{O}(k)$ using prefix sums, and then compute
$\sum_{i=1}^{k}\sum_{j=1}^{i-1}\mathrm{C}[i]\mathrm{C}[j]=\sum_{i=1}^{k}\mathrm{C}[i]\sum_{j=1}^{i-1}\mathrm{C}[j]=\sum_{i=1}^{k}\mathrm{C}[i]\mathrm{C_{sum}}[i-1]$ in $\mathcal{O}(k)$ time as well.  To simplify the implementation, we could also use a single loop, and keep track of $\mathrm{C_{sum}}[i-1]$ by accumulating $\mathrm{C}[i]$ to it cumulatively after we use the sum for our main computation.
...
int main() {
	...
	int distant_pairs = 0, prefix_sum = 0;
	for (int i = 0; i < cow_components.size(); i++) {
		distant_pairs += cow_components[i] * prefix_sum;
		prefix_sum += cow_components[i];
	}
...
public class CountCross {
	...
	public static void main(String[] args) throws IOException {
		...
		int distantPairs = 0, prefixSum = 0;
		for (int i = 0; i < cowComponents.size(); i++) {
			distantPairs += cowComponents.get(i) * prefixSum;
			prefixSum += cowComponents.get(i);
		}
