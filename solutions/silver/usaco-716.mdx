...
Since there will be at most $\mathcal{O}(N^2)$ connected components, the time complexity of computing the pairwise product of the counts can be $\mathcal{O}(N^4)$.
This is fast enough to pass; however, since our BFS is only $\mathcal O(n^2)$, we might as well squeeze the pairwise to product to $\mathcal{O}(n^2)$:
