...
For each pair of connected components $A$ and $B$, any cow from $A$ must cross a road with any cow from $B$.
Therefore, the answer is the sum of the pairwise products of the number of cows in each connected component.
We must take special care to only count each pair once.
