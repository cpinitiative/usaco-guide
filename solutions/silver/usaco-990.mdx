...
Let $b$ represent the minimum number of berries in any bucket that Elsie receives. Without loss of generality, we can assume that all of Elsie's buckets contain exactly $b$ berries. Our goal is to maximize the total number of berries placed into $K$ buckets, each containing at most $b$ berries, such that at least $K/2$ buckets contain exactly $b$ berries.
