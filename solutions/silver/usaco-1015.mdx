...
To do this for each coordinate, we can maintain a dictionary that stores the $y$-values for each $x$ and the $x$-values for each $y$.
Then, we sort the $y$-values for each $x$ and the $x$-values for each $y$, so that for any coordinate, we can find the indices in which it appears in the hashmaps using binary search.
After finding these indices, we need to find the distance from all $x$-values and distance from all $y$-values in the hashmap lists. We precompute **prefix sums** beforehand, and then compute the summations for the indices as follows:
