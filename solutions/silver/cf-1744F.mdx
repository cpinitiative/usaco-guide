...
[Official Editorial](https://codeforces.com/blog/entry/108101)
...
We need to find all subsegments of length $\leq 2 \cdot \texttt{mex}$ that contains $l \dots r$ and omit $\texttt{pos}$. First, let's solve for $\texttt{pos}>r$. Let's iterate $r$ to $\texttt{pos}-1$, finding all bounds of $l$ that satisfy the constraints for each $r$. This is not a time concern as long as we can find all possible values of $l$ in $\mathcal{O}(1)$, since each time we iterate we are increasing the size of the subsegment by $1$.
