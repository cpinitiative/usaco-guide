...
public class Main {
	static List<Integer>[] graph = new ArrayList[100001];
	static int[] visited = new int[100001];
	static int[] odd = new int[100001];
	static int timer = 1;
	...
	public static void dfs(int node, int parent) {
		...
		for (int i : graph[node]) {
			...
			if (visited[i] == 0) {
				dfs(i, node);
				if (odd[i] == 1) {
					ans.add(new Pair(i, node));
					odd[i] = 0;
				} else {
					ans.add(new Pair(node, i));
					odd[node] ^= 1;
				}
			} else if (visited[node] > visited[i]) {
				ans.add(new Pair(node, i));
	...
	public static void main(String[] args) throws IOException {
		...
		int oddSum = 0;
		for (int i = 1; i <= n; i++) { oddSum += odd[i]; }

		if (oddSum > 0) {
			pw.println("IMPOSSIBLE");
		} else {
			for (Pair p : ans) { pw.println(p.first + " " + p.second); }
		}
