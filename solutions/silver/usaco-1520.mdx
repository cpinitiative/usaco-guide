...
We are given a tree of waypoints rooted at waypoint $1$, with each edge assigned a difficulty and enjoyment value. We must answer $M$ queries to find the maximum total enjoyment from a path to a starting waypoint to the root, such that at most $c$ edges have difficulty greater than $s$.
