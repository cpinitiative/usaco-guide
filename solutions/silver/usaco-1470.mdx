...
Let's denote a desirable location's distance to the edge as $d_{desirable}$ and a cow's distance to the edge as $d_{cow}$

Recall earlier the amount of countributions that can occur is equal to $min(d_{cow}, d_{desirable}) + 1$
...
We can optimize yet again by implementing prefix arrays. All countributions from pairs limited by $d_{desirable}$ can be found quickly using the prefix array, and all countributions from pairs limited by $d_{cow}s$ can be found by multiplying $d_{cow}s$ by the number of pairs limited. 
