...
If 0-indexing, the contribution from $i$ and $j$ can be written as $\min(i + 1, n - j)$.
...
The amount of contributions created is equal to the number of operations that do not involve $i$.
...
Recall that the amount of contributions that can occur is equal to $\min(d_{cow}, d_{des})$.

Thus, for a given cow, we want to find out how many contributions are limited by $d_{des}$ when $d_{des} < d_{cow}$ of each desirable location, and how many are limited by $d_{cow}$ when $d_{cow} < d_{des}$. We can do this by setting up an array of all $d_{des}$, and binary searching over this array to see for how many desirable locations $d_{des} < d_{cow}$, and how many desirable locations $d_{cow} < d_{des}$.
