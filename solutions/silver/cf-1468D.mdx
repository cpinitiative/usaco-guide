...
public class Firecrackers {
	public static void main(String[] args) {
		...
		for (int t = 0; t < testNum; t++) {
			...
			int maxFirecrackers = Math.min(numOfFirecrackers, Math.abs(hooliganLocation - guardLocation) - 1);
	...
	private static boolean works(int[] explodingTimes, int fireCrackersExploded, int timeBeforeCaught) {
		int currTime = 1;
