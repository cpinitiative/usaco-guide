...
author: Nathan Gong, Raagav Bala
---

[Official Analysis (C++)](http://www.usaco.org/current/data/sol_ccski.html)

<Spoiler title="Hint 1">
Given a value for $D$, how can we check if all waypoints are reachable?
</Spoiler>

<Spoiler title="Hint 2">
Trying out every possible value of $D$ is inefficient. How can we speed up this approach?
</Spoiler>

<Spoiler title="Solution">
## Explanation

We can combine the answers for hints 1 and 2 to solve this problem. Concretely,

1. For a given $D$, check if all waypoints are reachable by all other waypoints using floodfill.
2. Use binary search over the range $0$ to the maximum possible $D$, $\max(\texttt{elevations}) - \min(\texttt{elevations})$.

So the solution is to run a binary search over $D$ in logarithmic time, using floodfill to check if $D$ guarentees that all waypoints are reachable from each other.
