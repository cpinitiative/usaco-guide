...
int main() {
	...
	for (int right = 0; right < flat_num; right++) {
		...
		if (curr_pokemon.size() == types.size()) {
			shortest_interval =
			    std::min(shortest_interval, right - closest_left + 1);
		}
	}
...
public class Everywhere {
	public static void main(String[] args) throws IOException {
		...
		for (int right = 0; right < flatNum; right++) {
			...
			currCaught.put(newCaught,
			               currCaught.getOrDefault(newCaught, 0) + 1);
			...
			if (currCaught.size() == types.size()) {
				shortestInterval =
				    Math.min(shortestInterval, right - closestLeft + 1);
			}
		}
