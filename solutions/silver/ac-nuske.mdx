...
11
```
$\texttt{prefix[0][1]} = 1$ as there is one connected component in the first row, and
$\texttt{prefix[1][0]} = 1$ as there is one connected component in the first column. However,
$\texttt{prefix[1][1]} = 1$ also as these two connected components get merged.
...
**Case 1:** $\texttt{grid[i][j]} = \texttt{grid[i - 1][j]} = \texttt{grid[i][j - 1]} = 1$ \
```
? 1
1 1
```
We name $\texttt{grid[i - 1][j]}$ $A$ and $\texttt{grid[i][j - 1]}$ $B$.
```
? A
B 1
```
Before we added $\texttt{grid[i][j]}$, $A$ and $B$ must have been part of separate connected
...
$\texttt{pref[i][j]} = \texttt{pref[i - 1][j]} + \texttt{pref[i][j - 1]} - \texttt{pref[i - 1][j - 1]} - 1$.

**Case 2:** $\texttt{grid[i][j]} = 1$, $\texttt{grid[i - 1][j]} = \texttt{grid[i][j - 1]} = 0$ \
```
? 0
0 1
```
...
**Case 3:** All remaining cases where $\texttt{grid[i][j]} = 1$ \
```
? 1
0 1
```
...
$\texttt{pref[i][j]} = \texttt{pref[i - 1][j]} + \texttt{pref[i][j - 1]} - \texttt{pref[i - 1][j - 1]}$.

**Case 4:** $\texttt{grid[i][j]} = 0$ \
No new connected component can be made by introducing $\texttt{grid[i][j]}$, and no two connected
...
int main () {
    ...
    for (int i = 0; i < Q; i ++) {
        ...
        // How many new connected components appear in the top row and leftmost column
