...
public class MinimizeDiameter {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(Math.max(Math.max(tree1Diameter, tree2Diameter), addedEdgeDiameter));
	}

	private static Edge dfs(List<List<Integer>> tree) { return dfs(tree, 1, 0, 0); }

	/** @return the farthest node and its distance from the given node. */
	private static Edge dfs(List<List<Integer>> tree, int node, int previous, int length) {
