...
public class MinimizeDiameter {
    ...
    /** @return the farthest node and its distance from the given node. */
    static Edge dfs(List<List<Integer>> tree, int node, int previous, int length) {
        Edge maxPath = new Edge(node, length);
        for (int i : tree.get(node)) {
            if (i == previous) {
                continue;
            }
            Edge other = dfs(tree, i, node, length + 1);
            if (other.second > maxPath.second) {
                maxPath = other;
            }
        }
        return maxPath;
    ...
    static Edge dfs(List<List<Integer>> tree) {
        return dfs(tree, 1, 0, 0);
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        pw.println(Math.max(Math.max(tree1Diameter, tree2Diameter), addedEdgeDiameter));
        pw.flush();
    }
}
```
