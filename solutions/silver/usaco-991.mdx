...
Firstly, observe that we can binary search on the answer, or $X$ in this case. For each $X$, we check if it is valid and update our binary search accordingly.

Notice that the constraint on $M$ is up to $10^{12}$, which leads us to believe that the solution will be in square root time. To more efficiently determine whether a given $X$ value is valid, we can simulate the process. However, we process all states where $Y$ does not change at once.

Additionally, notice that if Bessie gives more than $\sqrt{2n}$ distinct values of $Y$, then she will have definitely payed off the debt, since $1 + 2 + \ldots + \sqrt{2n} \ge N$, which means that the upper bound of the numebr of operations we must make is $\sqrt{2N}$.

Therefore, we can validate a value of $X$ in $\mathcal{O}(\sqrt{N})$ time, and the entire solution runs in $\mathcal{O}(\sqrt{N}log {N})$.
