...
Additionally, notice that if Bessie gives more than $\sqrt{2n}$ distinct values of $Y$, then she will have payed off the debt, since $1 + 2 + \ldots + \sqrt{2n} \ge N$, which means that the upper bound of the numebr of operations we must make is $\sqrt{2N}$.
