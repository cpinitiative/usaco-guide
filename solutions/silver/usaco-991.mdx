...
Additionally, notice that if Farmer John gives more than $\sqrt{2N}$ distinct values of $Y$, then he will have definitely paid off the debt, since $1 + 2 + \dots + \sqrt{2N} \ge N$, which means that the upper bound of the number of operations we must make is $\sqrt{2N}$, satisfying our time constraints.
