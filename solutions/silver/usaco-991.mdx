...
Firstly, observe that we can perform a binary search on the answer, or $X$ in this case. For each $X$, we check if it is valid and update our binary search accordingly.
...
Therefore, we can validate a value of $X$ in $\mathcal{O} (\sqrt{N})$ time, and the entire solution runs in $\mathcal{O}(\sqrt{N}\cdot \log {N})$.
