...
To efficiently determine whether a given $X$ value is valid, we can simulate the process. However, we process all states where $Y$, the number of gallons Farmer John pays back, does not change all at once.

Additionally, notice that if Farmer John gives more than $\sqrt{2N}$ distinct values of $Y$, then he will have definitely paid off the debt, since $1 + 2 + \ldots + \sqrt{2N} \ge N$, which means that the upper bound of the number of operations we must make is $\sqrt{2N}$, satisfying our time constraints.
