...
author: Brad Ma, Raviteja Kompalli, Kevin Sheng, Arnav Gokhale
---
...
Firstly, notice that we can binary search on the answer, or $X$ in this case.
For each $X$, we must check if it is valid, and update the binary search
accordingly. We can binary search on $X$ because, for smaller values of $X$, we
are giving more gallons of milk. Therefore, if it is possible for one $X$ value
to meet the requirement, it is possible for all of the values less than that $X$
to do so as well.

To efficiently determine whether a given $X$ value is valid, we can simulate the
process. However, we process all states where $Y$, the number of gallons Farmer John
pays back, does not change all at once.

Additionally, notice that if Farmer John gives more than $\sqrt{2N}$ distinct values
of $Y$, then he will have definitely paid off the debt, since
$1 + 2 + \ldots + \sqrt{2N} \ge N$, which means that the upper bound of the
number of operations we must make is $\sqrt{2N}$, satisfying our time
constraints.

Therefore, we can validate a given value of $X$ in $\mathcal{O} (\sqrt{N})$
time, and the entire solution runs in $\mathcal{O}(\sqrt{N}\cdot \log {N})$.
...
bool can_repay(long long num_gallons, long long within_days, long long at_least,
	...
	while (within_days > 0 && g < num_gallons) {
		...
		if (y < at_least) {
			long long leftover = ((num_gallons - g) + (at_least - 1)) / at_least;
...
int main() {
	...
	std::ofstream("loan.out") << low << endl;  // low == high, we can output either
}
```
