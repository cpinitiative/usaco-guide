...
If we pre-subtract each element in the inequality by the average $X$ we look for from the milk production array and compute its prefix sum, then we can reformulate the final inequality as a maximum subarray sum problem and use DP instead.
