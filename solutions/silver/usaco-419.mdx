...
Alternatively, one may subtract the average $X$ being verified from each value in $M$, resulting in an array where the average of a set in $M$ is smaller than or equal to $X$ if and only if the sum of the corresponding set in the modified array is less than or equal to $0$. This means that the maximum subarray sum of the modified array except the first and last elements is desired to minimize the average of the modified sum of the remaining elements, obtained using a greedy method.

To see how this is equivalent to the main approach, note that that the sum of $[l, r]$ in the modified array is the sum in the original array minus $X\cdot (r-l+1)$.  Using this to rewrite the prefix sum array and thus the last equation in the main solution will result the same optimization goal.
