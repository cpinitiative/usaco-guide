...
Now, if we find the prefix sum array $\texttt{pref}$ of the machine milk production array $M$, consider the condition we need to achieve an average $\le X$ by removing a subarray from $i$ to $j$ (right endpoint exclusive):
$$
