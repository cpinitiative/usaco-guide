...
rented, this would run in $O(NM)$, which is too slow for $N = 10^5$, $M = 10^5$.
...
Now, the total time complexity of milk calculations is $O(NlogM)$, which is adequate for $N = 10^5$, $M = 10^5$.
