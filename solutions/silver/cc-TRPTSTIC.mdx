...
vector<vector<ll>> grid,
    pref;  // Since N, M <= 1e6, we cannot just use an array

ll rangesum(ll row1, ll column1, ll row2,
            ll column2) {  // Requires row1 < row2; column1 < column2
	// BeginCodeSnip{We make this function to make applying prefix sums easier}
	// Sometimes, the range of squares the mentor can reach goes outside the
	// grid
	if (row1 < 0) row1 = 0;
	if (row2 >= N) row2 = N - 1;
...
bool test(ll distance) {  // Testing whether a certain distance will work
	...
	for (ll i = 0; i < N; i++) {
		for (ll j = 0; j < M; j++) {
			if (grid[i][j] ==
			    0) {  // The mentor has to at least be able to stay in this room
				continue;
			}
...
void solve() {
	...
	// Binary search for the answer (minimal = first true) and directly
	// outputting it
