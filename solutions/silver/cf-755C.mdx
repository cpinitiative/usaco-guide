...
So, we can count the number of distinct vertices in $P$ and divide by two to find the number of trees, being careful to count the case where $P[i]$ = $i$ as one tree, since it is just a isolated vertex to be counted as one tree.
