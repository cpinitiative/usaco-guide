...
## Explanation
The "remainder when the number of ones in the length-K window of $b$ with leftmost index $i$ is divided by two" is analogous to the XOR of the window. The only difference between the windows of $r_i$ and $r_{i+1}$ is that $r_i$'s window contains $b_i$, while $r_{i+1}$'s contains $b_{i+k}$.

$$r_i\oplus r_{i+1} = b_i\oplus b_{i+k}$$

Since we are given $r$, choosing the value of $b_i$ determines $b_{i+k}$, which in turn determines $b_{i+2k}$, $b_{i+3k}$, and so on. This splits $b$ into $k$ binary chains, each determined by one of the first $k$ characters of $b$. For each chain, we can set the first character to `0` and `1`, and compare which one will minimize the amount of `1` bits.

The only thing that restricts the first $k$ bits of $b$ is $r_1$, which will sometimes force us to flip one of our initial bits despite it increasing the number of `1` bits in the chain. We can check if our configuration satisfies $r_1$ by recording the bitwise XOR of the optimal first $k$ bits, and storing a running minimum $m$ the minimal cost of flipping a chain. If the bitwise XOR of the first k elements doesn't align with $r_1$, we can then flip that chain back and add $m$ to our count.

We can then apply the same idea to maximize the amount of `1` bits.

## Implementation

**Time Complexity:** $\mathcal{O}(\Sigma{N})$
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n-k; i++) {
            y += ((y[i] == '1')^(x[i] == '1')^(x[i+1] == '1'))+'0';
            c[i%k].first += y[y.size()-1]-'0';
            c[i%k].second++;
        }
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 0; t < test; t++) {
            ...
            for (int i = 0; i < k; i++) y.append('0');
            y.ensureCapacity(n); // optional, like reserve in C++

            for (int i = 0; i < n - k; i++) {
                int bit = ((y.charAt(i) - '0') ^ (x.charAt(i) - '0') ^ (x.charAt(i + 1) - '0'));
                y.append((char)(bit + '0'));
...
for _ in range(t):
    ...
    y = [False] * n  # initialize full length, first k bits are '0'

    for i in range(n - k):
        y[k+i] = y[i] ^ (x[i] == '1') ^ (x[i+1] == '1')
        c[i%k][0] += y[k+i]
        c[i%k][1] += 1
