...
For each city, we find the nearest tower to its left and right using binary search. We then calculate the distance to both towers and pick the smaller one—that's the minimum radius needed for that city.
Finally, we take the largest of these minimum radii across all cities.
