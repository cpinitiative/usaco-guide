...
## Solution 3 - Using a Sorted List
...
for city in cities:
    dist = float('inf')
    closest_tower = bisect.bisect_left(towers, city)

    if closest_tower != len(towers):
        dist = towers[closest_tower] - city

    if closest_tower > 0:
        dist = min(dist, city - towers[closest_tower - 1])
