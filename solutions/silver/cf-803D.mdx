...
If we can find whether formatting an ad with a given width within the line constraints is possible, we can binary search on the width, printing the minimum width that works.

Since spaces function identically to hyphens, we can treat them similarly. We can break the ad into blocks of word lengths, with each word length including the word plus a space or hyphen. The only exception is the last word, which will have neither.

Given a width, we will add as many words as possible to each line, creating a new line when we run out of room. If the number of lines we use is less than or equal to the maximum number of lines, then our width is fine. Note that if any word is wider than our given width, we must use a greater width.
