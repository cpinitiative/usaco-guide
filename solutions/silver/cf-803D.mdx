...
// returns true if most optimized ad of given width doesn't use greater than
// max_lines
bool f(int width) {
	int lines = 0;
	int cur_width = 0;
	...
	for (int word_length : word_lengths) {
		// if an individual word is wider than our ad width, we need a greater
		// width
...
int main() {
	...
	for (char i : ad) {
		word_lengths.back()++;
		// spaces and hyphens function the same, start new word after adding
		// char
