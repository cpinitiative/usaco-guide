...
First, let's construct the lexicographically greatest sequence without the operation, and call this $s$. We can do this easily using a monotonic stack. We iterate from left to right and while the stack's top element is less than the current element, we pop it. Then, we push the current element onto the stack. We also maintain the indices at which these elements appear in array $a$.
...
To understand this, consider the test case $[6, 4, 2, 5, 2, 3]$, where we consider moving 5 right after 6. The subarray $[4, 2]$ has a maximum of $4$, which is greater than the next element in $s$, 3, so this move is beneficial. However, if the $4$ were a $1$ or $2$, moving it wouldn't have been beneficial.

The only thing remaining is to find the optimal chosen element. To do this, we iterate $s$ from left to right, and whenever we find an element satisfying the condition, we choose this element to move and break. This is because future elements satisfying the condition aren't as optimal. Consider the array $[7, 5, 1, 6, 3, 4]$. We can either move $6$ right after $7$ or $4$ right after $6$, as they both satisfy the condition. However, the former leads to the answer $[7, 6, 5, 4]$ and the latter leads to the answer $[7, 6, 4, 3]$, which is lexicographically smaller.
...
int main() {
	...
	while (t--) {
		...
		for (int i = 0; i < n; i++) {
			while (!best.empty() && best.back().first < arr[i]) { best.pop_back(); }
			best.push_back({arr[i], i});
		}

		// find optimal operation
		best.insert(best.begin(), {n + 1, -1});
		best.push_back({-1, n});
		...
		for (int i = 1; i < (int)best.size() - 1; i++) {
			if (best[i].second != best[i - 1].second + 1) {
				int left = (i == 1) ? 0 : best[i - 1].second + 1;
				int right = best[i].second - 1;

				int mx = *max_element(arr.begin() + left, arr.begin() + right + 1);
				if (mx >= best[i + 1].first) {
					move_index = best[i].second;
					new_index = left;
					move_cow = best[i].first;
					break;
				}
			}
		}
		...
		if (move_index != -1) {
			arr.erase(arr.begin() + move_index);
			arr.insert(arr.begin() + new_index, move_cow);
		}

		std::vector<int> new_best;
		for (int i = 0; i < n; i++) {
			while (!new_best.empty() && new_best.back() < arr[i]) {
				new_best.pop_back();
			}
			new_best.push_back(arr[i]);
		}

		for (int i = 0; i < (int)new_best.size(); i++) {
			std::cout << new_best[i] << " \n"[i == (int)new_best.size() - 1];
		}
	}
}
```
