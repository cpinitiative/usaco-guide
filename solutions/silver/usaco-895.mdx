...
We can use Breadth-First Search (BFS) to explore each blob, starting from an unvisited '#' cell. During BFS, we can calculate the blob’s area by counting the number of '#' cells visited and determine its perimeter by carefully analyzing the surroundings of each '#' cell. The perimeter is determined by counting the number of sides of the cell that are either adjacent to a '.' cell or are on the boundary of the grid. For example, if a '#' cell is surrounded on all four sides by other '#' cells, it is considered to be completely inside the blob and does not contribute to the perimeter. However, for each side of a '#' cell that is either adjacent to a '.' cell or is on the edge of the grid (top, bottom, left, or right boundaries), that side contributes one unit to the perimeter.

As we traverse the grid, we can initiate a BFS for each unvisited '#' cell to compute the area and perimeter of all blobs. Within the BFS, for each '#' cell, we start by assuming that all four of its sides contribute to the perimeter and then reduce this count by one for each neighboring '#' cell in the grid (since shared edges do not add to the perimeter). We can track the largest area found, and if multiple blobs share the same largest area, we can choose the one with the smallest perimeter. Finally, we output the largest area and the corresponding smallest perimeter.
...
public class IcyPerimeter {
    ...
    private static int[] bfs(int row, int col, char[][] grid, boolean[][] visited, int n) {
...
pair<int, int> bfs(int row, int col, vector<vector<char>>& grid, vector<vector<bool>>& visited, int n) {
...
def bfs(row: int, col: int, grid: List[List[str]], visited: List[List[bool]], n: int) -> Tuple[int, int]:    
    queue = deque()
