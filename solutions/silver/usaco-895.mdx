        ...
        while (!q.isEmpty()) {
            ...
            for (int[] direction : DIRECTIONS) {
                ...
                if (nextRow >= 0 && nextRow < n && nextCol >= 0 && nextCol < n) {
                    if (grid[nextRow][nextCol] == '#') { // If the neighbor (nextRow, nextCol) is part of the same blob
                        q.add(new int[]{nextRow, nextCol}); // Add the neighbor (nextRow, nextCol) to the queue
    ...
    while (!q.empty()) {
        ...
        for (auto [dx, dy] : directions) {
            ...
            if (next_row >= 0 && next_row < n && next_col >= 0 && next_col < n) {
                if (grid[next_row][next_col] == '#') { // If the neighbor (next_row, next_col) is part of the same blob
                    q.push({next_row, next_col}); // Add the neighbor (next_row, next_col) to the queue
                    sides--; // Shared edges reduce the perimeter contribution
                }
            }
        }
    ...
    while queue:
        ...
        for dx, dy in directions:
            ...
            if 0 <= next_row < n and 0 <= next_col < n:
                if grid[next_row][next_col] == '#':  # If the neighbor (next_row, next_col) is part of the same blob
                    queue.append((next_row, next_col))  # Add the neighbor (next_row, next_col) to the queue
...
def main():
    ...
    with open("perimeter.out", "w") as fout:
        fout.write(f"{max_area} {min_perimeter}\n")
