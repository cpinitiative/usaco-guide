...
pair<int, int> bfs(int row, int col, const vector<vector<char>>& grid, vector<vector<bool>>& visited, int n) {
...
def bfs(row: int, col: int, grid: List[str], visited: List[List[bool]], n: int) -> Tuple[int, int]:
    queue = deque()
