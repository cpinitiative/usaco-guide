...
1. An empty cell on the border is always good, as if FJ places conveyers optimally,
   the cell will point outward.
2. A cell pointing into another good cell is also good. If FJ places conveyors
   onto empty cells optimally, the conveyor should point into an adjacent good
   cell if possible.
...
Whenever we "undo" a query, we DFS from the new empty cell. If it is good,
then we check if any cells around it become good. Otherwise, we leave it as is.

The floodfill is guaranteed to run in $\mathcal{O}(N^2)$ because good cells are
not revisited, meaning that $N^2$ cells will be visited at most.
