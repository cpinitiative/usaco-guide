...
1. An empty cell on the border is always good, since if FJ places conveyers optimally,
...
Whenever we "undo" a query, we perform DFS from the new empty cell. If it is good,
then we check if any cells around it have become good. Otherwise, we leave it as is.

The floodfill is guaranteed to run in $\mathcal{O}(N^2)$ since good cells are
not revisited, and therefore at most $N^2$ cells will be visited.
