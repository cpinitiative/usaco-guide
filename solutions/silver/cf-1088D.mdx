...
[Official Editorial](https://codeforces.com/blog/entry/63656)
...
- both $a$ and $b$ has bit $B$ activated
- neither $a$ and $b$ has bit $B$ activated
- only $a$ has bit $B$ activated
- only $b$ has bit $B$ activated

When it is the first two cases, the results for the two queries will be different. This is because the since we are using XOR on an activated bit in one number and not the other, there will always be one greater since they have the same bit at this position. If the first query yields $-1$ or second query yields a $1$, both $a$ and $b$ must have bit $B$ activated.
