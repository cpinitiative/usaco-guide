...
We can use recursive "flood fill" to find each connected group (moo network) and compute the perimeter of the bounding box for each group along the way.

We can iterate through each cow, launch a recursive search from that cow, and find all cows belonging to the same group. Then, we can calculate the minimal perimeter of a fence enclosing that group by considering each cow's coordinates. Finally, we output the smallest perimeter of all such fences.
