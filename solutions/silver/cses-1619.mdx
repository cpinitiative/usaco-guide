...
Due to Python's big constant factor, this mirrored code will TLE on one test
case.
...
Since "all arrival and leaving times are distinct," we can store if a given time is an arrival or departure, and sort all the arrival and leaving times in one array. This is fast enough to pass AC.
...
for i in range(2 * n):
	if i == 0:
		...
		at += 1
		curr = times[i][0]
	ppl_change[at] += times[i][1]  # Adjusted indexing here

ppl_amt = [0] * (2 * n + 1)

for i in range(1, 2 * n + 1):
	ppl_amt[i] = ppl_change[i - 1] + ppl_amt[i - 1]  # Adjusted indexing here
