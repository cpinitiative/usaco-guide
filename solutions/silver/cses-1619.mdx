...
author: Michael Cao, Neo Wang, Brad Ma, Alex Du
---
...
Due to Python's big constant factor, this mirrored code will TLE on one test case.
...
print(max_ppl)
```
Since "all arrival and leaving times are distinct," we can store if a given time is an arrival or departure, and sort all the arrival and leaving times in one array as in the below solution:
