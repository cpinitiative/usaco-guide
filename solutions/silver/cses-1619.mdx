...
Due to Python's high constant factor, this code may be too slow.
...
Since all arrival and leaving times are distinct, we can store if a given time is an arrival or departure, and sort all the arrival and leaving times in one array. This saves a bit of time by removing the need to sort a list of tuples, and passes all cases.
...
for _ in range(int(input())):
	...
	# Change is 1 when person enters, -1 when person leaves
	tmap[start] = 1
	tmap[end] = -1
	times.append(start)
	times.append(end)
...
Due to Python's big constant factor, this code may TLE.
