...
def solve():
    ...
    for _ in range(test_num):
        half_size = int(sys.stdin.readline().strip())
        ...
        for to_remove in nums.keys():
            start = max(nums.keys()) + to_remove
            at = start
            ...
            for _ in range(half_size):
                ...
                moves.append((largest, other))
            
            if len(moves) == half_size:
                valid_start = start
                valid_moves = moves
                break
