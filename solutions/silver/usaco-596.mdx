...
Recursive DFS (which the C++ code uses) is too slow, so here we must use an iterative BFS approach.
...
public class BuildGates {
	private static boolean[][] fence = new boolean[4003][4003];
	private static boolean[][] visited = new boolean[4003][4003];
	private static final int maxx = 2001, minx = 2001, maxy = 2001, miny = 2001;
