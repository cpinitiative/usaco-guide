...
**Time Complexity:** $\mathcal{O}(T \cdot \binom{7}{x} \cdot x! \cdot x)$
...
Because we do not know the values of the input array ahead of time, some of the candidate values could be invalid.

The next step would be to take all $N^3$ tuples of and check if this solution of $(A, B, C)$ by checking if all the values in the array satisfy $A, B, C, A+B, ..., A+B+C$.

## Implementation:
**Time Complexity:** $\mathcal{O}(T \cdot N^5)$
		...
		for (int i = 0; i < candidatesVector.size(); i++)
			for (int j = i; j < candidatesVector.size(); j++)
				for (int k = j; k < candidatesVector.size(); k++) {
					int a = candidatesVector[i], b = candidatesVector[j],
					    c = candidatesVector[k];
