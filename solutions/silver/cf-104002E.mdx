...
First note that for any even number $k \leq n$, a valid solution must have selected at most $k/2$ integers. This is the only requirement for a valid solution.

To solve this problem, we can use a greedy algorithm. For any even $k$, if we know the $k/2$ integers that make the highest sum, we can use this to find the $k/2+1$ integers for $k+2$.
...
int main() {
    ...
    for (int i = 2; i < n; i += 2) {
        ...
        // if it is worth it to give up a previous number to take both, give up the smallest
        ...
        // take the max of the two otherwise
        else {
