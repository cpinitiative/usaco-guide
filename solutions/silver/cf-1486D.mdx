...
To check whether the median $x$ is achievable, we need to count the number of elements that are $\ge x$ and the number of elements that are $< x$.

If the number of elements $\ge x$ is greater than the number of elements $< x$, we can say that this median is achievable.
Note that there must be strictly more elements greater than or equal to $x$ than less than $x$ because of how the problem defines the median.
...
The solution's time complexity in O(nlogn) which satisfies the problem constraints.
