...
To check whether a median at least $x$ is achievable on an interval, we need to count the number of elements that are $\ge x$ and the number of elements that are $< x$.
...
We can prove that if $f(i) - f(j) > 0$ and $i \ge j + k$, it is possible to have a median of $x$.
...
From here we can perform a linear search for the max difference keeping track of the minimum element in the prefix $f(0\dots i-k)$ which ensures that the size is always at least $k$. If it greater than $0$, then this median is possible to achieve.
