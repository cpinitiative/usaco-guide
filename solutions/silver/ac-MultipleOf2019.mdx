...
[Official Editorial](https://img.atcoder.jp/abc164/editorial.pdf#page=10)

## Explanation
In order to store sections of the string, we can make a suffix array $S$, such that:

$${S_i = s_n + 10*s_{n-1} +100*s_{n-2} + \dots + {10^{n-i}} * s_{i}}$$
...
It also means that if both $S_0 \bmod M = S_3 \bmod M$ (both suffixes have the same remainders), subtracting would lead to $(S_0 \bmod M) - (S_3 \bmod M) = 0$, meaning a substring that is divisible by $2019$.

So with $1234$, $(S_0 \bmod 3) - (S_3 \bmod 3) = (1234 \bmod 3) - (4 \bmod 3) = 1 - 1 = 0$. Because they subtract to zero, it means the substring from index $0$ to $2$ leads to a number divisible by the modulus. (The substring is $123$ which is divisible by 3).
...
Therefore, our answer is the counts of all the remainders in an array from $0\dots2018$ and ${N \choose 2}$.
