...
author: Qi Wang, Melody Yu (Video), Brad Ma, David Guo
---
...
int main() {
    ...
    sort(east_cows.begin(), east_cows.end(), [&](int a, int b) {
        return y_coordinates[a] < y_coordinates[b];
    });
    // sort the north cows by x-coordinate
    sort(north_cows.begin(), north_cows.end(), [&](int a, int b) {
        return x_coordinates[a] < x_coordinates[b];
    });
    ...
    for (int j : east_cows) {  // loop through all pairs of cows
        for (int k : north_cows) {
			 ...
			 */
            if ((!stopped[j]) && (!stopped[k]) && (x_coordinates[k] > x_coordinates[j]) && (y_coordinates[j] > y_coordinates[k])) {
                ...
                } else if ((x_coordinates[k] - x_coordinates[j]) < (y_coordinates[j] - y_coordinates[k])) {
