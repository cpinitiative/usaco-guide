...
First, notice that when you have spiders that are friends, the $\gcd$
requirement implies that they must share a common prime factor.  This implies
that all spiders with a common factor can directly pass messages to one another.

Note that "directly" passing messages from one another for *any* spiders that
share the common factor is expensive to maintain; $3\times10^5$ is already
enough spiders such that having many more orders of magnitudes of edges to
represent all possible communications naively simply isn't going to cut it.
...
have before we reach $3\times10^5$ ($2\times3\times5\times\cdots\times\17$ with
...
overall, but it may be dominated by factorization.
...
$\mathcal O(n\frac{\log n}{\log\log n}+n\log\log A)\approx\mathcal O(n\log\log n)$.
