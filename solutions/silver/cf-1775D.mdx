...
so our number of edges is bounded by $O(N \log N)$ (since the smallest prime number is $2$ and
most primes are larger than that, there's at most $\log N$ primes that needs to be multiplied
before reaching $N$).
...
**Exact Time Complexity:** $\mathcal O\left(N\frac{\log N}{\log\log N}+\frac A{\log A}+N\log\log A\right)$
where $A=\max(a_i)$.

This implementation uses a sieve for a little bit of speedup, which is not strictly
required for this problem. The above time complexity is for the sake of exactness
and obtained by numbered theory; for practical purposes, it is trivial that this
is bounded by $\mathcal O(N\log N)$.
