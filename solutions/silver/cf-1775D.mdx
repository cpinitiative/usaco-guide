...
As for the number of edges, consider that any integer $n$ has at most $\log{n}$ prime factors, since factorizing out each prime factor at least halves $n$. Thus, the number of edges is bounded by $\mathcal{O}(N \log{N})$.
