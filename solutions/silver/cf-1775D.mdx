...
int main() {
	...
	for (int s = 0; s < spider_num; s++) {
		int spider;
		cin >> spider;
		for (int f : (prime_factors[s] = primes(spider))) { trains[f].push_back(s); }
	}
	...
	set<int> taken;
	// each spider's predecessor in the bfs tree
	...
	while (!frontier.empty()) {
		...
		for (int s : frontier) {
			...
			for (int f : prime_factors[s]) {
				if (taken.count(f)) {
					// alrady used this prime factor b4, let's not waste time
					continue;
				}
				for (int ns : trains[f]) {
					if (come_from[ns] == -1) {
	...
	// backtrack from the end until we get to the start
	vector<int> path{end};
