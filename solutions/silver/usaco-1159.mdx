...
void solve() {
    ...
    ll a = 0;
    for(ll i = 0; i < n; ++i){
        if(a >= sz(startComponent)) break;
        ll dist = abs(startComponent[a] - i);
        while(a < sz(startComponent) - 1 && abs(startComponent[a + 1] - i) < dist){
            a++;
            dist = abs(startComponent[a] - i);
        }
    ...
    ll b = 0;
    for(ll i = 0; i < n; ++i){
        if(b >= sz(endComponent)) break;
        ll dist = abs(endComponent[b] - i);
        while(b < sz(endComponent) - 1 && abs(endComponent[b + 1] - i) < dist){
            b++;
            dist = abs(endComponent[b] - i);
        }
