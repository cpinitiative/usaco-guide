...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		// still be placed onto the top of the towers, process
		while (!towers.empty() && towers[0][0] <= (a[i][0] - k) && amt > 0) {
			if (towers[0][1] > amt) {
				towers[0][1] -= amt;
				amt = 0;
				break;
		...
		towers.push_back({a[i][0], a[i][1] - amt});
		count += a[i][1] - amt;
	}
	cout << count;
}
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < n; i++) {
			...
			cowGroups[i] = new Group(w, Math.min(a, m));
		}

		Arrays.sort(cowGroups, Comparator.comparingInt(group -> group.weight));

		// Queue that stores towers based on weight-count pairs
		Deque<long[]> towers = new ArrayDeque<>();
		towers.addFirst(new long[]{(long) -1e18, m}); // Initial large tower

		long answer = 0;

		for (Group g : cowGroups) {
			long w = g.weight;
			int a = g.amt;
			int remaining = a;
