...
However, this takes $\mathcal{O}(C\log C)$ operations where C is the total amount of cows, which is still too slow. To optimize this, instead of considering cows individually, we can group them up by weight and count. Then, by maintaining a queue of cow groups sorted by the weight, we can process all cows in O($N$).
...
int main() {
	...
	// Deque that stores the cows based on weight count pairs
	q.push_back({-1e9, m});
