...
However, this takes $O(c$ $logc$) operations, which is still too slow. To optimize this, instead of considering cows individually, we can group them up by weight and count. Then, by maintaining a queue with the groups of towers with sorted by the weight of the group, we can process all cows in O($N$).
