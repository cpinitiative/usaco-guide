...
<Warning>
Due to the strict time limits, this Python code recieves a TLE verdict on a test case.
</Warning>
...
# Variables to store graph data and visited nodes
neighbors = []
visited = []
threshold = 0
num_reachable = 0

# Function to search all vertices that can be reached through the current vertex v
def search_videos(v):
