...
We [DFS](/silver/graph-traversal#dfs) on an arbitrary point and calculate the distance to each point.
The point with the maximum distance is one of the endpoints of the diameter.

We can run DFS again from this point, where the maximum distance from this new point is our actual diameter.
...
At each node in the tree, we compute the heights of its two tallest subtrees.
By adding them together, we find the length of the longest path through that point.
Our answer is then the longest path we've found across all nodes.
