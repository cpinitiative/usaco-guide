...
We [DFS](/silver/graph-traversal#dfs) on an arbitrary node and calculate the distance to each node.
Since the diameter is the longest path, one of its endpoints must be furthest away from the starting point. Therefore, the node with the maximum distance is one of the endpoints of the diameter.
