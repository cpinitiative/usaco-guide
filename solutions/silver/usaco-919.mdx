...
We record for each rectangle its respective corner increments and decrements in an auxiliary two dimensional array. Then we run a two dimensional prefix sum over that array to rebuild the actual count of coats of paint in every cell. Finally, we scan the reconstructed grid and count how many cells have exactly the required number of coats.
