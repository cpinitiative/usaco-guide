...
author: Kevin Sheng, thetazero, Melody Yu, Ruben Jing
---
...
We can find the prefix array in order to quickly compute whether or not a group of cows has IDs summing to a multiple of $7$.
Given a prefix array $p$, the sum between a range $[l, r]$ is $p[r] - p[l-1]$. We must also make sure this sum is a multiple
of $7$ in order to be a valid group. These two conditions give us the following:
$$
\left(p[r] - p[l-1]\right)\%7 = 0 \\
p[r]\%7 - p[l]\%7 = 0 \\
p[r]\%7 = p[l]\%7
$$
Thus, we can simplify the solution to finding where two prefixes mod $7$ are equal.

Additionally, becase the prefix at $i$ is defined by
$$
p[i]\%7 = \left(a[i]+p[i-1]\right)\%7,
$$
we can find the prefix mod $7$ when precomputing to avoid large numbers.

In order to find the largest consecutive group, we try to find the largest distance between two equal prefixes. We can have
a list that stores the first occurrence of each remainder. Everytime we see a remainder again, we calculate the distance
