...
We use a greedy algorithm to find $\texttt{possible}(i)$. Firstly, we want to sort all houses by their address. For each $j$ from $1\dots n$, let $h_j$ be the address of the $j$th house. Place a fire hydrant at address $h_j + i$, and this hydrant will cover all houses in the range $[h_j, h_j + 2i]$. We can repeat this process for the first house to the right whose address does not fall into this range until every house can be connected with at least one first hydrant. Note that due to the circular nature of the street, we must do all calculations modulo $1000000$. If the number of hydrants needed is greater than the number of fire hydrants allowed, $\texttt{possible}(i)$ is false; otherwise, it is true.

## Implementation

**Time Complexity**: $\mathcal{O}(h^2 \cdot \log(\max(H_i)))$
