...
Note: The video solution might not be the same as other solutions. Code in C++, Python, and Java.
<YouTube id="IeOHxgaOfis" />
...
If during DFS, we find a move, represented by 'M' or 'O' followed by
...
After updating our board, we check if Bessie has won. We can hard code every winning possibility
since there are few cases to check.

During DFS, if we revisit a square in our current board state, we stop iterating. However,
