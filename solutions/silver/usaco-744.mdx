...
for r in range(width):
    for c in range(width):
        ...
        if color not in visible:
            visible[color] = {'min_r': r, 'max_r': r, 'min_c': c, 'max_c': c}
        else:
...
if len(visible) > 1:
    ...
    for color, bounds in visible.items():
        # initialize it as a difference array
        min_r, max_r, min_c, max_c = bounds['min_r'], bounds['max_r'], bounds['min_c'], bounds['max_c']
        rect_num[min_r][min_c] += 1
        rect_num[min_r][max_c + 1] -= 1
        rect_num[max_r + 1][min_c] -= 1
        rect_num[max_r + 1][max_c + 1] += 1
