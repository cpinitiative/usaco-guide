...
void join(int a, int b) { parent[find(a)] = parent[find(b)]; }

int main() {
	int n, m, k;
	std::cin >> n >> m >> k;

	for (int i = 0; i < 2 * (n + m); i++) { parent[i] = i; }
	...
	int cnt = -1;
	for (int i = 0; i < n + m; i++) {
		if (find(i) == find(i + n + m)) { return std::cout << "0", 0; }
		if (find(i) == i) { cnt++; }
	}
...
public class TableColoring {
	...
	static int[] parent = new int[400001];
	...
	private static void join(int a, int b) { parent[find(a)] = parent[find(b)]; }
...
parent = [i for i in range(400001)]
