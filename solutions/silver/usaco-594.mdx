...
bool valid_blast_radius(long long blast_radius, int k, std::vector<long long> bales) {
	/*
	...
	*/
	...
	long long last_blast_location = LLONG_MIN;
	int needed_cows = 0;
	for (long long bale_position : bales) {
...
int main() {
	...
	long long left = 0;
	long long right = bales[n - 1] - bales[0];
...
public class AngryCows {
	public static void main(String[] args) throws IOException {
		...
		long[] bales = new long[n];
		for (int i = 0; i < n; i++) { bales[i] = io.nextLong(); }

		Arrays.sort(bales);
		long left = 0;
		long right = bales[n - 1] - bales[0];
	...
	private static boolean validBlastRadius(long blastRadius, int k, long bales[]) {
		...
		long lastBlastLocation = Long.MIN_VALUE;
		int neededCows = 0;

		for (long balePosition : bales) {
