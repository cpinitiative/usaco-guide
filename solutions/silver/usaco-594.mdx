...
Notice that the value $R$ can exhibit between $0$ to $N$. Once R is less than the threshold we want to find, at least one hay bale will not detonate; greater than the threshold, no hay bale will survive. Perhaps we can use binary search for this property and incorporate it in our solution somehow?
</Spoiler>

<Spoiler title="Hint 2">
Say we already know what the value $R$ might be. What is the most efficient way to check if this $R$ value is valid? Can we do it in linear time?
</Spoiler>
