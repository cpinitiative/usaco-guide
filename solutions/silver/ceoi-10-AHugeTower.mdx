...
[Official Analysis](https://archive.org/download/oi-ted-ceoi-archive/CEOI/2010/)
...
Let $a_1, a_2, \dots, a_n$ be the block sizes, sorted from least to greatest.
Let $b_i$ be the number of indices $j$ such that $j < i$ and $a_i \leq a_j + d$.
Let $\texttt{ans}_i$ be the answer if the tower consisted of only the blocks $a_1, a_2, \dots, a_i$. Of course, this means that $\texttt{ans}_n$ will be the final answer.
