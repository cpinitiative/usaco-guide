...
Notice that if $C + W$ is odd or $C + O$ is even, the answer must be false, because we can't simplify the substring into a single $C$ without $O$ or $W$ characters remaining. This is because the parity of $C + W$ or $C + O$ is always constant, beacuse by performing the expansion or deletion operation, we can never change the parity. (For example, $CWCW$ -> $CW$ or $CW$ -> $COC$, both of which hold the same parity of C + W).
...
Otherwise we will be left with $W = 1$ and $O = 1$ meaning that $C$ was even and by combining the $WC$ pair into a singular $C$, we will be left with a odd amount of $C$, which can be simplified into one $C$ character.
