...
**Time Complexity:** $\mathcal{O}(N^2log(n))$
...
int ff(int r, int c, int color_1, int color_2) {
	...
	if (grid[r][c] != color_1 && grid[r][c] != color_2) { return 0; }
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			if (!visited[i][j]) { res1 = max(res1, ff(i, j, grid[i][j], grid[i][j])); }
		}
	}
	cout << res1 << "\n";
...
public class MultiMoo {
	...
	static int ff(int r, int c, int color1, int color2) {
		if (r < 0 || r >= n || c < 0 || c >= n) { return 0; }
		if (visited[r][c]) { return 0; }
		if (grid[r][c] != color1 && grid[r][c] != color2) { return 0; }
