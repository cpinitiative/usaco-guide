...
vector<vector<pair<int, int>>> find_removable_regions(const vector<string>& grid, int k) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < MAX_COLS; j++) {
            ...
            char color = grid[i][j];
            auto cells = find_region(grid, i, j);
            ...
            if (cells.size() >= k) {
                removable_regions.push_back(cells);
            }
        }
    }
...
// Simulate gravity by making non-empty cells fall to the lowest available position
...
public class MooyoMooyo {
    ...
    static final int[][] MOVES = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}}; // Movement directions
...
MAX_COLS = 10
MOVES = [(1, 0), (-1, 0), (0, 1), (0, -1)]  # Movement directions (up, down, left, right)
...
# Find all removable regions (regions of size >= k)
def find_removable_regions(grid, k):
    n = len(grid)
    removable_regions = []
    visited = [[False] * MAX_COLS for _ in range(n)]
...
def apply_gravity(grid):
    ...
    for col in range(MAX_COLS):
        ...
        for row in range(n - 1, -1, -1):
            if grid[row][col] != '0':
                grid[empty_row][col], grid[row][col] = grid[row][col], grid[empty_row][col]
                empty_row -= 1
