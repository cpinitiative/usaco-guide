...
The bound on $C$ is very small, what time complexity might this imply?
</Spoiler>
<Spoiler title="Hint 1 (answer)">
$\mathcal{O}(2^C \cdot C)$ or similar. This should inspire you to use bitmasks!
</Spoiler>
...
We may also compute this minimum distance with a [multi-source BFS](/silver/graph-traversal) in the same time complexity.
## Implementation
...
const int BIG = 1e9; // arbitrarily large integer
...
int main() {
	...
	for (int edit = 0; edit < c; edit++) {
		for (int mask = 0; mask < (1 << c); mask++) {
			...
			min_edits[mask ^ (1 << edit)] = min(min_edits[mask ^ (1 << edit)],
												min_edits[mask] + 1);
		}
	}
...
```py
BIG = 10**9  # arbitrary large integer
