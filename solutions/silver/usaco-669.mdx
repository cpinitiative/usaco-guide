...
**Time Complexity:** $\mathcal{O}(N^2 \log(2 \cdot 25000^2))$
...
int main() {
    ...
    for (int t = 0; t < N; t++) {
        ...
        for (int j = 0; j < N; j++) {
            if (dist[j] != INT_MAX) {
                dist[j] = min(dist[j], d2(cows[i], cows[j]));
            }
        }
    }
    cout << m << '\n';
        ...
        for (int t = 0; t < N; t++) {
            ...
            for (int j = 0; j < N; j++) {
                if (dist[j] != Integer.MAX_VALUE) {
                    dist[j] = Math.min(dist[j], d2(cows[i], cows[j]));
                }
            }
        }
        pw.println(m);
