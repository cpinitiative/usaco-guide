...
<YouTube id="DpIUSBBhNFU" />

# Solution 1 (DSU)

## Explanation

We can build edges between every pair of cows. After sorting the edges based on weight we can use a [DSU](/gold/dsu). Doing so ensures that we include every cow only once through the smallest weights possible.
