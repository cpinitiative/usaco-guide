...
title: Moocast
author: Neo Wang, Óscar Garries, Kevin Sheng, David Zhou
---

[Official Analysis (Java)](http://www.usaco.org/current/data/sol_moocast_gold_dec16.html)

## Explanation (Kruskal's)

We can build edges between every pair of cows. We can then construct an MST by sorting the edges based on weight. Using a [DSU](/gold/dsu), we ensure that we include every cow in the network only once through the smallest weights possible.
...
def all_reachable(power, cows):
    ...
    while q:
        curr = q.popleft()
        for c in range(n):
            if not reached[c] and dist_sq(cows[curr], cows[c]) <= power:
                reached[c] = True
                q.append(c)
