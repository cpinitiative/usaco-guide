...
int main() {
	...
	while (high - low > MAX_ERROR) {
		mid = (high + low) / 2;
		curr_min_time = get_time(
		    mid);  // returns time it would take if friends were to meet at mid
...
public class Solution {
	static ArrayList<Double> locations = new ArrayList<>();
	...
	public static double get_time(double loc) {
		...
		for (int i = 0; i < locations.size(); i++) {
			...
			if (i_time > max_time) {
				left_has_max =
				    false;  // reset to false, as we have a new maximum
				right_has_max = false;
	...
	public static void main(String[] args) {
		...
		double low = 0;
		double high =
		    0;  // high will be the location of the farthest north friend
		double mid;
