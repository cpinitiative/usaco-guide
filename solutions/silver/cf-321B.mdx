...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // sort by type and then by strength
        Collections.sort(jiro, new Comparator<Card>() {
            @Override
            public int compare(Card a, Card b) {
                if (a.type == b.type) {
                    return Integer.compare(a.strength, b.strength);
                }
                return Boolean.compare(b.type, a.type);
            }
        });

        Collections.sort(ciel, new Comparator<Card>() {
            @Override
            public int compare(Card a, Card b) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < m; j++) {
                // if we haven't used j and this card can beat Jiro's card
                if (!done[j] && ((jiro.get(i).type && ciel.get(j).strength >= jiro.get(i).strength) || 
                                (!jiro.get(i).type && ciel.get(j).strength > jiro.get(i).strength))) {
                    done[j] = true;
