...
We can first precompute the wu values for each pair of strings. Then, we can prefix sum up the number of strings that gives each wu value of each string (represented by `pref_wu` in the following implementation). Thus, for each query we can utilize output our prefix sum array.
...
minus wu of $(i \oplus j)$ will also give wu values of the bits that are the same. Also, since $k$ is at most $100$, we can precompute the answer for every $k$ of a string.
...
int main() {
	...
	for (int i = 0; i < (1 << n); i++) {
		for (int j = 0; j < (1 << n); j++) {
			...
			if (wu_value <= max_k) {
				pref_wu[i][wu_value] += str_count[j];
			}
		}
