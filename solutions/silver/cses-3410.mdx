...
Hence, for each point the sum and difference of the coordinates matters. Thus, we'll keep track of the maximum and minimum of the sum and difference.
When a new point comes up, we updates the values. The maximum of the values: $\text{maxsum}-\text{minsum}$ and $\text{maxdiff}-\text{mindiff}$ is the answer for the four cases.
