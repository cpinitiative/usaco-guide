...
into the same group in $\mathcal{O}(N)$ time.

Now we can just process all nodes with $\texttt{degree}+1\le\dfrac{N}2$, with DFS.

### Proof

We can handle all nodes with $\texttt{degree}+1>\dfrac{N}2$ in $\mathcal{O}(N)$ time.
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		// if any two nodes have degree+1 > n/2, they are part of the same
		// component
		if (n - adj[i].size() > n / 2) {
	...
	for (int i = 0; i < n; i++) {
		if (!group[i]) {
			...
			// if a new group has been created, e.g. curgroup = cur, increase
			// cur
			if (curgroup == cur) { cur++; }
		}
	}
