...
To check if a certain number of tickets reaches a minimal contribution of $k$, we can observe that in order to minimize the number of tickets sold, we should maximize the total revenue from selling these tickets. If, however, it is impossible to sell at least $k$ tickets, then output $-1$.
