...
Greedily, it is optimal to maximize the total revenue from selling these tickets since that will minimize the total number of tickets. If it is impossible to sell at least $k$ tickets, then output $-1$.

Additionally, the problem indicates that only $x\%$ of the $a$-th ticket and $y\%$ of the $b$-th ticket could contribute to the total ecological contribution. We can precompute the multipliers $pct$ (namely: x, y, and x+y) over all $t$ tickets, then greedily match the largest multiplier with highest value ticket by sorting the multipliers $pct$ and tickets $p$ in decreasing order. Our total ecological contribution is $\sum\limits_{i=1}^{t} p_i \cdot pct_i$.
