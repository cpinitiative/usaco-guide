...
public class GuessTheAnimal {
	...
	public static void main (String[] args) throws IOException {
		...
		for (int x = 0; x < characteristics.length; x++) { // initialize arraylists
		...
		for (int i = 0; i < n; i++) { // read input
			...
			for (int x = 0; x < k; x++) {
				...
				characteristics[i].add(string); // add to the array of arraylists
			}
		}
		...
		for (int i = 0; i < n; i++) { // loop through the combinations
			for (int j = i + 1; j < n; j++) {
				largest = Integer.max(largest, numCommon(i, j)); // keep track of the largest
			}
		}
