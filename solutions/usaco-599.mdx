...
int main() {
	...
	vector<vector<vector<int>>> dst(n,
																	vector<vector<int>>(n,
																											vector<int>(2, INT32_MAX)));

	// i, j, 0 - clockwise dist from i -> j
	// i, j, 1 - counter-clockwise dist from i -> j
	for (int i = 0; i < n; i++) {
		for (int j = i; j < i + n; j++) {
			if (i == j) {
				dst[i][j][0] = 0;
				...
				int x = (j + n) % n;
				int y = (j - 1 + n) % n;
				dst[i][x][0] = dst[i][y][0] + calc_dist(x, y);
			}
		}
	}
	for (int i = 0; i < n; i++) {
		for (int j = i; j > i - n; j--) {
			if (i == j) {
				...
				int x = (j + n) % n;
				int y = (j + 1 + n) % n;
				dst[i][x][1] = dst[i][y][1] + calc_dist(x, y);
			}
		}
	}
	...
	for (int i = 0; i < n; i++) {
		...
		/*
		 * two type of possible angles (90 deg either on inside or outside)
	...
	for (int i = 1; i < n; i++) {
		hsh.push_back((hsh.back() * P + dst[(i - 1 + n) % n][i][0]) % MOD);
		hsh.push_back((hsh.back() * P + ang[i]) % MOD);
	}
	hsh.push_back(hsh.back() * P + dst[n - 1][0][0]);
	...
	function<int(int i, int j)> get_hash = [&](int i, int j) {
		...
		return ((hsh[j] - ((hsh[i] * poly[j - i]) % MOD) + MOD) % MOD);
	};

	int ans = 0;

	for (int i = 1; i < (int)(hsh.size()) - 2; i++) {
		int fin = -1;

		for (int len = 1; len < (int)(hsh.size()) - i; len++) {
			int cr = get_hash(i, i + len);
			int occ = 0;

			for (int j = 0; j < (int)(hsh.size()) - len; j++) {
		...
		if (fin != -1) {
			...
			a = min(dst[fin][0][0], dst[fin][0][1]) + dst[strt][fin][0];
		}
		// default time to get to exit from strt
		int b = min(dst[strt][0][0], dst[strt][0][1]);
