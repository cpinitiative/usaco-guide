...
Instead of treating the towns as nodes, we're going to treat the regions themselves as nodes instead, with
the walls still being the edges.
We BFS from each node, logging the minimum distance from every region to every other region.
Finally, we try all meeting regions and print out the one that minimizes the walls the memebers have to cross.
...
for r in range(region_num):
	for w in regions[r]:
		if w in prev_walls:
			# dupes might hppen but that doesn't really matter
