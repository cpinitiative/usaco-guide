...
In this problem, you're given $n$ machines and asked the minimum time these machines need to work to create $t$ produces such that the $i$-th machine creates a product in $k_i$ time.
...
Overall, the $n$ machines can create ‎‎\sum\limits_{i=1}^n $\lfloor \frac{ans}{k_i} \rfloor$ products. If this sum $\geq t$, then $ans$ is valid.
