...
author: Ryan Kim, Andrew Cheng, Mohammad Nour Massri
---
...
public class GCDBoard {
	...
	public static void main(String[] args) throws IOException {
		...
		// Prefix will find the GCD from the start
		int[] gcdPrefix = new int[MAX_N];

		// Suffix will find the GCD from the end
		int[] suffix = new int[MAX_N];
		...
		for (int x = N; x >= 1; --x) {
			// GCD for suffix
			suffix[x] = gcd(suffix[x + 1], arr[x]);
		}

		int ans = 1;

		// 'x' is the index we are going to remove
		for (int x = 1; x <= N; ++x) {
			// Removing index 'x is the same as finding the GCD of everything
			// before it and after it
			ans = Math.max(ans, gcd(gcdPrefix[x - 1], suffix[x + 1]));
		}
