...
public class Main {
	public static void main(String[] args) throws IOException {
		...
		int[] nums = new int[N];
		String[] stringNums = input.readLine().split(" ");
		for (int i = 0; i < N; i++) {
			nums[i] = Integer.parseInt(stringNums[i]);
		}
		input.close();

		// prefixGCD[i] = GCD of nums[0], nums[1], ..., nums[i]
		// suffixGCD[i] = GCD of nums[i], nums[i + 1], ..., nums[N - 1]
		...
		// Find maximum gcd after taking out nums[i]
		int maxGCD = 0;
		for (int i = 1; i <= N; i++) {
