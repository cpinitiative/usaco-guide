...
void solve() {
	...
	for(int i = 1; i <= maxD; i++) {
		int amount = 0;
        //Amount calculates how many groups of "i" berries can be put into a basket
		for(int j = 0; j < N; j++) {
			amount += A[j] / i;
		}
		...
		if(amount >= K) {
            //If there are enough "i" sections for both Bessie and Ellie, then Bessie can collect (K / 2) * i berries
			mx = max(mx, (K / 2) * i);
			continue;
		}
		MOD = i;
		sort(all(A), cmp);	
        //Gives all of the rest over mod numbers to Bessie. 
