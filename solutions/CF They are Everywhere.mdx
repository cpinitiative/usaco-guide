...
int main() {
    ...
    for (int right = 0; right < flat_num; right++) {
        ...
        while (closest_left + 1 <= right
                && curr_pokemon.count(flats[closest_left + 1])
                && curr_pokemon[flats[closest_left + 1]] > 1) {
            closest_left++;
