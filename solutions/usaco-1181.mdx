...
Suppose we are currently looking at cow $i$ and cow $i+1$ has a greater hunger value, then it is sufficient to decrease cows $i+1$ to the same hunger
...
Note that at every step if we are looking at cow $i$, we are making sure cows from 1 to $i$ have the same hunger values. 
