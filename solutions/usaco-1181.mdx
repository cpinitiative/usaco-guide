...
```python
# Check if a proposed levelling works given some corn amounts.
def check(same_lvl, corn, hunger_lvl):
	hunger_lvl[0] -= corn[0]
	hunger_lvl[-1] -= corn[-1]
...
def solve(n_cows, hunger_lvl, corn):
	find_min_level = []
	...
	works = check(same_lvl, corn, hunger_lvl)
	return sum(corn) * 2 if works else -1
