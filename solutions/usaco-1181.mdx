...
Let's work through a sample case:

4 6 4 4 6 4

if $i$ = 1 and since value of $i+1 > i$, we should decrease values of $i+1$ and $i+2$ until value of $i+1 = i$, afterwards:

4 4 2 4 6 4

if $i$ = 2 and since the value of $i > i+1$, we should decrease the values of every other adjacent pairs before $i+1$ until they are equal, afterwards:

2 2 2 4 6 4

Now continue the pattern. After $i = 3$,
...
Finally, we have successfully equalized the hunger values of all the cows!
