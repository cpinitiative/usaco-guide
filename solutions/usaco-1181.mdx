...
We can solve this problem greedily using one loop and making prefixes of cows have equal hunger as we loop from left to right. Thus, we can break this
problem up into cases:

Suppose the cows are numbered from $1$ to $n$, and we are currently looking at cow $i$. If cow $i+1$ has a greater hunger value, then it is sufficient to decrease cow $i+1$ to the same hunger valueas cow $i$. To do this we will pick the pair of cows $i+1$ and $i+2$ and decrease their hungers until cow $i+1$ has the same hunger value as cow $i$. Of course, if cow $i+2$ has a negative hunger value, then it is impossible and we output $-1$.
...
decrease their hunger values together (cows $1$ and $2$, cows $3$ and $4$, $\ldots$, cows $i-1$ and $i$). After this, all the cows before $i+1$ will be decreased to the same hunger value as cow $i+1$. However, when $i$ is odd, it is impossible to decrease it's hunger unless we also decrease the hunger value for cow $i+1$, so we output $-1$.
