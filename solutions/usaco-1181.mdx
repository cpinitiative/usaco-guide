...
equal - we only can decrease the hunger for all the cows before $i+1$. To do this we select adjacent pairs of cows starting with the odd numbered cows
(cow 1 and 2 get their hungers decreased together, cow 3 and 4 ... cow $i-1$ and cow $i$). After this, all the cows before $i+1$ are decreased to the same
hunger value as cow $i+1$, and we can move on. However, there is the case where $i$ is odd, in which it is impossible to decrease unless we also decrease
the hunger value for cow $i+1$, so we output -1.
