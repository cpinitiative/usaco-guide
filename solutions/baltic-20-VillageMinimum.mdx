...
nodes need to travel would then be equal or larger than 4 (they are travelling
over at least one transiting node between them). If we instead just swap them
