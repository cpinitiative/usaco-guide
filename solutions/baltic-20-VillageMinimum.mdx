...
To illustrate this idea, let's consider the following example: Nodes 2 and 3 are
not swapped yet. Now, if we swap node 3 and 4 directly, the total distance
travelled would be $2 * 2 = 4$. It will never be less than just swapping the
nodes with their neighbors, which in this case means swapping node 1 with 2 and
then node 2 with 3, resulting in a total distance of $2 * (2 * 1) = 4$.
...
them directly will be more than 4.
