...
As each cow $i$ only wants to visit one of the other cows $j$, we can interpret the input as a functional graph with edges $i \to j$. The resulting graph is not necessarily connected, so we have to find out all connected components in this graph and solve them separately. 

For each connected component of the functional graph, let's observe that there must be one and only one cycle in it. Moreover, all cows can visit their buddies before their departure except one who wants to visit the cow initiating the visit chain in the cycle. For other paths that lead to the cycle, we can always visit them first before processing the cycle, so they will always be able to "moo". Therefore, the maximum amount of "moos" is the sum of all $v$ in the connected component subtracted by the minimum $v_i$ in the cycle. 

For each cow, we first check if it is already visited. If not, we then want to find out all other cows in this connected component by using a reversed graph and adding up the "moos". After that, we run Floyd's algorithm to determine the cycle and find the minimum "v" value in it. As each cow will only be processed once (or twice if in the cycle), we can achieve a time complexity of $\mathcal{O}(N)$. 
...
public class Visits {
	...
	static void mark(int node) {
		...
		for (Integer child : reversed_graph.get(node)) {
			mark(child);
		}
