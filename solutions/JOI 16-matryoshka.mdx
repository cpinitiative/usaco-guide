...
**Time Complexity**: $\mathcal O((N + Q) \log (\max H_i))$ or $\mathcal O((N + Q) \log N)$.
...
of the sequence of dolls (not the whole sequence like before). We can thus use a segment tree
to store, for each doll $i$, what that length is when considering only dolls with $H_j \leq H_i$.
