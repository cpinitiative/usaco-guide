...
int main()
{
	...
	int max_ans = 0;
    	...
    	if(valid) {
        	for(auto i: status){
			if(i == 2) {
				// days that have a status of 2 are breakouts, so they count in both answers
				min_ans++;
				max_ans++;
			}
        		else if(i == 0) max_ans++; // days with no required status are assumed to have a breakout for max_ans
		}
