...
"""
As long as there is no contradiction, we use the rules following:
1.  If the entry log says a day from now has an outbreak, 
	then that day must have an outbreak,
	every other day between them must have no outbreaks.
2.  Otherwise, if the entry log says -1, 
	then that day may have a potential outbreak.
"""
...
if valid:
	print(str(outbreaks) + " " + str(outbreaks + potential), file = fout)
else:
	print(-1, file = fout)
```
