...
Note: when placing the second rectangle on the left and right sides, make sure $w_{1} + w_{2} <= W$, otherwise, the second rectangle won't fit.
...
Calculate these placements, and chose the placement that requires the least distance.
