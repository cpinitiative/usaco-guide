...
void solve(){
	...
	if (w1 + w2 > total_width) {
		amount_needed_left = 1e9;
		amount_needed_right = 1e9;
	}

	if (h1 + h2 > total_height) {
		amount_needed_top = 1e9;
		amount_needed_bottom = 1e9;
	}
	
	// neat trick to get the min out of multiple elements
	int ans = min({amount_needed_left, amount_needed_right, amount_needed_top, amount_needed_bottom});
