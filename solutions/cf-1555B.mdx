---
id: cf-1555B source: CF 1555B title: B. Two Tables authors: David Zhang, Lewis Everett
---
...
maximise its distance from table 1. E.g., if placing table 2 to the left of table 1, we will assume
its leftmost edge will be touching the left wall of the room. Determining the solution is then
simply a case of simulating all of these cases and finding which involves moving table 1 the
smallest distance.
...
In this case, table 2 is placed to the left of table 1. We can therefore calculate its overlap with
table 1 as follows: $w$ - $x_{1}$. Here the overlap is 1, meaning we must move table 1 one space
from its starting position to fit table 2 in the room. We must then compare this answer to the
answers we get from placing table 2 to the right, top or bottom of table 1. The smallest of all of
these is our final answer.
...
It is possible that the two tables don't overlap at all. In this case the above calculations would
return a negative number so we should instead return 0 to indicate that table 1 doesn't have to move
at all.

Note: when placing table 2 on the left and right sides, make sure $w_{1} + w_{2} \leq W$, otherwise
table 2 won't fit in the room. Similarly, when placing table 2 on the top and bottom sides, make
sure $h_{1} + h_{2} \leq H$, otherwise table 2 won't fit in the room.
...
public class TwoTables {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++) {
			...
			if ((x2 - x1) + w <= W) { // Check if combined table widths can fit in the room
				int left = Math.max(0, w - x1); // Determine overlap when placing the table on the left
				int right = Math.max(0, x2 - (W - w)); // Determine overlap when placing the table on the right
				answer = Math.min(left, right);
			}
			if ((y2 - y1) + h <= H) { // Check if combined table heights can fit in the room
				int top = Math.max(0, y2 - (H - h)); // Top overlap
				int bottom = Math.max(0, h - y1); // Bottom overlap
