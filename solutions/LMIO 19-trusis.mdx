...
Let the heights of the poles be $a_1,\ldots,a_N$. If the heights of poles
$i_1<i_2<\cdots<i_k$ do **not** change, then we should have
...
Note that it is impossible for both of these conditions to hold if
$i_j>i_{j+1}$. To finish, we can just sort the points $(b_i,c_i)$ in
lexicographical order for each $1\le i\le N$ and run an LIS algorithm on the
$y$-coordinates.
