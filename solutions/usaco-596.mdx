...
Under these observations, we can use a set of visited edges and visited nodes, and keep track of the number of contiguous closed regions. This solution is about 10 times faster than using flood fill and works in $\mathcal{O}(N\log N)$ due to $\log N$ set lookup and insertion.
