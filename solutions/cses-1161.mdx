...
As we want to selete both shortest sticks and then insert the combined new stick, we will want to have a data structure where we can get the smallest value in the collection and add new value to it. The insert operation can be done easily with a priority queue in $\mathcal{O}(\log n)$ time while getting the smallest value takes constant time.

This gives us a time complexity of $\mathcal{O}(n \log n)$ in total. 
