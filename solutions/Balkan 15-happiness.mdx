...
Firstly, only the two least elements in each bucket could potentially be bad (since $(2^k + a) + (2^k + b) \geq 2^{k + 1}$). This narrows down the number of elements we need to check significantly.
...
We can use multisets to store the elements in the buckets, so the complexity of this algorithm is $O(KQ (\log 10^{12} + \log KQ))$. In practice, this solution runs faster than the first solution.
