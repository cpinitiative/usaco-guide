...
If we choose the numbers that we check this way, then we will only check $\mathcal{O}(\log 10^{12})$ numbers for each query (since the $i$-th number we check is always at least $2^i - 1$).
...
Secondly, we can store the sum of the elements in each bucket and be able to find the prefix sums of buckets in $\mathcal{O}(\log 10^{12})$ time.

We can use multisets to store the elements in the buckets, so the complexity of this algorithm is $\mathcal{O}(KQ (\log 10^{12} + \log KQ))$. In practice, this solution runs faster than the first solution.
