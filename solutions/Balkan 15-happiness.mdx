...
title: Balkan OI 2015 Happiness
...
- Check whether there exists some number $x < \sum_{i \in M} i$ such that $\sum_{i \in M, i \leq x}i < x$.
...
However, the numbers that we want to insert can get very large (up to $10^{12}$). Since we have to answer queries online, using a BIT is out of the question.

Luckily, we can simply use a sparse segment tree instead!

We still need to find a way to check whether the $x$ described above exists though.
...
This means that the next number we need to check is $\sum_{i \in M, i \leq k}i + 1$.

If we choose the numbers that we check this way, then we will only check $O(\log 10^{12})$ numbers for each query (since the $i$-th number we check is always at least $2^i - 1$).

The complexity of this algorithm is thus $O(KQ \log^2 10^{12})$.
...
A smarter solution is to split the elements of $M$ into the buckets $[2^i, 2^{i + 1})$ for each $i$ from 0 to $\log 10^{12}$. Notice how there are exactly $\log 10^{12} + 1$ buckets.

Why is this convenient?

Firstly, only the two least element in each bucket could potentially be bad (since $(2^k + a) + (2^k + b) \geq 2^{k + 1}$). This narrows down the number of elements we need to check significantly. (It turns out that only checking each element instead of each number less than the sum of the elements in $M$ is sufficient.)

Secondly, we can store the sum of the elements in each bucket and be able to find the prefix sums of buckets in $O(\log 10^{12})$ time.

We can use multisets to store the elements in the buckets, so the complexity of this algorithm is also $O(KQ \log 10^{12})$, albeit with a much better constant factor.
