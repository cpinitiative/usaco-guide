...
number of sweets a boy $i$ could give to a girl $j$ is $b_i$. Thus, the minimum number of sweets he could give out in total is $b_i \cdot m$. Thus, the answer to this problem is lower bounded by $\sum\limits_{i=1}^{n} b_i \cdot m$.
