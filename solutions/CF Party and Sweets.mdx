...
We are not done yet, however. The previous sum does not satisfy the condition
that $g_i$ is the maximum number of sweets a single girl received. For this to
be satisfied, while still maintainting a minimum sum of sweets, we add
$\sum\limits_{i = 1}^{m} (g_i - c_i)$ to the answer, where $c_i$ represents the
chosen $m$ sweets to "replace" in order to satisfy $g_i$ being the maximum
amount of candies a single girl received.

$c_i$ can be found by choosing the highest $m$ sweets which still satisfies the
condition that $b_i$ is the minimum number of sweets that a boy gave.
