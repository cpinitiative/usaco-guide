...
LCA precomputation is known to take $O(n \log n)$ with sparse table. Additionally, updates and queries both take $O(\log n)$ each, the complexity of the LCA and BIT queries. As there are $q$ updates and queries in total, the total complexity for the querying step is $O(q \log n)$. Hence, the total complexity is $O((n+q) \log n)$.
