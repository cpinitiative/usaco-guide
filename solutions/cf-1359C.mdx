...
- Case 1: pouring an equal number of cups of hot and cold water is equivalent to pouring one cup of each. Otherwise,
- Case 2: there must be exactly one more cup of hot water poured than cold water poured. Thus, there will be $c_i + 1$ cups of hot water and $c_i$ cups of cold water for some $c \geq 0$.
- It can be proven, using induction, that the average barrel temperatures when $c$ is odd is a monotonically decreasing function. Thus, we can binary search on the maximum number of odd cups $c$ which gives a temperature of at least $t$ by checking the odd integers around it.

The answer will be either be the barrel with $2$, $c$, or $c + 2$ cups.
	...
	*/
	...
	if (maximumCups == 1e6 + 1) {
		...
		*/
		...
		double minAbsDifference = min(abs(desired_temp - avgBarrel1), min(abs(desired_temp - avgBarrel2), abs(desired_temp - hot_cold_avg)));
