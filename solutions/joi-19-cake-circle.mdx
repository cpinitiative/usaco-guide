...
First, given some set of cakes $S$, it is always optimal to arrange them in sorted order by color (this can be shown inductively). The total color is $2(\max(S) - \min(S))$. 
...
Let $\texttt{opt}(l)$ be the optimal right endpoint for some left endpoint. We can observe that $\texttt{opt}$ is a non-decreasing function. This motivates a divide and conquer solution. 
