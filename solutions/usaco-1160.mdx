...
## Alternate Solution
An alternate solution to this problem is to use complementary counting. Consider a given value of $k$. 
Count the number of pairs of intervals $i$ and $j$ which satisfy $a_i + a_j > k$ or $b_i + b_j < k$. It is clear that 
any pair that satisfies $a_i + a_j > k$ cannot satisfy $b_i + b_j < k$, and vice versa. So, these two are 
