...
// https://codeforces.com/contest/1316/problem/E (input ommitted due to length)
int main() {
	int pool_size;
	int team_size;
	int aud_size;
	std::cin >> pool_size >> team_size >> aud_size;
	vector<pair<int, int>> aud_contrib(pool_size);
	for (int i = 0; i < pool_size; i++) {
		std::cin >> aud_contrib[i].first;
	...
	for (int up_to = 1; up_to <= pool_size; up_to++) {
		...
		for (int subset = 0; subset < (1 << team_size); subset++) {
			...
			if (curr_aud < aud_size && max_strength[up_to - 1][subset] != -1) {
				max_strength[up_to][subset] = std::max(
