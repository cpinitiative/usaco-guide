...
int main() {
	...
	std::queue<pair<int, bool>> queue;
	queue.push({0, false});
	while (!queue.empty()) {
		auto [curr, drank_water] = queue.front();
		queue.pop();

		// Drink water (if we haven't drank it already)
		if (!drank_water) {
			pair<int, bool> water{curr / 2, true};
...
queue = deque([(0, False)])
while queue:
	curr, drank_water = queue.pop()

	# Drink water (if we haven't drank it already)
	if not drank_water:
		water = (curr // 2, True)
		if water not in fullness:
			queue.appendleft(water)
			fullness.add(water)
	...
	for fruit in (orange, lemon):
		...
		if after <= max_fullness and to_add not in fullness:
			queue.appendleft(to_add)
			fullness.add(to_add)

best = max([n[0] for n in fullness])
