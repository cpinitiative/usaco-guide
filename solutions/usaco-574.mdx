...
Instead of thinking too hard about a knapsack solution (although it is relatively straightforward), a BFS/DFS can be done to iterate though **all** attainable states and simply choose the one with maximum fullness. The scene can be thought of as a graph by defining each edge to be the transition from fullness $i$ to $j$. Each state is defined by $<$fullness, has Bessie drank water yet (true/false)$>$. Since there are $2T$ states and each state is visited at most once, the time and space complexity of this solution is $O(T)$. Note that the sole purpose of the BFS/DFS is to do an exhaustive search, iterating through all attainable states, as aforementioned. Below is a BFS implementation:
