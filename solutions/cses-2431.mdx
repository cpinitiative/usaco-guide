...
the last group used in the sum. From there, we can calculate the exact number
that the $k$th digit is in, which we can then use to find the value of the $k$th
