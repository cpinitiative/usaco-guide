...
int main(void) {
	// open the teleport.in for reading and teleport.out for writing/priting
    freopen ("teleport.in", "r", stdin);
    freopen ("teleport.out", "w", stdout);

	// define variables needed; i is the going to be input array
	// j is going to be best strategy's distance required
    int i[4], j;
    cin >> i[0] >> i[1] >> i[2] >> i[3];

    j = abs(i[0] - i[1]); // caluculate the distance without travel
	// if it is greater than the sum with teleportation then change it to sum with teleportation
    if (j > abs(i[0] - i[2]) + abs(i[1] - i[3])) {
        j = abs(i[0] - i[2]) + abs(i[1] - i[3]);
    }
    // repeat this again with the other possible method
    if (j > abs(i[0] - i[3]) + abs(i[1] - i[2])) {
        j = abs(i[0] - i[3]) + abs(i[1] - i[2]);
    }

	// print the solution to teleport.out
    cout << j << "\n";
}
```
</CPPSection>
