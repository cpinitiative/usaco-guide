...
int main() {
	...
	// define variables needed; arr is the going to be input array
	int arr[4];
	cin >> arr[0] >> arr[1] >> arr[2] >> arr[3];

	/*
	dist is going to contain the best case scenario's strategy
	it is going to first take the distance without travel
	compare it with the first and second combination to figure
	out wheter it is the smallest/best case scenario for teleporting
	*/
	int dist = abs(arr[0] - arr[1]); // calculate the distance without travel
	if (dist > abs(arr[0] - arr[2]) + abs(arr[1] - arr[3])) {
		dist = abs(arr[0] - arr[2]) + abs(arr[1] - arr[3]);
	}
	// repeat this again with the other possiblity given below
	if (dist > abs(arr[0] - arr[3]) + abs(arr[1] - arr[2])) {
		dist = abs(arr[0] - arr[3]) + abs(arr[1] - arr[2]);
	}
