...
the $\texttt{i}$th cow in our list will have $\texttt{n-i}$ cows with heights greater than or equal to it. So to
get our final answer, we can initialize a cumulative product which will contain our answer, and for every
$\texttt{i}$ between 1 and $\texttt{n}$, we just have to multiply this cumulative product by the number of
stalls that the $\texttt{i}$th cow could possibly use, minus $\texttt{(n-i)}$.
...
// Create a list with the ith element of this list containing the number of stalls that the ith cow in our sorted cows list can use.
for i in range(n):
    for j in range(n):
        if cows[i] <= stalls[j]:
            possible_places[i] += 1
