...
the ith cow in our list will have $\texttt{n-i}$ cows with heights greater than or equal to it. So to
...
stalls that the ith cow could possibly use, minus $\texttt{(n-i)}$.
