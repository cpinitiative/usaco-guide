...
Let the node that Gigel goes to from node $i$ be $t_i$. Since the graph made from the directed edges $i \rightarrow t_i$ is a [functional graph](/silver/func-graphs), we can use [binary jumping](/plat/bin-jump) or any other efficient method to find the final node.
