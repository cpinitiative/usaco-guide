...
int main() {
	...
	for (int i = 0; i < S.length(); i++) {
		...
		if ((out.length() >= T.length()) && out.substr(out.length() - T.length()) == (T)) {
...
public class Censoring {
	public static void main(String[] args) throws IOException {
		...
		String S = br.readLine(); // String S, what we want to censor
		String T = br.readLine(); // String T, the censored word
		StringBuilder out = new StringBuilder(""); // StringBuilder solves the problem of immutable strings, so less memory is taken up
		
		for (int i = 0; i < S.length(); i++) {
			// Adding each character to the result one at a time
			out.append(S.substring(i, i + 1)); 
			
			// Checking the length of the result and whether the censor word exists
