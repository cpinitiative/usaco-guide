...
with open('fenceplan.in') as read:
	...
	coors = [[int(i) for i in read.readline().split()] for _ in range(n)]
	for _ in range(m):
...
for i in range(n):
	if not visited[i]:
		...
		# the code searches for the minimum x and y coordinates that make up the rectangle
		# bfs technique is used so that the program does not revisit/check already checked rectangles
		# while still checking all possible cases
		...
		min_perimeter = min(min_perimeter, ((bx + by) - (ax + ay)) * 2)
