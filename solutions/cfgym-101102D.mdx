...
Let's first simplify this problem, and then apply the solution of that problem to this one.

Say we had a matrix of all `0`'s and `1`'s. How many submatrices of all `1`'s are there?

We first define a 2D array, $\texttt{longest\_consec}$, that contains for each cell how many `1`'s there are to the right.  
...
finding out for each cell the number of submatrices of all `1`'s
that have said cell as an upper left corner.

How do we do this?  
First, let's initialize a stack $\texttt{rel\_streaks}$ and a number $\texttt{curr\_sum}$.  
$\texttt{rel\_streaks}$ will have elements each with two things:
...
$\texttt{curr\_sum}$, after processing, will contain
		...
		for (int t = 0; t < testNum; t++) {
			...
			for (int c = 0; c < colNum; c++) {
				...
				for (int r = rowNum - 1; r >= 0; r--) {
					...
					relStreaks.addFirst(new int[] {longestConsec[r][c], popped});
