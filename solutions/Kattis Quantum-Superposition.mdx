...
We can use dynamic programming by processing each node topologically to find the lengths of all the routes to the current node we are considering. We can store
the lengths in a bitset for each node. When we consider a node, we can add 1 to all the lengths reaching the previous node and insert it into the bitset for the current
