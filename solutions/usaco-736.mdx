...
We'll iterate through every character in every genome, and determine whether, for each position, there are any characters that appear in any genome of a spotted and plain cow. If there are, this position isn't a possible solution, so we can end this part of our search and move on to the next position to save on runtime. If there aren't, this is a possible solution, so add one to our solution storage variable. 
