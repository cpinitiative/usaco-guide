...
// Use `unordered_map` to achieve O(1) time complexity in queries
unordered_map<char, int> spotties;
...
int main()
{
  ...
  for (int i = 0; i < m; i++)
  {
    // Let's clear characters history
    spotties.clear();
    ...
    for (int j = 0; j < 2 * n; j++)
    {
      ...
      {
        // Spotty cows
        // Check character `c` as seen
        spotties[c]++;
      }
      else
      {
        // Plain cows
        // Check if the gen has been seen on spotty cows
        if (spotties.find(c) != spotties.end())
          break;
      }

      // If the for-loop hasn't been broken yet, then we found another gen
      if (j == 2 * n - 1)
        ans++;
    }
  }
