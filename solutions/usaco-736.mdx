...
with open('cownomics.in', 'r') as read:
	data = [i.strip() for i in read.readlines()]
...
spotted = ['' for i in range(n)]
plain = ['' for i in range(n)]

for i in range(n):
	spotted[i] = fin.readline().strip()

for i in range(n):
	plain[i] = fin.readline().strip()
...
for i in range(m):
	...
	for j in range(n):
		if spotted[j][i] in seen:
			break
	else:  # We never broke out of the above loop
		ans += 1
