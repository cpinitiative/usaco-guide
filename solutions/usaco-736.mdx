...
int main() {
    ...
    for (int i = 0; i < M; i++) {
        // initialize letter counters for spotty and plain strings
	map<char, int> spottyCt, plainCt;
	spottyCt['A'] = spottyCt['C'] = spottyCt['G'] = spottyCt['T'] = 0;
        plainCt['A'] = plainCt['C'] = plainCt['G'] = plainCt['T'] = 0;
	// count occurrences of each letter in each string at index M
        for (int j = 0; j < N; j++) {
            spottyCt[spotty[j][i]]++;
            plainCt[plain[j][i]]++;
        }
	// if none of spotty's letters reused in plain, index M is valid solution
        bool valid = true;
