...
To answer an query, we can split the range $[l, r]$ into $\mathcal O(\log N)$ segments. To calculate the answer, we just repeatedly merge these segements until there is one left and take the $\texttt{pref}$. 
...
## Solution 2 - With Lazy Proagation
