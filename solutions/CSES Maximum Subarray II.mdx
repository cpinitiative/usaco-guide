...
Notice that we are trying to maximize $\textrm{pfx}[i] - \textrm{pfx}[j]$. Since $j$ is guranteed to be within the window $[i-b,i]$, we can construct a sliding window of size b, and compute $\max_{A\le i \le B}(\textrm{pfx}[i]-\textrm{pfx}[j])$.
