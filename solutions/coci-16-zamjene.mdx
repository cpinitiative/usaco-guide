...
struct Hash {
	...
	/** @return the hashes that can be paired with this one to make a sum of 0 */
...
class DominikArray {
	private:
		...
		vector<Hash> hash;  // the current hash of a component
		vector<Hash> req_hash;  // the needed hash for a component to be sortable
