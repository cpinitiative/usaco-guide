...
public class BackForth {
	...
	public static void main(String[] args) throws IOException {
		...
		PrintWriter written = new PrintWriter("backforth.out");
		written.println(possible.size());
		written.close();
	}

	static void getPossible(
		int day,
		int aTank, List<Integer> aBuck,
		int bTank, List<Integer> bBuck
	) {
		// Last day, add the amount of milk in the first tank.
		if (day == 4) {
			possible.add(aTank);
		...
		// This transfers every possible bucket from barn x to barn y.
		for (int i = 0; i < aBuck.size(); i++) {
			// Bucket to transfer.
			int t = aBuck.get(i);
			// Creates a new copy of available buckets, and transfers the ith one.
			List<Integer> newA = new ArrayList<>(aBuck);
			newA.remove(i);
			List<Integer> newB = new ArrayList<>(bBuck);
			newB.add(t);
