...
## Implementation
**Time Complexity:** $\mathcal{O}(\sqrt{N}*logN)$
...
public class LoanRepayment {
	...
	static boolean isOk (long numGallons, long withinDays, long atLeast, long x) {
		long g = 0;
		while (withinDays > 0 && g < numGallons) {
			long y = (numGallons - g) / x;
			if (y < atLeast) {
				long leftover = ((numGallons - g) + (atLeast - 1)) / atLeast;
				return (leftover <= withinDays);
			}
			long maxMatch = (numGallons - (x * y));
	...
	public static void main (String[] args) throws IOException {
		...
		// keep running until we narrowed it down to one number
		while (low < high) {
