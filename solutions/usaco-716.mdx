...
inline bool operator<(const Road& r1, const Road& r2) {
	return tie(r1.sr, r1.sc, r1.er, r1.ec)
			< tie(r2.sr, r2.sc, r2.er, r2.ec);
}
...
int main() {
	...
	connected_cow_num = [&] (int r, int c, int prev_r, int prev_c) -> int {
		if (r < 0 || c < 0 || r >= side_len || c >= side_len
				|| visited[r][c]
	...
	for (int r = 0; r < side_len; r++) {
		for (int c = 0; c < side_len; c++) {
			if (!visited[r][c]) {
				...
				if (comp_sz != 0) {
					cow_components.push_back(comp_sz);
				}
			}
		}
	}
...
def connected_cow_num(r: int, c: int, prev_r: int, prev_c: int) -> int:
	if (r < 0 or c < 0 or r >= side_len or c >= side_len
			or visited[r][c]
