...
If there are more nodes, we can treat it similar to 2 nodes by partitioning all the nodes into 2 groups and making this problem a bipartite graph problem. All trees here are biparte graphs, meaning no nodes will have multiple colors. We can partition the nodes into two groups using DFS, assuming that any two neighbor nodes belong to different groups.
