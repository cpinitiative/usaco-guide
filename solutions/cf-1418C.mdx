...
If we define $\texttt{dp}[i][j]$ as the minimum number of skip point needed to get to the $i$th boss on turn $j$ 
...
So, the answer would be $\min(dp[N][0], dp[N][1])$.
...
void solve() {
	...
	/*
	 * dp[i][j] = the min amt of skip points 
