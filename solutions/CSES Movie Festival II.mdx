...
The first step is the same as that of [Movie Festival](/silver/greedy-sorting#example---the-scheduling-problem); sort the movies in increasing order of end time. For each movie in order, we will assign it to one of the $K$ members to watch (or none of them).

Keep track of the time each member finishes watching all of his currently assigned movies in a sorted collection (e.g. a [`multiset`](https://en.cppreference.com/w/cpp/container/multiset) in C++). Initially, the collection consists of $K$ zeroes.
