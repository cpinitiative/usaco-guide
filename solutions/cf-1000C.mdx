...
int main(){
	int n; cin >> n;
	vector<pair<ll, ll>> a(n); // This stores the original endpoints to use in our frequency array later
	...
	vector<ll> coords;
	for (auto& c: points) {
		mp[c] = cur; // Assign c to cur, where cur is our compressed coordinate for "a".
