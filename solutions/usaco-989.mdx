...
def fastest_time(dist: int, max_speed: int) -> int:
    """
    Calculates answer for a given
    distance and max speed Bessie can have
    when completing the race
    """
    speed_up_dist = 0
    # Amount of distance where Bessie's speeding up
    ...
    while True:
        ...
        if (speed_up_dist + slow_down_dist) >= dist:
            return time
        # If we're above the speed limit, add the current
        # speed to the part where we slow down as well.
        if curr_speed >= max_speed:
            slow_down_dist += curr_speed
            time += 1
            # Check again if we've reached or passed the finish line
...
# reading input
ans = []
with open('race.in', 'r', encoding='utf-8') as read:
    k, n = map(int, read.readline().strip().split())
    for test_cases in range(n):
        x = int(read.readline().strip())
        ans.append(fastest_time(k, x))

with open('race.out', 'w', encoding='utf-8') as out:
    for a in ans:
        print(a, file=out)
