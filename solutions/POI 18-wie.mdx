...
polynomial. and can be evaluated in constant time.
Otherwise, let $v = \texttt{solve}(k, P)$, $u_\text{even} = \texttt{solve}(2k,
P_\text{even})$, and $u_\text{odd} = \texttt{solve}(2k, P_\text{odd})$. We have
two cases when computing each $v[i]$:
