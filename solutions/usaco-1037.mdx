...
int main()
{
  ...
  // checks if pair {patientZero, k} is consistent with handshakes data over time
  ...
  cout << possiblePatientZero.size() << " " << minK << " " << (maxK == t ? "Infinity" : to_string(maxK)) << endl;
  return 0;
}
```
