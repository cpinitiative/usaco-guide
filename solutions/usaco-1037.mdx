...
int main() {
	...
	auto is_consistent = [&](int patient_zero, int k) {
		...
		for (int i = 0; i < n; i++) {
			if (tmp_infected[i] != cows_infected[i]) {
				return false;
			}
		}
		return true;
