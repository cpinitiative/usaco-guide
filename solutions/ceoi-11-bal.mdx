...
The problem can be solved by simulation. In particular, we want to inflate the balloons from left to right. The difficulty is to find out when the new balloon $b$ touches one of the inflated balloon $a$ on its left, and how to check this for all previous inflated balloons efficiently. For notation, $a_r$ is the radius and $a_x$ the x-coordinate of balloon $a$. 
...
// class Balloons causes compilation error on ojuz, so bal
public class bal {
