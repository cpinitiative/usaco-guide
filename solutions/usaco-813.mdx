...
struct Boot { int max_depth, max_steps, index; };
...
int main() {
	...
	for (int i = 1; i < tile_num - 1; i++) {
		tiles_by_depth.push_back(i);
	}
	sort(
		tiles_by_depth.begin(), tiles_by_depth.end(),
...
for b in boots:
	...
	& update tile_at & needed_step accordingly
	"""
	while (tile_at < len(tiles_by_depth)
			and tiles[tiles_by_depth[tile_at]] > b.max_depth):
