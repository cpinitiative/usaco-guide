...
title: Milk Factory
author: Mrinall Umasudhan, Jesse Choe, and Ananth Kashyap
---
...
class Graph{
	...
	int DFSUtil(int v,boolean visited[]){
		...
		while(i.hasNext()){
			int n = i.next();
			if(!visited[n]){
				// recurse to the next edge/station in its path 
				// and increase the number of stations visited.
...
public class factory{
	public static void main(String[] args) throws IOException{
		...
		for(int i = 0; i < N - 1; i++){
			// Meaning one can walk from station A to station B.
			g.addEdge(sc.nextInt() - 1, sc.nextInt() - 1); 
		}
		for(int i = 0; i < N; i++){
			int ans = g.DFS(i);
			if(ans == N){
				// If the number of visited stations from the DFS is equal to the total number of stations then output.
				out.println(i + 1); 
				used = true;
				break;
			}
...
This solution follows the logic outlined in the official solution.
...
# Create a list holding all factories that have an outgoing conveyer belts
outgoing_locations = [0]*max(outgoing)

for i in range(n-1):
	outgoing_locations[conveyer_belts[i][0] - 1] += 1
...
for i in range(n - 1):
	if outgoing_locations[conveyer_belts[i][1] - 1] == 0:
		if conveyer_belts[i][1] != ans:
			ans = conveyer_belts[i][1]
			counter += 1
