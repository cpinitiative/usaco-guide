...
<Warning>
We cannot use brute force to find the tower with the smallest possible cube on 
top because that will yield a time complexity of $\mathcal{O}(N^2)$, which is 
too slow.
</Warning>

## Implementation 1: Binary Search + Dynamic Array
...
represented as a
[TreeMap](https://usaco.guide/silver/intro-ordered?lang=java#multisets) in
