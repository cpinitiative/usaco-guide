...
// n - number of cows
// p - outgoing edges
// cycle: -2 = unvisited, -1 = currently being visited, else = visited
int n, p[INF], cycle[INF];

bool in_cycle = false;
void dfs(int node) {
    // seen this node before
    if (cycle[node] != -2) {
        // currently being visited - found a cycle
        if (cycle[node] == -1) {
            in_cycle = true;
            // node will cycle back to itself
            cycle[node] = node;
        }
        return;
    }
    // set this node to "currently visited"
    cycle[node] = -1;
    // visit the next node
    dfs(p[node]);
    // you've exited the cycle
    if (cycle[node] != -1) {
        in_cycle = false;
        return;
    }
    ...
    // node is out of the cycle
    // so it will cycle back to the first node
    // it visits in the cycle
    else {
        cycle[node] = cycle[p[node]];
    }
}
...
//    ofstream cout("shuffle.out");
    ...
    int ans = 0;
    for (int i = 0; i < n; i++) {
        // if this node cycles back to itself, it means it is never empty
        if (cycle[i] == i) ans++;
    }
    cout << ans << '\n';
