...
const int TOTAL = 52;  // Crossing Points' Map Length 
...
int main()
{
	...
	for (int i = 0; i < TOTAL; i++)
	{
		...
		{
			// keep track of all cows that have an entry/exit point
			...
			// Add all pairs to the set of crossings if not already in it
			for (int k : appearances)
			{
				bool already_present = crossings.count({recordings[i], k});
				already_present |= crossings.count({k, recordings[i]});
				if (!already_present)
				{
	...
	std::cout << crossings.size() << std::endl;
}
```
