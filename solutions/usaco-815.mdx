...
1. if log is tampered and cows would NOT breakout: $\texttt{dp}[i][j]=\texttt{dp}[i][j-1]+1$

2. if log is tampered and cows would breakout (So $a[j] \not = 0$): $\texttt{dp}[i][j]=\texttt{dp}[i-1][j-1]+1$

3. if log is NOT tampered (Then $i$-th breakout will be on $(j-a[j])$-th day): $\texttt{dp}[i][j] = \texttt{dp}[i-1][j-a[j]-1]+\texttt{range\_ans}[j-a[j]][j]);$
$$\texttt{range\_ans}[l][r]$$ represents minimum cost
to replace range $a[l]...a[r]$ with $0...(r-l)$ respectively.
...
int main() {
	...
	for(i=1; i<=n; i++) cout<<dp[i][n]<<endl;
}
```
