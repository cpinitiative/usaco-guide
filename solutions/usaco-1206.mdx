...
all gifts in the strongly connected component through a series of reassignments and is guaranteed to be able to receive one 
or more preferred gifts (including itself) based on SCC definition. Obviously it is only optimal to assign cow $i$ gift it
prefers over its own, or the original gift.
## Implementation
