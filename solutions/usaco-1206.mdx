...
all gifts in the strongly connected component through a series of reassignments based on SCC definition. It is also guaranteed to be able to receive one 
or more preferred gifts (including itself) since we only are only adding these as outgoing edges.
## Implementation
