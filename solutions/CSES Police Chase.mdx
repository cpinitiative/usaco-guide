...
First, we run a max flow algorithm on the graph. Then, we run another BFS from node $1$. If node $a$ is reachable but node $b$ is not and there was an edge $a\rightarrow b$, then we know that edge must be removed.
