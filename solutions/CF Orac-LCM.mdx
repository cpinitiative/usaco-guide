...
Consider fixing $j$. How can the gcd of all lcms of nums[$j$] and all nums[$i$] (such that $i$ is in range $[0,j]$) be quickly computed? The result would be nums[$j$]$*C$, where $C$ is the gcd of all nums[$i$], and we can keep track of this gcd on the run.
