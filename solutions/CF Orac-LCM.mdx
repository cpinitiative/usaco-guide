...
Consider fixing $j$. How can the gcd of all lcms of $a_j$, and all $a_i$ (such that $i$ is in range $[0,j]$) be quickly computed? The result would be $a_j*C$, where $C$ is the gcd of all $a_i$, and we can keep track of this gcd on the run.
