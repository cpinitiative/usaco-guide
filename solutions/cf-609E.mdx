...
Binary lifting technique can be used to find the heaviest edge in the path from node $u$ to $v$. 
Therefore we only need to find the heaviest edge from $u$ to $lcm(u,v)$ and $v$ to $lcm(u,v)$.
