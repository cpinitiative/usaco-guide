...
If FJ's $i$th cow is greater than FP's $j$th cow, we can add $\texttt{dp}[i - 1][j - 1][k - 1]$, because by pairing up those two cows, we've essentially downsized the remaining spots by one.
...
int main() {
	...
	*/
	vector<vector<vector<long long>>> dp(n + 1, 
									vector<vector<long long>>(m + 1, vector<long long>(k + 1)));
