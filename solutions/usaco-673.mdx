...
If FJ's $i$th cow is greater than FP's $j$th cow, we can add $\texttt{dp}[i - 1][j - 1][k - 1]$, because by pairing up those two cows, we only have $k - 1$ more spots to consider.
