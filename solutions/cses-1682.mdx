...
Let's say $\texttt{can[u][v]}$ is $\texttt{true}$ if you can go from vertex $u$
to vertex $v$ through a series of edges. Additionally, let's define the directed
graph given in the statement as $G$ and the reverse of it (where an edge
