...
We can create a prefix sum and check whether it is equal to $x$. If it is, then increment $res$, the answer. Furthermore, we can have a map that keeps track of these prefix sums and when the prefix sums isn't equal to $x$, we can check whether there are subarrays with prefix sum equal to $x - \texttt{prefixSum}$. This will ensure that we can remove/add the sum of the subarray that has $x - \texttt{prefixSum}$ from/to our current subarray to build our answer.
