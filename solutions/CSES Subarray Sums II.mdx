...
We can have a map that keeps track of the prefix sums, we can check whether there are subarrays with prefix sum equal to $\texttt{prefixSum} - x$. This will ensure that we can remove/add the sum of the subarray that has $\texttt{prefixSum} - x$ from/to our current subarray to build our answer.
