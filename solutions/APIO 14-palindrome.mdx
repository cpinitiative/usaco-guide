...
First, construct the [palindromic tree](http://adilet.org/blog/palindromic-tree/) from the string. There are $O(N)$ distinct palindromes, so our tree will have $O(N)$ nodes.
