...
Let $\text{covered}[S]$ be the maximum prefix of people we can pay off using the notes defined in the subset, and $\text{leftover}[S]$ be the leftover amount of money we have after we pay off the prefix of the people.
