...
$\texttt{lambda}$ where the number of subarrays used is $\geq k$.

Now, for a given lambda, we're going to calculate the number of people in an
optimal food arrangement such that we subtract $\texttt{lambda}$ for every
person.
...
person $i$ is being used. Let $\texttt{cnt}[i][j]$ represent the number of
...
\max\begin{cases}\{\texttt{dp}[i - 1][0] + a[i] - \texttt{lambda}, \texttt{cnt}[i - 1][0] + 1\}\\
