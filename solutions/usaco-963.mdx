...
author: Krish Thawani (Java), Hua Zhi Vee (C++), Ben Dodge 
---
...
We create a 2D array of booleans which states if cow $A$ has a ranking higher than
cow $B$ in any point of time. We read every ranking, then for all $\frac{N(N-1)}{2}$
