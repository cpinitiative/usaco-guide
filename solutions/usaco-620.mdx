...
title: Milk Pails
author: Óscar Garries, Neo Wang, Nathan Gong, Akshar Yeccherla
---
...
x, y, k, m = map(int, filein.read().split())
MAX_OPS = 101

sol = float('inf')

vis = [[[False for a in range(MAX_OPS)] for b in range(MAX_OPS)] for c in range(MAX_OPS)]


def ff(curX:'int', curY:'int', curK:'int'):
    global sol

    if curK > k or vis[curX][curY][curK]:
        return
    ...
    # case 3
    if curX > y - curY:
        ff(curX - y + curY, y, curK + 1)
    else:
        ff(0, curY + curX, curK + 1)

    if curY > x - curX:
        ff(x, curY - x + curX, curK + 1)
    else:
        ff(curX + curY, 0, curK + 1)
