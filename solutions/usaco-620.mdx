...
title: Milk Pails
author: Óscar Garries, Neo Wang, Nathan Gong, Akshar Yeccherla, George Pong
---
...
sol = float('inf')
vis = [[[False for a in range(MAX_OPS + 1)] 
	for b in range(MAX_OPS + 1)] 
...
def generate_permutations(p1: int, p2: int):
    ...
    return [(x, p2), (p1, y), (0, p2), (p1, 0), (p1 + da, p2 - da), 
			(p1 - db, p2 + db)]
