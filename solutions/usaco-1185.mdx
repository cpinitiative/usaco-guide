...
In any case where they're equal, it's always possible to reduce them both to zero! (by decreasing adjacent elements).

However, with three cows:

$[2, 2, 2]$

We can't apply the same transformation. In fact, for any odd number of cows, it's impossible to guarantee that we can decrement all the elements to zero.

This motivates us to split the problem into two cases, if $N$ is even or if $N$ is odd.
...
More formally, if we let $\texttt{dp}[i][j] =$ the number of tuples that can be formed if the first $i - 1$ elements are set to zero and the $i$th element is hunger value $j$, then this state affects all states $\texttt{dp}[i + 1][j]$ such that $h_i \leq k \leq h_{i + 1}$.
