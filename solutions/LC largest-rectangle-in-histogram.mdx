...
Actually, we only need to go through the heights in one direction. When we see `(i, heights[i])`, we process all rectangles with right end at `i-1` and height at least `heights[i]`.
