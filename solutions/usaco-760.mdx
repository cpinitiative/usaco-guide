...
void solve(){
	int n; cin >> n;
	int cowID[n];
	for(int i = 0; i < n; i++) cin >> swapOrder[i];
	for(int i = 0; i < n; i++) cin >> cowID[i];

	// For loop iterates 3 times and simulates each swap backwards during every iteration
	for(int i = 0; i < 3; i++){
		// Temporary array to assist with the swapping as not to change the values of the cowID array
		int temp[n];
		for(int j = 1; j <= n; j++){
			int e = find(n, j);
			/* 
		...
		for(int j = 0; j < n; j++) cowID[j] = temp[j]; 
	}

	for(int i = 0; i < n; i++) cout << cowID[i] << ln; 
}
