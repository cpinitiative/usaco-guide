...
Instead, let's try and go through all possible values of $j$ and see how many values of $i$
can be paired with it. To do this, we need to see how many numbers in $A$ are equal to $B_{C_j}$,
which can be done with a map and some precalculation.

<Info>
The official solution goes through all values of $i$ instead, but it's still pretty much
the same idea.
</Info>
...
public class Main {
	public static void main(String[] args) throws IOException {
		...
		// Stores for each number in A how many times it appears in total
		HashMap<Integer, Integer> aOccNum = new HashMap<>();
		for (int i : a) {
