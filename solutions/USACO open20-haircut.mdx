...
First, we need to count the number of inversions. (Fun fact: the number of inversions is also a measure of how sorted an array is. If an array has 0 inversions, it's perfectly sorted in ascending order. If it has $\frac{n(n - 1)}{2} inversions though, it's perfectly sorted in descending order.) 
...
To do this, we use a BIT as a frequency table, keeping track of the number of hairs before index $i$ of each possible value. If our BIT is named `tree`, `tree[i]` will tell us how many times $i$ is seen in the array of hairs. As long as we fill out the frequency table in the order the elements appear in the array, we can use the frequency table to also count the number of inversions, because the frequency table only contains hairs to the left of the hair we are currently considering.

That leads us to this algorithm:

1. Initialize a BIT so that the frequency of each value is 0.
2. Create an array `inversions`. `inversions[j]` will store the number of inversion pairs with the second index being $j$
3. For each hair $j$ in the order it appears in the array:

    a. Count the number of taller hairs that appear before hair $j$ in the array (which is just the frequencies of all values greater than $A[j]$). Store this in `inversions[j]`.

    b. Increment the frequency of $A[j]$ in the BIT.

The number of inversions would then be the sum of all the values we computed in step 3a. However, we also need to consider the haircuts. If we cut all hairs greater than length $n$ to $n$, what does that mean? It means that there can be no inversions with a $j \geq n$, because the definition of an inversion is that $i > j$, but if $j \geq n$ then $i > n$ must also be true. However, if this is true, then $i$ and $j$ would both have been cut to length $n$, and $(i, j)$ would no longer be an inversion.
