...
If we let $d_i$ denote the maximum depth in node $i$'s subtree, then the
...
Since $d_i = \max_{c \in i \text{'s children}}(d_c) + 1$, the above expression
simplifies to $\mathcal O(N)$ - a significant improvement from our first
solution!
