...
The [Official Editorial](https://codeforces.com/blog/entry/44259) uses a divide and conquer approach. However, like many divide and conquer optimization problems, this can also be solved using [convex hull trick](https://usaco.guide/plat/convex-hull-trick?lang=cpp).
...
By expanding this expresion, we can express this in terms of $\texttt{pre}$ and $\texttt{ips}$:

$$
\sum_{i = l}^r a_i\cdot(i - l+1) = \texttt{ips}[r]-\texttt{ips}[l-1]-(l-1)(\texttt{pre}[r]-\texttt{pre}[l-1])
$$

For this problem, we need to choose a contiguous subarray in order to maximize the required score. Let use define a function $f$, such that $f(r)$ denotes the maximum score of a subarray that ends at $r$. We can thus define $f$ as follows:
...
Because of this property, we can use convex hull trick. Each value can be represented as a linear function of the form 
...
**Complexity**: $\mathcal{O}(n)$ or $\mathcal{O}(n \log n)$. The impementation below runs in $\mathcal{O}(n \log n)$ because it uses [Line Container](https://usaco.guide/adv/line-container/). It is possible to get a linear time complexity by taking advantage of the fact that $l$ and $\texttt{pre}[r]$ are both non-decreasing.
...
/**
 * - Source: https://usaco.guide/adv/line-container?lang=cpp
 * - Supports insertion of linear functions and querying
 * of the maximum value of any function at some given x
 * - Useful for Convex Hull Trick
 * - Similar to LiChao Tree
*/
...
int N;
long long A[200005], pre[200005], ips[200005];
