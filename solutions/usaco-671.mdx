...
sys.stdin, sys.stdout = open('lasers.in', 'r'), open('lasers.out', 'w')
...
class Fencepost:
	def __init__(self, x: int, y: int):
		self.__slots__ = ['x', 'y']  # faster attribute access
		self.x = x
		self.y = y
...
x_lines = defaultdict(list)  # stores all indices of fenceposts on line x = key
y_lines = defaultdict(list)  # stores all indices of fenceposts on line y = key
...
# stores (idx of fencepost, direction of laser).
queue = deque([(0, HORIZ), (0, VERTI)])
