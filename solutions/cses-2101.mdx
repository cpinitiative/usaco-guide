...
However, this hints at a faster solution. Note that our slow approach bears many similarities with [Kruskal's MST algorithm](/mst/#kruskals), which implies that the path between $u$ and $v$ on any minimum spanning tree will minimize the maximum edge weight. For a more formal proof, see [here](https://www.geeksforgeeks.org/minimum-bottleneck-spanning-treembst/). 
****
All that is left to do is to efficiently query for the minimum node on a tree path. One way to do this is with [binary jumping](/plat/binary-jump). 
