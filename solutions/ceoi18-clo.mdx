...
This drastically simplifies the problem. Now we can simply define $\texttt{max_profits}[t][c]$ as the maximum profit we can gain from the first $t$ transactions given that we have $c$ cores left.
