...
We can find the values for $\texttt{trav}_i$ quickly with [Dijkstra's Algorithm](/gold/shortest-paths). To get $\texttt{occ}_i$, we'll store the parents of nodes during our Dijkstra's Algorithm, so we can backtrack to get the counts of cows that pass through. We can compute $\texttt{occ}$ in $\mathcal{O}(N^2)$ within the time limits because $N \leq 10^4$.
