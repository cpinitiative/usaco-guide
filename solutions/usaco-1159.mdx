...
ll cost(ll a, ll b){
	if(comps[a].size() > comps[b].size()){
		ll dist = MAXN;
		for(ll u: comps[b]){
			ll i = lower_bound(comps[a].begin(), comps[a].end(), u) - comps[a].begin();
			if(i > 0){
				dist = min(dist, abs(comps[a][i - 1] - u));
			}
			dist = min(dist, abs(comps[a][i] - u));
		}
		return dist * dist;
	} else {
		ll dist = MAXN;
		for(ll u: comps[a]){
			ll i = lower_bound(comps[b].begin(), comps[b].end(), u) - comps[b].begin();
			if(i > 0){
...
void solve(){
	ll n, m; cin >> n >> m;
	forn(i, n) comp[i] = -1, adj[i].clear(), comps[i].clear();
	forn(i, m){
		ll a, b; cin >> a >> b;
	...
	for(ll c = 1; c < cur; c++){
		res = min(res, cost(comp[0], c) + cost(c, comp[n - 1]));
	}
	cout << res << endl;
}
