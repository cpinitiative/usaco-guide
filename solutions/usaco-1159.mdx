...
### Explanation
We can also find the components of each graph by storing the edges in a Disjoint Set Union/Union Find Structure, and query the elements in O(n) time. Afterward, we can find the answer with a two pointer search.
