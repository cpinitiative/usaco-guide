...
At first glance, these subtasks seem impossible - Dijkstra's algorithm always has a complexity of $O(E \log V)$!

... or does it?

Note that the $O(E \log V)$ complexity is only true if there are no negative edges.
