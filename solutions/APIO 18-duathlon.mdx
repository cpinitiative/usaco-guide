...
What makes a bad triple? A triple $(S, C, F)$ is bad if the paths from $S$ to $C$ and $C$ to $F$ both pass through some bottleneck.
...
If we remove $P$ from the graph, we are left with the smaller tree containing $B$ and several other smaller trees.

If $S$ and $F$ are both in the same smaller tree that doesn't contain $B$, then $C$ can't be in the smaller tree containing $B$.

The sum of such pairs $(S, F)$ is thus the number of bad triples each pair $(B, P)$ contributes to the total number of bad triples.
