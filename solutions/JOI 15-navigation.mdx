...
To solve the last subtask, you'll need to make full use of both the island numbers _and_ the numbers written on the flags.

</Spoiler>

<Spoiler title="Hint 2">

Given two adjacent vertices $u$ and $v$, we should be able to tell which of $u$ and $v$ is closer to $T$ given the pairs $(u,F[u])$ and $(v,F[v])$.

</Spoiler>

<Spoiler title="Hint 3">

Write on the flags such that $F[u]\oplus F[v]=0$ implies that $\min(u,v)$ is closer to $T$ (while $F[u]\oplus F[v]=1$ implies that $\max(u,v)$ is closer to $T$).
