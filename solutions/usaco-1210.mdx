...
The probablity that you get less than or equal to $E_{x-1}$ test cases is ${\sum_{i=0}}^{E_{x - 1}} p_i$. We can multiply this by the $E_{x-1}$ to get the expected value of $E_x$ if we stop before the $x$th submission.

The probablity that you get more than $E_{x-1}$ test cases is ${\sum_{i=\lfloor E_{x-1} \rfloor + 1}}^T p_i$. To get the expected value of getting more than $E_{x-1}$ test cases is the sum of $i$ from ${\lfloor E_{x-1} \rfloor + 1}$ to $T$ of the probability of getting $i$ test cases multiplied by the $i$ or ${\sum_{\lfloor E_{x-1} \rfloor + 1}}^T p_i i$.

In addition, these formulas for both cases can help us mathematically prove that you can merge Case 3 with either Case 1 or 2. Since we merged it with Case 1, it will become $E_{x-1} \cdot p_{E_{x-1}}$. If we had merged it with Case 3, it would also become $E_{x-1} \cdot p_{E_{x-1}}$. This happens because $p_i * E_{x-1} = p_i * i$ only when $i=E_{x-1}$.
...
Claim: We can process multiple queries if the values of $a = {\sum_{i=0}}^{\lfloor E_{x - 1} \rfloor} p_i$ and $b = {\sum_{i=\lfloor E_{x - 1} + 1 \rfloor}}^{T} i p_i$ don't change between queries.
...
However, the values of $a$ and $b$ won't always remain the same. We can binary search on the max number of submissions Bessie can make before the values of $a$ and $b$ change. $a$ and $b$ will change when the value$\text{ of } {\lfloor E_x \rfloor}$ changes. The conditions for binary search are fullfilled because $E_x$ is a non-decreasing function.
