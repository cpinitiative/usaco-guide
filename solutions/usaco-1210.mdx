...
However, the values of $a$ and $b$ won't always remain the same. We can binary search on the max number of submissions we can do before the values of $a$ and $b$ change. $a$ and $b$ will change when the value$\text{ of } {\lfloor E_x \rfloor}$ changes. The conditions for binary search are fullfilled because $E_x$ is an non-decreasing function.
