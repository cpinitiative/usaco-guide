...
Essentially we can binary search on the next value that will change the values of $a$ and $b$. This happens when $\lfloor E_x \rfloor$ changes. Since there are at most $T$ values of $E_x$, the resulting time complexity will be $O(T^2 + T \log K)$.
