...
The probablity that you get less than or equal to $E_{x-1}$ test cases is ${\sum_{i=0}}^{E_{x - 1}} p_i$. We can multiply this by the $E_{x-1}$ to get the expected value of $E_x$ if we stop before the $x$th submission and we get $E{x-1} \cdot {\sum_{i=0}}^{E_{x - 1}} p_i$
