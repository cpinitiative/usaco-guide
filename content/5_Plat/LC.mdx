...
export const problems = {
    ...
    probs: [
      ...
      new Problem("ojuz", "BOI - 2circles", "balkan11_2circles", "Very Hard", false, ["Geometry", "Binary Search"], "We binary search for the optimal radius. To test whether a given radius works, we deflate the polygon by that radius and check whether the maximum distance between 2 vertices is at least twice that radius. We can deflate the polygon using half-plane intersection and we can check the maximum distance between 2 vertices using rotating calipers.")
    ]
};
