...
int main() {
    ...
    int res = 0;
    for (int mask = 0; mask < (1 << M); mask++) {
		// sos[mask] now contains the number of words that are disjoint from subset mask
		int valid_words = n - sos[mask];
...
for i in range(n):
    st = input()
    mask = (1 << (ord(st[0]) - 97)) | (1 << (ord(st[1]) - 97)) | (1 << (ord(st[2]) - 97))
    sos[mask] += 1
