...
Let's denote the $i$-th point as $\texttt{pts}[i]$.

Our initial stack elements are $\texttt{pts}[N-1], \texttt{pts}[0], \texttt{pts}[1]$, which corresponds to $(1,2), (2,0), (5,3)$. Then, starting from $\texttt{pts}[2]$, we process each point.

First, we check if the turn between $\texttt{pts}[0], \texttt{pts}[1], \texttt{pts}[2]$ is counterclockwise, which it is not. Hence, the point $\texttt{pts}[1]$ cannot be part of the convex hull. We pop it and insert $\texttt{pts}[2]$.
...
Then, we check whether the turn between $\texttt{pts}[2], \texttt{pts}[3], \texttt{pts}[4]$ is counterclockwise, which it is. However, because $(1, 2)$ is already in our stack, we have completed the cycle, and our convex hull is complete.
