...
Then, we check the turn $(2,0),(3,2),(5,3)$ which is clockwise. Hence, we pop the stack until the counterclockwise property can be satisfied. In this case, only $(5,3)$ is popped. Afterwards, we push $(2, 3)$ to our stack.

Our stack now is then $(1,2),(2,0),(5,3)$.

Then, we check whether the turn between $\texttt{pts}[2], \texttt{pts}[3], \texttt{pts}[4]$ is counterclockwise, which it is, so we push $(2,3)$ to our stack.
