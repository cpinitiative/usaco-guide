...
<Resources>
	<Resource source="cp-algo" title="Graham Scan" url="https://cp-algorithms.com/geometry/grahams-scan-convex-hull.html" starred />
...
The Graham Scan algorithm works in 3 steps. First, it sorts all of the $n$ points by their counterclockwise angle around an abitrary pivot $P_0$. This algorithm uses the bottommost (and rightmost if there is a tie), point as $P_0$. 

To start the creation of the convex hull, we choose 3 points. The pivot, the first, and the last point based on our initial sorting.
...
There are two only 2 cases for our 3 points: either the turn from $v[i-2] \rightarrow v[i-1] \rightarrow v[i]$ is counterclockwise or not. If the turn is not counterclockwise, then we know that our current set of polygons cannot be convex, and thus we must pop the stack and include the candidate point. Otherwise, we can simply add the candidate point to our stack.

To understand why, consider those three points: the top two points in the stack, and our candidate point. Since our turn is counterclockwise, we are looking for such points that make a counterclockwise turn.
...
int main()
{
    ...
    for(;;) {
        ...
        if(!n) break;
        vP pts;
        set<P> pt; // points are not necessarily distinct so we have to keep track of the unique ones
