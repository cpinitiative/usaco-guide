...
Denote our stack as $\texttt{C}$, the top element of $\texttt{C}$ as $\texttt{C}[i]$ and $\texttt{cand}[j]$ as the $j$-th sorted point.
...
Let's denote the $i$-th sorted point as $\texttt{cand}[i]$.

Our initial stack elements are $(1,2),(2,0)$. Then, starting from $\texttt{cand}[2]$, we process each point.

First, we check if the turn between $(1,2), (2,0)$, and $(3,2)$, is counterclockwise, which it is. Hence, the point $\texttt{cand}[2]$ is inserted on our stack.
...
Our stack now is then $(1,2),(2,0),(6,2),(5,3),(2,3)$, which is the convex hull of all our points.
