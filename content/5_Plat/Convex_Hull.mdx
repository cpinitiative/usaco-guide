...
A counterclockwise turn from $a \rightarrow b \rightarrow c$ means that $c$ is to the left of the line from $a$ to $b$.

Denote our stack as $\texttt{hull}$, the top element of $\texttt{hull}$ as $\texttt{hull}[i]$ and $\texttt{cand}[j]$ as the $j$-th sorted candidate point.

There are 2 cases for our 3 points: either the turn from $\texttt{hull}[i-1] \rightarrow \texttt{hull}[i] \rightarrow \texttt{cand}[j]$ is counterclockwise or not. If the turn is not counterclockwise, then we know that our current set of points cannot be convex, and thus we pop the top element off $\texttt{hull}$ and proceed to check convexity of the point until the turn is counterclockwise. Otherwise the turn is counterclockwise, and thus we include it in our hull.
...
Animation created with [GeoDeb](https://github.com/lukakalinovcic/geodeb).

Convex Hull visualization with more points can be found [here.](https://lukakalinovcic.github.io/geodeb/gallery/hull.html)
...
2. Maintaining the stack:
	...
	Let's denote the $j$-th sorted point as $\texttt{cand}[j]$.
