...
Then, we maintain a stack containing the points of the convex hull, such that the set of points in the stack is convex. As a result, we only have to check the candidate point against the topmost 2 points.

In order for the hull to be convex, every three consecutive points of the stack must contain a counterclockwise turn.
...
P operator*(const T& l, const P& r) { 
    return r*l; 
}
P operator/(const P& l, const T& r) { 
    return P(l.f/r,l.s/r);
...
vi hullInd(const vP& v) {
	...
	trav(c,cand) { // for every point
		while (sz(C) > 1 && cross(v[end(C)[-2]],v[C.bk],v[c]) <= 0) {
            C.pop_back(); // pop until counterclockwise and size > 1
        }
		C.pb(c); 
    }
