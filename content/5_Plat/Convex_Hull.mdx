...
We maintain a stack containing the points of the convex hull, such that the set of points in the stack is convex. As a result, we only have to check the candidate point against the topmost 2 points.
...
Denote our stack as $\texttt{C}$, the top element of $\texttt{C}$ as $\texttt{C}[i]$ and $\texttt{cand}[j]$ as the $i$-th sorted point.

There are 2 cases for our 3 points: either the turn from $\texttt{C}[i-1] \rightarrow \texttt{C}[i] \rightarrow \texttt{cand}[j]$ is counterclockwise or not. If the turn is not counterclockwise, then we know that our current set of points cannot be convex, and thus we pop the top element off $\texttt{C}$ and proceed to check convexity of the point until the turn is counterclockwise. Otherwise the turn is counterclockwise, and thus we include it in our hull.
...
[Animated](https://convexhull.tiiny.site/)
...
Let's denote the index of $i$-th sorted point as $i$.

Our initial stack elements are $(1,2),(2,0)$. Then, starting from point $2$, we process each point.
