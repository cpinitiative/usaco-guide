...
Let $freq[i]$ equal the number of times $i$ occurs in our set.
In our array of coordinate compressed values, we want to find the first
indice $x$ such that $\sum_{i=0}^{x} freq[i]$ is $\geq K$.
...
checking the maximum in our left and right childs is enough to know if we
should walk left or right. But with sum, we also need to consider the range $[1, l)$ and how it might affect our answer.

To handle keeping track of the prefix of $[1, l)$, we first set our prefix value to a neutral value, then join it with our left child's answer every time we walk right.
