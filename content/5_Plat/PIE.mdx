...
<Resources>
    ...
    <Resource source="wiki" title="Inclusion-exclusion  principle" url="https://en.wikipedia.org/wiki/Inclusion%E2%80%93exclusion_principle">Wiki</Resource>
</Resources>

The inclusion-exclusion principle relates to finding the size of the union of some sets.

Verbally it can be expressed as following:
<center>
...
The mathematical formulation of the principle is as follows:
$$
...
A dynamic programming approach with bitmasking would look like this: $dp[i][mask]= \text{the number of strings of length i that match all the patterns in set, but none other patterns } $.
The recurrence is: $dp[i][mask \& j] \text{+=} dp[i-1][j] \text{ where j is a set of patterns that match charachter c at position i}$. The code below is the implementation.
...
int howMany(vector<string> patterns, int k) {
    ...
    for(int mask = 0; mask < (1 << (int)patterns.size()); mask++) {
        if(__builtin_popcount(mask) == k) {
            for(int supermask = mask; supermask < (1 << (int)patterns.size()); supermask++) {
