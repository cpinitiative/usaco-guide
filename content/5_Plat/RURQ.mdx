 ...
 * U = update type, which will be the Query struct
 */
template <class T, class U> class LazySegtree {
  private:
	...
	void apply(int v, int len, U x) {
		if (x.type == ADD) {
			t[v] += x.val * len;
			if (lz[v].type != SET) {
				lz[v] = Query{ADD, lz[v].val + x.val};
			} else {
				lz[v] = Query{SET, lz[v].val + x.val};
			}
		} else {
			t[v] = x.val * len;
			lz[v] = x;
	...
	void range_update(int v, int l, int r, int ql, int qr, U x) {
		if (qr < l || ql > r) { return; }
		if (ql <= l && r <= qr) {
			apply(v, r - l + 1, x);
		} else {
  ...
  public:
	LazySegtree(const vector<T> &a) : sz((int)a.size()), t(4 * sz), lz(4 * sz) {
		build(1, 0, sz - 1, a);
	}
