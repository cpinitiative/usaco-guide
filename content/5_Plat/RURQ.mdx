...
void pushdown(ll p){
	...
	if(tree[p].lzSet != 0){
		...
		tree[lc].val = (tree[lc].r - tree[lc].l + 1) * tree[p].lzSet;
		tree[rc].val = (tree[rc].r - tree[rc].l + 1) * tree[p].lzSet;
		...
		tree[lc].val += (tree[lc].r - tree[lc].l + 1) * tree[p].lzAdd;
		tree[rc].val += (tree[rc].r - tree[rc].l + 1) * tree[p].lzAdd;
		tree[p].lzAdd = 0;
	}
	return;
}
...
void add(ll p, ll l, ll r, ll a, ll b, ll val){
	if(a > r || b < l)return;
	if(a <= l && r <= b){
...
void set(ll p, ll l, ll r, ll a, ll b, ll val){
	if(a > r || b < l)return;
	if(a <= l && r <= b){
		tree[p].val = (tree[p].r - tree[p].l + 1) * val;
		tree[p].lzAdd = 0;
