...
-	Range add ($lzAdd$) when $lzSet$ equals 0: Simply add the new value to the pre-existing value. 

-	Range add ($lzAdd$) when $lzSet$ doesn't equal to 0: Add the new value to $lzSet$ and clear $lzAdd$. 

-	Range set when $lzAdd$ equals 0: Simply update the $lzSet$ value. 

-	Range set when $lzAdd$ doesn't equal to 0: Again simply update the $lzSet$ value since a set update will override all previous add update. 
...
void pushdown(int p){
	...
	if(tree[p].lzSet != 0){
		...
		else tree[rc].lzSet += tree[p].lzAdd;
