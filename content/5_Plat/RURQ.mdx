...
$\mathcal{O}(\log{N})$, we lazily apply updates by storing them at the tree 
nodes they affect.
...
have to do for each node.

<Spoiler title = "Horrible Queries Implementation">
...
class LazySegtree {
  private:
    ...
    ll range_sum(int v, int l, int r, int ql, int qr) {
        ...
        push_down(v, l, r);
        int m = (l + r) / 2;
        return range_sum(2 * v, l, m, ql, qr) + 
  ...
  public:
    ...
    ll range_sum(int ql, int qr) { return range_sum(1, 0, sz - 1, ql, qr); }
};
...
class LazySegtree {
  private:
	const int sz;
	vector<ll> tree;     // tree[i] = sum of this node's range
  ...
  public:
	LazySegtree(const vector<ll> &a) : sz(a.size()), tree(4 * sz), lazy(4 * sz) {
		build(1, 0, sz - 1, a);
	}
...
We can also use a somewhat generic template:
