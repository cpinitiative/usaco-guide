...
Since we only need to keep track of $2X+1$ values for each $i$, this solution runs in $O(nX)$ time, which is fast enough with $X=5\cdot 10^5$ using bitset.
...
- Bipartite matching in $O(N^3)$ (dense graph with $N$ vertices on each side)
  - Though $O(NM)$ [Kuhn's algorithm](https://codeforces.com/blog/entry/83036?#comment-703057) is probably fast enough ...
