...
class DisjointSet:
	...
	def __init__(self, sz) -> int:  # Construct the DSU
		self.sz.append(1)
		for i in range(1, sz + 1):
			self.parent[i] = i
			self.sz.append(1)

	def find(self, k) -> int:  # Find the root of k's component
		if self.parent[k] == k:
			return k
		return self.find(self.parent[k])

	def find_size(self, x) -> int:  # Find the x's component's size
		return self.sz[self.find(x)]

	def union(self, a, b) -> int:  # Combine two components into one
		x = self.find(a)
		y = self.find(b)
		self.sz[y] += self.sz[x]
		self.parent[x] = y
