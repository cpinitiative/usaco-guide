...
export const problems = {
	...
	ad: [
		...
		new Problem(
			'IZhO',
			'2017 - Bootfall',
			'https://oj.uz/problem/view/IZhO17_bootfall',
			'Hard',
			false,
			['Knapsack', 'Bitset'],
			''
...
<!-- 
		new Problem(
			'LMiO',
			'2017 - Faktai',
			'https://oj.uz/problem/view/LMIO17_faktai',
			'Easy',
			false,
			['Bitset'],
			''
...
tl;dr some operations are around 32-64x faster compared to a boolean array. See the [C++ Reference](http://www.cplusplus.com/reference/bitset/bitset/) for the operations you can perform.

<Resources>
	<Resource source="CPH" title="Bit Optimizations" url="https://cses.fi/book/book.pdf#page=110">speeding up Hamming distance</Resource>
	<Resource
		source="CF"
...
<FocusProblem problem={problems.school[0]} />
...
A naive knapsack solution would be as follows. For each $0\le i\le \texttt{comps.size()}$, let $\texttt{dp}[i][j]=1$ if there exists a subset of the first $i$ components whose sizes sum to $j$. Then the answer will be stored in $\texttt{dp}[i]$. This runs in $\mathcal{O}(N^2)$ and is too slow if implemented naively, but we can use bitset to speed it up!
...
<FocusProblem problem={problems.cow[0]} />
...
<FocusProblem problem={problems.lots[0]} />
...
Consider the case when $\sum a_i\ge c$. The intended solution runs in $\mathcal{O}(n\cdot \max(a_i))$; see [here](https://github.com/bqi343/USACO/blob/master/Implementations/content/various/Knapsack.h) for more information. However, we'll solve it with bitset instead.

As with the first problem in this module, let $\texttt{dp}[i][j]=1$ if there exists a subset of the first numbers components that sums to $j$. This solution runs in $\mathcal{O}(n\cdot \sum a_i)$ time, which is too slow even if we use bitset.
...
- Bipartite matching in $\mathcal{O}(N^3)$ (dense graph with $N$ vertices on each side)
  - Though $\mathcal{O}(NM)$ [Kuhn's algorithm](https://codeforces.com/blog/entry/83036?#comment-703057) is probably fast enough ...
...
<Resources>
	<Resource source="GFG" title="_Find_next() function in C++ bitset" url="https://www.geeksforgeeks.org/_find_next-function-in-c-bitset-with-examples/">only resource I could find :P</Resource>
</Resources>
...
In USACO Camp, a similar problem appeared with $N\le 10^5$ and a $6$ second time limit (presumably to allow $\mathcal{O}(N\log ^2N)$ solutions to pass). I had already done this problem but ~~forgot how I had solved it~~ decided to try something new. Try to guess what I did!
