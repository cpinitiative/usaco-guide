...
Label the cows from $0\dots N-1$. For two cows $x$ and $y$ set `adj[x][y]=1` if
...
> Given $n$ ($n\le 2\cdot 10^4$) positive integers $a_1,\dots,a_n$
> ($a_i\le 2\cdot 10^4$), find the max possible sum of a subset of
> $a_1,\dots,a_n$ whose sum does not exceed $c$.
