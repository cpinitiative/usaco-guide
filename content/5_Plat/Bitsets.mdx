...
export const problems = {
    ...
    ad: [
      ...
      new Problem("CSES", "BOI - Nautilus", "https://cses.fi/247/submit/B", "Normal", false, ["Bitset"], "After each minute, store a binary matrix of where Nautilus could possibly be. Moving in any direction is equivalent to shifting all values in the matrix in that direction and then taking the bitwise AND of the grid and the boolean matrix of where Nautilus could be at time 0. We can use an array of bitsets to perform these operations efficiently."),
