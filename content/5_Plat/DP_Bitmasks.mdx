...
Let our dp states be defined as $dp[S][i]$ being the number of routes that visit all the cities in the subset $S$ and end at city $i$. The transitions will then be:
$$
dp[S][i] = \sum_{x \in adj[i]} dp[S \text{\textbackslash} i][x] \text{ if $x \in S$}
$$
where $S \text{\textbackslash} i$ is the subset $S$ without city $i$.
