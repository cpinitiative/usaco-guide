...
Some number theory problems, in particular those that involve making GCD equal to $1$, may be optimized using a sort of bitmask over primes.
If each number were represented by a bitmask of its prime divisors, choosing a set with GCD $1$ is equivalent to choosing a set of bitmasks that AND to $0$.

For example: the set $\{6, 10, 15 \}$ can be represented by $\{0b011, 0b101, 0b110 \}$ (in binary), where the bits correspond to divisibility by $[2, 3, 5]$.
Then, we can see that $\{6, 10 \}$ doesn't have GCD $1$ because $0b011 \& 0b101 = 0b001 \neq 0$.
Likewise, $\{6, 10, 15 \}$ has GCD $1$ because $0b011 \& 0b101 \& 0b110 = 0b000 = 0$.
