...
The main idea is that we don't have to store all the nodes at all times - we create nodes only when needed. In a normal segment tree, an update only affects $\mathcal{O}(\log N)$ nodes, so in a sparse segment tree, we only store $\mathcal{O}(Q \log N)$ nodes!
...
It's possible to reduce the memory of a sparse segment tree to $\mathcal{O}(N)$, as described [here](https://codeforces.com/blog/entry/83969).

</Optional>

### Problems

Some of these problems are solvable with a normal segment tree if you use coordinate compression. Sparse segment trees are rarely ever _required_ to solve a particular problem, but they can make things a lot more convenient.
