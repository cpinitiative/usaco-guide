...
Let $m_1=\frac{P_1.y - P_2.y}{P_1.x - P_2.x}$ be the slope of the linear function that goes through the $P_1$ and $P_2$ points and
$m_2=\frac{P_2.y - P.y}{P_2.x - P.x}$ be the slope of the linear function that goes through the $P_2$ and $P$ points.

The point are collinear if and only if the slopes are equal i.e. $m_1 = m_2 \Leftrightarrow \frac{P_1.y - P_2.y}{P_1.x - P_2.x} = \frac{P_2.y - P.y}{P_2.x - P.x}
...
And this way we obtained the previously mentioned formula. Now, this formula doesn't only tell us whether the points are collinear, but
where the point $P$ is located.

<img src="/src/demo-collinear.png" alt="Points">
