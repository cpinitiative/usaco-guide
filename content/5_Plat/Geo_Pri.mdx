...
This problem combines the Euclidean MST (EMST) with lowest common ancestor queries.

We can compute the EMST with a normal MST algorithm with Kruskal's or Prim's, but
unfortunately they're too slow for the given bounds.
...
class BinaryJumping {
  ...
  public:
	...
	/** @return the max value on the path from x to y */
...
/** @return the Manhattan distance from point a to b */
int dist(const Point &a, const Point &b) {
	return abs(a.x - b.x) + abs(a.y - b.y);
}

/** @return the set of points that make up the MST of the given edges */
...
To improve the previous discussed algorithm we'll reduce the complete graph - with $N^2$ edges - to a much smaller one; this way the $\mathcal{O}(N \codt \log{N})$ Kruskal algorithms will
run on fewer edges, thus speeding up the computation. There several ways of reducing the number of edges; two of them are:
- [Delaunay Triangulation](https://en.wikipedia.org/wiki/Delaunay_triangulation) has a nice property: the minimum spanning tree is a subster of the triangulation, which consists of triangles whose circumcircle do not contain any other of the points.
- Only consider the nearest point in each of the $8$ octants of the plane.

We'll continue to discuss the second approach. The image below depicts the octants with an arbitrary point $P$ at the centre. Let's take a closer look at the octant containing points
$Q$ and $R$. Point $Q$ is the closest point in the octant - the dashed line traverses point with equal manhattan distance to point $P$. We'll assume that $PR$ is considered for
the spanning tree. According to the image $|PQ| < |PR| \iff |PQ| + |QR| < |PR| + |QR|$, thus the minimum spanning tree will contain edges $PQ$ and $QR$. Consequently, edge $PR$ can be ignored or removed.

![Illustration of Octants](./assets/octants.png)

This approach significantly reduces the number of edges. This reduces the problem to that of finding the nearest neighbour in each octant. We'll analyze the same octant; the other can be computer similarly. The closest point in the octant is the one with the maximum value of $x-y$
