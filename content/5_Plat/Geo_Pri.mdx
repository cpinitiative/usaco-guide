...
The solution to cast a ray from the point $P$ going in any fixed direction - preferably to the right. If the point is located 
on the outside of the polygon the ray will intersect its edges an even number of times.  If the point is on the inside of the polygon then 
it will intersect the edge an odd number of times.

This approach is called [Ray casting algorithm](https://en.wikipedia.org/wiki/Point_in_polygon).
