...
struct Segment { long long a, b, x, y, idx;  };
...
long long orientation(Point a, Point b, Point c) { return (b.x-a.x) * (c.y-a.y) - (c.x-a.x) * (b.y-a.y); }
...
int main() {
    ...
    out << (ans > 1 ? second_segment+1 : first_segment+1);
