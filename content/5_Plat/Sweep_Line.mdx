...
The key observation is that although there are many points, they are spread across a small surface i.e. $0 \leq x, y \leq 1000$. So instead of using Kruskal's or Prim's
algorithm, we can use Dijkstra's. Starting from an arbitrary point, from the first one in the code below, run Dijkstra algorithm. The priority queue
