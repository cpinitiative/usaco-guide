...
The key observation is that although there are many point, they are spread across a small surface i.e. $0 \le x, y \le 1000$. So instead of using  Kruskal's or Prim's 
algorithm we can use a Dijkstra approach style. Starting from an arbitrary point, from the first one in the code below, run Dijkstra algorithm. The priority queue
will sort the points after the distance to the MST.
...
int d[NMAX][NMAX];
bool mat[NMAX][NMAX]; // True if (x,y) point is in the input
...
int main() {
	...
	auto inside = [](int x, int y) { return 0 <= x && 0 <= y && x <= 1000 && y <= 1000; };
	...
	while(!pq.empty()) {
		...
		for(int dir = 0; dir < 4; dir++) {
			int newx = x + dx[dir];
			int newy = y + dy[dir];
			if( inside(newx, newy) && (d[newx][newy] == -1 || dist + 1 < d[newx][newy]) ) {
				d[newx][newy] = dist + 1;
