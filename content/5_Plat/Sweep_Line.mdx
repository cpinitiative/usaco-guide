...
*Time Complexity:* $\mathcal{O}(S^2)$, where S is the grid size
...
int d[GRID_SZ][GRID_SZ];     // The distance from the starting point
bool mat[GRID_SZ][GRID_SZ];  // mat[x][y] = if (x, y) point is in the input
