...
The key observation is that although there are many points, they are spread across a small surface i.e. $0 \leq x, y \le 1000$, so instead of using Kruskal's or Prim's
algorithm, we can use Dijkstra's. Starting from an arbitrary point, run Dijkstra's algorithm with the priority queue that will sort the points after the distance to the MST.
...
int d[NMAX][NMAX]; // The distance from the starting point
bool mat[NMAX][NMAX];  // mat[x][y] = if (x, y) point is in the input
