...
Because each step now runs in linear time, by the [master theorem](https://en.wikipedia.org/wiki/Master_theorem_(analysis_of_algorithms)) $T(n) = 2T(n / 2) + \mathcal{O}(n)$, our solution now runs in $\mathcal{O}(n\log n)$. 
