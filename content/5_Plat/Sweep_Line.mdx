...
### Solution
Let's simplify the problem a little bit and focus on finding any overlapping segments. The segment we are looking for is one of the two.
...
We simulate this by sorting all the segment endpoints by x and walking through this sorted array (called 'events' in the code below). As we scan, we keep track of active segments
