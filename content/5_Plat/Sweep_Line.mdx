...
The active set of segments is ordered by $y$ coordinate. If two segments overlap, they are adjacent in the set, so every time we insert or remove
...
// To check if two segments intersect we will use the signed area of the ABC
// triangle. This can be derived from the cross product of the vectors AB and
// AC.
