...
pair<Point, Point> strip_solve(
    vector<Point> &points) {  // Brute force for points with near mid_point
...
pair<Point, Point> solve(vector<Point> &points, int l,
                         int r) {  // Solve the problem for range [l, r]
	if (l == r) { return INF; }
	int mid = (l + r) / 2;
	pair<Point, Point> ans_left =
	    solve(points, l, mid);  // The smallest distance in range [l, mid]
	pair<Point, Point> ans_right =
	    solve(points, mid + 1, r);  // The smallest distance in range [mid+1, r]
