...
A brute force matching algorithm that computed $
...
It can be shown that for each point $p$, there is $\mathcal{O}(1)$ points that satisfy this property. Because each point in $R'$ is at least $\delta_r \geq \delta$ apart, arranging the points in the worst case would result in 6 and spoints in the corners and sides of the bounding rectangle.
...
Because each step now runs in linear time, by the mastern theorem $T(n) = 2T(n / 2) + \mathcal{O}(n)$, our solution now runs in $\mathcal{O}(n\log n)$ (note its similarities to merge sort). 
