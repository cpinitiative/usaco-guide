...
### Solution
Let's simplify the problem a little bit and focus on finding any of overlapping segments. The segment we are looking for is on of the two.

To find a pair of overlapping segments we use a sweep line approach. Imagine sweeping a vertical line across the scene from left to right, pausing at every segment endpoint.
We simulate this by sorting all the segment endpoints by x and walk through this sorted array (called 'events' in the code below). As we scan we keep track of active segments
by using a set (called 'active_segments' in the code below). When we hit the beginning point of a segment, we add it to the active set, and when we hit the ending point of a segment, we remove it from the active set. 
The operation of inserting or removing takes $\mathcal{O}(\log n)$.

The active set of segments is ordered by $y$ coordinate. If two segments overlap, it means that they are adjacent in the set. So every time we insert or remove 
a segment we check if the adjacent segments overlap.
...
struct Point { 
    int x, y, segment_idx; 
    bool operator<(const Point &oth) { return x == oth.x ? y < oth.y : x < oth.x; }
};
struct Segment { int a, b, x, y, idx;  };

double coordinate(Segment a) {    
    if(a.b == a.y) return a.y;
    return a.b + (a.y - a.b) * (sweep_line_x-a.a) / (a.x - a.a);
}

bool operator<(Segment a, Segment b) { return a.idx != b.idx && coordinate(a) < coordinate(b); }
bool operator==(Segment a, Segment b) { return a.idx == b.idx; }
int orientation(Point a, Point b, Point c) { return (b.x-a.x) * (c.y-a.y) - (c.x-a.x) * (b.y-a.y); }

bool intersect(Segment a, Segment b) {
    Point p1 = {a.a, a.b}, p2 = {a.x, a.y}, p3 = {b.a, b.b}, p4 = {b.x, b.y};
    if( orientation(p1, p2, p3) * orientation(p1, p2, p4) > 0 ) return false;
    if( orientation(p3, p4, p1) * orientation(p3, p4, p2) > 0 ) return false;
    return true;
}
    ...
    for(int i = 0; i < (int)events.size(); i++) {
        ...
        if(it != active_segments.end()) {
            ...
            if(it != active_segments.begin()) { 
                it--;
                if(intersect(*it, segments[first_segment])) {
                    second_segment = it->idx;
                    break;
                }
            }
