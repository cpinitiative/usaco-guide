...
From here, we need a fast way to query the $\text{LCA}$ of a range of nodes. One way to do this is to maintain the DFS order of the nodes, and simply take the $\text{LCA}$ of the nodes with the earliest and latest traversal within the range. This can be done with any range query data structure, such as a sparse table or segment tree.
...
namespace SGT {
    ...
    /** @return edge which is the LCA of the nodes w/ lowest and highest traversal time in [l, r] */
