...
public class Springboards {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Query the full range to account for the transition from the last springboard to (N, N)
        // rangeMin returns min(ans[j] - x2[j] - y2[j]). We add 2*N to complete the formula.
