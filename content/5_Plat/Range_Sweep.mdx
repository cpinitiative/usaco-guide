...
class BIT:
	...
	def __init__(self, size):
		...
		self.bit = [0] * (
			size + 1
		)  # Binary Indexed Tree (BIT) array initialized with zeros
	...
	def pref_sum(self, ind):
		...
		while ind > 0:
			...
			ind -= (
				ind & -ind
			)  # Move to the previous index that contributes to the current index
		return total
...
if __name__ == "__main__":
	...
	for y, type, x1, x2 in events:
		...
		if type == 1:
			...
			ans += bit.pref_sum(x2) - bit.pref_sum(
				x1 - 1
			)  # Calculate sum of horizontal range using BIT
		elif type == 3:
