...
$$
\texttt{ans}[i] = \min\limits_{j < i, x_2[j] \le x_1[i], y_2[j] \le y_1[i]}(\texttt{ans}[j] + \texttt{dist}(j, i))
$$

**Full Solution:** $\mathcal{O}(P \log P)$

Optimizing the DP involves the use of a min point update range query segment tree. Let's first expand $\texttt{dist}(i, j)$ in the transition formula.
