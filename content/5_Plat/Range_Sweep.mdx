...
We can sweep from bottom to top (by the $y$ coordinate); storing two nodes for vertical segments (one for start and one for end) and one node for horizontal segments.
