...
struct Point {
	int x, y, i, type;
	Point(int x, int y, int i, int type) : x(x), y(y), i(i), type(type) {}
...
int main() {
	...
	for (int i = 0; i < P; ++i) {
		int x1, y1, x2, y2;
		cin >> x1 >> y1 >> x2 >> y2;
		events.emplace_back(x1, y1, i, -1);  // start point
		events.emplace_back(x2, y2, i, 1);   // end point
	...
	for (Point p : events) {
		if (p.type == -1) {
			ans[p.i] = p.x + p.y + S.query(0, y_index(p.y));
		} else {
			S.upd(y_index(p.y), ans[p.i] - p.x - p.y);
		}
	}

	// adds 2N to include the transition from the last springboard to the
	// endpoint
	cout << S.query(0, 2 * P - 1) + 2 * N;
}
```
...
Bonus: Rather than thinking of this problem as minimizing the distance walked, think of this problem as maximizing the distance covered by springboards to simplify the DP transition.
