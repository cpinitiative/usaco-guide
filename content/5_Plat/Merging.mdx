...
A set doesn't have to be an `std::set`. Many data structures can be merged, such as `std::map` or `std:unordered_map`. However, $\mathcal{O}(1)$ time of `std::swap` may not apply for some low-level data structures: swapping two [arrays](http://www.cplusplus.com/reference/array/array/swap/) takes time linear in the sum of the sizes of the arrays, and the same goes for [GCC policy-based data structures](https://codeforces.com/blog/entry/11080). For two `__gnu_pbds::tree` or `__gnu_pbds::gp_hash_table` `a` and `b` we can use `a.swap(b)` in place of `swap(a,b)` to reach $\mathcal{O}(1)$ time.

<IncompleteSection>	

is there a list of data structures that support swap in constant time?
