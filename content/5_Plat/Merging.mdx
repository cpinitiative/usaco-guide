...
**Proof:** Suppose that you always merge from the smaller subtree into the
larger subtree. Then, each element will be merged at most $O(\log N)$ times,
because the subtree size will double every time the element is merged into a
larger subtree. In total, we process at most $O(N\log N)$ elements.

Note that when we use the merging technique in the code, each merge will take
less time than in our proof. So, we will process at most $O(N \log N)$ elements,
each taking at most $O(\log N)$ time.
