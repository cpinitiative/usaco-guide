...
Find LCA of node $a$ and $b$ as described above. Then, the distance between the two nodes would be the $\texttt{depth}[a] + \texttt{depth}[b] - 2 \cdot \texttt{depth}[\textrm{func(a, b)}]$.
...
int main() {
    ...
    F0R(i, Q) {
        ...
        pr(depth[a] + depth[b] - 2 * depth[find(a, b)],"\n");
    }
