...
First, let $\texttt{st}, \texttt{en}, and \texttt{up}$ be the time-in, time-out, and ancestor table for all nodes in the tree. 
...
With this information, we can declare node $a$ an ancestor of $b$ if $\texttt{st}[a] <= \texttt{st}[b] && \texttt{en}[a] >= \texttt{en}[b]$
