...
In our ```jmp(int x, int d)``` implementation, we can test if we jump in powers of two by using the $\&$ operator. If the $i$th bit on the right is toggled, then we jump. For example, a jump of $13$ would correspond to the binary number $1101$. We would jump 3 times on bits $$i$$.

<Spoiler title="Illustration of the jump method">

![Binary Lifting Diagram](https://i.imgur.com/Mu1UZI4.png)

</Spoiler>

To calculate the columns required for the ```up``` array, use the formula $\textrm{ceil}(\log_2{N})$ which in our case simplifies to $\textrm{ceil}(\log_2{(2\cdot 10^5)}) = 18$.
...
void dfs(int x) { // build jump table
    FOR(i, 1, MS) up[i][x] = up[i-1][up[i-1][x]];
    trav(y,adj[x]) if (y != up[0][x]) {
        up[0][y]=x;
        dfs(y);
    }
}
...
int jmp(int x, int d) {
    ...
    return x ? x : -1; // modfication to return -1 if not found
}
