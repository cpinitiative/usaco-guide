...
In our ```jmp(int x, int d)``` implementation, we can test if we jump in powers of two by using the $\&$ operator. If the $i$th bit on the right is toggled, then we jump. For example, a jump of $13$ would correspond to the binary number $1101$. We would jump 3 times on bits $0, 1, 3$ (in that order) from the right.
...
To calculate the columns required for the ```up``` array, use the formula $\lceil (\log_2{N}) \rceil$ which in our case simplifies to $\lceil (\log_2{(2\cdot 10^5)})\rceil = 18$.
