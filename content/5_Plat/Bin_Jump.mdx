...
First, let $\texttt{tI}, \texttt{tO}, and \texttt{up}$ be the time-in, time-out, and ancestor table for all nodes in the tree. 
...
With this information, we can declare node $a$ an ancestor of $b$ if $\texttt{tI}[a] <= \texttt{tI}[b] && \texttt{tO}[a] >= \texttt{tO}[b]$
