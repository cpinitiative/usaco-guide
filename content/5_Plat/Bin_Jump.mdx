...
export const problems = {
	...
	general: [
		...
		new Problem(
			...
			false,
			['LCA'],
			'The answer is the number of pairs of paths that overlap. If we split each path A-B into the 2 paths A-lca(A, B) and B-lca(A, B), then we can count how many pairs of those paths overlap and subtract the number of pairs of paths that have the same lca and 2 outgoing edges from the lca. The case where the tree is a line is a classical problem and we can adapt it to work on a general tree.'
		),
	],
};
