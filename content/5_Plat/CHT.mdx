...
In this module, we'll focus on the special case of CHT where "slopes" of functions are monotonic. This specific case is solvable in $O(N)$ using a `std::deque` in C++.

(For the more general $O(N \log N)$ CHT, see [the LineContainer module](/plat/LC)).
