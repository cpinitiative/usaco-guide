...
We partition the array into $\sqrt N)$ blocks, where each block covers an
interval of size $\sqrt{N}$. For each block, there exists a corresponding
`update` and `query` operation.

**Update Queries:** $\mathcal{O}(1)$

To update an element at location $x$, we can find the corresponding block using
the formula $\frac{x}{\texttt{blocksz}}$, where $\texttt{blocksz} = \sqrt{N}$ and apply the corresponding difference
between the element currently stored at $x$, and the element we want to change
it to.
...
$$
\sum_{i = 0}^{R} \texttt{blocks}[i] + \sum_{\lfloor R \rfloor \cdot \texttt{blocksz}}^r \texttt{nums}[i]
$$

where $\texttt{blocks}[i]$ represents the total sum of the $i$-th block, where the $i$-th block represents the sum of the elements from the range $[i\sqrt{N},{(i + 1)}\sqrt{N})$, and $R$
represents $\frac{r}{\texttt{blocksz}}$. Since each of these blocks are of size
$\sqrt{N}$, there can be up to $\sqrt{N} + 1$ blocks.

Finally, to find our total difference between $[l, r]$, we can perform two sum
queries for the quantity
$\sum_{0}^{r}\texttt{nums}[i] - \sum_{0}^{l-1}\texttt{nums}[i]$ which produces
the answer of $\sum_{l}^{r} \texttt{nums}[i]$
