...
If we try prefix sums, the commplexity is still $\mathcal{O}(N^2)$.
...
We can apply the DP algorithm to the steps where $A_i * x >= \sqrt(N)$ because the jump is bigger, thus resulting in a faster loop
For the remaining cases where $A_i * x < \sqrt(N)$, we can apply prefix sums.
