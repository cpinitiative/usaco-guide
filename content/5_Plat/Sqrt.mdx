...
One approach in $\mathcal{O}(N^2)$ is DP.
<LanguageSection>
<CPPSection>
```cpp 
vector<int> dp(n, 1);
...
Another approach still in $\mathcal{O}(N^2)$ is prefix sums.
<LanguageSection>
<CPPSection>
```cpp 
for(int i = n-1; i >= 0; i--) {
...
We can apply the first algorithm to those steps $A_i * x >= \sqrt(N)$, because the jump is bigger so 
the for loop is faster, and the second one for $A_i * x < \sqrt(N)$ .
