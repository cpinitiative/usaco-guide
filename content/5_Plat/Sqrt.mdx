...
We partition the array into blocks of size $\texttt{blocksz}=\lfloor \sqrt{N}
\rfloor$. Each block stores the sum of elements within it, and allows for the
...
\texttt{blocksz},(i + 1)\cdot \texttt{blocksz})$, and $R=\left\lfloor
\frac{r}{\texttt{blocksz}} \right\rfloor$.
...
long long blocks[MAX_BLOCKS]; // 450^2 > 2e5
int nums[MAX_BLOCKS * MAX_BLOCKS];
int block_size;
