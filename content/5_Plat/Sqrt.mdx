...
interval of size $\texttt{blocksz}=\lfloor \sqrt{N} \rfloor$. For each block, there exists a
corresponding `update` and `query` operation.

**Update Queries:** $\mathcal{O}(1)$

To update an element at location $x$, we can find the corresponding block using
the formula $\frac{x}{\texttt{blocksz}}$, where
$\texttt{blocksz} = \lfloor \sqrt{N} \rfloor$ and apply the corresponding
difference between the element currently stored at $x$, and the element we want
to change it to.
...
where $\texttt{blocks}[i]$ represents the total sum of the $i$-th block and the
$i$-th block represents the sum of the elements from the range
$[i\sqrt{N},{(i + 1)}\sqrt{N})$, and $R$ represents
$\frac{r}{\texttt{blocksz}}$.

Finally, $\sum_{i=l}^{r} \texttt{nums}[i]$ is the difference between the two
prefix sums $\sum_{i=0}^{r}\texttt{nums}[i]$ and
$\sum_{i=0}^{l-1}\texttt{nums}[i]$ queries for the quantity
