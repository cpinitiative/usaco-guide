...
We'll use [euler tour technique](/gold/tree-euler) to flatten the tree into an array, upon which we can apply MO's algorithm. Now, if node $x$
...
**Time Complexity:** $\mathcal{O}(N\cdot\log{N}+(N + Q)\sqrt{N})$
...
int main() {
	...
	for (int i = 1; i <= q; i++) {
		...
		int l = get_lca(x, y);
		if (l == x) {  queries.push_back({start[x], start[y], -1, i}); } 
		else { queries.push_back({en[x], start[y], l, i}); }
	}
