...
To perform a sum query from $[0...r]$, calculate

$$
\sum_{i = 0}^{r} \texttt{blocks}[i]
$$

where $\texttt{blocks}[i]$ represents the total sum of the $i$-th block. Since
each of these blocks are of size $\sqrt{N}$, there can be at most $\sqrt{N}$
blocks.

Finally, to find our total difference between $[l, r]$, we can calculate

$$
\sum_{i = 0}^{r} \texttt{blocks}[i] - \sum_{i = 0}^{l - 1} \texttt{blocks}[i] = \sum_{i = l}^{r} \texttt{blocks}[i]
$$
