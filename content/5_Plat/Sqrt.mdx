...
Low constraints (ex. $N=5\cdot 10^4$) and/or higher time limits (greater than 2s) can be signs that sqrt decomp is intended.

Some solutions have complexity $\mathcal{O}(N\sqrt{N\log N})$ ...

optimization tips? (ez to get TLE with bad constant ..., )
...
can get AC with suboptimal complexity ... Fast $O(N\sqrt N\log N)$ where the $\log N$ factor comes from a BIT can beat $O(N\sqrt N)$ (depending on the test data ...)
