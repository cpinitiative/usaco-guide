...
We partition the array into blocks of size $\texttt{block_size}=\lceil \sqrt{N}
...
the formula $\frac{x}{\texttt{block_size}}$.
...
$$
\sum_{i = 0}^{R-1} \texttt{blocks}[i] + \sum_{R \cdot \texttt{block_size}}^r \texttt{nums}[i]
$$
...
\texttt{block_size},(i + 1)\cdot \texttt{block_size})$, and $R=\left\lceil
\frac{r}{\texttt{block_size}} \right\rceil$.
