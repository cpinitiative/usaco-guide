...
export const problems = {
	diviSample: [
		new Problem('CSES', 'Range Minimum Queries I', '1647', 'Very Easy', false, [], ''),
	],
	general: [
		new Problem('JOI', 'Secret', 'JOI14_secret', 'Easy', false, [], 'joi-14-secret'),
...
In the [prerequisite module](/gold/tree-euler#range-minimum-query), it was shown that we can get $\mathcal{O}(1)$ time queries with $\mathcal{O}(N\log N)$ time preprocessing when $\ominus$ denotes `min`. But how do we generalize to other associative operations?
