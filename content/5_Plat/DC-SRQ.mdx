...
In the [prerequisite module](/gold/tree-euler#range-minimum-query), it was shown that we can get $O(1)$ time queries with $O(N\log N)$ time preprocessing when $\ominus$ denotes `min`. But how do we generalize to other associative operations?
