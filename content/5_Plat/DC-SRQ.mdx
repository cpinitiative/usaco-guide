...
In the [prerequisite module](/gold/tree-euler#sparse-tables), it was shown that we can get $\mathcal{O}(1)$ time queries with $\mathcal{O}(N\log N)$ time preprocessing when $\ominus$ denotes `min`. But how do we generalize to other associative operations?
