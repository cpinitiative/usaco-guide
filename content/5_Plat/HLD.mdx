...
Below is an example implementation of Heavy Light Decomposition based on the resources above. See the solution below, as well as the solutions for [Subtrees & Paths](/solutions/hr-SubtreesPaths) and [Query on a tree again!](/solutions/spoj-QueryOnATreeAgain), for examples of how this implementation can be used.
...
class HLD {
  ...
  public:
    HLD(vector<vector<int>> adj_, int _R)
        : N(adj_.size()), R(_R), adj(adj_), par(N, -1), siz(N, 1), depth(N), rt(N), pos(N),
        segtree(vector<ll>(N,0ll)) // modify if need
    ...
    void modifySubtree(int v, T val) {
        segtree.range_update(pos[v] + VALS_IN_EDGES, pos[v] + siz[v] - 1, Query{ADD,val}); // modify if need
    }
};
```
