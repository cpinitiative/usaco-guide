...
Since the process requires performing $\mathcal{O}(\log N)$ Segment Tree operations, the total time for a path query or update is $\mathcal{O}(\log N) \cdot \mathcal{O}(\log N) = \mathcal{O}(\log^2 N)$.
...
Now, to change the value at node $i$ to $x$, we can just update the value in the segment tree. To query the maximum value in the path from $a$ to $b$, we first find the [Lowest Common Ancestor](/gold/lca-euler#lca). We combine the path from $a$ to $lca(a,b)$ and the path from $b$ to $lca(a,b)$ to find our answer.
...
[this link](https://www.spoj.com/problems/GRASSPLA/) to submit.
