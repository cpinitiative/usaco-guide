...
Additionally, by using a [Segment Tree](/gold/PURS#segment-tree) (or similar structure), we can process queries on a contiguouse segment of any heavy chain in $\mathcal{O}(\log N)$ time.
