...
// most array sizes will be doubled to prevent out of bounds

V<vi> calc(2000, vi(1001,0)); // calc[i][j] stores the # of steps to get all cows distance j away to door i
V<vi> dp(8, vi(1001,INF));
int rot;

void compdp(int k, int begin, int end, int rl, int rr){
	// fixed k, begin and end are the ends of the array, rl and rr are the bounds on the last door used
	int mid = (begin+end)/2;
	
	int bestcost = INF;
	int last = -1;
	// bestcost is min amount moved, last is the last door used
	for(int lst= rl; lst<=min(mid,rr); lst++){
		int cost = dp[k-1][lst-1] + calc[lst+rot][mid-lst+1];
		if(cost < bestcost){
			bestcost = cost;
			last = lst;
		}else if(cost==bestcost && lst < last){
			last = lst;
		}
	}
	dp[k][mid] = bestcost;
	if(begin==end) return;
	compdp(k, begin, mid, rl, last);
	compdp(k, mid+1, end, last, rr);
}
signed main(){
	int n,k;
	cin >> n >> k;
	...
	for(int i=0; i<n; i++)for(int j=1; j<=n; j++){
		calc[i][j] = calc[i+n][j] = calc[i][j-1] + a[i+j-1]*(j-1);
	}
