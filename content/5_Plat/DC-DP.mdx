...
const int maxn = 1000;
const int maxk = 7;
vector<vector<ll>> calc(2*maxn, vector<ll>(maxn+1, 0));  // calc[i][j] stores the # of steps to get all
                                // cows distance j away to door i
vector<vector<ll>> dp(maxk+1, vector<ll>(maxn+1, INF));
int rot;
...
void compdp(int k, int begin, int end, int rl, int rr) {
	...
	for (int lst = rl; lst <= min(mid, rr); lst++) {
		ll cost = dp[k - 1][lst - 1] + calc[lst + rot][mid - lst + 1];
		if (cost < bestcost) {
			bestcost = cost;
			last = lst;
