...
Essentially, we just nest the loops that one would find in a 1D BIT to get N-dimensional BITs. We can then use <abbr title="Principle of Inclusion-Exclusion">[PIE](https://brilliant.org/wiki/principle-of-inclusion-and-exclusion-pie/)</abbr> to query subrectangles.
