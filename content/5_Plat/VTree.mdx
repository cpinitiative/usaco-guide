...
$S$ is the *virtual tree* or *auxiliary tree* of $S_0$. We can also define $p'_v$, the virtual parent of $v$ as the lowest node in $S$ such that $p'_v \neq v$ and $\text{LCA}(p'_v,v) = p'_v$, and $v$ as a virtual child of $p'_v$. Here is an example of a virtual tree, with nodes in $S_0$ colored purple, and the rest of the virtual tree in blue:

![VTree](https://ibb.co/67M0f8hY)
...
int tin[MAX_N], tout[MAX_N], sub[MAX_N], col[MAX_N], 
    lift[MAX_N][LG], timer;
