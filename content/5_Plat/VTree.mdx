...
inline int is_ancestor(int u, int v) { return tin[u] <= tin[v] && tout[v] <= tout[u]; }
...
vector<int> vtree(vector<int> &key) {
	if (!sz(key)) return {};
	vector<int> res = key;
	sort(all(key), sort_tin);
	for (int i = 1; i < sz(key); i++) res.push_back(lca(key[i - 1], key[i]));
...
int range_query(int QL, int QR, int CL = 0, int CR = L - 1, int CI = 1) {
	...
	return min(range_query(QL, QR, CL, (CL + CR) / 2, 2 * CI),
...
void solve() {
	...
	for (int dep = 0; dep < n; dep++) {
		...
		for (int v : vt) {
			...
			if (sz(vg[v])) {
				int su = 0;
				for (int u : vg[v]) su += dp[u];
				dp[v] = min(dp[v], su);
			}
		}
		if (sz(vt)) ans += dp[vt[sz(vt) - 1]];
	}
	cout << ans << "\n";
}
