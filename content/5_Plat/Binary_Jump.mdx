...
up = [[0 for j in range(NMAX)] for i in range(LOGMAX)]

def jmp(x:int, k:int) -> int:
...
for i in range(n-1): up[0][i+2] = boss[i]

for i in range(1, LOGMAX):
...
up = [[0 for j in range(LOGMAX)] for i in range(NMAX)]
	
boss = list(map(int, input().split()))
for i in range(2,n+1): g[boss[i-2]].append(i)
	
def dfs(node:int, parent:int):
    up[node][0] = parent
    for i in range(1, LOGMAX): up[node][i] = up[up[node][i-1]][i-1]
    for son in g[node]:
        if son != parent:
            depth[son] = depth[node] + 1
            dfs(son, node)
	
def jump(x:int, d:int) -> int:
    for i in range(LOGMAX):
        if((1<<i) & d):
            x = up[x][i]
    return x
