...
def kth_ancestor(x: int, k: int) -> int:  # return the kth ancestor of x
...
LOGMAX = math.ceil(math.log2(n))

up = [[-2 for j in range(n)] for i in range(LOGMAX + 1)]
