...
There are ways to do this in $\mathcal{O}(K)$ time for a binary tree if you don't need to return the values in sorted order (see [here](https://www.sciencedirect.com/science/article/pii/S0890540183710308)).
...
Essentially, we start with the entire search space and then we _fracture_ it into _subspaces_ based on the children of the root. Then we can finish with either of the two approaches.
