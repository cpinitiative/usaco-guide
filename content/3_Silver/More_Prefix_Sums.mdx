...
public class ForestQueries {
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i < N + 1; i++) {
			for (int j = 1; j < N + 1; j++) {
				pfx[i][j] = arr[i][j] + pfx[i - 1][j] + pfx[i][j - 1] -
				            pfx[i - 1][j - 1];
			}
		}
...
<Resources>
	<Resource source="Codeforces" title="An Introduction To Difference Arrays" starred />
</Resources>
...
int main() { 
	...
	vector<tuple<int, int, int>> updates(m);
	for(tuple<int, int, int> &update : updates) {
		cin >> get<0>(update) >> get<1>(update) >> get<2>(update);
	}
	vector<long long> s(m + 2, 0), add(n + 2, 0);
	for(int i = 0; i < k; i++) {
		int x, y;
		cin >> x >> y;
		s[x]++;
