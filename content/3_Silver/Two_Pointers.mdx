...
To accomplish this, we can define $\texttt{left}$ and $\texttt{right}$ to **represent** the beggining and end of the **segment**. However, both will start at the beggining of the array. These numbers can be thought of as pointers, hence the name "two pointers".
...
We can move the right pointer only to index $1$:
<center>
<table>
	<tbody>
		<tr>
...
The sum of the values in this range is 4, and there are 2 values. So, the current maximum is 2. Now, moving the left pointer up by 1, we can subtract 3 from the sum of values, to get 1. So, the current sum of values is 1. The array then looks like:
<center>
<table>
	<tbody>
		<tr>
...
Now, we can move the right pointer up by $2$, to the end. This makes the sum of values precisely $5$ (since $1$ (previous cur val) $+2+1=5$, and has a length of 3. So, the maximum is now $3$.
<center>
<table>
	<tbody>
		<tr>
...
At this point we are done, since the right pointer has hit the end. So, the answer is $3$.
