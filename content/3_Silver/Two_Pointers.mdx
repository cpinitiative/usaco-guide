...
The sum of the values in this range is 4, and there are 2 values. So, the current maximum is 2. By incrementing the left pointer by 1, we can subtract 3 from the sum of values to get 1. The array then looks like:
<center>
<table>
	<tbody>
		<tr>
...
Now, we can move the right pointer to the end. This makes the sum of values $4$, since $1$ (previous cur val) $+2+1=4$, and the length of the segment equal to 3. So, $\texttt{ans}$ is now $3$.
<center>
<table>
	<tbody>
		<tr>
...
Since the right pointer has reached the end of the array, we are done at this point. This leaves us with $3$ as our answer.
