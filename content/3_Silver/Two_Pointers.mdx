...
<Quiz>
  <Quiz.Question>
    ...
    <Quiz.Answer correct>
      Keeping track of two pointers to find a pair of indices that satisfy some condition.
      <Quiz.Explanation>
        Correct.
  ...
  <Quiz.Question>
    When would a solution be impossible when finding two distinct elements in an array which add up to a target while doing a collision two pointers algorithm? (two pointers move from the ends of the array to the middle). 

	```
	// find two distinct elements in an array 
