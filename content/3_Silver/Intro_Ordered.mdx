...
export const problems = {
	...
	sample: [
		new Problem('CSES', 'Bit Inversions', '1188', 'Hard', false, [], '@B'),
	],
	general: [
		...
		new Problem('Silver', 'Convention II', '859', 'Normal', true, []),
		new Problem('Gold', 'Snow Boots', '813', 'Hard', true, []),
		new Problem(
			'CF',
...
<Resources>
	<Resource source="CPH" title="4.4 - Set Iterators" starred></Resource>
</Resources>
...
In java, iterators are helpful in looping through sets.

Iterators used on **HashSet** would have the elements in a random order:
```java
...
```
But in **TreeSet** the elements are in a sorted order:
```java
...
Instead of creating an iterator and looping through it, Java provides a for-each loop which creates a hidden iterator and loops through it automatically:
```java
Set<Integer> set = new TreeSet<Integer>();
set.add(1); set.add(3); set.add(0); set.add(-2);
for(int i : set){
...
You shouldn't modify sets when traversing it with set iterators like in any other iteterators for Collections (this **INCLUDES** when using a for-each loop). The only modification possible is using the iterator `remove()` method which can only be used once before calling a `next()` method.
...
<Resources>
	<Resource source="CSA" url="heaps" title="Heaps"></Resource>
</Resources>

A **priority queue** (or **heap**) supports the following operations: insertion of elements, deletion of the element considered highest priority, and retrieval of the highest priority element, all in $O(\log N)$ time according to the number of elements in the priority queue. Priority queues are simpler and faster than sets, so you should use them instead whenever possible.
