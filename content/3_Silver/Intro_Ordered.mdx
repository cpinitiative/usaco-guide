...
{/_ Hard, would recommend skipping (1700 on CF) _/} {/_ Hard, do not attempt
until Gold/Plat (2100 on CF) _/} {/_ First solve problem for $n$-th tournament
only. Extremely hard, do not attempt (2700 on CF) _/}
...
One limitation of ordered sets is that we can't efficiently access the $k^{th}$
...
- If you want to remove a value _once_, make sure to use
  `multiset.erase(multiset.find(val))` rather than `multiset.erase(val)`. The
  latter will remove _all_ instances of `val`.
