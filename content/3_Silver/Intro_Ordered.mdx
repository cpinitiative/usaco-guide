...
<Resources>
	...
	<Resource source="CP2" title="2.2.2 - Non-Linear Data Structures"></Resource>
</Resources>
...
We can replace `s.upper_bound(7)` with `upper_bound(begin(s),end(s),7)` which was the syntax that we used for vectors in the prerequisite module. Although the latter version will return iterators to the same elements, its time complexity is linear in the size of the set `s` rather than logarithmic, so make sure to avoid it!
