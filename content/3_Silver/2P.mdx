...
of an interval. It can also be used to track two values in order to satisfy a specific condition. 
...
We can then remove the $\texttt{left}$-indexed book from $\texttt{cur}$, the total time of current segment, and increment $\texttt{left}$ by 1.

**The $\texttt{right}$ pointer does not restart after an iteration, it simply keeps moving right; therefore, the time complexity is $\mathcal{O}(N)$.**
