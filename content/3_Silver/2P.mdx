---
id: 2P
title: 'Two Pointers'
author: Darren Yao
prerequisites:
  - sorting-custom
description: 'Iterating two monotonic pointers across an array to search for a pair of indices satisfying some condition in linear time.'
frequency: 2
---

export const problems = {
	sample: [
		new Problem(
			'CSES',
			'Sum of Two Values',
			'1640',
			'Easy',
			true,
			[],
			'@CPH 8.1'
		),
		new Problem(
			'CSES',
			'Subarray Sums I',
			'1660',
			'Easy',
			false,
			[],
			'@CPH 8.1'
		),
	],
	ad: [
		new Problem(
			'CF',
			'Cellular Network',
			'https://codeforces.com/contest/702/problem/C',
			'Easy',
			false,
			['2P'],
			'cellular-network'
		),
		new Problem(
			'CF',
			'Books',
			'https://codeforces.com/contest/279/problem/B',
			'Easy',
			true,
			['2P'],
			'cf-books'
		),
		new Problem(
			'CSES',
			'Sum of Three Values',
			'1641',
			'Easy',
			true,
			['2P', 'Sorting'],
			'cses-1641'
		),
		new Problem(
			'Silver',
			'Paired Up',
			'738',
			'Easy',
			false,
			['2P', 'Sorting'],
			''
		),
		new Problem(
			'CF',
			'Cellular Network',
			'problemset/problem/702/C',
			'Easy',
			false,
			['2P'],
			'cellular-network'
		),
		new Problem(
			'Silver',
			'Diamond Collector',
			'643',
			'Normal',
			true,
			['2P', 'Sorting'],
			'usaco-643'
		),
		new Problem(
			'Silver',
			'Sleepy Cow Herding',
			'918',
			'Normal',
			false,
			['2P', 'Sorting'],
			'usaco-918'
		),
		new Problem(
			'CF',
			'Garland',
			'problemset/problem/814/C',
			'Normal',
			false,
			['2P'],
			''
		),
		new Problem(
			'CEOI',
			'2010 - A Huge Tower',
			'https://oj.uz/problem/view/CEOI10_tower',
			'Hard',
			false,
			['2P', 'Sorting'],
			'ceoi-10-tower'
		),
	],
};

<!-- 	new Problem(
		'CF',
		'Jury Meeting',
		'problemset/problem/853/B',
		'Hard',
		false,
		[]
	),
	new Problem(
		'Baltic OI',
		'2009 - Rectangle'
		'https://contest.yandex.com/ioi/contest/566/problems/E/',
		'Insane',
		false,
		[]
	), -->

<Problems problems={problems.sample} />

## Resources

<Resources>
	<Resource source="CPH" title="8.1 - Two Pointers" starred>
		solutions to the problems above
	</Resource>
	<Resource source="IUSACO" title="14.1 - Two Pointers">
		above + mention of max subarray sum
	</Resource>
	<Resource
		source="CF"
		title="CF Edu"
		url="https://codeforces.com/edu/course/2/lesson/9"
		starred
	>
		video explanation of two pointers
	</Resource>
	<Resource source = "YouTube" title = "Two-Pointer Technique" url = "https://www.youtube.com/watch?v=gLfVHfe_DD0&list=PLtLxhThTxJMVMvWJoE2p8Z84tA8suGzvn&index=17">
		A video explanation of common problems that the two-pointer technique could be applied to. Has a bit of Python code but mostly consists of the intuition and problem-solving approach for these problems.
	</Resource>
</Resources>

## Problems

<Problems problems={problems.ad} />
