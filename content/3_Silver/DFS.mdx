...
contributors: Andrew Wang, Jason Chen, Albert Ye
prerequisites:
  - intro-graphs
  - complete-rec
...
<Quiz>
	<Quiz.Question>
		Can a DFS with adjacency list be used to find shortest paths in linear time?
		<Quiz.Answer>
			Yes
			<Quiz.Explanation>
				Incorrect. Worst case, a DFS shortest path algorithm would have to iterate through <i>every</i> path before finding the shortest. Working shortest path algorithms will be covered later in the Gold section.
			</Quiz.Explanation>
		</Quiz.Answer>
		<Quiz.Answer correct>
			No
			<Quiz.Explanation>
				Correct! Worst case, a DFS shortest path algorithm would have to iterate through <i>every</i> path before finding the shortest. Working shortest path algorithms will be covered later in the Gold section.
			</Quiz.Explanation>
		</Quiz.Answer>
	</Quiz.Question>

	<Quiz.Question>
		If a graph has two connected components, does there exist a node where we could start a DFS that visits every node in the graph?
		<Quiz.Answer>
			Yes
		</Quiz.Answer>
		<Quiz.Answer correct>
			No
			<Quiz.Explanation>
				Correct! DFS can only search one connected component at a time, because it cannot search for nodes without any edges reaching them.
			</Quiz.Explanation>
		</Quiz.Answer>
	</Quiz.Question>
</Quiz>
