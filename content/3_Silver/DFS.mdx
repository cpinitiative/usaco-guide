...
## DFS With Stack
A danger in using DFS when dealing with large bounds is the possibility of a common runtime error, **stackoverflow error**. This commonly occurs when $N > 10^5$ due to the limited memory of the built-in "stack" of recursive calls. To resolve this, we can implement our solution iteravely by making our own stack instead of recursively calling a function. You can imagine adding to this stack being the equivalent of calling the recursive function, and popping from a stack being the same as executing the function.
