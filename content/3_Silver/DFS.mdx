...
export const problems = {
	...
	general: [
		...
		new Problem(
			...
			'Normal',
			false,
			['Connected Components', 'Binary Search'],
			'usaco-669'
		),
		new Problem(
			'Silver',
...
For example, the goal of the focus problem above is to add edges such that the entire graph is a single connected component.
...
In the above focus problem, we are tasked to assign each node (friend) of the graph to one of two colors (teams), subject to the constraint that edges (friendships) connect two nodes of opposite colors. In other words, we need to check whether the input is a bipartite graph and, if it is, output a valid coloring.
