...
public class CodeforcesBadge {
    ...
    static void floyd(int curr){
        ...
        a=curr;
        // finds the entry cycle node, as well as, fills in the ans array of nodes in the cycle
        while(a!=b) {
            ans[b]=b;
            a=arr[a];
            b=arr[b];
        ...
        int cycleFirst=a;
        ans[cycleFirst]=cycleFirst;
        a=curr;
        // fills the ans array of nodes, outside the cycle
        while(a!=cycleFirst){
            ans[a]=cycleFirst;
            a=arr[a];
        }
