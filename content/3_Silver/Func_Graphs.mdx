...
/*
 * inside the floyd(a) function, a vertex has already been calculated(whose
 * ans[b]!=-1), then, there are 2 cases: either, that node:b is the first node
 * to a cycle or, non cycle node, which when followed, leads to a cycle for both
 * the cases, ans for the nodes from a to b, is ans[b] itself this operation,
 * happens in function fillup(a,b)- a being the user's input and b being the
 * vertex, whose answer has already been generated
 */
