...
int main()
{
	...
	// we need vector of vectors since at nodes combining the cycle with other
	// part of the connected component,
	// there would be more than one outgoing arrows in the reversed adjacency list
	radj.assign(N, {});
	for (int i = 0; i < N; i++)
		radj[adj[i]].push_back(i);

	ans.assign(N, -1);
	// we run the Floyd's algorithm for each connected component
	for (int i = 0; i < N; i++)
		if (ans[i] == -1)
			floyd(i);
...
public class Badge {
	...
	/*
	 * we need list of lists for the reversed adjacency list since at nodes
	 * combining the cycle with other part of the connected component, there would
	 * be more than one outgoing arrows in the reversed adjacency list
	 */
