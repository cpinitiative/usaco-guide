...
> We may ask the following questions: if we begin our walk at the starting node, what
is the first node in the cycle and how many nodes does the cycle contain?

However, CPH did not mention how to extend Floyd's Algorithm to answer the first part of the question above; try to do so here!
...
First we run Floyd's Algorithm, which finds some node on the cycle after $2c\left\lceil \frac{t}{c}\right\rceil$ queries. Then we can find the first node in the cycle after another $t$ queries!
