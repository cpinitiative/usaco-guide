...
### Solution 3
Without using any adjacency lists. `floyd(x)` generates answers for all connected vertices. `fillup(a,b)` fills up the vertices, from `a` to `b`, `b` being the coordinate, whose `ans[b]` has already been calculated `floyd(x)`
<LanguageSection>
<JavaSection>
```java
...
/*
 * inside the floyd(a) function, a vertex has already been calculated(whose ans[b]!=-1), then, there are 2 cases:
 * either, that node:b is the first node to a cycle
 * or, non cycle node, which when followed, leads to a cycle
 * for both the cases, ans for the nodes from a to b, is ans[b] itself
 * this operation, happens in function fillup(a,b)- a being the user's input and b being the vertex, whose answer has already been generated
 */
...
public class CodeforcesBadge {
    ...
    static void fillUp(int curr, int firstCycle){
		...
		// fillup from curr to the vertex, whose answer has already been calculated
    ...
    static int floyd(int curr){
        ...
        while(a!=b){
			/* 
			 * while finding cycle, if a node is found out to be already calculated, go to the fillUp function, where, the answer from vertex curr to this vertex a is ans[a]
			 */
            if(ans[a]!=-1){
                fillUp(curr, a);
                return 0;
        ...
        int cycleFirst=a;
        a=curr;
		// from a to the first node joining cycle and non cycle, we fill ans with that vertex
        while(a!=cycleFirst){
            ans[a]=cycleFirst;
            a=arr[a];
        }
