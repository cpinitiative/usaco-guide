...
public class XorPyramid {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // Determine if the i-th term in Pascal's Triangle
            // is odd by determining the number of 2s in the factorization of n choose k.
            if (pref[n - 1] - pref[i] - pref[n - i - 1] == 0) {
                // If there are no 2s, then the i-th term is odd.
                // This means num will show up in our answer, so we xor it to our answer.
                ans ^= num;
            }
        }
