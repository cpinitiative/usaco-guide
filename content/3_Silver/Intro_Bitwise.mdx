...
Perhaps one of the most common binary operation in practice is[bitwise xor](https://en.wikipedia.org/wiki/Bitwise_operation#XOR).
The special property that differenciates it from the other bitwise operations is that xor is its own inverse, i.e. $x \oplus x = 0$.

<FocusProblem problem = "sample2" />

### Explanation

Simulating the process, similarly to the [Pascal's triangle](https://en.wikipedia.org/wiki/Pascal%27s_triangle), isn't efficient, 
because of $n$ being too large. One good practice when dealing such problems is analyzing how the answer is affected by each single value. Keeping in mind that xor is its 
own inverse operation, a value $x$ xored with itself an **even** number of times will result in $0$, thus cancelling each other out; on the other hand, the value $x$ xored 
...
As you can see, the values $B$ does not appear in the final result, thus it can be ignore. Additionally, the bottom row values in
...
the parity of the frequency tells us that value $A$ contributes to the result, while. value $B$ is cancelled out.


### Implementation

Because we are only interested in the parity of the binomial coefficient, there is no need of computing it. We can simply check the parity by
counting the power of two in $\binom{n}{k}=\frac{n!}{k! \cdot (n-k)!}$. With this in mid, precompute $pref_i=p$ where $p$ is the power of two in $i!$ factorization.
