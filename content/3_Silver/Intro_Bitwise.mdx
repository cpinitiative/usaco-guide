...
own inverse, a value $x$ xored with itself an **even** number of times will result in $0$, thus cancelling each other out; on the other hand, the value $x$ xored 
...
As you can see, the value $B$ does not appear in the final result, thus it can be ignore. Additionally, the bottom row values in
...
the parity of the frequency tells us that value $A$ contributes to the result, whil value $B$ is cancels out itself.


### Implementation

Because we are only interested in the parity of the binomial coefficient, there is no need of computing its actual value. We can simply check the parity by
