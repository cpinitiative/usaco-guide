...
set in the final answer, that is, why are they set to zero or one. For this reason,
we'll consider each bit in independently. In this case, we'll check for every bit position, i.e.
for every power of two, whether or not it's set in every contiguous subsequence xor-sum.
...
int main() {
	...
	// For every bit position, check if it's set on in the xor-sum of every
	// subsequence
	for (int i = 0; i < 30; i++) {
		int s = 0;
