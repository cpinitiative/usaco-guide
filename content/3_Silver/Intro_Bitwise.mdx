...
One good practice for the xor operation is analyzing how the bits are
set in the final answer. In this case, we'll check for every bit position, i.e.
for every power of two, whether or not it's set on in every subsequence xor-sum.
This way, we transformed the problem into counting the number of subsequences with
xor-sum non-zero in an array formed by the bits in same position in all value.
...
int main() { 
	...
	// Check for every bit position if it's set on in the xor-sum of every subsequence
	for(int i = 0; i < 30; i++) {
		int s = 0;
		...
		for(int a : v) {
			s ^= (a >> i) & 1; 
			/*
			 * Count the # of sequences ending at this position with xor-sum non-zero
			 * by counting the prefixes of the inversed bit, i.e. pref[i] ^ pref[j] = 1.
			 * Update the answer by adding the # of such sequnces multiplied by the respective
			 * power of two.
 			 */
			ans += pref[s ^ 1] * 1LL << i;
			// Update the prexies
