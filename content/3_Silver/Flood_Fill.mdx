...
Recursive implementations of floodfill can sometimes lead to a stack overflow.
To avoid that, we can use a stack data structure in an iterative implementation
as follows:
...
void floodfill(int r, int c) {
	// Note: you can also use an deque for a BFS-based approach
...
public class RoomCount {
	...
	public static void floodfill(int r, int c) {
		...
		while(!frontier.isEmpty()){
			...
			visited[r][c] = true;
			for (int i = 0; i < 4; i++) {
				frontier.add(new int[] {r + R_CHANGE[i], c + C_CHANGE[i]});
			}
		}
	}
