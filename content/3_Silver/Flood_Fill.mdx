...
title: Flood Fill
author: Darren Yao
contributors: Kevin Sheng, Albert Ye
prerequisites:
  - dfs
...
<Quiz>
	<Quiz.Question>
		What is the minimum number of starting nodes for flood fills we would need to visit every node in an undirected graph with four components?
		<Quiz.Answer>
			1
			<Quiz.Explanation>
				Incorrect. Each DFS can travel one connected component, so a graph with four connected components would require $\mathbf{4}$ DFS runs.
			</Quiz.Explanation>
		</Quiz.Answer>
		<Quiz.Answer>
			2
	...
	<Quiz.Question>
		Which of the following is not information we could get from the results of an unmodified floodfill algorithm?
		<Quiz.Answer>
