...
[DFS](https://usaco.guide/silver/dfs). The algorithm to
...
- _Stack overflow_ if you didn't include the appropriate compiler options for
  [adjusting the stack size](/general/cpp-command#adjusting-stack-size)
...
A non-recursive implementation of flood fill adds adjacent nodes to a stack or queue, similar to [BFS](https://usaco.guide/gold/bfs), and is usually implemented as follows:
...
<Quiz>
	<Quiz.Question>
		How many times will we have to run DFS to perform a flood fill on a graph G with 4 connected components?
		<Quiz.Answer>
			1
			<Quiz.Explanation>
				Incorrect. Each DFS can travel one connected component, so a graph with four connected components would require **4** DFS runs.
			</Quiz.Explanation>
		</Quiz.Answer>
		<Quiz.Answer>
			2
		...
		<Quiz.Answer correct>
			4
			<Quiz.Explanation>
				Correct! Each DFS can travel one connected component, so a graph with four connected components would require **4** DFS runs.
			</Quiz.Explanation>
		</Quiz.Answer>
	</Quiz.Question>
	
	<Quiz.Question>
		Observe the following grid. A `.` is a traversable point, and a `#` is an impassable wall.
```..A.#.
...##.
..#.#.
###...```
		How many nodes can be reached from $A$, excluding $A$?
		<Quiz.Answer correct>
			8
			<Quiz.Explanation>
				Correct! From $A$, you can only reach the eight other points that are not blocked by a wall:
				```BBAB#.
BBB##.
BB#.#.
###...```
		...
		<Quiz.Answer>
			9
			<Quiz.Explanation>
				Incorrect. We exclude $A$ from our calculations.
			</Quiz.Explanation>
		</Quiz.Answer>
		<Quiz.Answer>
			15
			<Quiz.Explanation>
				Incorrect. You cannot reach any of the nodes on the other side of the wall, so the other seven unoccupied nodes cannot be visited.
			</Quiz.Explanation>
		</Quiz.Answer>
		<Quiz.Answer>
			24
			<Quiz.Explanation>
				Incorrect. Walls cannot be traversed.
			</Quiz.Explanation>
		</Quiz.Answer>
	</Quiz.Question>

	<Quiz.Question>
		Which of the following is not information we could get from the results of a floodfill algorithm?
		<Quiz.Answer>
			The number of connected components of a graph
			<Quiz.Explanation>
				Incorrect. We find the number of connected components by tracking the number of times DFS is executed.
			</Quiz.Explanation>
		</Quiz.Answer> 
		<Quiz.Answer>
			The number of nodes in each connected component
			<Quiz.Explanation>
				Incorrect. We can find the number of nodes in each connected component by iterating through all nodes after running floodfill and determining which connected component each node is in.
			</Quiz.Explanation>
		</Quiz.Answer>
		<Quiz.Answer correct>
			The shortest path between two nodes
			<Quiz.Explanation>
				Correct! This cannot be found with floodfill, because floodfill doesn't keep track of distances.
			</Quiz.Explanation>
		</Quiz.Answer>
		<Quiz.Answer>
			The largest connected component of a graph
			<Quiz.Explanation>
				Incorrect. We can find the largest connected component of a graph by taking the number of nodes in each connected component and finding the one with the most nodes.
		</Quiz.Answer>
	</Quiz.Question>
