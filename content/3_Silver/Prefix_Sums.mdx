...
<Quiz>
  ...
  <Quiz.Question>
    **Challenge Problem**: With the idea of prefix sums in mind, at first glance it may seem logical to apply this method to return range products in constant time. Instead of adding, we multiply, and instead of subtracting, we divide. Assume that we calculate the answer modulo a large prime ($10^9 + 7$, for instance), and the array has normal constraints ($N \leq 10^5$, $1 \leq a_i \leq 10^9$). What additional challenges may we face?
    <Quiz.Answer>
...
$$
In short, modular division is not as straight forward as modular addition, subtraction, or multiplication. However because the problem states that the modulo is prime, this is still possible using the modulo inverse (although this knowledge isn't necessary for silver). See the [Modulo Arithmatic](https://usaco.guide/gold/modular?lang=cpp) module for more details.
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
</Quiz>
