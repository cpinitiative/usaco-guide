...
An naïve approach to this problem is to compute the intervals that cover each point by looping through each index from $[a,b]$ individually and incrementing by 1, resulting in an $\mathcal{O}(NQ)$ time complexity (where N is the maximum size of our interval and Q is the number of queries), which easily times out (think what happens when the interval [0, N] is queried many times).

We can do better. It's easy to see that an increment of $x$ (before computation) in $\texttt{arr}[i]$ causes all subsequent $\texttt{prefix}[i...N]$ (after computation) to increase by $x$. This concept can be conceptualized through increment and decrement points. An increment point increases (and decrement decreases) all subsequent cells. (Note that our decrement point is located at $B+1$ because the interval is inclusive.)
...
<center>
	<table className='text-center'>
		<tr>
			<td>0</td>
			<td>0</td>
			<td className='bg-green-200'>2</td>
			<td>0</td>
			<td>0</td>
			<td>0</td>
		</tr>
...
<center>
	<table className='text-center'>
		<tr>
			...
			<td>2</td>
			<td>2</td>
			<td className='bg-red-200'>0</td>
		</tr>
	</table>
</center>

In this above problem, we are only incrementing by 1, so our $x$ is consistent. As a result, when we encounter a starting point, we can increment by 1, and for an endpoint, decrement by 1. For our problem, we don't need to include a prefix array because we aren't querying a range, so a single variable suffices.
...
int main() {
    ...
    F0R(i, 2*N) { // coordinate compression
        ...
        pfx[l+1] += customers[i].second; // merge all interval end and starts here
    }

    FOR(i, 1, 2*N+1) {
        pfx[i] += pfx[i-1]; // we have combined arr and pfx otherwise do pfx[i] += arr[i] - pfx[i-1]
    }
...
We can apply a similar concept to 2d matrices. Since we define $\texttt{prefix}[i][j]$ as the number of points between $(0,0)$ and $(i, j)$, each point $(i, j)$ is equal to the sum of itself and all the points between the rectangles formed by $(0, 0), (i, j-1)$ and $(0, 0),(i-1, j)$. However, since we are double counting the square $(i-1, j-1)$, we subtract it from our equation. This can be expressed by

$$
\texttt{prefix}[i][j] = \texttt{prefix}[i][j] = \texttt{prefix}[i][j] + \texttt{prefix}[i-1][j] + \texttt{prefix}[i][j-1] - \texttt{prefix}[i-1][j-1]
$$

Given the sample case in [Forest Queries](https://cses.fi/problemset/task/1652), the algorithm goes as follows:

We first increment every point by 1.
...
Then, we apply the preprocessing step.

$$
\sum_{i=1}^{N} \sum_{j=1}^{N} \texttt{prefix}[i][j] = \texttt{prefix}[i][j] + \texttt{prefix}[i-1][j] + \texttt{prefix}[i][j-1] - \texttt{prefix}[i-1][j-1]
$$
...
Notice how all queries can now be processed using 

$$
\begin{aligned}
\texttt{arr}[i][j]=&\,\texttt{prefix}[A][B]
        - \texttt{prefix}[a-1][B] \\
        &- \texttt{prefix}[A][b-1] + \texttt{prefix}[a-1][b-1]
\end{aligned}
$$
