...
<Quiz>
  ...
  <Quiz.Question>
    We are given the prefix sum array $P = [2, 7, 14, 15, 19]$. Compute the sum of values _of the original array_ using the prefix sum array for ranges $[1, 4]$ and $[2, 5]$ (inclusive, 1-indexed).
    <Quiz.Answer>
      13; 12
      <Quiz.Explanation>
  ...
  <Quiz.Question>
    **Challenge Problem**: With the idea of prefix sums in mind, at first glance it may seem logical to apply this method to products. Instead of adding, we multiply, and instead of subtracting, we divide. Assume that we calculate the answer modulo a large prime ($10^9 + 7$, for instance). What additional challenges may we face?
    <Quiz.Answer>
    ...
    <Quiz.Answer>
      **Time Limit**: Multiplying, dividing, and modulo are relatively slow operations, and this will make the prefix product just as fast as iterating through the array.
      <Quiz.Explanation>
        Incorrect. While it is true that multiplying and dividing are relatively slow, this difference is hardly enough to impact the runtime by a significant amount. 
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
    ...
    <Quiz.Answer correct>
      **Division under Modulo**: We cannot directly use division as a substitute for subtraction, because division is not distributive under modulo. 
      <Quiz.Explanation>
        Correct! This is to say, for some $a, b$ such that $a$ is divisible by $b$, 
$$
\frac a b \mod m \neq \frac {a \mod m} b \mod m
$$
. Because the problem statement specifies that $m$ is prime, we can circumvent this problem by multiplying by $b^{-1} \mod m$ instead of dividing by $b$. For more information about this, see the [Modular Arithmatic Module](https://usaco.guide/gold/modular?lang=cpp). Note that knowledge of number theory  is generally not needed to score well on silver. 
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
</Quiz>
