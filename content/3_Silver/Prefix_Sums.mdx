...
$$
\texttt{prefix}[i][j] = \texttt{arr}[i][j] = \texttt{prefix}[i][j] + \texttt{prefix}[i-1][j] + \texttt{prefix}[i][j-1] - \texttt{prefix}[i-1][j-1]
$$

Given the sample case in [Forest Queries](https://cses.fi/problemset/task/1652), the algorithm goes as follows:

We first build array $\texttt{arr}$ which increments every point by 1.
