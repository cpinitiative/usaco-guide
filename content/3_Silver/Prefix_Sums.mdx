...
This results in a $\mathcal{O}(nV)$ time complexity (where $0 \leq a < b \leq V$), which easily times out (think what happens when the interval $[0, V]$ is queried $n$ times).
