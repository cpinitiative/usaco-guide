...
A naïve approach to this problem is to create an array $\texttt{ctr}$, where $\texttt{ctr}[i]$ is the number of intervals that cover each point $i$. We can do this by looping through each interval $[a,b]$ and increasing $\texttt{ctr}[i]$ by $1$ for each index in $a \leq i \leq b$.
