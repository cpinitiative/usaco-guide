...
author: Darren Yao, Dustin Miao, KJ Karaisz
description:
  'Computing range sum queries and range additions in constant time over a fixed 1D array.'
prerequisites:
frequency: 3
---
...
<FocusProblem problem="haybale-stacking" />

<Spoiler title = "Hint">
What if you did prefix sums, but in reverse?
</Spoiler>

<Spoiler title = "Solution">
Consider the array $[0, 1, 0, 0, 0]$. Running prefix sums on this gives us $[0, 1, 1, 1, 1]$. Using prefix sums, we can implicitly add to a suffix by changing one number. 
Then, to support any subarray, we can subtract from the suffix after the range we wanted to update using the same strategy.
</Spoiler>
