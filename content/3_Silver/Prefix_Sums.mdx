...
<Quiz>
  ...
  <Quiz.Question>
    **Challenge Problem**: With the idea of prefix sums in mind, at first glance it may seem logical to apply this method to return range products in constant time. Instead of adding, we multiply, and instead of subtracting, we divide. Assume that we calculate the answer modulo a given $M$ and the array has normal constraints ($N \leq 10^5$, $1 \leq a_i \leq 10^9$). What additional challenges may we face?
    <Quiz.Answer>
    ...
    <Quiz.Answer correct>
      **Division under Modulo**: We cannot directly use division as a substitute for subtraction, because division is not distributive under modulo. 
      <Quiz.Explanation>
        Correct! Division under modulo is not the same as addition, subtraction, or multiplication. To divide involves multiplying by the modulo inverse, which may or may not exist depending on $M$ and the value to be divided (the two should be relatively prime for there to be one). One way we could conceivably do this is if $M$ is prime itself. See [here](https://usaco.guide/gold/modular) for more details
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
</Quiz>
