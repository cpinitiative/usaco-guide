...
Prefix sums are a technique used to precompute the cumulative sums of an array to quickly find the sum of any subarray.
Using $\mathcal{O}(N)$ precomputation time, subsequent queries can be found in $\mathcal{O}(1)$ time.
This technique can also be generalized to invertible operations,
such as [bitwise xor](https://usaco.guide/silver/intro-bitwise#xor-operation) or multiplication.
