...
In our above problem, our $x$ is fixed at $1$. As a result, when we encounter a starting point, we can increment by 1, and for an endpoint, decrement by 1. We actually cannot compute the prefix directly as $V \leq 10^9$, as we will run out of memory when creating an array of size $V$.
