...
<Quiz>
  ...
  <Quiz.Question>
    **Challenge Problem**: With the idea of prefix sums in mind, at first glance it may seem logical to apply this method to return range products in constant time. Instead of adding, we multiply, and instead of subtracting, we divide. Assume that we calculate the answer modulo a large prime ($10^9 + 7$, for instance). What additional challenges may we face?
    <Quiz.Answer>
...
$$
Because the problem statement specifies that $m$ is prime, we can circumvent this problem by multiplying by $b^{-1} \mod m$ instead of dividing by $b$. For more information about this, see the [Modular Arithmetic Module](https://usaco.guide/gold/modular?lang=cpp). Note that knowledge of number theory  is generally not needed to score well on silver. 

Another possible problem that could occur is if any of the values are $0$.
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
</Quiz>
