...
int main() {
	...
	for (auto& [width, edge] : edges) {
		cin >> edge.first >> edge.second >> width;
	}
...
If you don't like all the accesses to `.first` and `.second`,
an alternative is to use an `std::vector` or maybe even an
...
int main() {
	...
	// You can replace vector<int> with array<int, 3> if you like
...
but that would make indexing a little nastier.
...
- If `compare(x, y)` is `true`, then `compare(y, x)` must be false.
- If `compare(x, y)` is `true` and `compare(y, z)` is `true`, then `compare(x, z)` must be true.
...
struct Edge {
	...
	friend bool operator<(const Edge &x, const Edge &y) { return x.width < y.width; }
};
```
...
- If `compare(x, y) > 0`, then `compare(y, x) < 0`.
- If `compare(x, y) > 0` and `compare(y, z) > 0`, then `compare(x, z) > 0`.
