...
<Quiz>
  <Quiz.Question>
    When are custom comparators used?
    <Quiz.Answer>
      When sorting integers in default order.
      <Quiz.Explanation>
        Incorrect. Sorting integers in default order does not require custom comparators as the default comparator in the sort function suffices.
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      Comparing the relationships of elements in a single-element array.
      <Quiz.Explanation>
        Incorrect. Comparators are only needed for data with two or more elements to compare.
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer correct>
      Sorting custom objects or sorting values in non-default order.
      <Quiz.Explanation>
        Correct. You will need to use a custom comparator as there is no default option that is able to sort the custom data, or no option to sort in the specific way that you want.
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
  <Quiz.Question>
    Which input constraints require coordinate compression for the following problem: Given $N$ unique integers, in where each integer is $N_i$ ($1 \leq N_i \leq M$), the solution requires you to create an array where the values of the $N$ integers given are represented by the array's indices.
    <Quiz.Answer correct>
      $1 \leq N \leq 10^6$ and $1 \leq M \leq 10^9$
      <Quiz.Explanation>
        You would need to create an array of length M, and since M can be up to 10^9, that is too large. Instead, you can compress the N unique values into N unique values in the range of M. For example, [1, 2, 10^9] becomes [1, 2, 3], and therefore your array will only have a size of N.
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      $1 \leq N \leq 10^9$ and $1 \leq M \leq 10^6$
      <Quiz.Explanation>
        You would not need coordinate compression since the largest M can be is 10^6 and making an array of size M will typically not exceed the memory limit.
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      $1 \leq N \leq 10^6$ and $1 \leq M \leq 10^6$
      <Quiz.Explanation>
        You would not need coordinate compression since the largest M can be is 10^6 and making an array of size M will typically not exceed the memory limit.
      </Quiz.Explanation>
    </Quiz.Answer>
