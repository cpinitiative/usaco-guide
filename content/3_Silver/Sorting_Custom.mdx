...
The code creates a sorted list of the given values with duplicates removed. The list can be [binary serached](/silver/binary-search) to get the index or "compressed value" of any element. At the same time, this method of creating a sorted list of the given values allows one to work backwards to get the original values from their compressed values. If the original values are important, they can be used with this implementation.
