...
Coordinate compression is another useful technique related to sorting that can
also be used in conjuction with custom comparators. Coordinate compression
describes the process of replacing each value in a list with its index if that
...
there are only $N = 10^6$ such integers, we can coordinate-compress their
...
discuss how coordinate compression is applied. Since the solution uses 2D-prefix
sums (another Silver topic), it is helpful if all point coordinates are
coordinate-compressed to the range $0$ to $N-1$ so they can be used as array
indices.

Below you will find an excerpt of solution code to Rectangular Pasture that uses
coordinate compression. Observe how a custom comparator is used to sort the
points (either by `x` or by `y`):
