...
export const problems = {
	sample: [
		new Problem('Silver', 'Wormhole Sort', '992', 'Insane', false, [], '@Later'),
	],
	general: [
		new Problem(
			'CSES',
			...
			false,
			[],
			'Just sort endpoints of intervals and process in order.'
		),
		new Problem(
			'Silver',
			'Lifeguards',
		...
		new Problem('Silver', 'Mountain View', '896', 'Easy', false, [], ''),
...
<Resources>
	...
	>
		short overflow of what this module will cover
	</Resource>
</Resources>
...
We won't discuss the full solution here, as some of the concepts necessary for solving this problem will be introduced later in Silver. However, any solution to this problem starts by sorting the edges in nondecreasing order of weight. For example, the sample contains the following edges:
...
We can also use [lambda expressions](/general/cpp-tips#lambda-expressions) in C++11 or above:
