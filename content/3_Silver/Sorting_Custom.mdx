...
for s in range(n):
    for e in range(s + 1, n):
		...
		btm = min(points[s][1], points[e][1])

        above = psa[end][n] - psa[srt-1][n] - psa[end][top] + psa[srt-1][top]
        below = psa[end][btm-1] - psa[srt-1][btm-1]
        ans += (above + 1) * (below + 1)
...
**Note:** The above Python solution does not sucessfully pass all test cases on
USACO even with the correct time complexity of $\mathcal{O}(N^2)$ since Python
is too slow.
