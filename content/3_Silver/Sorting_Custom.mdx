...
prerequisites:
  ...
  - intro-graphs
description:
  "If we use custom objects or if we want to sort elements in an order other
  than the default, then we'll need to define a custom comparator. We may also
  want to coordinate compress when values are in a large range but we only need
  to work with their relative order."
---
...
Coordinate compression describes the process of replacing each value in a list with its index if that
...
When we have values that can take on a large range of possible values, but we
...
with implementation. If we have a set of integers ranging from $0$ to $10^9$, we
...
they can be used as array indices.

Below you will find the solution code to Rectangular Pasture, which uses
...
The solution uses a lambda function as the custom comparator, which our guide
...
The problem statement of Rectangular Pasture ensures that all given `x` and `y` coordinates are distinct. However, coordinate compression can also be applied to problems where values are repeated and where we want to remove duplicate values. The following code shows ways of setting up coordinate compression with the removal of duplicate values:
...
```cpp
BeginCodeSnip{}
cmp = v; //"cmp" starts as a copy of a vector v, but will end up with the elements of v in sorted order with duplicates removed
sort(begin(cmp), end(cmp)); //v is a vector of a type for which no custom comparator is necessary, though you can add a custom comparator to this sort() call as necessary
cmp.erase(unique(begin(cmp), end(cmp)), end(cmp)); 
auto cid = [&](int x) { return lower_bound(begin(cmp), end(cmp), x) - begin(cmp); }; // cid gives the index of x in cmp by "binary searching"
EndCodeSnip
```
...
```java
BeginCodeSnip{}
TreeSet<Integer> set = new TreeSet<Integer>(v); //v is a Collection of values - replace this with whatever you like
ArrayList<Integer> cmp = new ArrayList<Integer>(set); //cmp is the sorted set of values of v with duplicates removed
EndCodeSnip
```
</JavaSection>
...
The code creates a sorted list of the given values with duplicates removed. The list can be [binary serached](/silver/binary-search) to get the index or "compressed value" of any element. At the same time, this method of creating a sorted list of the original values allows one to work backwards to get the original values from their compressed values. If the original values are important, they can be used with this implementation.
