...
author: Darren Yao, Eric Wei, Neo Wang, Jesse Choe
...
This problem has two types of solutions one of which is called the  [Kadane's Algorithm](https://en.wikipedia.org/wiki/Maximum_subarray_problem#Kadane's_algorithm). However, understanding Kadane's algorithm is **optional** for this problem, since it can also be solved using prefix sums.

<Spoiler title="Solution">

### Solution

Suppose that we store the prefix sums at each index in the array. Intuitively, the maximum subarray sum ending at each index is the difference between the maximum prefix sum and the minimum prefix sum. Thus, we can iterate each element in the array and store the minimum prefix sum up to the current index. Then, we can find the maximum subarray sum by subtracting the current prefix sum to the minimum prefix sum.
...
int main(){
	int n; cin >> n;
	vl dp(n+1, 0); // Prefix sum array initially filled with 0's
	for(int i=1; i<=n; i++){
		ll x; cin >> x;
		dp[i] = dp[i-1] + x; // Compute the prefix sum at each element
	}
	ll max_subarray_sum = dp[0], min_prefix_sum = dp[0];
	for(int i=1; i<=n; i++){
		// The maximum subarray sum is the maximum difference the current prefix sum and the minimum prefix sum up to the current index
		max_subarray_sum = max(max_subarray_sum, dp[i] - min_prefix_sum);
		min_prefix_sum = min(min_prefix_sum, dp[i]);
	}
...
Note: It is recommended that you skip the following solution.

</Warning>

<Spoiler title = "Alternative Solution">

### Solution

Suppose we fix the first element of a subarray. The Kadane's Algorithm solution involves starting a new subarray sum at an element if that element is greater than the current subarray sum with an addition to the current element. If the subarray sum ending at the current element is greater than the current element, then we continue the subarray sum. Otherwise, we set the new subarray sum equal to the current element and restart our subarray sum. To find the maximum subarray sum over all possible sums, we store an integer $S$, representing the maximum subarray sum at each iteration of the array. When we finish iterating the array, we print $S$.

Watch this [video](https://www.youtube.com/watch?v=jnoVtCKECmQ&t=611s) for a more detailed explanation of the approach above.
...
int main(){
	...
	// Continue the subarray sum or start a new subarray sum beginning at the current element
