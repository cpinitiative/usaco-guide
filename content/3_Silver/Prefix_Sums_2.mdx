...
author: Darren Yao, Eric Wei, Neo Wang, Qi Wang, Jesse Choe, Kevin Sheng
description:
...
max_subarrary_sum = arr[0]
min_pref_sum = 0
running_pref_sum = 0
for i in arr:
    running_pref_sum += i
    max_subarrary_sum = max(max_subarrary_sum, running_pref_sum - min_pref_sum)
    min_pref_sum = min(min_pref_sum, running_pref_sum)
print(max_subarrary_sum)
```
...
for r in range(side_len):
    for ci, c in enumerate(input()):
        tree = c == '*'
        tree_prefixes[r + 1][ci + 1] += (tree_prefixes[r][ci + 1] + tree_prefixes[r + 1][ci]
                                         - tree_prefixes[r][ci] + tree)
