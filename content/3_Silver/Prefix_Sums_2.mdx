...
Consider the prefix sum array $p$. The subarray sum $a_j \dots a_{i-1}$ where $j\leq i$ is $p[i]-p[j]$, so we are looking for the maximum possible value of $p[i]-p[j]$ over $0 \leq i \leq j \leq N$. 

For a fixed right bound $i$, the maximum subarray sum is

$$$
p[i]-\min_{j\leq i}{p[j]}
$$$

Thus, we can keep a running minimum to store $\min\limits_{j\leq i}{p[j]}$ as we iterate through $i$. This yields the maximum subarray sum for each possible right bound, and the maximum over all these values is our answer.
...
[Video explanation of the max subarray sum using Kadane's Algorithm](https://www.youtube.com/watch?v=jnoVtCKECmQ&t=611s).
