...
Suppose that we store the prefix sums at each index in the array. Intuitively, the maximum possible $p[i]-p[j]$ where $j\leq i$. Thus, we can iterate each element in the array and store the minimum prefix sum up to the current index. Then, we can find the maximum subarray sum by subtracting the current prefix sum to the minimum prefix sum.
