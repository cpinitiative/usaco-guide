...
Consider the prefix sum array $p$. The subarray sum $a_i \dots a_{j-1}$ where $i\leq j$ is $p[j]-p[i]$, so we are looking for the maximum possible value of $p[j]-p[i]$ over $0 \leq i \leq j \leq N$. 
...
Thus, we can keep a running minimum to store $\min\limits_{i\leq j}{p[i]}$ as we iterate through $i$. This yields the maximum subarray sum for each possible right bound, and the maximum over all these values is our answer.
...
Kadane's Algorithm finds the largest subarray sum using a greedy method. Check this
[video tutorial](https://www.youtube.com/watch?v=jnoVtCKECmQ&t=611s) for more details.
