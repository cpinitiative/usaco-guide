...
public class Main {
	...
	public static List<Integer> rep = new ArrayList<Integer>();
...
int main() {
	...
	cout << rep_cities.size() - 1 << '\n';
    for (int i = 0; i < rep_cities.size() - 1; i++) {
        cout << rep_cities[i] + 1 << ' ' << rep_cities[i + 1] + 1 << '\n';
    }
}
```
...
<Quiz>
	...
	<Quiz.Question>
		What's the time complexity of these graph traversal algorithms?
