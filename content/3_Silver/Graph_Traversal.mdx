...
### Application: State-Space Graphs

<FocusProblem problem="statesample"/>

A **state-space graph** refers to conceptualizing some kind of state in each node. For instance, consider the [Towers of Hanoi](https://en.wikipedia.org/wiki/Tower_of_Hanoi). We can assign the starting state a node, and for each move from that state, assign the resulting state a node and the move connecting them an edge. We can continue to do this for each possible state in the game until a goal state is met. Note that the edges in this graph are *directed*, because being able to go from a state $A$ to a state $B$ doesn't necessarily mean we can go from $B$ to $A$.

In the above focus problem, we want to find all winning states reachable from the starting state.
...
## Solution - Maze Tac Toe
We can conceptualize a state representing the game using the Bessie's position in the maze and the maze-tac-toe state. with this, we can do DFS to find all of the states in the same component as the starting state, and of those, the winning states. This is fast because the number of states is bounded by $25^2 \cdot 3^9$, and DFS is $O(n + m)$.
