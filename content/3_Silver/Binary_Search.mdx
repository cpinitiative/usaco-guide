...
public class BinarySearch {
    ...
    public static int lastTrue(int lo, int hi, Function<Integer, Boolean> f) {
        for (--lo; lo < hi; ) {
            int mid = lo+(hi-lo+1)/2;
            // find the middle of the current range (rounding up)
            if (f.apply(mid)) lo = mid;
                // if mid works, then all numbers smaller than mid also work
            else hi = mid-1;
            // if mid does not work, greater values would not work too
            // so we don't care about them
...
See [Java Function Interface](https://docs.oracle.com/javase/8/docs/api/java/util/function/Function.html) if you're not familiar with the Function interface used.
