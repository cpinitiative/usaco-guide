...
<Quiz>
  ...
  <Quiz.Question>
    Suppose we were tasked to find the maximum value `m` in the range $[l,r], 0 \leq l < r \leq 10^9$ such that a given monotonic boolean function `f(m)` returns `true`. The following code has an error in it that causes an infinite loop. Find the corrected version. Assume that an `f(l)` always evaluates to true (ie there exists an `m` satisfying the given conditions). 
