...
<Quiz>
  ...
  <Quiz.Question>
    Suppose a program involving binary search runs in $\mathcal{O}(\log n)$ time for some $n$. At what values of $n$ would runtime be an issue? A typical online judge can handle around $10^9$ operations in a second. 
    <Quiz.Answer>
      $9$
      <Quiz.Explanation>
        Incorrect. 
    ...
    <Quiz.Answer correct>
      $10^{10^{9}}$
      <Quiz.Explanation>
        Correct. The logarithm of this value is a bit more than $10^9$. A pure, logarithmic binary search will almost never run into limits regarding time limit. 
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer correct>
      $TREE(9)$ (learn more about the TREE function [here](https://googology.wikia.org/wiki/TREE_sequence))
      <Quiz.Explanation>
    	Correct. This number is way too large, to the extent where simply proving that it is finite is a challenge. 
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
  <Quiz.Question>
    Suppose we were tasked to find some maximum value such that a given monotonic boolean function `someBooleanFunction(m)` returns `true`. The following code has an error in it that causes an infinite loop. Find the corrected version. 
    ```cpp
int findMaxTrue(int l, int r, function<bool(int)> someBooleanFunction) {
	while (l < r) {
		int m = (l + r) / 2;
		if (someBooleanFunction(m))
			l = m;
		else 
			r = m - 1;
	}
...
int findMaxTrue(int l, int r, function<bool(int)> someBooleanFunction) {
	...
	return l;
      ...
      ```
      <Quiz.Explanation>
        Incorrect. This will still cause an infinite loop. For instance, if for `someBooleanFunction` we have
        ```cpp
        bool someBooleanFunction(int m) { return m <= 7; }
        ``` 
        and call `findMaxTrue(1, 11, someBooleanFunction)`, the program will never terminate (it will be stuck at $l = 6, r = 7$).
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      ```cpp
...
int findMaxTrue(int l, int r, function<bool(int)> someBooleanFunction) {
	...
	return l;
      ...
      ```
      <Quiz.Explanation>
        Correct. The reason for an infinite loop is when $r$ is one more than $l$ and `someBooleanFunction(l)` evaluates to `true` . $m$ will be defined as $\lfloor \frac {l + r} 2 \rfloor = l$, and will thus evaluate to true, which means that $l$ and $r$ have not changed in the loop. By adding $1$ to the expression  $$\lfloor \frac {l + r + 1} 2 \rfloor$ will instead evaluate to $r$. 
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
</Quiz>
