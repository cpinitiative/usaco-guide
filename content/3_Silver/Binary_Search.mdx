...
def last_true(lo: int, hi: int, test: Callable[[int], bool]):
	"""
	Binary Search
...
# all numbers satisfy the condition
print(last_true(2, 10, lambda x: True))  # outputs 10

print(last_true(2, 10, lambda x: x * x <= 30))  # outputs 5

# no numbers satisfy the condition
print(last_true(2, 10, lambda x: False)) #  outputs 1
```
...
def lastTrue(lo: int, hi: int, test: Callable[[int], bool]):
	"""
	Binary Search
	:param lo: lower bound
	:param hi: upper bound
...
def firstTrue(lo: int, hi: int, test: Callable[[int], bool]):
	hi += 1
	while lo < hi:
		mid = (lo + hi) // 2
		if test(mid):
...
public class MaxMed {
	...
	public static boolean medReachable(int x) {
		...
		long opsNeeded = 0;
		for (int i = (n - 1) / 2; i < n; i++)
			opsNeeded += Math.max(0, x - a.get(i));
		return opsNeeded <= k;
	}
...
def firstTrue(lo: int, hi: NotImplemented, test: Callable[[int], bool]):
	hi += 1
	while lo < hi:
		mid = lo + (hi - lo) // 2
		if test(mid):
...
int main() {
	...
	for (int i = 0; i < Q; i++) {
		...
		int B;
		cin >> A >> B;
		cout << upper_bound(begin(v), end(v), B) 
					- lower_bound(begin(v), end(v), A) << "\n";
	}
}
```
...
public class Haybales {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < Q; ++i) {
			int A = io.nextInt();
			int B = io.nextInt();
			io.println(atMost(B) - atMost(A - 1));
		}
		io.close();
	}
