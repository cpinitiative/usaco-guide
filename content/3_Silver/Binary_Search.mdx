...
All values currently greater than or equal to the median, must remain greater than or equal to the median. i.e

If after sorting an array we have the array
$[1,1,2,3,4,4,5,5,6,8,8]$. The current median is $4$, to increase the median to $6$, we have to increase the current median by $2$ and we also have to increase the $5$'s to $6$.

Following this idea, to increase the median to a value $x$, we will need to increase the value of $array[i]$ (for $i$ starting from  $\frac{n + 1}{2}$ (the  position of the median) down to $n$), if it is less than the value $x$.
