...
See [Java Predicate Interface](https://docs.oracle.com/javase/8/docs/api/java/util/function/Predicate.html) if you're not familiar with the `Predicate` interface used.
...
This approach is based on interval jumping. Essentially, we start from the beginning of the array, make jumps, and reduce the jump length as we get closer to the target element. We use powers of 2, very similiar to [Binary Jumping](/plat/bin-jump).
