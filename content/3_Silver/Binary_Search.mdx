...
<Warning>
It is NOT recommended to use tenary expression in the python implementation of binary search because python tenary expression does not accept statements.
</Warning>
...
```java
public static int firstTrue(Predicate<Integer> f, int lo, int hi) {
	for (hi ++; lo < hi; ) {
		int mid = lo+(hi-lo)/2;
		if(f.test(mid)) hi = mid; else lo = mid+1;
	}
	return lo;
}
```
