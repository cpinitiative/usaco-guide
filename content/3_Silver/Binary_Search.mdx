...
<Quiz>
  ...
  <Quiz.Question>
    ...
    <Quiz.Answer correct>
      $10^{10^{9}}$
      <Quiz.Explanation>
        Correct. The logarithm of this value is a bit more than $10^9$. A pure, logarithmic binary search will almost never run into issues regarding time limit. 
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
  <Quiz.Question>
...
```
<Quiz.Explanation>
        Correct. The reason for an infinite loop is when $r$ is one more than $l$ and `someBooleanFunction(l)` evaluates to `true` . $m$ will be defined as $\lfloor \frac {l + r} 2 \rfloor = l$, and will thus evaluate to true, which means that $l$ and $r$ have not changed in the loop. By adding $1$ to the expression, $\lfloor \frac {l + r + 1} 2 \rfloor$ will instead evaluate to $r$ which will break the loop. 
</Quiz.Explanation>
</Quiz.Answer>
