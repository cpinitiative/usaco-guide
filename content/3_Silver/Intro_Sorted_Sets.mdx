...
you want an implementation from the standard library.
...
Additionally, an implementation of `SortedDict` from the `sortedcontainers`
library can be found below. All operations shown below are $\mathcal{O}(\log N)$
time, except for an $\mathcal{O}(N \log N)$ initialization and $\mathcal{O}(N)$
time to get all items, keys, or values.
