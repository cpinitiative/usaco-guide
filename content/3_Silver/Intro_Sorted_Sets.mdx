...
author: Darren Yao, Benjamin Qi, Andrew Wang, Aadit Ambadkar
prerequisites:
  - intro-sets
description:
...
## Sorted Sets (and the additional functions that come with them)
...
- To remove __all__ values, use `ms.erase(val)`

<Warning>
	Using `ms.erase(val)` erases __all__ instances of `val` from the multiset. To remove one value, use `ms.erase(ms.find(val))`!
</Warning>
