...
In Python, we delete and retrieve the element of **lowest** rather than highest priority.
...
Because of a heap's structure, printing out `pq` will **not** print out the elements in sorted order in Python. The comments below are a **representation** of what the heap contains, **not** what the contents of `pq` actually are.
...
pq = []
heapq.heapify(pq)  # This line is not necessary since pq starts as an empty list. However, for lists of length greater than 1, heapify is required to turn the list into a heap.
...
class BitInversion {
	public static TreeMap<Integer, Integer> ret =
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < s.length() - 1; i++) {
			if (s.charAt(i) != s.charAt(i + 1))
				dif.add(i + 1);  // initialize dif
		}
		for (int it : dif) {
			if (dif.higher(it) != null)
				add(dif.higher(it) - it);  // initialize ret
		}
