...
# Note:
# The numbers appear sorted when iterating due to the hashing implementation
# Sets are not ordered, so do not expect ordered/sorted behavior
```
...
sorted_set = SortedSet([3, 2, 4, 1, 5])
iterator = iter(sorted_set)
...
iteration. It can also be simply used in cases where you just want the first or any element.
