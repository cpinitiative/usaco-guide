...
<CPPSection>
```cpp
// number of elements in the list for which you want to find the sum over all subsets
int n = 20;
vector<int> a(1 << n);
vector<int> sos(1 << n);
for (int i = 0; i < (1 << n); i++) {
	// iterate over all other sets and checks whether it is a subset of i
	for (int j = 0; j < (1 << n); j++) {
		if ((i & j) == j) { sos[i] += a[j]; }
	}
}
...
Define $\texttt{SOS}(mask, x)$ to be the sum of subsets of $mask$ such that the first $x$ bits of the subset are identical to the first $x$ bits of mask.
...
$\texttt{SOS}(mask, x) = \left\{
	\begin{array}{ c l }
		\texttt{SOS}(mask, x - 1) + \texttt{SOS}(mask - 2^i, x - 1) & \quad \textrm{if } |2^i \wedge mask| > 0 \\
		\texttt{SOS}(mask, x - 1) & \quad \textrm{otherwise}
	\end{array}
\right.$
...
vector<int> a(1 << n);
// dp array that keeps track of the sum over subsets
