...
If the $x+1$th bit of $mask$ is a $1$ then we can transition to this state from both subsets where the $x$th bit is turned off and turned on because both of these cases would be subsets.

More formally,

$SOS(mask, x) = \left\{
	\begin{array}{ c l }
		SOS(mask, x - 1) + SOS(mask - 2^i, x - 1) & \quad \textrm{if } 2^i \wedge mask \\
