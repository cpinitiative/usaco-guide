...
- The $j$-th bit of mask is 0, meaning that cell $(i, j)$ is **covered**.
	- Case 1: we used a vertical tile to cover it.
		- Cell $(i - 1, j)$ must have been empty before this, so there are $\texttt{dp}[i][j - 1][mask \oplus 2^j]$ ways to do this.
	- Case 2: we used a horizontal tile to cover it.
		- This is only possible if $j > 0$.
		- Cells $(i, j - 1)$, $(i - 1, j - 1)$, and $(i - 1, j)$ must have been covered, so there are $\texttt{dp}[i][j - 2][mask]$ to do this.
- The $j$-th bit of the mask is 1, meaning that cell $(i, j)$ is **empty**.
	- Cell $(i - 1, j)$ must have been covered before this, so there are $\texttt{dp}[i][j - 1][mask \oplus 2^j]$ ways to do this.
