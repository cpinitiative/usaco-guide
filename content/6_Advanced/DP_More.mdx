...
- All cells from cell $(i, j + 1)$ to cell $(N - 1, M - 1)$ are **empty**.
- $mask$ represents whether each of the remaining $N$ cells are **empty**, with the $k$-th bit corresponding to the cell in column $k$.

For example, the following state would contribute toward $\texttt{dp}[3][1][10100_2]$:
![](broken-profile-state-example.png)

We now have two cases when calculating $\texttt{dp}[i][j][mask]$:
- The $j$-th bit of the mask is 0, meaning that cell $(i, j)$ is **covered**.
	- Case 1: we used a vertical tile to cover it.
		- Cell $(i - 1, j)$ must have been empty, so there are $\texttt{dp}[i][j - 1][mask \oplus 2^j]$ ways to do this.
	- Case 2: we used a horizontal tile to cover it.
		- This is only possible if $j > 0$.
		- Cells $(i, j - 1)$, $(i - 1, j - 1)$, and $(i - 1, j)$ must have been covered, so there are $\texttt{dp}[i][j - 1][mask \oplus 2^{j - 1}]$ ways to do this.
		- This corresponds to `if (j && !(mask & (1 << j)) && !(mask & (1 << j - 1)))` in the code below.
- The $j$-th bit of the mask is 1, meaning that cell $(i, j)$ is **empty**.
	- Cell $(i - 1, j)$ must have been covered, so there are $\texttt{dp}[i][j - 1][mask \oplus 2^j]$ ways to do this.
		- This is the same as case 1 of when the $j$-th bit of the mask is 0, so we handle them simultaneously in the code below.
...
the first two dimensions of the DP array, as $\texttt{dp}[i][j]$ depends only on $\texttt{dp}[i][j - 1]$.
