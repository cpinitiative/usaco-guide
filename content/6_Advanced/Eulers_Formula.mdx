...
Bessie is searching for scenic valleys on an N×N grid where each cell has a unique height. A valley is a contiguous, non-holey region where every cell within the region is lower than the cells on its border. The goal is to calculate the sum of the sizes of all valleys.

**Applying Euler's Formula**:

Euler's formula for planar graphs is given by V−E+F=2, where:
- V is the number of vertices,
- E is the number of edges,
- F is the number of faces (including the exterior).

In the context of the grid and valleys:
- Vertices (V): Represent cells in the grid.
- Edges (E): Represent the connections (up, down, left, right) between cells.
- Faces (F): Represent the valleys, holes, and the exterior region.

To identify valleys, Euler's formula can help distinguish between holey and non-holey regions:
- Identify Contiguous Regions: Use graph traversal algorithms (e.g., DFS or BFS) to find all edgewise-contiguous regions.
- Determine Holiness: For each region, check if its complement is pointwise-contiguous.
- Calculate Valley Sizes: For non-holey regions where all internal cells are lower than border cells, sum the sizes.
