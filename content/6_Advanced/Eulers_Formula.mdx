...
pair<int, int> indexToPair(int n, int idx) {
    return {idx / n, idx % n};
}

// Convert a 2D grid coordinate (row, column) to a 1D index
int pairToIndex(int n, int r, int c) {
    return r * n + c;
}
...
int main() {
    ...
    int n, m;
    cin >> n >> m;
    vector<vector<bool>> grid(n, vector<bool>(n, false)); // Initialize the grid with all water
    DisjointSets dsu(n * n); // Initialize DSU for the grid cells

    int islandCount = 0; // Current number of islands
    int islandsWithoutLakeCount = 0; // Number of islands without freshwater lakes
