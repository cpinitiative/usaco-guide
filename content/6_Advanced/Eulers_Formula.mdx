...
Bessie is searching for scenic valleys on an N×N grid where each cell has a unique height. A valley is a contiguous, non-holey region where every cell within the region is lower than the cells on its border. The goal is to calculate the sum of the sizes of all valleys. By converting the grid into a graph where cells are vertices and adjacencies are edges, Euler's formula helps ensure the correct identification and counting of valleys by verifying the structural integrity of regions and their borders. Each valley, being a non-holey region, can be analyzed as a face in this graph.
