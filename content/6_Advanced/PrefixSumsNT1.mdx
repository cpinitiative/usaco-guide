...
When $i \leq \sqrt{n}$, there are only $\mathcal{O}(\sqrt{n})$ distinct values for
$\left\lfloor\frac{n}{i}\right\rfloor$. Similarly, when $i > \sqrt{n}$,
$\left\lfloor\frac{n}{i}\right\rfloor < n$ has only $\mathcal{O}(\sqrt{n})$ distinct
...
We can calculate both each of these contribution in $\mathcal{O}(\sqrt n)$ time.
...
For $x\in Q_N$ we can compute $h_x$ in $\mathcal{O}(\sqrt x)$ time since if $ij\le x$, then
...
Additionally, $\sum_{x\in Q_N}\sqrt x\le \mathcal{O}(\sum_{i=1}^{\sqrt N}\sqrt {N/i})\le \mathcal{O}(N^{3/4})$.

Bonus: It is possible to reduce this to $\mathcal{O}(N^{2/3})$ time (see [this comment](https://codeforces.com/blog/entry/117635?#comment-1041002)).
...
Bonus: You can optimize this to $\mathcal{O}(N^{2/3})$ by applying the method introduced
in the second resource. More specifically, $g$ and its prefix sums can be precomputed
up to $N^{2/3}$ in $\mathcal{O}(N^{2/3})$ time using a linear sieve. Then computing the remaining
prefix sums takes $\mathcal{O}(\sum_{i=1}^{N^{1/3}}\sqrt{N/i})=\mathcal{O}(N^{2/3})$ additional time.
