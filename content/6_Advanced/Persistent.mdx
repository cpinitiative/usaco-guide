...
In C++, one can implement this to run in $\mathcal O(\log N)$ time per
query and $\mathcal O(1)$ time per update by using an array of `vector`s.
...
update by using a binary-tree-like structure where array elements are the leaves.
...
void update_item(int index, int value, int prev_time, int curr_time) {
	// Updates the array item at a given index and time
	roots[curr_time] = new Node(roots[prev_time]); // First, copy the root
	roots[curr_time] = update(roots[curr_time], index, value); // Then update it
}
...
trees, it's relatively straightforward to convert one into a persistent
segment tree - just add range queries!

However, note that it's not possible to support range updates with range queries
in persistent segment trees, since lazy propagation doesn't play well with path
copying.
