...
try view one of those dimensions as time. In this problem, we'll view the index
of each array as its time dimension.
...
- Type 3 queries involve copying the root of the segment tree at some time and
  appending it to the array of segment tree roots at each time.
