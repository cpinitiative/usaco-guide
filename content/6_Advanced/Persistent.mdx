...
int build(int tl, int tr, const vector<int> &arr) {
	...
	int mid = (tl + tr) / 2;
	int nxt_l = build(tl, mid, arr);
	int nxt_r = build(mid + 1, tr, arr);

	tree[timer] = join(nxt_l, nxt_r);
	return timer++;
}

int upd(int v, int pos, int val, int tl, int tr) {
	if (tl == tr) {
		tree[timer] = {val, 0, 0};
		return timer++;
	}
	...
	int mid = (tl + tr) / 2;
	auto [_, nxt_l, nxt_r] = tree[v];
	if (pos <= mid) {
		nxt_l = upd(nxt_l, pos, val, tl, mid);
	} else {
		nxt_r = upd(nxt_r, pos, val, mid + 1, tr);
	}
...
ll qry(int v, int ql, int qr, int tl, int tr) {
	if (qr < tl || tr < ql) { return 0ll; }
	if (ql <= tl && tr <= qr) { return tree[v].sum; }

	int mid = (tl + tr) / 2;
	return qry(tree[v].l, ql, qr, tl, mid) + qry(tree[v].r, ql, qr, mid + 1, tr);
}
...
int main() {
	...
	vector<int> roots = {build(0, n - 1, a)};
	...
	for (int t = 0; t < q; t++) {
		...
		if (type == 1) {
			...
			cin >> pos >> val;
			pos--;
			roots[k] = upd(roots[k], pos, val, 0, n - 1);
		} else if (type == 2) {
			int a, b;
			cin >> a >> b;
			a--, b--;
			cout << qry(roots[k], a, b, 0, n - 1) << "\n";
		} else if (type == 3) {
			tree[timer] = tree[roots[k]];
			roots.push_back(timer);
			timer++;
		}
	}
}
