...
The graph was generated for the probability of answering $4\cdot10^4$ queries correctly in a row. This assumption is reasonable because the test data consists of only 1 large test case. Note that by increasing $S$ to $200$, which still fits under the time constraint, the probability of answering $10^8$ queries correctly is still $0.98$, making our solution resilient against multiple testcases, if they existed.
