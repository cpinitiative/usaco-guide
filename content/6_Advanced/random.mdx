...
Our approach has a time complexity of $\mathcal{O}\left(S\cdot n\log a + S \cdot d(a)^2 + S\cdot \sqrt{a}\right)$, with the log factor occuring due to gcd computation, and the sqrt due to factoring.
...
The graph was generated for the probability of answering $10^4$ queries correctly in a row. This assumption is reasonable because the test data consists of only 1 large test case. Note that by increasing $S$ to $150$, which still fits under the time constraint, the probability of answering $10^6$ testcases correctly is still $0.98$, making our solution resilient against multiple testcases, if they existed.
...
us range_min(int l, int r, int j) {
