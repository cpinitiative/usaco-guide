...
unsigned long long n, a[MAX_N], dv[MAX_N], cnt[MAX_N], ans, x, ind, d;
...
int main() {
	...
	for (int s = 0; s < S; s++) {
		...
		fill(cnt,cnt+ind,0);
		for (int i = 0; i < n; i++)
			cnt[lower_bound(dv,dv+ind,gcd(a[i],x))-dv]++;
		for (int i = 0; i < ind; i++)
			for (int j = 0; j < i; j++)
...
def clt_probability(k, S):
	...
	Parameters:
	    k: The parameter defining the number of draws for each minimum:
	             - The first group uses k draws.
	             - The second group uses 2k draws.
	    S: The number of independent trials (sums over S minima).

	Returns:
		The approximated probability that the sum of the minima from 2k draws is less than that from k draws.
	"""
