...
- Take our answer to be the minimum of all $d$ such that $\geq \frac{n}{2}$ numbers in $a$ are divisible by $d$

If we take $S = 10$, and estimate the number of testcases in Codeforces to be $100$, our probability of success is $(\frac{1023}{1024})^{100} \approx .97$, which is acceptable.

### Implementation

Our approach has a time complexity of $\mathcal{O}\left(S\cdot n\log a + S \cdot d(a)^2 \right)$, with the log factor occuring due to gcd computation.

<Warning>

Due to the high volume of input required for this problem, it may be useful (though not required) to use faster methods of input such as `scanf` or a custom input class to pass this problem.
...
unsigned long long n, a[mxn], dv[mxn], cnt[mxn], ans, x, ind, d;
...
signed main() {
	...
	for (int s = 0; s < S; s++) {
		...
		fill(cnt, cnt + ind, 0);
		for (int i = 0; i < n; i++)
			cnt[lower_bound(dv, dv + ind, __gcd(a[i], x)) - dv]++;
		for (int i = 0; i < ind; i++)
			for (int j = 0; j < i; j++)
...
def clt_probability(k, S):
	...
	Parameters:
	    k (int): The parameter defining the number of draws for each minimum:
	             - The first group uses k draws.
	             - The second group uses 2k draws.
	    S (int): The number of independent trials (sums over S minima).

	Returns:
	    float: The approximated probability that the sum of the minima from 2k draws is less than that from k draws.
	"""
...
inline void point_update(int i, int v) {
	// 1 <= v <= n is the original color
	for (int j = 0; j < S; j++) {
		seg[L + i][j] = va[v][j];
		for (int cv = (L + i) / 2; cv; cv /= 2)
			seg[cv][j] = min(seg[2 * cv][j], seg[2 * cv + 1][j]);
	}
}
inline int range_min(int l, int r, int j) {
	// we can be sloppy since only 10k queries
	int res = inf;
...
namespace HLD {
const int mxn = 5e5 + 5;
int f[mxn], depth[mxn], heavy[mxn], head[mxn], pos[mxn], cp = 0;
vector<int> adj[mxn];
...
int dfs(int v) {
	...
	for (int c : adj[v])
		if (c != f[v]) {
			f[c] = v, depth[c] = depth[v] + 1;
			int csz = dfs(c);
			sz += csz;
			if (csz > msz) { msz = csz, heavy[v] = c; }
		}
	return sz;
}
void decompose(int v, int h) {
	head[v] = h, pos[v] = cp++;
	for (int i = 0; i < S; i++) SGT::seg[SGT::L + pos[v]][i] = va[c[v]][i];
	if (heavy[v] != -1) decompose(heavy[v], h);
	for (int c : adj[v]) {
		if (c == f[v] || c == heavy[v]) continue;
		decompose(c, c);
	}
}
inline void init() {
...
inline int path_min(int a, int b, int i) {
	int res = inf;
	for (; head[a] != head[b]; b = f[head[b]]) {
		if (depth[head[a]] > depth[head[b]]) swap(a, b);
		res = min(res, SGT::range_min(pos[head[b]], pos[b], i));
	}
	if (depth[a] > depth[b]) swap(a, b);
	res = min(res, SGT::range_min(pos[a], pos[b], i));
	return res;
}
}  // namespace HLD
...
void solve() {
	cin >> n >> m;
	for (int i = 1; i <= n; i++)
		for (int j = 0; j < S; j++) { va[i][j] = rand() % (inf + 1); }
	...
	while (m--) {
		...
		if (tp == 1) {
			...
			long long c1 = 0, c2 = 0;
			for (int i = 0; i < S; i++) {
				c1 += HLD::path_min(a, b, i);
				c2 += HLD::path_min(c, d, i);
			}
			if (c1 < c2) {
				cnt++;
				cout << "Yes\n";
