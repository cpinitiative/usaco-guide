...
2. $g(n, k) = g(n - \lfloor n/k \rfloor - 1, k)$; it can be seen as a [sliding window](/gold/sliding-window) on the nimber values. Any window of length $g(\frac{n}{k},k)$
with the right boundary in the range $[n-\frac{n}{k},n)$ will encompass the values: $1, 2, \ldots, \frac{n-k}{k}$.
