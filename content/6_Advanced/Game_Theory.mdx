...
<Info title="NIM game with increases">
In some cases there could be allowed to also **add** stones to a chosen pile, so when a turn comes, you can remove or add stones. It turns out that adding doesn't change the how winning and losing states are determined i.e. adding operation doesn't affect the outcome of the game.
</Info>
...
We will assume that from a state with xor-sum $0$ we can also reach another state with xor-sum $0$. Let's pick an arbitrary pile with $y$ stones and we would like to remove $x$ stones ($x \in [1, y]$).
