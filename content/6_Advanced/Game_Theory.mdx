...
The [Sprague-Grundy theorem](https://en.wikipedia.org/wiki/Sprague%E2%80%93Grundy_theorem) defines "nimbers" or "grundy values" of any impartial game as the unique one-pile NIM game that the state of the game - let it be $s$ - is equivalent to, taking into account the **reachable** states denoted as $s_i \,\,, i \in [1, k]$. These values can be computed using the recursive formula:

$$
\tag{1}
n(x) = \text{mex}\{\text{s_1, s_2, s_3, ..., s_k}\}
$$
...
We can apply the Sprague-Grundy theorem $(2)$. Let $g(n,k)$ be the nimber value for a particular pile. Additionally, I highly recommend taking a particular case and analyzing the patterns in the grundy values. Two observation can be inferred from the pattern:
