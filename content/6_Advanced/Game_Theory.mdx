...
The [Sprague-Grundy theorem](https://en.wikipedia.org/wiki/Sprague%E2%80%93Grundy_theorem) defines "nimbers" or "grundy values" of any impartial game as the unique one-pile NIM game that the state of the game is equivalent to. These values can be computed using the recursive formula:
