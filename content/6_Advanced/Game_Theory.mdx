...
It remains to demonstrate that from any winning state we can bring the other player in a losing state. Let $s$ be the xor-sum of all the pile sizes. If there is a pile with $y$ stone such that $y \gt y \oplus s$, then we can remove
$y - y \oplus s$ stones from this pile, remaining $y - (y - y \oplus s) = y \plus s$
