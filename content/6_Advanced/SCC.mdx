...
In the first phase, the algorithm performs a depth-first search (DFS) on the original graph to determine the exit times of vertices, akin to creating a topological sort. As each vertex finishes processing, it is pushed onto a stack. This stacking ensures that, in the next phase, nodes are processed in the order of their decreasing finish times. This order is crucial for effectively identifying the SCCs.
