...
During this DFS, the algorithm explores all outgoing edges from the current vertex, updating its `low` value to track the lowest discovery time reachable. The DFS processes nodes according to the edges defined by the original graph. By tracking discovery times and following this specific order, the DFS run can identify all vertices of one SCC before extracting them and moving on to the next.
