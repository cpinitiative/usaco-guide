...
We know that the transposed graph will have the same SCCs as the original because reversing an edge doesn't change the reachability between two components. Since each SCC is maximal and disjoint, reversing edges doesn't break any connections in the original graph.
