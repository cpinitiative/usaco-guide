...
struct TwoSatSolver {
    ...
    void dfs1(int v) {
        ...
        order.push_back(v); // Append node to order list after visiting all reachable nodes
    }
    ...
    bool solve() {
        ...
        for (int i = 0; i < n_vertices; i += 2) {
            if (comp[i] == comp[i + 1]) {
                return false; // If variable and its negation are in the same component, unsatisfiable
			}
            assignment[i / 2] = comp[i] > comp[i + 1]; // Assign true/false based on component ids
        }
