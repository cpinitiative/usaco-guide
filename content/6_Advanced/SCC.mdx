...
We know that the transposed graph will have the same SCCs as the original because reversing an edge doesn't change the reachability within the two components. Since each SCC is maximal and disjoint, reversing edges doesn't break any undirected connections in the original graph.
