...
struct TwoSatSolver {
	...
	/*
	'add_disjunction' is responsible for adding a clause of the form (𝑎 ∨ 𝑏) to
	the 2-SAT problem. In the context of 2-SAT, each clause specifies a logical
	disjunction of two literals, and the task is to determine if there is a way
	to assign truth values to the variables so that all clauses are satisfied.
	The 'add_disjunction function translates each clause into implications and
	updates the implication graph accordingly.
	*/
	void add_disjunction(int a, int na, int b, int nb) {
		// na and nb signify whether a and b are to be negated
