...
We know that the transposed graph will have the same SCCs as the original because reversing an edge doesn't change the reachability within the two components. Since each SCC is maximal and disjoint, reversing edges doesn't break any connections in the original graph.
...
For visualisation, refer to the TopCoder resource posted [here](https://www.topcoder.com/thrive/articles/kosarajus-algorithm-for-strongly-connected-components).
