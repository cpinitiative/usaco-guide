...
public class Main {
	static boolean[] vis;
	static List<Integer>[] adj;    // // Adjacency list of neighbor
	static List<Integer>[] adj_t;  // adj_t is the transpose of adj
	static int[] id;
	...
	private static void dfs(int x, int pass, int num) {
		vis[x] = true;
		List<Integer> ad = (pass == 1) ? adj[x] : adj_t[x];
		for (int e : ad) {
			if (!vis[e]) { dfs(e, pass, num); }
		}
		S.add(x);
