...
## Explanation
Kosaraju's algorithm is an efficient method for finding the strongly connected components (SCCs) of a directed grpahs. The algorithm works in two main phase. In first phase, it performs the DFS on the original graphs $G$ and record the exit time (i.e. reversed topological sort of graph $G$).

In the second phase, the graph is transposed, meaning all the edges are reversed. A new DFS is performed, but this time it processes nodes in the order defined by the topological sort from the first phase. By reversing the edges and processing nodes in this specific order, each DFS run in this phase will identify all vertices of one SCC before moving to the next SCC. This is because in the transposed graph, the SCCs are still strongly connected, and the exit times help in isolating them during the DFS.
