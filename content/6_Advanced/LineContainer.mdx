...
struct Halfplane {
	...
	/** @return if a point r is outside the half-plane */
	bool out(const Point &r) { return cross(pq, r - p) < -eps; }
};

/** @return the intersection of a set of half-planes */
std::vector<Point> halfplane_intersect(std::vector<Halfplane> &halfplanes) {
	...
	for (int i = 0; i < int(halfplanes.size()); i++) {
		...
		while (len > 1 && halfplanes[i].out(inter(active_halfplanes[len - 1],
