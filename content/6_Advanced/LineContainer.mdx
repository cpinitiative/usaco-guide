...
std::vector<Point> halfplane_intersect(std::vector<Halfplane>& halfplanes) {
    ...
    for (int i = 0; i < int(halfplanes.size()); i++) {
        ...
        while (len > 1 && halfplanes[i].out(intersection(active_halfplanes[len - 1], active_halfplanes[len - 2]))) {
            active_halfplanes.pop_back();
            len--;
        }

        // Remove invalid half-planes from the beginning of active set
        while (len > 1 && halfplanes[i].out(intersection(active_halfplanes[0], active_halfplanes[1]))) {
    ...
    for (int i = 0; i < len - 1; i++) {
        intersection_points[i] = intersection(active_halfplanes[i], active_halfplanes[i + 1]);
    }
    intersection_points.back() = intersection(active_halfplanes[len - 1], active_halfplanes[0]);
...
Point intersection(const Halfplane& h1, const Halfplane& h2) {
    ...
    return Point(h1.p.x + (num1 / den) * h1.pq.x, h1.p.y + (num1 / den) * h1.pq.y);
}

int main() {
    std::vector<Halfplane> halfplanes;

    // Example: Adding half-planes (adjust as needed)
    halfplanes.emplace_back(Point(1, 1), Point(1, -1));
    halfplanes.emplace_back(Point(1, -1), Point(-1, -1));
    halfplanes.emplace_back(Point(-1, -1), Point(-1, 1));
    halfplanes.emplace_back(Point(-1, 1), Point(1, 1));

    // Compute intersection points of half-planes
    std::vector<Point> intersection_points = halfplane_intersect(halfplanes);
