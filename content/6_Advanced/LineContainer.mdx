...
<FocusProblem problem="HPIsam" />

A half-plane is a planar region consisting of all points on one side of an infinite straight line, and none on the other. We can apply computational geometry here, as an intersection can be represented as a convex polygon, where every point inside it is also inside all half-planes. If we can construct this polygon, we can retrieve the intersections.
...
![halfplane1](./assets/halfplane1.png)

This half-plane has an equation $y = 2x -2$ and can be represented as $P = (1,0)$ with the direction vector $PQ$ as $(1,2)$.
Let's now look at various ways of solving this problem, optimizing as we continue.
...
![halfplane4](./assets/halfplane4.png)
...
* Sort the set of half-planes by angle; it takes $\mathcal{O}(N\log N)$ time.
...
struct Halfplane {
	...
	bool out(const Point &r) const { return cross(pq, r - p) < -eps; }

	// Comparator for sorting half-planes by angle
	bool operator<(const Halfplane &rhs) const { return angle < rhs.angle; }
};

// Function to compute intersection of two half-planes
Point intersection(const Halfplane &h1, const Halfplane &h2) {
	long double den = cross(h1.pq, h2.pq);
	long double num1 = cross(h2.pq, h1.p - h2.p);

	// Calculate intersection point
	return Point(h1.p.x + (num1 / den) * h1.pq.x,
	             h1.p.y + (num1 / den) * h1.pq.y);
}

// Function to compute the intersection of a set of half-planes
long double halfplane_intersect(std::vector<Halfplane> &halfplanes) {
	...
	for (int i = 0; i < int(halfplanes.size()); i++) {
		...
		while (len > 1 &&
		       halfplanes[i].out(intersection(active_halfplanes[len - 1],
		...
		while (len > 1 && halfplanes[i].out(intersection(
		                      active_halfplanes[0], active_halfplanes[1]))) {
	...
	for (int i = 0; i < len - 1; i++) {
		intersection_points[i] =
		    intersection(active_halfplanes[i], active_halfplanes[i + 1]);
	}
	intersection_points.back() =
	    intersection(active_halfplanes[len - 1], active_halfplanes[0]);

	// Calculate area using shoelace formula
	long double area = 0;
	for (int i = 0; i < len; i++) {
		area +=
		    intersection_points[i].x * intersection_points[(i + 1) % len].y -
		    intersection_points[i].y * intersection_points[(i + 1) % len].x;
	}
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < c; j++) { std::cin >> points[j].x >> points[j].y; }

		// Convert polygon to half-planes
		for (int j = 0; j < c; j++) {
			Point a = points[j];
			Point b = points[(j + 1) % c];
			halfplanes.push_back(Halfplane(b, a));
		}
	}

	// Compute intersection area of all half-planes
	long double intersection_area = halfplane_intersect(halfplanes);

	// Output result with precision up to 1e-2
	std::cout << std::fixed;
	std::cout.precision(3);  // To ensure absolute error less than 1e-2
