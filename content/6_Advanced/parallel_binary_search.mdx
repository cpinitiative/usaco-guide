...
Both of the above resources provide a solution to [Meteors](https://szkopul.edu.pl/problemset/problem/7JrCYZ7LhEK4nBR5zbAXpcmM/site/?key=statement) using **parallel binary search** technique.

<Info>
This section is based on the definitions given in [Binary Search](/silver/binary-search) module.
</Info>
## Tutorial
...
## Pseudocode
Let $\text{event[i]}$ corresponds to some stuff that needed be done at that particular index $\text{i (lo <= i <= hi)}$
...
struct Disjoint_Set_Union {
    ...
    Disjoint_Set_Union(int n) : n(n) {
        dsu.resize(n+1); // 1-based indexing. Can also be used as 0-based indexed system.
...
void parallel_binary_search(int l, int r, int level, vector <int> &candidate) {
    ...
    for(int c : candidate) {
        if (DSU[level].Find(query[c].first) != DSU[level].Find(query[c].second)) {
...
int main() {
    ...
    // [0....m-1] are the possible answers. 'm' is equivalent of -1 for the answer.
