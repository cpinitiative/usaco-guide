...
struct Node {
  ...
  // search stack -> will allow us to find parents of nodes without explicitly storing them
  ...
  // gets top of search stack, which will contain the path from root (bottom) to the current node (top)
  T top() {
  ...
  int max() {
    return ::max({l ? l->max() : 0, r ? r->max() : 0, val});
  }
...
struct Node {
  ...
  // search stack -> will allow us to find parents of nodes without explicitly storing them
  static inline vector<T> st = {};
  Node(ll v, T l = nullptr, T r = nullptr) : val(v), s(v), sz(0), l(l), r(r), rev(false) { pull(); }
