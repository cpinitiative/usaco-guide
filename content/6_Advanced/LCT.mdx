...
Unfortunately, this observation alone is not enough to prove the amortized
...
struct Node {
  ...
  // Store Nodes in contiguous array rather than allocating with `new` to prevent MLE
  static Node t[2 * N];
  ...
  void splay() {
    for (;p;rotate()) {
      if (p->p)  {
        if (side() == p->side()) { p->rotate(); } // zig-zig
        else { rotate(); } // zig-zag
      }
    }
  }
...
struct SplayTree {
  ...
  pair<SplayTree, SplayTree> split(int i) { // splits into {[0, i), [i, )}
    if (i == (root ? root->sz : 0)) { return make_pair(SplayTree{root}, SplayTree{nullptr}); }
  ...
  friend ostream& operator<<(ostream& os, SplayTree s) { return os << s.root; }
} splay;
