...
Introduces a modified version of the prime counting function, considering the number of integers not divisible by primes up to a certain threshold.

It involves a combination of linear sieving and recursive calculation, achieving a complexity of $O(N^{2/3}\log^{1/3}N)$.
