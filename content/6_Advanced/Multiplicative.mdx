...
description: This module delves into the topic of multiplicative functions and their properties.
frequency: 0
---
### Prerequisite Skills
...
You are given integers $ N, A, $ and $ B $. Find $ \pi(N) $ (the number of primes no greater than $ N $), and print $ \sum_{i} p_{Ai+B} $ for nonnegative integers $ i $ with $ \sum_{i} p_{Ai+B} \leq N $.
...
- $ 0 \leq A < B \leq N $
- $ 0 \leq B < A \leq N $
- $ 0 \leq X \leq 10^6 $ where $ X = \# \{ i \in \mathbb{Z}_{\geq 0} \mid \sum_{i} p_{Ai+B} \leq N \} $
