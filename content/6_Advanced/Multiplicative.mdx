...
// return pi(x) = the number of prime numbers between 2 and N
long long counting_primes(const long long N) {
	...
	for (int p = 3; p <= v; p += 2)
		if (!skip[p]) {
			...
			for (int k = 0; k < s; k++) {
				...
				if (d <= v) {
					large[ns] = large[k] - (large[small[d >> 1] - pc]) + pc;
				} else {
					large[ns] = large[k] - small[(N / d - 1) >> 1] + pc;
				}
	...
	for (int l = 1; l < s; l++) {
		...
		if (e < l + 1) break;
		long long t = 0;
		for (int k = l + 1; k <= e; k++) t += small[(M / rough[k] - 1) >> 1];
