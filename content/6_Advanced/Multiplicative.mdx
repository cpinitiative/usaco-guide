...
description: The module provides a introduction to prefix sum techniques applied to multiplicative functions. It defines arithmetic, multiplicative, and completely multiplicative functions, showcasing examples like the divisor function, Euler's totient function, and the Möbius function. It explains techniques such as Dirichlet convolution, Möbius inversion, and binomial inversion for efficient manipulation of multiplicative functions. The module also includes examples demonstrating prefix sum methods used in problems like summing divisors, totient calculations, enumerating primes, and counting primes, along with detailed explanations and implementations.
frequency: 0
---
...
// BeginCodeSnip{Sieve Implementation}
vector<int> prime;
bool is_composite[MAX_N];
...
It achieves a complexity of $O(N^{3/4}\log^{1/2}N)$.
...
There exists a faster way to compute primes in O(n^2/3), see [Maksim1744’s Codeforces Blog](https://codeforces.com/blog/entry/91632) for more details. Below is the implementation using the fenwick code snippet from the [PURS module](https://usaco.guide/gold/PURS?lang=cpp#solution---dynamic-range-sum-queries-with-a-bit)
