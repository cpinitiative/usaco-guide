...
There exist an $O(\frac{n^{2/3}}{\sqrt[3]{\log n}})$ implementation, see [Maksim1744’s Codeforces Blog](https://codeforces.com/blog/entry/91632) for more details. Below is the implementation using the fenwick code snippet from the [PURS module](https://usaco.guide/gold/PURS?lang=cpp#solution---dynamic-range-sum-queries-with-a-bit). Note that the fastest solutions to this library checker problem look like they run in $O(\frac{N^{3/4}}{\sqrt{\log N}})$
