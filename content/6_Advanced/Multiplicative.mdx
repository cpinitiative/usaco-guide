...
void solve() {
	linearSieve(N);
	int x = Prc / A + (Prc % A >= B + 1);
	printf("%d %d\n", Prc, x);
	for (int i = B + 1, cnt = 1; i <= Prc; i += A, ++cnt)
		printf("%d%c", Prs[i], " \n"[cnt == x]);
}
...
For the complete explanation of the solution, please refer to https://codeforces.com/blog/entry/91632
