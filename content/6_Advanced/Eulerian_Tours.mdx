...
First let's define what is an **Eulerian** path: ``A path that goes through every edge once``. Similarly, an **Eulerian cycle** is an Eulerian path that starts and ends
with the same node.
