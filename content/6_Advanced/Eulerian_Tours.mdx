...
public class EulerianCycle {
	...
	static void dfs(int node) {
		while (!g.get(node).isEmpty()) {
			int[] edge = g.get(node).remove(g.get(node).size() - 1);
			int son = edge[0], idx = edge[1];
			if (seen[idx]) continue;
			seen[idx] = true;
			dfs(son);
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < m; i++) {
			int x = sc.nextInt() - 1;
			int y = sc.nextInt() - 1;
			g.get(x).add(new int[] {y, i});
			g.get(y).add(new int[] {x, i});
			degree[x]++;
			degree[y]++;
		}
		...
		if (path.size() != m + 1) {
			...
			for (int node : path) { System.out.print((node + 1) + " "); }
			System.out.println();
		}
	}
}
