...
An important condition is that a graph can be an Eulerian graph if and only if every node has an even degree.

Now, to properly find the Eulerian cycle we run a slightly modified DFS.
The DFS goes through only unvisited edges and the same edge can be processe multiple times throuoght the DFs, so we remove it from the graph at the first visit.
