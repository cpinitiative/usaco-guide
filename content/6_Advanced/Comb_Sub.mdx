...
Some intuition for this can arise if we think of the subtree rooted at $i$ as node $i$ in an undirected graph of $n$ singletons.
Merging two subtrees of sizes $X$ and $Y$ is equivalent to adding $X\cdot Y$ edges between pairs of nodes from different subtrees.
After all merges, we have a complete graph with $\mathcal{O}(N^2)$ edges.
