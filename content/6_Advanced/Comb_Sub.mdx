...
The counter will always be equal to $\binom{N}{2}$ at the end of this process; every pair of ones contributes one to the answer. Adding $a\cdot b$ to the counter corresponds to merging two subtrees of sizes $a$ and $b$ into one subtree of size $a+b$ in $\mathcal{O}(ab)$ time, which in total will result in a complexity of $\mathcal{O}(N^2)$.
