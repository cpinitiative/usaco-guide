...
> Think of each subtree rooted at $i$, as a node $i$ in an undirected graph of $n$ singletons.
> Merging two subtrees $A$ and $B$ of sizes $X$ and $Y$ is equivalent to adding at most $X\cdot Y$ edges between pairs of nodes from different subtrees.
