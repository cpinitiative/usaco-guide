...
The three points of the triangle given are $(0, 0), (X, Y), \& (A, B)$.
...
where the unknowns are $A$ and $B$, and the known constants are $Y$ and $-X$. 

Let $g = gcd(X, Y)$. The solution is possible only if $g \mid 2$ 
(i.e., Bézout's Identity) — otherwise no such $(A, B)$ exists.
...
Multiplying this by $2 / g$, we get:
