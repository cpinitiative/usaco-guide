...
which takes $O(N\log \sigma)$ integers using the most straightforward approach. However, if we can reduce this to $O(N\log \max(N,\sigma))$
*bits* of information, we can pack these bits into $O(N\log \max(N,\sigma) / W)$ words where
$W$ is the [word](https://en.wikipedia.org/wiki/Word_(computer_architecture)#Variable-word_architectures) size ($W=64$ on a 64-bit architecture). This is $O(N)$ words assuming $2^W>\max(N,\sigma)$ (that is, all the integers we're working with fit into a single word).
...
with length divisible by $W$, taking $O(N)$ bits. To answer a query for the
$r$th prefix sum, we start with the $\lfloor r/W\rfloor\cdot W$th prefix sum and then use
built-in operations that run in constant time to add the contribution of the
remaining bits (like $\texttt{\_\_builtin\_popcountll}$ to count the number of bits in a 64-bit word).
