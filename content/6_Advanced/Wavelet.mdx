...
Notice that a index can be in at most $log(M)$ segments
...
<Info title="Left occurrences">
  Note that $\texttt{left\_occurrences}$ is the amount of indices between l and r that their value is less than mid  
...
struct Segment {
    ...
    Segment(int l, int r,const vector<pair<int,int>> &indices) :l(l), r(r), mid((r + l) / 2), indices(indices) {
        calculate_b();
    }
    ...
    void update() {
        ...
        if (r - l > 1) {
            ...
            partition_copy(indices.begin(), indices.end(), leftIndices.begin(),rightIndices.begin(),
    ...
    int find_k_smallest(int a, int b, int k) {
        ...
        if (k <= inLeft) {
            ...
            return right->find_k_smallest(a - lb,b - lr, k - inLeft); // Appears in right
        }
    }
};
