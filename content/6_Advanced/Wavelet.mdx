...
This reduces the problem of counting the number of occurrences in a prefix.
...
We can traverse down to the leaf to remove the old element and traverse again to add the new element. These updates only change our indices vector and prefix vector.

To make erasing and adding values easier, we can have the indices vector store sets instead of vectors.
...
Instead, we can use a sparse segment tree. These support the operations:
...
This approach is not memory efficient and requires a segment tree implementation.
