...
$W$ is the [word](https://en.wikipedia.org/wiki/Word_(computer_architecture)) size ($W=64$ on a 64-bit architecture). This is $O(N)$ words assuming $2^W>\max(N,\sigma)$ (that is, all the integers we're working with fit into a single word).
