...
struct Segment {
	...
	Segment(int l, int r, vector<pair<int, int>> indices)
	    : l(l), r(r), mid((r + l) / 2), indices(std::move(indices)) {
		calculateB();
	}
	...
	void update() {
		...
		if (r - l > 1) {
			...
			for (auto [ind, val] : indices)
				if (val < mid) leftIndices.emplace_back(ind, val);
				else rightIndices.emplace_back(ind, val);
