...
| ----- | ---- | ---- |
| Module Solution 1 | $\mathcal{O}(\log \sigma\log N)$| $\mathcal{O}(N\log \sigma)$ |
| Module Solutions 2a / 2b | $\mathcal{O}(\log \sigma)$ | $\mathcal{O}(N\log \sigma)$ |
| Module Solution 3 | $\mathcal{O}(\log \sigma)$ | $\mathcal{O}(N)$ |
| Persistent Segment Tree | $\mathcal{O}(\log \sigma)$ | $\mathcal{O}(N\log \sigma)$ |
...
This takes $\mathcal{O}(N\log \sigma)$ time and memory.
...
Querying the count in a single node takes $\mathcal{O}(\log N)$ time using binary search, and the tree has depth $\mathcal{O}(\log \sigma)$, so in a total a query takes $\mathcal{O}(\log \sigma\log N)$ time.
...
which takes $\mathcal{O}(N\log \sigma)$ integers using the most straightforward approach. However, if we can reduce this to $\mathcal{O}(N\log \sigma)$
*bits* of information, we can pack these bits into $\mathcal{O}(N\log \sigma / W)$ words where
$W$ is the [word](https://en.wikipedia.org/wiki/Word_(computer_architecture)) size ($W=64$ on a 64-bit architecture). This is $\mathcal{O}(N)$ words assuming $2^W>\sigma$ (that is, all the integers we're working with fit into a single word).

It remains to describe how to store a single length-$N$ bitvector in
$\mathcal{O}(N)$ bits while still allowing constant time prefix sum queries. Specifically,
we can store the original bitvector in $N$ bits and only the sums of prefixes
with length divisible by $W$, taking $\mathcal{O}(N\log N/W)$ bits, which is $\mathcal{O}(N)$ bits assuming $2^W>N$. To answer a query for the
