...
The first resource also discusses how to support updates to $a$ of the form
$\text{swap}(a_i, a_{i+1})$.
...
bitvectors at each level into a single bitvector of length $N$ before taking prefix sums.
...
Here we discuss how to remove a factor of $\log \sigma$ from the memory complexity.
...
which takes $O(N\log \sigma)$ integers. However, if we only store $O(N\log \sigma)$
*bits* of information, we can pack these into $O(N\log \sigma / W)$ integers where
$W$ is the word size (64). This is $O(N)$ since $2^W>\sigma$.
