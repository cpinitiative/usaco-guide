...
int N, Q;
ll Tsum[400005], Tmax[400005], Tmin[400005], Lazy[400005];
 
void pushdown(int t, int tl, int tr) {
    if (Lazy[t] != -1) {
        int tm = (tl + tr) >> 1;
 
        Tsum[t << 1] = Lazy[t] * (tm - tl + 1);
        Tmax[t << 1] = Lazy[t];
        Tmin[t << 1] = Lazy[t];
        Lazy[t << 1] = Lazy[t];
 
        Tsum[t << 1 | 1] = Lazy[t] * (tr - tm);
        Tmax[t << 1 | 1] = Lazy[t];
        Tmin[t << 1 | 1] = Lazy[t];
        Lazy[t << 1 | 1] = Lazy[t];
...
inline void push_add(int t, int tl, int tr, ll v) {
    if (v == 0)
        return;
    T[t].sum += (tr - tl + 1) *v;
    T[t].max1 += v;
...
inline void push_max(int t, ll v, bool l) {
    if (v >= T[t].max1)
        return;
...
inline void push_min(int t, ll v, bool l) {
    if (v <= T[t].min1)
        return;
...
inline void pushdown(int t, int tl, int tr) {
    if (tl == tr)
        return;
    // sum
    int tm = (tl + tr) >> 1;
