...
- If $m \le x / 2$, then after the modulo operation $x$ must be strictly less than
  $m$
- If $x / 2 < m \leq x$, then $x \mod m = x - m$. This then reduces to
  the second case.
...
We can store $\texttt{cur\_max}$ and $\texttt{cur\_min}$ to compute them
...
it is still necessary to maintain a lazy tag to propagate updates. Clearly, this
...
to its base state of all 0s. If there are no point updates, then the time
...
It's possible to solve this problem without a $\texttt{min}$ tag, but using 
both a $\texttt{min}$ and a $\texttt{max}$ allows range updates, which 
aren't strictly pertinent to this particular problem but is still 
...
const int MAXN = 100000 + 1;  // 1-based

int N;
ll tsum[MAXN * 4];  // Sum tag
ll tmax[MAXN * 4];  // Max tag
ll tmin[MAXN * 4];  // Min tag
ll lazy[MAXN * 4];  // Lazy updates
