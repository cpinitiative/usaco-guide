...
We will store $\texttt{cur_max} in a seperate array as a seperate (mergable) value. Although it is possible that a single query processes $\mathcal{O}(n)$ nodes, over all queries this amortizes to the acceptable time complexity of $\mathcal{O}((n + q)\log a)$. 
...
Although strictly speaking, **The Child and Sequence** is not a segment tree beats problem, the techniques used in it are extremely similar. 
