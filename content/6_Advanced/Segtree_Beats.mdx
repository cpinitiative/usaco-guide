...
We will store $\texttt{cur\_max}$ in a seperate array as a seperate (mergable) value. Although it is possible that a single query processes $\mathcal{O}(n)$ nodes, over all queries this amortizes to the acceptable time complexity of $\mathcal{O}((n + q)\log a)$. 

Consider adding in operations of type 3. Although the implementation is
relatively straightforward (simply a point update on segment tree), the proof of complexity from the previous section
...
to its base state of all 0s. Note that each update operation runs in $\mathcal{\Omega}(\log n)$, so if there are no point updates, then the time
complexity is $\mathcal{O}(n \log a \log n)$.
...
$\mathcal{O}(n \log a \log n + q_p \log a) = \mathcal{O}((n \log n + q)\log a)$.

Although strictly speaking, **The Child and Sequence** is not a segment tree beats problem, the techniques used in it are closely related. In short, segment tree beats is a technique that allows a non-polylogarithmic range update complexity that amortizes to $\mathcal{O}(n \log n)$ or $\mathcal{O}(n \log^2 n)$
