...
**Note:** Because we are no longer doing range updates with lazy propogation,
there is no need for a lazy tag.

We will store $\texttt{cur\_max}$ in a seperate array as a seperate (mergable)
...
The issue, again, is that in lazy propogation, it is difficult to update the sum
...
children dispite being in the query range. In other words, when a node satisfies
