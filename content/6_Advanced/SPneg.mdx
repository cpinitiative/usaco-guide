...
As mentioned in [cp-algorithms](https://cp-algorithms.com/graph/finding-negative-cycle-in-graph.html), we run Bellman-Ford $N$ times (instead of $N-1$ times). if there is an update to distances in the $N$th iteration, then there is an negative cycle.
