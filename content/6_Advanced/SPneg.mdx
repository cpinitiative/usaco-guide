...
As mentioned in [cp-algorithms](https://cp-algorithms.com/graph/finding-negative-cycle-in-graph.html), we relax the edges N times. If we perform an update on the $N$th iteration, there is an negative cycle.
