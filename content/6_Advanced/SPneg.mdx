...
all edges V-1 times. Hence, it runs in O(E * V).
...
distance to that point is minus infinity. Notice that all points that are reachable
...
do a bfs with those points as sources.
...
int main(){
    ...
    while(!(n == 0 && m == 0 && q == 0 && s == 0)){
        ...
        for(int i = 0; i < q; i++){
            ...
            if(is_negative_inf[target]) { cout << "-Infinity\n"; }
            else if(dist[target] == INF) { cout << "Impossible\n"; }
