...
As mentioned in [cp-algorithms](https://cp-algorithms.com/graph/finding-negative-cycle-in-graph.html), we run Bellman-Ford $N$ times (instead of $N-1$ times). If we perform an update on the $N$th iteration, there is an negative cycle.
