...
and any other vertex should have at most $V-1$ edges, where $V$ stands for the number
of vertices in the graph. Because of that, the algorithm iterates through at most
all edges $V-1$ times. Hence, it runs in $\mathcal{O}(E \cdot V)$.

If the graph has a negative cycle, we can detect a vertex in this cycle by running
...
> Given variables $x_1,x_2,\dots,x_N$ with constraints in the form
