...
As the title says, above is described [Edmonds-Karp](https://en.wikipedia.org/wiki/Edmonds%E2%80%93Karp_algorithm) algorithm.
...
To find the maximum flow we'll handle a **preflow** which satisfies all the constraints of the flow, but not necessary the conservation law. It means that the incoming flow can exceed the outgoing flow.

Let's define the excess flow of node u as: $in_u - out_u$, where $in$ is the incoming flow and $out$ is the outgoing flow.

The algorithm starts with an initial preflow where the source has an excess flow. Every stage it picks a node with excess flow and tries to **push** the excess to its neighbours, if the capacity supports it.
...
edge $(u, v)$. The process stops when there are no more excess node left in the flow network.

Another important feature of the algorithm is the **labeling** function $h$, also know as the height function, which assignes each node an
integer. One labeling in valid if satisfies the following conditions: $h_source = |V|$, $h_sink = 0$ and $h_u \le h_v + 1$ if there is an edge from node $u$
to node $v$ with positive capacity, i.e. it supports more flow. The height function helps us know where to send the excess flow, where it's needed. It's like water, it can only
flows from top to bottom.

To summarize: Start with a valid preflow and a valid labeling. In each step, for every excess node, try to push excess flow to node's neighbours. After each step
check if the flow and the labeling are still valid. If they are and there are no more path between the $source$ and the $sink$ it means it found the maximum flow.

The difference between Edmond's Karp (or Ford-Fulkerson) and Push-Relabel algorithm is that the former one keeps a valid flow all the time and improves it while
there are augmenting paths, while in the Push-Relabel there doesn't exist an augmenting path at any time, and it improves the preflow untile it reaches the maximum flow.
