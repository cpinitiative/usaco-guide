...
The only difference between this and a normal flow is that the incoming flow can exceed the out coming flow.

Let's define the excess flow of node u as: $in_u - out_u$, where $in$ is the incoming flow and $out$ is the outgoing flow.

The algorithm starts with an initial preflow where the source has an excess flow. At every stage, it picks a node with the excess flow and tries to **push** the excess to its neighbors, if the capacity supports it.
...
edge $(u, v)$. The process stops when no more excess nodes are left in the flow network.
...
to node $v$ with positive capacity, i.e. it supports more flow. The height function helps us know where to send the excess flow, and where it's needed. It's like water, it can only
flow from top to bottom.

To summarize: Start with a valid preflow and a valid labeling. In each step, for every excess node, try to push excess flow to the node's neighbors. After each step
check if the flow and the labeling are still valid. If they are and there are no more paths between the $source$ and the $sink$ it means it found the maximum flow.

The difference between Edmond's Karp (or Ford-Fulkerson) and Push-Relabel algorithm is that the former keeps a valid flow all the time and improves it while
