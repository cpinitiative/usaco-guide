...
The algorithm starts with an initial preflow where the source has an excess flow. Every stage it picks a node with excess flow and tries to push the excess to its neighbours, if the capacity supports it. The process stops when there are no more excess node left in the flow network. 
