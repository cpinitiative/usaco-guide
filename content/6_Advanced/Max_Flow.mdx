...
The solution is a greedy approach. The download speed (the flow) can be improved as long as we can find a non-negative capacity path from the source (the server) and the sink (Kotivalo's computer). After that we subtract the minimum capacity on the found path from the edge capacities.
...
long long max_flow(vector<vector<long long>> g, int n, int source, int sink) {
	...
	// Find a way from source to sink with non-negetive capacities
	auto reachable = [&]() -> bool {
		queue<int> q;
		q.push(source);
		while (!q.empty()) {
