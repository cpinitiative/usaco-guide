...
The solution is a greedy approach. The download speed - the flow - can be improved as long as we can find a non-negative capacity path from the source - the server
and the sink - Kotivalo's computer. After that we substract the minimum capacity on the found path from the edge capacities.
...
long long max_flow(long long g[][NMAX], int n, int source, int sink) {
	long long flow = 0;
	vector<int> parent(n, -1);

	// BFS for a path from source to sink with non-negetive capacities
	auto reachable = [&]() -> bool {
		queue<int> q;
		q.push(source);
		while(!q.empty()) {
	...
	// While there is a way from source to sink with non-negative capacities we can increase the downloading speed - the flow
	while(reachable()) {
		int node = sink;
		...
		node = sink;
		while(node != source) {
			// Substract the current flow from capacity edges
...
int main() {
	...
	cout << max_flow(capacity, n, 0, n-1) << '\n';
