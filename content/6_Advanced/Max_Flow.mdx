...
Is's recommended that you solve the first problem - Download Speed - in the section before trying this one.
...
Here is an animation of how the code below works:
...
int main() {
	...
	for (int node = 1; node <= n; node++) {
		if (Pair[node]) { cout << node << ' ' << Pair[node] - n << '\n'; }
	}
}
```
</CPPSection>
