...
bool check(array<int, K> &b, int x) { return (reduce(b, x) == 0); }

vector<int> rebuild_path(const vector<int> &path, int v) {
	array<int, K> b{0};
	vector<int> res;
	if (add(b, arr[v])) { res.push_back(v); }
	for (auto x : path) {
		if (add(b, arr[x])) { res.push_back(x); }
	}
	return res;
}
...
int LCA(int x, int y) {
	...
	for (int i = K - 1; i >= 0; i--) {
		if (!is_ancestor(p[x][i], y)) { x = p[x][i]; }
	}
	return p[x][0];
}
...
bool query(int x, int y, int k) {
	...
	int lca = LCA(x, y);
	for (auto v : up[x]) {
		if (!is_ancestor(v, y)) { add(b, arr[v]); }
	}

	for (auto v : up[y]) {
		if (!is_ancestor(v, x)) { add(b, arr[v]); }
	}
...
int main() {
	...
	while (q--) {
		...
		auto bas = basis[x];
		for (int i = 0; i < 20; i++) {
			if (basis[y].t[i] >= t.dep[l]) { bas.add(basis[y].a[i]); }
		}
