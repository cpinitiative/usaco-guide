...
If the first $2$ characters of `a` are already less than or greater than those of `b`, then nothing changes, really.
...
To solve this problem, we're going to have to create another array based on the suffix array that computes the **longest common prefix** (LCP) between adjacent elements of the suffix array.
...
int main() {
	...
	for (int i = 0; i < n - 1; i++) {
		...
		while (str.charAt(i + curr_cmp) == str.charAt(prev + curr_cmp)) {
			curr_cmp++;
		}
...
public class SuffixArray {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < n - 1; i++) {
			...
			int currCmp = startAt;
			while (str.charAt(i + currCmp) == str.charAt(prev + currCmp)) {
				currCmp++;
			}
