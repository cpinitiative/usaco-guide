...
export const problems = {
	minEx: [new Problem('CSES', 'Coin Grid', '1709', 'Easy', false, [], '')],
	path: [
		new Problem(
			'Kattis',
			'The Wrath of Khan',
	...
	cut: [
		new Problem('CSES', 'Police Chase', '1695', 'Easy', false, [], ''),
		new Problem(
			'Old Gold',
			'Cow Steeplechase',
			'93',
		...
		new Problem(
			'AC',
			'ARC E - MUL',
			'http://arc085.contest.atcoder.jp/tasks/arc085_c',
			'Hard',
			false,
			[],
			''
...
<FocusProblem problem={problems.minEx[0]} />

## Resources

The resources below include many clever applications of min cut, including the [Closure Problem](https://en.wikipedia.org/wiki/Closure_problem).

<Resources>
	<Resource
		source="CPC"
		title="10 - Network Flow"
		url="10_graphs_3_network_flow" starred
	>
		Slides from "Algorithm Design." Min-Cut Max-Flow Theorem, applications of flow / min cut.
	</Resource>
	<Resource source="CPH" title="20.3, 20.4 - Flows & Cuts" starred>
		Brief descriptions of flow / min cut applications.
	</Resource>
</Resources>

## Solution - Coin Grid

This problem asks us for the **minimum node cover** of a bipartite graph. Construct a flow graph with vertices labeled $0\ldots 2N+1$, source $0$, sink $2N+1$, and the following edges:
...
<Resources>
	<Resource source="CPH" title="20.4 - Path Covers" starred></Resource>
	<Resource
		source="Wikpedia"
