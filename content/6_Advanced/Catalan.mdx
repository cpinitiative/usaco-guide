...
def factorial(p: int) -> None:
	fac[0] = 1
	for i in range(1, MAXN + 1):
		fac[i] = fac[i - 1] * i % p


"""Precomputes modular inverses of factorials from 0 to MAXN in O(n + log p) time."""


def inverses(p: int) -> None:
	inv[MAXN] = exp(fac[MAXN], p - 2, p)
...
def choose(n: int, r: int, p: int) -> int:
	return fac[n] * inv[r] % p * inv[n - r] % p
...
if n % 2 != 0:
	...
	factorial(MOD)
	inverses(MOD)
	result = exp(n // 2 + 1, MOD - 2, MOD) * choose(n, n // 2, MOD) % MOD
	print(result)
```
</PySection>
