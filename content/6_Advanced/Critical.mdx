...
3. Compute the $idom$ for all nodes by applying the $5$-th property of $sdoms$ previously mentioned.
...
for(int i = n-1; i >= 0; i--) {
    ...
    for(int node : bucket[i]) {
        int p = Find(node, 0);
        if(sdom[p] == sdom[node]) { dom[node] = sdom[node]; } 
        else { dom[node] = p; }
    }
...
4. After steps $2$ and $3$, the $idom$ value has been set, for every node $u$, or it has been set to an ancestor $v$ of $u$ and $idom(u)=idom(v)$. Now if we start processing nodes after (0) increasingly, then for any vertex $u$:
