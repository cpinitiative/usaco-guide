...
The dominator of the starting node $1$ is itself. The set of dominators for any other node $u$ is the intersection of the set of dominators for all ancestors
$p$ of node $u$.
...
int main() {
	...
	set<int> dominators;
	for (int i = 1; i <= n; i++) {
		if (dom[n][i]) { dominators.insert(i); }
	}
...
$$
$$
sdom(u)= min(v \vert \text{there is a path from v to u: } v = v_0 \rightarrow v_1 \rightarrow v_2 \rightarrow v_3 \dots \rightarrow v_k = u \\ \text{ such that } e(v_i) \geq e(u), \forall i \in (1,k))
$$
...
for (int i = n - 1; i >= 0; i--) {
	// Iterate over incoming edges applying (2)
