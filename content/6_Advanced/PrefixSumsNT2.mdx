...
It achieves a complexity of $\mathcal{O}(\frac{N^{3/4}}{\sqrt{\log N}})$.

### Implementation

**Time Complexity: $\mathcal{O}(\frac{N^{3/4}}{\sqrt{\log N}})$**
...
There exists an $\mathcal{O}(\frac{n^{2/3}}{\sqrt[3]{\log n}})$ implementation; see [Maksim1744’s Codeforces blog](https://codeforces.com/blog/entry/91632) for more details.
Below is an implementation with a BIT.
Note that the fastest solutions to this library checker problem look like they run in $\mathcal{O}(\frac{N^{3/4}}{\sqrt{\log N}})$.

### Implementation

**Time Complexity: $\mathcal{O}(\frac{n^{2/3}}{\sqrt[3]{\log n}})$**
