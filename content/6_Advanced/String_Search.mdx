...
Let's denote $dp_i$ as the maximum diameter of a palindrome centered at $i$. Manacher's algorithm makes use of the previously determined $dp_j$, where $j < i$ in
calculating $dp_i$. The main idea is that for a palindrome centered at $i$ with the borders $left$ and $right$ the $dp_j$ ($i < j \le right$) values are - probably - mirrors
of the $dp_k$ ($left \le k < i$) values on the left side of the palindrome. Probably because for some $j$ the maximum palindrome might cross the right border. 
...
string menacher(string s) {
	...
	for(int i = 0; i < arr.size(); ) {
		...
		dp[i] = right - left + 1;
 		...
 		// The values on the right side of the palindrome [i+1, right] we'll probably be the same as the ones on the left side.
 		// Probably because some might even go beyond the current [left, right] palindrome.

		int new_center = right + (i%2 == 0 ? 1 : 0);
		for(int j = i + 1; j <= right; j++) {
			// i - (j - i) represents the mirror of j on the left side of the palindrome.
			// It's possible that the left mirror of j might go beyond the current palindrome - it crosses the left border.
			// To avoid taking the wrong answer, take the minimum of the left mirror of j and the diameter of palindrome centered at j ending at right.
			...
			// Update the max diameter
	 		...
	 		// If the palindrome centered at j reaches the right bordes of the current palindrome it might go even beyond.
