...
The Z-algorithm is very similar to KMP, but the z-algorithm uses a different function than $\pi$ and it has an interesting different application than string matching. As I've mentioned
z-algorithm doesn't use $\pi$ function, instead it uses the **z-function** defined as: $Z_i$ = the length of the longest string that is both the prefix of string $S$ and $S_{i...n}$ - the suffix
starting from $i$.

Here are the values of the z-function for some strings:
- aabxaayaab $\rightarrow$ $Z=[0,1,0,0,2,1,0,3,1,0]$
- aabxaabxcaabxaabxay $\rightarrow$ $Z=[0,1,0,0,4,1,0,0,0,8,1,0,0,5,1,0,0,1,0]$
Taking a closer look at the second example for $Z_9=8$:

<center>
$S_{9...}$ = **aabxaabx**ay
<br/>
$S$ = **aabxaabxcaabx**aabxay
</center>

To efficiently compute the z-array, we maintain the $[l, r]$ interval such that $S_{l...r}$ is also a prefix, i.e. $Z_l=r-l+1$. Let's denote $i \in [l, r]$ an arbitrary position, this gives us the following cases:
1. If $i + Z_{i-l} < r$ then we know that $Z_i = Z_{i-l}$
2. Otherwise $i + Z_{i-l} \geq r$, meaning that the answer can expand beyond $r$, thus we compare character by character from there on.
