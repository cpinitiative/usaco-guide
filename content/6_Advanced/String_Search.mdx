...
Let's denote $dp_i$ as the maximum diameter of a palindrome centered at $i$. Manacher's algorithm makes use of the previous determined $dp_j$, where &j < i& in
...
of the $dp_k, left < k < i$ values on the left side of the palindrome. Probably because some for some $j$ the maximum palindrome might cross the right border. 
