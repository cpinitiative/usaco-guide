...
<FocusProblem problem="cses-1731"/>
...
void insert(string word) {
	// Node 0 has 26 sons - a through z.
	int node = 0;
	for (auto c : word) {
...
int main() {
	...
	for (int i = s.size() - 1; i >= 0; i--) {
		...
		for (int j = i; j < s.size(); j++) {
			...
			// If the stop[node] is marked as true it means that it's the end of
			// a word. Add to dp[i] the number of ways s[j+1...s.size()] can be
			// formed.
