...
string menacher(string s) {
	...
	for (int i = 0; i < arr.size();) {
		...
		// The values on the right side of the palindrome [i+1, right] we'll
		// probably be the same as the ones on the left side. Probably because
		// some might even go beyond the current [left, right] palindrome.
		...
		for (int j = i + 1; j <= right; j++) {
			...
			// If the palindrome centered at j reaches the right bordes of the
			// current palindrome it might go even beyond. Considering this make
			// the new_center = j.
