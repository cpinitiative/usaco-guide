...
calculating $dp_i$. The main idea is that for a palindrome centered at $i$ with the borders $left$ and $right$ the $dp_j$ ($i < j <= right$) values are - probably - mirrors
of the $dp_k$ ($left <= k < i$) values on the left side of the palindrome. Probably because for some $j$ the maximum palindrome might cross the right border. 
