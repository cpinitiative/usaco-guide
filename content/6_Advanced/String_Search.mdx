...
int nodes = 1; 
int trie[NMAX][SIGMA];
int fail[NMAX]; // fail[u] = the failure link for node 
int seen[NMAX]; // check if a node has been visited in trie 
int ans[NMAX]; // ans[i] = the number of occurrences of word i
