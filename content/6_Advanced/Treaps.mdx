...
This code solves [this problem](https://judge.yosupo.jp/problem/dynamic_sequence_range_affine_range_sum), but also provides a generalizable template.
...
void push(Node *treap) {
  ...
  if (treap->f) { // need to affine this range
    ...
    if (treap->l) {
      treap->l->f = treap->f(treap->l->f);
    }
    if (treap->r) {
      treap->r->f = treap->f(treap->r->f);
    }
    treap->f = F{1, 0};
  }
}
...
void upd(int l, int r, function<void(Node *)> f) { // updates range [l, r)
...
template <typename R>
R query(int l, int r, function<R(Node *)> f) { // queries range [l, r)
  Node *a, *b, *c;                             // a: [0, l); b: [l, r); c: [r, )
  split(treap, a, b, l);
  split(b, b, c, r - l);
  assert(b);
...
signed main() {
  ...
  while (q--) {
    ...
    case 2:
      cin >> l >> r;
      upd(l, r, [](Node *x) { x->rev ^= true; });
      break;
    case 3:
      cin >> l >> r >> w >> b;
      upd(l, r, [=](Node *x) { x->f = F{w, b}(x->f); });
      break;
    case 4:
      cin >> l >> r;
      cout << query<mint>(l, r, [](Node *x) { return x->sum; }) << endl;
      break;
    }
  }
}
