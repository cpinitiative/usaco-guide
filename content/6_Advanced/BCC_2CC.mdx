...
Now we're left with two cases. If node `c` isn't a critical one it means that a path from `a` to `b` can avoid  the node. Otherwise, if node `c` is a critical one we 
