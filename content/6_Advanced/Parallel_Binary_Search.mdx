---
id: parallel-binary-search
title: 'Parallel Binary Search'
author: Muaath Alqarni
description: 'Answering multiple binary search queries parallelly'
prerequisites:
  - binary-search
frequency: 0
---


## Resources

<Resources>
	<Resource source="CF" title="Parallel Binary Seach [tutorial]" url="45578" />
</Resources>

<FocusProblem problem="sam" />

## Solution - New Roads Queries
Let's think how to binary search a single query.
It's clear that we can use DSU with linear search, but also we can do binary search with DSU in $O(N \cdot \log_2{N})$.

Now let's use parallel binary search:
Sweep through the array of edges and union them in the DSU,
whenevery you find a query its median = $i$, if the two nodes where connected, it means shrink to left, otherwise shrink to right.

You need to do the sweep $O(\log_2{N})$ times, so you know the exact time the nodes where connected.
And you need to sort the queries by median before every sweep thats $O(Q \cdot \log_2{Q})$
And $O(\alpha{N})$ for the DSU


Total Complexity: $O((N + Q) \cdot \log_2{N}^2 \cdot \alpha{N})$

## Implementation - New Roads Queries
<LanguageSection>

<CPPSection>

```cpp
#include <bits/stdc++.h>
#define ll long long
#define pii pair<int, int>
using namespace std;

const int N = 2e5 + 1;

int n, m, q;
vector<pii> edges;

struct dsu {
	int ccc = 0;  // Connected Components Count
	int cnt[N], par[N];
	void init(int sz) {
		ccc = sz;
		for (int i = 0; i <= sz; i++) par[i] = i, cnt[i] = 1;
	}
	int root(int node) {
		if (par[node] == node) return node;
		return root(par[node]);
	}
	bool link(int x, int y) {
		x = root(x), y = root(y);
		if (cnt[x] < cnt[y]) swap(x, y);
		if (x == y) return false;
		par[y] = x;
		cnt[x] += cnt[y];
		ccc--;
		return true;
	}
};

// Parallel Binary Search Query
struct pbs {
  public:
	int idx, l, r;
	int a, b;
	int mid() { return (l + r) / 2; }
} qrs[N];

int main() {
	ios::sync_with_stdio(0), cin.tie(0);
	cin >> n >> m >> q;
	for (int i = 1; i <= m; i++) {
		int a, b;
		cin >> a >> b;
		if (a > b) swap(a, b);
		edges.push_back({a, b});
	}

	for (int i = 0; i < q; i++) {
		cin >> qrs[i].a >> qrs[i].b;
		qrs[i].idx = i;
		qrs[i].l = 1, qrs[i].r = m + 1;
	}
	dsu cur;
	int lg = ceil(log2(m));
	while (lg--) {
		cur.init(n);
		sort(qrs, qrs + q,
		     [](const pbs &x, const pbs &y) { return x.mid() < y.mid();
                });
		int idx = 0;
		for (int i = 0; i < m; i++) {
			cur.link(edges[i].first, edges[i].second);
			while (idx < q && qrs[idx].mid() == i + 1) {
				if (qrs[idx].l >= qrs[idx].r) {
					idx++;
					continue;
				}
				if (cur.root(qrs[idx].a) == cur.root(qrs[idx].b))
					qrs[idx].r = qrs[idx].mid();
				else qrs[idx].l = qrs[idx].mid() + 1;
				idx++;
			}
			if (idx == q) break;
		}
	}
	sort(qrs, qrs + q,
	     [](const pbs &x, const pbs &y) { return x.idx < y.idx; });
	for (int i = 0; i < q; i++)
		cout << (qrs[i].l == m + 1 ? -1 : (qrs[i].a == qrs[i].b ? 0 : qrs[i].l))
		     << '\n';
}
```

</CPPSection>

</LanguageSection>

## Problems

<Problems problems="gen" />
