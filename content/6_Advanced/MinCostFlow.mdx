...
$u,v,w$, the following holds: $w \ge \pi(u)-\pi(v)$. Then, each edge weight can be
...
Since there are $J$ iterations (for each job) and each iteration takes $\mathcal{O}(W^2)$
time, the overall time complexity is $\mathcal{O}(J W^2)$.
