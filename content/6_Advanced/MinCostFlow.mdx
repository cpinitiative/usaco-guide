...
The assignment problem is best understood as a MCMF problem. In our formulation,
we will assign $W$ workers to $J$ jobs, $J \ge W$, and the cost of assigning the
$i$th job to the $j$th worker is $A_{i,j}$.
...
- $(J_i, W_j)$ with $(1, A_{i,j})$
- $(W_j, E)$ with $(1, 0)$

The answer will be the minimum cost of the max flow of the graph.

To solve this, we will use the MCMF algorithm detailed above. In every iteration
of the algorithm, we will assign a job to a worker. In each iteration, we will
first assign the job to an auxilary worker, then we will try to find an
augmenting path with minimum cost. To optimize this, we will only run Dijkstra
on the worker nodes. For details, refer to the code.
		...
		while (job[w_cur] != -1) {   // Dijkstra step: pop min worker from heap
			...
			for (int w = 0; w < W; w++) {
				if (!vis[w]) {
					...
					if (ckmin(dist[w], dist[w_cur] + edge)) {
                                                prv[w] = w_cur;
                                        }
					if (ckmin(min_dist, dist[w])) {
                                                w_next = w;
                                        }
				}
			}
...
More hungarian algorithm problems
