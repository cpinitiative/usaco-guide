...
To sort static arrays, use `sort(arr, arr + N)` where $N$ is the number of elements to be sorted. Range can also be specified by replacing `arr` and `arr + N` with the intended range.
...
In order to sort a dynamic array, use `sort(v.begin(), v.end())` (or `sort(begin(v),end(v))`). The default sort function sorts the array in ascending order.
