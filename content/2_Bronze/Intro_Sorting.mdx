...
<Resources>
	<Resource source = "YouTube" title = "Sorting and Searching" url = "https://www.youtube.com/watch?v=ZhDWGaVJdOw&list=PLtLxhThTxJMVMvWJoE2p8Z84tA8suGzvn&index=12">
		A video source that demonstrates how to sort an array in C++, Java, and Python. Start at time 0:49 and end at 5:42.
	</Resource>
</Resources>
...
In order to sort a dynamic array, use `sort(v.begin(), v.end())` (or `sort(begin(v),end(v))`). The default sort function sorts the array in ascending order. Similarly, we can specify the range. For example, `sort(v.begin() + 1, v.end() + 1)` sorts indices $[1, 4)$.
...
Note: There are some more sorting problems in the [Intro to Sets](/bronze/intro-sets) module.
