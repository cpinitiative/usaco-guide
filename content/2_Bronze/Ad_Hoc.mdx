...
- missing the observation the matching loop rests on: relabel the cows so that
  the target order is $1 \dots N$, and then cow $a_j$ must be moved exactly when
  some earlier $a_i$ has $a_i > a_j$, so the answer counts the elements that are
  not a prefix maximum
- missing a proof that no cow outside that count needs to be moved
