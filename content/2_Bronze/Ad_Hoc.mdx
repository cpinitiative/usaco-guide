...
for i in range(k):
	...
	if cow == 1:  # already fixed, nothing we can do
		...
		exit(0)
"""
returns a boolean that represents whether it is
possible to construct a valid ordering with given
fixed elements
"""
...
Save New Duplicate &Edit Just Text Twitter import java.io.*;
import java.util.*;
