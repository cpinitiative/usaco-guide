...
If the element is at the target position, we simply move forward. Otherwise, if it is behind the current pointer, we need to move it manually and mark it as moved.
