...
An alternative for variable-length arrays are "doubly-linked lists." They are implemented by "linking" or pointing each element to the element before and after it. We recommend you look at [this Wikipedia article](https://en.wikipedia.org/wiki/Doubly_linked_list) for more information. These support $\mathcal{O}(1)$ insertion and removal, if you have access to the pointer of the element beforehand.
...
Notice how you can create infinite loops with doubly-linked lists. In fact, doubly-linked lists are a great way to represent functional graphs. You can read more about functional graphs [here.](https://usaco.guide/silver/func-graphs)
