...
contributors: Nathan Wang, Abutalib Namazov, Aadit Ambadkar
description:
...
Essentially, every standard library data structure supports the `size()` method,
which returns the number of elements in the data structure, and the `empty()`
...
Technically, you can solve all Bronze problems without using anything from this
module aside from arrays. So the rest of this module isn't strictly necessary
for Bronze (although highly recommended).
...
Another type of datastructure is a "doubly-linked list." They are implemented by "linking" or pointing each element to the element before and after it. We recommend you look at [this Wikipedia article](https://en.wikipedia.org/wiki/Doubly_linked_list) for more information. These support $\mathcal{O}(1)$ insertion and removal if you have access to the iterator of the element to insert before or remove beforehand.
	
<Warning>
	Doubly-Linked lists cannot be accessed by indexes. This is a major reason why they are not used, as opposed to vectors or regular arrays.
</Warning>
	
The C++ object for a doubly-linked list is the `list` object.

```cpp
list<int> li
```

Alternatively, you could implement such a list on your own:
...
```
A sample implementation of this is provided below:
...
Notice how you can create infinite loops with doubly-linked lists. Doubly-linked lists can be used to represent functional graphs. You can read more about functional graphs [here.](https://usaco.guide/silver/func-graphs)
...
<Optional title="Additional Notes About C++ Data Structures">

See [here](/general/notes-cpp-ds).
