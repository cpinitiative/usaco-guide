...
export const problems = {
    bubble: [
      new Problem("HR", "Bubble Sort", "https://www.hackerrank.com/challenges/ctci-bubble-sort/problem", "Very Easy", false, [], "Implement the Bubble Sort algorithm. Read any of the resources if you don't know how to do that, or just google."),
      ],
    easy: [
      new Problem("CSES", "Distinct Numbers", "1621", "Easy", false, [], "cses-1621"),
      new Problem("CF", "Kayaking", "contest/863/problem/B", "Easy", false, [], "Loop over all pairs of people too put in the single Kayaks. Then, sort the remaining people and pair $a_i$ with $a_{i + 1}$ because this minimizes the sum of absolute values (draw some cases if you're confused why)."),
    ],
};
...
The default way to store data in python is using a list, which can simply be initialized as follows:

```py
list = []
```

Python lists are generic. This means that they can store any kind of data type, including objects.

Lists support many useful methods which will be explored further in the section below.
...
In C++ and Python, we can give a dynamic array an initial size. The code below creates a dynamic array with $30$ zeroes. 
...
Note that pre-allocation makes very little difference in python, but the equivalent is as follows:
...
Although you do **not** need to know how sorting is implemented for Bronze, you should know how to use built-in methods to sort a (possibly dynamic) array.
