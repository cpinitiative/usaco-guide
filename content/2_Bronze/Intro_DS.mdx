...
You already know one of the simplest data structures: the **array**! 

<LanguageSection>

<CPPSection>

In C++11, in addition to *normal arrays*, there exists an [`array` class](http://www.cplusplus.com/reference/array/array/) in the STL.
For example, an `array` of `int`s can be initialized using the following line of code:
...
We can also use **iterator**s. An iterator allows you to traverse a container by pointing to an object within the container. However, they are **not** the same thing as pointers.

For example, `v.begin()` returns an iterator pointing to the first element of the vector `v`. Apart from the standard way of traversing a vector (by treating it as an array), you can also use iterators:
