...
Every standard library data structure supports the `size()` method, which
...
You can solve all Bronze problems without using anything from this module aside
...
<Quiz>
  <Quiz.Question>
    ...
    <Quiz.Answer>
      `v.length()`
      <Quiz.Explanation>
        Incorrect. The property `length()` does not exist on an `std::vector`
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      `v.count()` 
      <Quiz.Explanation>
        Incorrect. The property `count()` does not exist on an `std::vector`
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      `v.resize()`
      <Quiz.Explanation>
        Incorrect. The property `resize()` resizes the vector to the specified number of elements
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
  <Quiz.Question>
    Which of the following would print out `5`?
    <Quiz.Answer correct>
      ```cpp
      int a = 3, b = 4, c = 5;
      tuple<int, int, int> t = tie(a, b, c);
  ...
  <Quiz.Question>
    ...
    <Quiz.Answer correct>
      Correct. Data structures can store different types of data.
      <Quiz.Explanation>
...
<Quiz>
  <Quiz.Question>
    ...
    <Quiz.Answer>
      `list.length`
      <Quiz.Explanation>
        Incorrect. The property `length` exists in arrays, but not `ArrayList`s
      </Quiz.Explanation>
    </Quiz.Answer>
  </Quiz.Question>
  <Quiz.Question>
