...
Therefore, it is correct to say that the time complexity of a linear function, $\mathcal{O}(n)$, is $\mathcal{O}(n/2)$, $\mathcal{O}(2n)$, $\mathcal{O}(n^2)$, $\mathcal{O}(2^n)$, $\mathcal{O}(n^n)$, etc.


But typically, we write the simplest function out of those that are the most restrictive, thus, $\mathcal{O}(n)$.


<Optional title="P vs. NP">

P refers to the class of problems that can be solved within polynomial time. (ie. $\mathcal{O}(n^2)$, $\mathcal{O}(n^3)$, $\mathcal{O}(n^100)$...)
NP, known as nondeterministic polynomial, is the set of problems with solutions that can be verified in polynomial time. In simple words, a nondeterministic turing machine can 
guess the solution which is then verified by polynomial time methods.

A common example of NP is the game of Sudoku, where a solution is easily verifiable, but not easily computable (exponentially slower as grid size increases), though 
there may exist a solution that does solve the solution under polynomial time.

“P vs NP” is the classic problem that asks whether every problem that can be verified in polynomial time can also be solved. This question is currently unsolved. 
