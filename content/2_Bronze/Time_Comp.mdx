...
Let $f$ and $g$ be non-negative functions from $\mathbb{R}_{\ge 0}$ to $\mathbb{R}_{\ge 0}$. If there exists
positive constants $n_0$ and $c$ such that $\texttt{f}(n) \le c \cdot \texttt{g}(n)$ whenever $n \ge n_0$, we
say that $\texttt{f}(n) = \mathcal{O}(\texttt{g}(n))$.

Therefore, it is correct to say that the time complexity of a linear function, $\mathcal{O}(N)$ is $\mathcal{O}(N^2)$, $\mathcal{O}(2^N)$, $\mathcal{O}(N^N)$, etc.

But typically, we write the most restrictive function, thus, $\mathcal{O}(N)$.
...
NP, known as nondeterministic polynomial, is the set of problems with solutions that can be verified in polynomial time, but not 
solved in polynomial time.

A common example of NP is the game of Sudoku, where a solution is easily verifiable, but not easily computable (exponentially slower as grid size increases).

“P = NP” is the classic problem that determines whether a problem that can be verified in polynomial time can also be solved 
in polynomial time.
