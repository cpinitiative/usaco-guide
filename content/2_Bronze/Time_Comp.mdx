...
NP, short for nondeterministic polynomial time, is the set of problems with solutions that can be verified in polynomial time. In other words, a nondeterministic turing machine can guess a solution, which can then be verified by polynomial time methods.
