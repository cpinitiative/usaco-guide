...
<Resources>
	...
	<Resource source="PAPS1" title="5 - Time Complexity">
...
<!-- For example, if our code currently runs in $\mathcal{O}(n^2)$ time, perhaps we can modify our code to make it run in $\mathcal{O}(n^2/32)$ by using a bitset. (Of course, with Big O notation, $\mathcal{O}(n^2) = \mathcal{O}(n^2/32)$.) -->
