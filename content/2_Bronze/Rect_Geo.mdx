...
The formula for finding the area of an individual rectangle is $w \cdot l$, so we can just retrieve these lengths with our coordinates. Below, $tr_x$ and $tr_y$ represent the top right $x$ and $y$ coordinates, and $bl_x$ and $bl_y$ represent the bottom left $x$ and $y$ coordinates.
 	
1. $\texttt{width} = tr_x - bl_x$
2. $\texttt{length} = tr_y - bl_y$
3. $\texttt{area} = w \cdot l$
...
Rectangles can be represented by $2$ points each, the top right and the bottom left. We'll call these points $tr_a$ and $tr_b$, and $bl_a$ and $bl_b$, for rectangles $a$ and $b$. 
