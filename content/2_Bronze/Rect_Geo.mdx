...
author: Darren Yao, Michael Cao, Benjamin Qi, Allen Li, Andrew Wang
...
export const problems = {
	...
	sam: [new Problem('Bronze', 'Blocked Billboard', '759', 'Easy', false, ['rect']),],
	general: [
...
Since all the intervals lie between the range, $[0, 100]$, we can just mark each point in the interval as painted and count the number of marked points.
...
int main() {
    ...
    for (int i=0; i<=100; i++) painted[i] = false; 
...
It's possible to just simply calculate the answer by adding the original length minus the overlapping length.

$(b-a)+(d-c)+interception(a,b,c,d)$

The following solution demostrates this $O(1)$ solution.
...
int main() {
    ...
    int ans = 0;
    ans += (b-a); // add area of first interval
    ans += (d-c); // add area of second interval
    if (intersect(a, b, c, d)) { // if it intersects we have to subtract the intersecting area
        ans -= min(b, d)-max(a, c);
    } 
