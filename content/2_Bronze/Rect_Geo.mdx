...
int main() {
	...
	int union = (b - a) + (d - c);  // the sum of the two intervals
	int intersection = max(min(b, d) - max(a, c), 0);  // subtract the intersection
	int total = union - intersection;
...
union = (b - a) + (d - c)  # the sum of the two intervals
...
Code runs marginally faster in Python when placed in a function, so we can use
this to get all 10 test cases (When taken out of the function, the code passes
only 9).
