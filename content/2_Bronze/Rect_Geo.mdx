...
{/* Unfortunately, C++ doesn't have a built in rectangle `struct` or `class`, so
you need to write the functions yourself. Here is the code from the official
analysis (modified slightly): */}
...
{/* Unfortunately, Python doesn't have a built in rectangle class, so you need
