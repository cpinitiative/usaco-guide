...
<Info title="To Code or Not To Code">
Since we're just discussing formulas here, we won't focus on the code,
though each of these should be easy to implement in each language.
</Info>

### Finding area
	
The formula for finding the area of an individual rectangle is $w \cdot l$, so we can just retrieve these lengths with our coordinates. 
...
First, we notice that rectangles can be represented by $2$ points each, the top left and 
the bottom right. We'll call these points $tr_a$ and $tr_b$, and $bl_a$ and $bl_b$, for rectangles 
$a$ and $b$. 
	
There are only two cases where the rectangles do not intersect: 

1. $tr_{a_y}$ $\leq$ $bl_{b_y}$ or $bl_{a_y}$ $\geq$ $tr_{b_y}$.  
2. $bl_{a_x}$ $\geq$ $tr_{b_x}$ or $tr_{a_x}$ $\leq$ $bl_{b_x}$. 
...
<LanguageSection>
	...
	<PySection>
		...
		def intersect(s1, s2):
			...
			or bl_a_y >= tr_b_y or tr_a_y or bl_b_y):
				return False
			else:
				return True
		```
...
First, we'll find this rectangle's $x$ and $y$ coordinates. 
$x$ = min($tr_{a_x}$, $tr_{b_x}$) - max($bl_{a_x}$, $bl_{b_x}$). 
$y$ = min($tr_{a_y}$, $tr_{b_y}$) - max($bl_{a_y}$, $bl_{b_y}$).
...
Multiply the $x$ and $y$ values together to find the overlapping area. 
