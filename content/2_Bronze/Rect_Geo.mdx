...
Rectangles can be represented by $2$ points each, the top right and the bottom left. We'll call these points $tr_a$ and $tr_b$, and $bl_a$ and $bl_b$, for rectangles $a$ and $b$. In examples with just one rectangle, we'll omit it to $tr_x$ and $tr_y$.
</Info>
