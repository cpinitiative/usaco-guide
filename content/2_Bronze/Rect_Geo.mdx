...
public class Paint {
	...
	public static void main(String[] args) throws IOException {
		...
		int total = 0;
		for (boolean i : painted) { total += i ? 1 : 0; } // Count the total amount of intervals painted
...
for i in range(a, b):
	painted[i] = True # Add Farmer John's painted interval
for i in range(c, d):
	painted[i] = True # Add Bessie's painted interval
...
int main() {
	...
	for (int i = 0; i < 3; ++i) {
		...
		y2 += MAX_POS / 2;
		for (int x = x1; x < x2; x++) {
			for (int y = y1; y < y2; y++) { visible[x][y] = i < 2; } // Mark billboard area as visible, truck as not visible
		}
	}

	int ans = 0;
	for (int x = 0; x < MAX_POS; x++) {
		for (int y = 0; y < MAX_POS; y++) { ans += visible[x][y]; } // Count all visible billboard squares
	}
	cout << ans << "\n";
}
```
...
public class Billboard {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 3; ++i) {
			...
			y2 += MAX_POS / 2;
			for (int x = x1; x < x2; x++) {
				for (int y = y1; y < y2; y++) { visible[x][y] = i < 2 ? 1 : 0; } // Mark billboard area as visible, truck as not visible
			}
		}

		int ans = 0;
		for (int x = 0; x < 2000; ++x) {
			for (int y = 0; y < 2000; ++y) { ans += visible[x][y]; } // Count all visible billboard squares
		}
	...
	for i in range(3):
		...
		for x in range(x1, x2):
			for y in range(y1, y2):
				visible[x][y] = i < 2 # Mark billboard area as visible, truck as not visible

	ans = 0
	for x in range(MAX_POS):
		for y in range(MAX_POS):
			ans += visible[x][y] # Count all visible billboard squares
	print(ans)
...
print(
	...
	- intersect(rects[1], rects[2]) # Subtract area of second billboard covered by truck
)
```
