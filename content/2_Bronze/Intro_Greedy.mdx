...
int main() {
    ...
    cin >> A >> B;
    ll ans = 0;
    bool mismatched = false; // Tracks if we are currently in a mismatched segment
    for (ll i = 0; i < n; i++) {
        // If the current cows are different, we are in a mismatched segment
        if (A[i] != B[i]) {
            // If we just entered a new mismatched segment, increment the answer
            if (!mismatched) {
                mismatched = true;
                ans++;
            }
        } else {
            // If cows match, we are not in a mismatched segment anymore
            mismatched = false;
        }
    }
    // Output the number of mismatched segments, which is the minimum number of flips needed
    cout << ans << endl;
}
```
...
public class Solution {
    public static void main(String[] args) throws IOException {
        ...
        int ret = 0;
        // Repeat until both strings are equal
        while (!new String(a).equals(new String(b))) {
            ...
            int rhs = n - 1;
            // Find the last position where a and b differ
            while (a[rhs] == b[rhs]) rhs--;
            // Flip the entire segment from lhs to rhs (inclusive)
            for (int i = lhs; i <= rhs; i++) {
                if (a[i] == 'G') a[i] = 'H';
                else a[i] = 'G';
            }
        }
        // Output the number of flips performed
        io.println(ret);
        io.close();
    }
...
# Build a list where each element is True if the cows at that position differ, False otherwise
s = [False] + [x != y for x, y in zip(a, b)]  # difference list
# Count the number of times a new mismatched segment starts (i.e., [False, True] pairs)
