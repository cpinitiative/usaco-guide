...
description: 'Sequence containers representing arrays that can change in size.'
frequency: 1
prerequisites:
  - intro-sorting
  - basic-cpp-understanding
---
...
C++ contains a built in data structure called a vector, which is a sequence container representing an array that can change in size. They use contiguous storage locations for their elements, which means that their elements can also be accessed using offsets on regular pointers to its elements, and just as efficiently as in arrays.
...
The operations on a C++ [`vector`](http://www.cplusplus.com/reference/set/set/) include:
...
Additional functions that sets support are discussed in the [Silver module](/silver/intro-ordered).
...
Insertions, deletions, and searches are all $\mathcal{O}(\log N)$, where $N$ is the number of elements in the map.
...
Colloquially, maps are referred to as **dicts** in python. They act as hash maps, so they actually have $\mathcal{O}(1)$ insertion, deletion, and searches.
...
To iterate over `dict`s, there are three options. Dicts will be returned in the same order of insertion in [Python 3.6+](https://stackoverflow.com/questions/39980323/are-dictionaries-ordered-in-python-3-6). You can iterate over the keys:
...
While you are free to change the _values_ in a map when iterating over it (as demonstrated above), be careful about inserting and deleting keys while iterating.

<LanguageSection>

<PySection>

This code will give a runtime error (although similar code _will_ create a `map` with 11 entries in C++):
...
If we compile using [`-D_GLIBCXX_DEBUG`](/general/debugging-lang/#-d_glibcxx_debug) and run the above, then
