...
export const problems = {
    probs: [
      new Problem("Bronze", "Diamond Collector", "639", "Easy", false, ["Nested Loop"], "Loop over all diamonds as the diamond with minimum size, and then take all the diamonds with size no more than K greater. Find the maximum over all possibilities."), // 99.9
      new Problem("Bronze", "Milk Pails", "615", "Easy", false, ["Nested Loop"], "Loop over the number of first-type operations you perform, and keep performing the second operation until you overflow the bucket."), // 99.71
      new Problem("Bronze", "Bovine Genomics", "736", "Normal", false, ["Nested Loop"], "Loop over each position. Then, loop over each non-spotty cow in a nested loop and check whether a spotty cow has the same character as a non-spotty cow to figure out whether the position explains spottiness."), // 99.73
      new Problem("Bronze", "Cow Gymnastics", "963", "Normal", false, ["Nested Loop"], "Loop over all pairs of cows, $(a, b)$, and check if cow $a$ did better than cow $b$ in all K practice sessions."), // 99.93
      new Problem("Bronze", "Where Am I?", "964", "Normal", false, [], "Loop over all $K$, and for each $K$, loop over all pairs of substrings of length $K$ and check whether they are equal."), // 99.48
      new Problem("Bronze", "Triangles", "1011", "Normal", false, [], "Brute force all possible right triangles by looping over all triples of points and checking whether they form a right triangle. If so, compute the area and find the maximum over all right triangles."), // 98.02
      new Problem("Bronze", "Lifeguards", "784", "Normal", false, [], "Loop over every lifeguard and try to fire each one. Then, compute the time still covered by the remaining lifeguards and add the lifeguard you fired back. Over all possible lifeguards to fire, take the maximum time still covered."), // 98.76
      new Problem("Bronze", "Circle Cross", "712", "Hard", false, [], "Two intervals $a_1, a_2$ and $b_1, b_2$ cross if min($a_2$, $b_2$) $\geq$ max($a_1, b_1$). Loop over each pair of characters and check if they cross. You may need to divide by $2$ to deal with overcounting, if your program counts $a_1, a_2$ crossing with $b_1, b_2$ and $b_1, b_2$ crossing with $a_1, a_2$."), // 89.6
      new Problem("Bronze", "Load Balancing", "617", "Very Hard", false, [], "Observe that a lot of placements of vertical or horizontal fences are equivalent. Specifically, a fence should be adjacent to a cow, so loop over all $N ^ 2$ unique placements of a vertical and horizontal fence. This idea is similar to Coordinate Compression."), // 82.79
      new Problem("Bronze", "Bull in a China Shop", "640", "Very Hard", false, [], "Brute force all possible pairs of figurines and all possible ways to shift vertically / horizontally. This requires $6$ nested for loops, and is not very fun to implement. Good luck :)"), // 47.38
    ],
};
...
compare difficulty / quality of these problems with those from sources mentioned in "Expected Knowledge"? (../intro/expected) ex. VPlanet, CodeSignal?
