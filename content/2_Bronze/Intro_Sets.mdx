...
deletions, and searches are all $\mathcal{O}(1)$. Unordered sets are implemented
by
...
[`std:set`](https://www.cplusplus.com/reference/set/set/) in the `<set>`
header.
...
with `std::map` and unordered maps are implemented with `std::unordered_map`.
