...
To iterate over maps, you can use a simple loop.
...
The values in the map are pairs of (key, value). We use the `auto` keyword so you can iterate over any type of key and value in the pairs. You can use these pairs normally, as introduced in [this module](https://usaco-guide.vercel.app/bronze/intro-ds#pairs--tuples).
...
When passing by value (the first example), you can only access the (key, value) pairs. When you pass by reference (second example), if you modify the values of the pairs you iterate through, it changes in the actual map as well.
