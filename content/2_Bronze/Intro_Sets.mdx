...
- the `m[key] = value` operator assigns a value to a key and places the key and
  ...
  Please read [this article](https://stackoverflow.com/questions/10124679/what-happens-if-i-read-a-maps-value-where-the-key-does-not-exist)
  to find out why `m[key]` is set to 0.
...
<Quiz>
  <Quiz.Question>
    What is the time complexity of insertions, deletions, and searches in an ordered set of size $N$?
    <Quiz.Answer>
      $\mathcal{O}(1)$
    </Quiz.Answer>
    <Quiz.Answer>
  ...
  <Quiz.Question>
    ...
    <Quiz.Answer correct>
      $0$
      <Quiz.Explanation>
        The element $3$ isn't contained in $s$, so the output should be $0$, since ```s.count(x)``` is supposed to output $1$ when the $x$ is present in the set and $0$ when $x$ isn't present in the set.
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      $-1$
...
```
  ...
  <Quiz.Question>
    ...
    <Quiz.Answer correct>
      $0$
      <Quiz.Explanation>
        After ```m.erase(2)``` is called, there is no longer a value associated with the key $2$, meaning ```m[2]``` is $0$. Please read [this article](https://stackoverflow.com/questions/10124679/what-happens-if-i-read-a-maps-value-where-the-key-does-not-exist) to understand why ```m[2]``` would be $0$ in this case.
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      $9$
  ...
  <Quiz.Question>
    ...
    <Quiz.Answer correct>
      $3$
      <Quiz.Explanation>
        `m[2]` is set to $5$ at first, but then later on it is changed to $3$. So, the answer is $3$.
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      $5$
