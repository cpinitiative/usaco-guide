...
Modifying a Collection (`Set`, `Map`, etc.) in the middle of a while-loop is unlikely to work, as it will probably cause a [ConcurrentModificationException](https://docs.oracle.com/javase/7/docs/api/java/util/ConcurrentModificationException.html). See the following snippet for an example:
...
In some cases it would be better to use `Iterator` and the `.remove()` method to remove elements in a set, like in the next code snippet:
