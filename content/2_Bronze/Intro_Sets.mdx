...
Some possible operations on a C++
[`std::set`](http://www.cplusplus.com/reference/set/set/) named `s` include:
...
Some possible operations on a `TreeSet` named `set` include:
...
deletion, and searches. Some possible operations on a Python `set` named `s`
include:
...
Some possible operations on a C++
[`std::map`](http://www.cplusplus.com/reference/map/map/) named `m` include:
...
The map stores entries as [pairs](/bronze/intro-ds#pairs--tuples) in the form
`{key, value}`. To iterate over maps, you can use a `for` loop. The `auto`
...
The first method is generally preferred over the second because the second will
make a copy of each element that it iterates over.

Additionally, you can pass by reference when iterating over a map, allowing you
to modify the values (but not the keys) of the pairs stored in the map:
...
<Quiz>
  ...
  <Quiz.Question>
    ...
    <Quiz.Answer correct>
      $0$
      <Quiz.Explanation>
        Recall that `s.count(x)` outputs $1$ when $x$ is present in `s` and $0$ otherwise. As $3$ isn't contained in $s$, the output should be $0$. 
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      $-1$
  ...
  <Quiz.Question>
    ...
    <Quiz.Answer correct>
      $4$
      <Quiz.Explanation>
        There are a total of $4$ unique numbers in $s$, so the answer is $4$.
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      $7$
