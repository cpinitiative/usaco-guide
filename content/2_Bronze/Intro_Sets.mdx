...
and the other using **hashing**. For Python, we'll introduce the built-in sets
and maps but keep in mind that they do _not_ store elements in sorted order.
...
[`std::set`](http://www.cplusplus.com/reference/set/set/) implements sorted sets
in C++. Some operations on an `std::set` named `s` include:
...
In _unordered_ sets, elements are stored in a random order. Insertions,
deletions, and searches are all $\mathcal{O}(1)$, where $N$ is the number of
elements in the set. Unordered sets are implemented in Java by the `HashSet`
class.
...
additional ones. Some useful ones are listed below (all $\mathcal{O}(\log N)$):

- `first()`, which returns the smallest element in the set and `last()`, which
  returns the largest element.
- `lower(x)`, which returns the largest element strictly smaller than `x` and
  `higher(x)`, which returns the smallest element strictly larger than `x`.
- `floor(x)`, which returns the largest element that's less than or equal to `x`
  and `ceiling(x)`, which returns the smallest element that's greater than or
  equal to `x`.

You can also iterate through a `TreeSet` in sorted order using a for-each loop.
...
System.out.println(set.higher(4)); // null
System.out.println(set.ceiling(4)); // 4
System.out.println(set.floor(3)); // 2

// You can iterate through a TreeSet set in sorted order
