...
```
*We use `std::map` because it can handle very large keys efficiently and only stores keys that are set. Accessing a key not present in the map returns `0` by default for integer values.*
...
public class Main {
	public static void main(String[] args) throws IOException {
		...
		HashMap<Long, Long> a = new HashMap<Long, Long>(); // Map to store assigned indices
...
```
*We use `HashMap<Long, Long>` to efficiently store and retrieve only the indices that are set. The `getOrDefault` method returns `0` if the key is not present.*
...
```
*We use a Python dictionary to store only the indices that are set. The `get` method returns `0` if the key is not present.*
