...
`unordered_set` actually has worst-case $\mathcal{O}(N)$ behavior, and it's
possible to construct a test case for Distinct Numbers such that a program
that uses `unordered_set` times out.
