...
[custom hash function](/gold/hashmaps#custom-hashing) for structures/classes
like vectors and pairs.

<Warning title="Unordered Set Performance">

`unordered_set` actually has worst-case $\mathcal{O}(N)$ behavior, and a program
that uses it times out on Distinct Numbers. For more on unordered maps and sets,
check out [this module](/gold/unordered).
...
Refer to the [More Operations on Sorted Sets](/silver/intro-sorted-sets) module
for more detail.
...
a [custom hash function](/gold/hashmaps#custom-hashing) for custom classes.
...
[More Operations on Sorted Sets](/silver/intro-sorted-sets) module for more
detail.
...
to construct test cases where Python `set`s and `dict`s are extremely slow. See
[this CF post](https://codeforces.com/blog/entry/101817) for more information.
...
No - historically, no USACO problem has included an anti-hash test. However,
these sorts of tests often appear in Codeforces, especially in educational
rounds, where open hacking is allowed.
...
It's also possible to change the values while iterating over the keys (or over
the values themselves, if they're mutable):
...
To iterate over `dict`s, there are three options, all of which involve for
loops. Dicts will be returned in the same order of insertion in
[Python 3.6+](https://stackoverflow.com/questions/39980323). You can iterate
over the keys:
...
`unordered_set` actually has worst-case $\mathcal{O}(N)$ behavior, and
a program that uses it times out on Distinct Numbers.
For more on unordered maps and sets, check out [this module](/gold/unordered).
...
<Quiz>
	...
	<Quiz.Question>
		Which of the following is the correct output of this code?
		```cpp
		s = set() s.add(1) s.add(4) s.add(5) s.add(2) print(3 in s)
		```
		<Quiz.Answer>
			$1$
			<Quiz.Explanation>
...
Incorrect. The output is a `bool`.
	...
	<Quiz.Question>
		Which of the following is the correct output of this code?
		```cpp
		m = dict() m[0] = 1 m[2] = 5 m[3] = 18 m[2] = 3 m[4] = 14 print(m[2])
		```
		<Quiz.Answer>
			$1$
			<Quiz.Explanation>
