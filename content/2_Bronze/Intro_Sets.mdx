...
- the `m[key] = value` operator assigns a value to a key and places the key and
  ...
  Note that `m[key]` is set to 0 because 0 is the default value for when the $m$
  doesn't contain $key$ or when `m[key]` is null. 
  [Here](https://stackoverflow.com/questions/10124679/what-happens-if-i-read-a-maps-value-where-the-key-does-not-exist)
  is a more detailed explanation as to why `m[key]` is set to 0.
...
<Quiz>
  ...
  <Quiz.Question>
    What is the time complexity of insertions, deletions, and removals in an ordered map of size $N$?
    <Quiz.Answer>
      $\mathcal{O}(1)$
    </Quiz.Answer>
    <Quiz.Answer>
  ...
  <Quiz.Question>
    ...
    <Quiz.Answer correct>
      $0$
      <Quiz.Explanation>
        After ```m.erase(2)``` is called, there is no longer a value associated with the key $2$, meaning ```m[2]``` is $0$.
      </Quiz.Explanation>
    </Quiz.Answer>
    <Quiz.Answer>
      $9$
