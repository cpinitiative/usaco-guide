...
(huh? isn't there solution code above and explanation of bin expo) - michael

-->

### Modular Inverse with Exponentiation

The **modular inverse** is the equivalent of the reciprocal in real-number arithmetic; to divide $a$ by $b$, multiply $a$ by the modular inverse of $b$. We'll only consider **prime** moduli here.
...
<FocusProblem problem={problems.dp[3]} />
...
[**Fermat's Little Theorem**](https://en.wikipedia.org/wiki/Fermat%27s_little_theorem) (not to be confused with Fermat's **Last** Theorem) states that for any **prime number** $p$, all integers $a$ not divisible by $p$ satisfy $a^{p - 1} \equiv 1 \pmod{p}$. Consequently, $a^{p-2} \cdot a \equiv 1 \pmod{p}$. Therefore, $a^{p - 2}$ is a modular inverse of $a$ modulo $p$.
...
### Permutations and LCM

Let's define the order of permutation $p$ as the number of times you need to apply $p$ to itself to get the identity permutation, or the permutation consisting of integers $1 \cdots N$ where $N$ is the length of permutation $p$.
