...
$a \& b$ is just the carry bits themselves, since a bit's $1$ only if it's a $1$ in both $a$ and $b$, which is exactly what we need. We multiply this by $2$ to shift all the bits to the left by one, so every value carries over to the next bit.
...
int main() {
	...
	for (int i = 4; i <= n; i++) { a.push_back(sum(i - 1, i) - a.back()); }
	sort(a.begin(), a.end());
