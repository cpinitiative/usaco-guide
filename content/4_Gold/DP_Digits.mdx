...
<CPPSection>
```cpp
// Code: https://raw.githubusercontent.com/thecodingwizard/competitive-programming/master/USACO/2014open/silver/odometer.cpp
...
ll solveDP(int loc, int k, bool largest, bool started, int targ, int targ2){
	if(loc == num.size()){
		if (!started) return 0;
		if (targ2 != -1) {
			if (k == 20) return 1;
	...
	for(int i = 0; i <= 9; i++){
		...
		ans += solveDP(loc + 1, k + (isStarted ? (targ == i ? 1 : -1) : 0), isLargest, isStarted, targ, targ2);
	}
...
ll solve(ll x){
	...
	for(int i = 0; i <= 9; i++){
		reset();
		ans += solveDP(0, 20, true, false, i, -1);
	}
	ll dup = 0;
	for(int i = 0; i <= 9; i++){
		for(int j = 0; j <= 9; j++){
			reset();
			dup += solveDP(0, 20, true, false, i, j); 
		}
	}
	return ans - (dup / 2);
}
