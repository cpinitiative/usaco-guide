...
For now, let's assume that we want to find the minimum of our function. Given that 
our function is unimodal, this means that the function **strictly decreases** first,
reaches its minimum, then **strictly increases**.
...
> 
> Thus, we can conclude that $m_1$ remains on the strictly decreasing section of our function. Because of this, we can conclude that our mode will not be in the range $[l, m_1)$, so we can eliminate this section from our search space.
...
```cpp 
template <typename F>
double find_min(double l, double r, const F &f) {
...
```cpp 
template <typename F>
int find_min(int l, int r, const F &f) {
    while (r - l > 3) {
        int m1 = l + (r - l) / 3;
        int m2 = r - (r - l) / 3;
        f(m1) > f(m2) ? l = m1 : r = m2;
    ...
    int res = INT_MIN;
    for (int i = l; i <= r; i++) { res = std::min(res, f(i)); }
...
is often a better option, as it's shorter and requires less call to our function 
$f(x)$.
...
int main() {
    ...
    const auto pref_sum = [&](int l, int r) -> ll {
...
This is because we can only allow our function to be flat at the mode! If we have 
