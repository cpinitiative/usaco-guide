...
void dijkstra(int src) { // Source and destination
	for (int i = 0; i < N; ++i) dist[i] = LLONG_MAX; // Set all distances to infinity
	...
	while (pq.size()) {
		ll dis, node; tie(dis, node) = pq.top();
		pq.pop(); dis *= -1;
...
It's important to include the line `if (dis != dist[node]) continue;`. This ensures that when all edge weights are non-negative, we will never go through the adjacency list of any vertex more than once. Removing it will cause TLE on the last test of the above problem!
...
last problem involves finding a negative cycle. Should SSSP negative be introduced earlier?
