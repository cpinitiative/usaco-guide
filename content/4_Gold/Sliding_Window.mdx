...
struct MaxQueue {
	...
	/*
	 * Remove the first element from our MaxQueue by poping the top element from
	 * s2. Since elements in s1 are stored in the order of how we added them, i.e.
	 * the last added element is at the top of s1, we just have to pop all of them
	 * out and push them into the stack s2. After that, these elements in s2 will
	 * be in reversed order, i.e. the first added element will be at the top of s2
	 * and we can pop them out as from normal stacks. We only move elements from
	 * s1 to s2 when s2 is empty.
	 */
	void dequeue() {
		if (s2.empty()) {
