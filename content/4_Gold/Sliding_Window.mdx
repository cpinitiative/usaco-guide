...
We can keep a pointer to the left boundary of the window and expand the right
boundary until we find a song that has already been accounted for. The song can
be quickly looked up with a set. 
...
for (int e : nums) {
	while (!increasing.empty() && increasing.back() > e) {
		increasing.pop_back();
	}
...
// increasing should be [1, 2, 3, 4]

for (int e : nums) {
	while (!decreasing.empty() && decreasing.back() < e) {
		decreasing.pop_back();
	}
...
vector<int> maxSlidingWindow(vector<int>& nums, int k) {
	...
	for (int i = 0; i < nums.size(); i++) {
		...
		if (i >= k - 1 && !d.empty() && d.front() == nums[i - k + 1]) {
			d.pop_front();
		}
	}
