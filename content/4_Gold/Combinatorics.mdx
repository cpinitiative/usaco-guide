...
The [Catalan numbers](https://en.wikipedia.org/wiki/Catalan_number) are a sequel of positive integers that can be very useful in counting problems in combinatorics. Some of the well-known applications are: count certain types of lattice path in a grid, counting binary trees, number of valid paranthesis strings of length $n$, and many more. The $n$-th Catalan can be expressed as following with respect to the central binomial coeffiecients:

$$
C_n=\frac{1}{n+1}\cdot \binom{2n}{n}=\frac{(2n)!}{(n+1)!\,n!}
$$

The Catalan numbers has the recurrence formula:
...
The first $5$ Catalan values are:
...
- Number of valid paranthesis epressions

The number of valid strings paranthesis expresions of size $2n$ consisting of $n$ left brackets and $n$ right brackets is equal to $C_n$. For example, $C_3=5$ and there are
$5$ valid expressions of size $6$ (i.e. $2n = 2 \cdot 3 = 6$):
- ()()()
- (())()
- ()(())
- ((()))
...
In a more formal way, we can rewrite this as: Considering right paranthesis $+1$ and left paranthesis $-1$, then the number of strings of length $2n$ encompassing $n$ values of $+1$ and $n$ values of $-1$ satisfying: $\sum_{i=1}^{2n}v_i \,\,\,\,, \forall i \in[1,2n]$ is equal to $C_n$.

- The number of full binary trees with $n+1$ leaves is equal to $C_n$. The following image shows the $5$ cases for binary trees with $4$ leaves:

![binary-trees](./assets/catalan-binary-trees.png)

- The number of monotonic lattice paths along the edges of a $n \times n$ grid, that don't pass above the diagonal, is $C_n$. The paths start in the lower left corner and ends in the upper right corner. For example, there are $C_4=14$ paths in a $4 \times 4 grid$:

![lattice points](./assets/catalan-lattice-paths.png)

- Consider a convex polygon with $n+2$ sides divided into $n$ triangles by connecting vertices with non-intersecting lines. The number of different ways to divide the polygon in this way is
equal to $C_n$. Let's take a look, for example, at the particular case $n=3$:

![polygons](./assets/catalan-polygons.png)

- The number of mountain ranges - see [Dyck words](https://en.wikipedia.org/wiki/Dyck_language) - of length $2n$ with $n$ upstrokes and $n$ downstrokes is equal to $C_n$
