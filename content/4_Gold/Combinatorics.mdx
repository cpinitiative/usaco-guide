...
Let $X$ represent the probability distribution of maximum number of candies a child gets. To get $E[X]$, we just need $P(X = x)$ for $1 \leq x \leq k$. $(\frac{x}{k})^n$ is the probability that each child gets at most $x$ candies. To get $P(X = x)$, we must subtract out the probability that each child gets strictly less than $x$ candies, which is $(\frac{x - 1}{k})^n$.

Therefore, $P(X = x) = (\frac{x}{k})^n - (\frac{x - 1}{k})^n$, allowing us to easily calculate $E[X]$.

### Implementation

**Time Complexity:** $\mathcal{O}(nk)$ using naive exponentiation or $\mathcal{O}(k \log n)$ using binary exponentiation or std::pow.
...
void solve() {
	...
	double out = 0;
	for (int i = 1; i <= k; i++) {
		out += i * (pow((double)i / k, n) - pow((double)(i - 1) / k, n));
	}

	// set output precision to 6 decimal points
...
Let $E[X_u]$ be the probability that node $u$ is chosen in a sequence of operations. Let's consider how an operation affects node $u$.
1. All nodes $v$ that cannot reach $u$ will not affect $u$ if chosen in an operation.
2. All nodes $v$ that can reach $u$ will immediately delete $u$ if chosen in an operation.
Therefore, $E[X_u]$ is the probability (in this case probability and expected value go hand in hand) that $u$ is chosen before any node $v \neq u$, which is $\frac{1}{a_u}$, where $a_u$ is the number of nodes $v$ that can reach $u$.

From linearity of expectation, $E[X_1 + X_2 + \dots + X_n] = E[X_1] + E[X_2] + \dots + E[X_n]$, solving the problem.
...
int a[N];
int n;
// adding 1 to a[i] for all nodes i that are reachable from node u
void dfs(int u) {
	vis[u] = 1, a[u]++;
	for (int v = 0; v < n; v++) {
		if (!vis[v] && adj[u][v]) { dfs(v); }
	}
}
...
void solve() {
	...
	double out = 0;
	for (int i = 0; i < n; i++) { out += 1.0 / a[i]; }

	// must be set high to be precise enough to pass
