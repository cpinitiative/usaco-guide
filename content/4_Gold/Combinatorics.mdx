...
Additionally, we can optimize this recursion formula from $\mathcal{O}(2^n)$ to $\mathcal{O}(n^2)$ using [Dynamic Programming](https://usaco.guide/gold/intro-dp?lang=cpp). You can learn more about how DP is used to optimize the binomial coefficient recursive formula [here](https://www.geeksforgeeks.org/binomial-coefficient-dp-9/).
...
Fortunately, we can use [modular inverses](https://usaco.guide/gold/modular?lang=cpp) to divide $n!$ by $k!$ and $(n - k)!$ modulo $p$. We can precompute all factorials in $\mathcal{O}(n)$ time and inverse factorials in $\mathcal{O}(n\log MOD)$ (although, not necessary, you can also precompute inverse factorials in $\mathcal{O}(n)$) time.
...
The first method for calculating binomial factorials is too slow for this problem since the constraints on $a$ and $b$ are $(1 \leq b \leq a \leq 10^6)$. However, we can use the second method to answer each of the $n$ queries in constant time by precomputing factorials in  and modular inverses.
