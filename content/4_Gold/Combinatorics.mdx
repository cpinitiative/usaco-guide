...
- Consider a convex polygon with $n+2$ sides divided into $n$ triangles by connecting vertices with non-intersecting lines. The number of different ways to divide the polygon in this way is equal to $C_n$. Let's take a look, for example, at the particular case for $n=3$ in which we have $C_4=$:

![polygons](./assets/catalan-polygons.png)

- The number of mountain ranges - see [Dyck words](https://en.wikipedia.org/wiki/Dyck_language) - of length $2n$ consisting of $n$ upstrokes and $n$ downstrokes is equal to $C_n$
