...
id: combo
title: 'Combinatorics'
author: Jesse Choe, Aadit Ambadkar
prerequisites:
  - divisibility
  - modular
...
```cpp
/** Computes nCr mod p using naive recurssion */
...
Additionally, we can optimize this recursion formula from $\mathcal{O}(2^n)$ to $\mathcal{O}(n^2)$ using [dynamic programming](/gold/intro-dp) (DP). We can cache the values of smaller binomials into a dp, and then call them in O(1) time when needed. The code below shows a bottom-up implementation of this type of DP.
...
Fortunately, we can use [modular inverses](/gold/modular) to divide $n!$ by $k!$ and $(n - k)!$ modulo $p$. A naive implementation of this would take too long, as computing inverse factorials online can be very time costly. Instead, we can precompute all factorials in $\mathcal{O}(n)$ time and inverse factorials in $\mathcal{O}(n\log MOD)$ (although not necessary, you can also precompute inverse factorials in $\mathcal{O}(n)$ time). See the code below for the implementation.
