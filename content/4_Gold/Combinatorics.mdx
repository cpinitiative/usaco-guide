...
Additionally, we can optimize this recursion formula from $\mathcal{O}(2^n)$ to $\mathcal{O}(n^2)$ using [Dynamic Programming](/gold/intro-dp). You can learn more about how DP is used to optimize the binomial coefficient recursive formula [here](https://www.geeksforgeeks.org/binomial-coefficient-dp-9/).
