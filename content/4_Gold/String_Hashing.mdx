...
id: hashing
title: 'Hashing'
author: Benjamin Qi, Andi Qu, Peng Bai
...
## Tutorial
<Resources>
  <Resource source="CF" title="XOR Hashing [TUTORIAL]" url = "https://codeforces.com/blog/entry/85900"></Resource>
</Resources>
...
<FocusProblem problem="exxor" />

### Solution
We first generate hash values for all values in $a$ and $b$ and store this in a map to make sure equal elements have the same value. Using this map, we can build arrays $\texttt{hasha}$ and $\texttt{hashb}$, corresponding to the hash values at each index for arrays $a$ and $b$, respectively.

To deal with duplicate elements, we set $\texttt{hasha}[i] = 0$ if the first occurrence of $a[i]$ is not at index $i$ in $a$ and $\texttt{hashb}[i] = 0$ if the first occurrence of $b[i]$ is not at index $i$ in $b$ for all $i$. This can be done by maintaining a set of values already encountered.

Let $\texttt{prefa}$ and $\texttt{prefb}$ be the prefix XOR array of $\texttt{hasha}$ and $\texttt{hashb}$, respectively. Answering queries now just requires checking if $\texttt{prefa}[x] = \texttt{prefb}[y]$.
...
long long rnd() {
	return uniform_int_distribution<long long>(0, LLONG_MAX)(rng);
}

void solve() {
	int N, Q;
	cin >> N;
	vector<long long> hasha(N), hashb(N);
	map<int, long long> hashval;
	set<int> s;
	for (int i = 0; i < N; i++) {
		int x;
		cin >> x;
		if (!hashval.contains(x)) { hashval[x] = rnd(); }

		if (!s.contains(x)) { hasha[i] = hashval[x]; }
	...
	for (int i = 0; i < N; i++) {
		...
		if (!s.contains(x)) { hashb[i] = hashval[x]; }
