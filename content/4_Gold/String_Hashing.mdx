...
of the two strings. See [rng-58's blog post about hashing](http://rng-58.blogspot.com/2017/02/hashing-and-probability-of-collision.html)
for how this probability is derived.
...
$m$ and $n$ are on the order of $10^5$, $M=10^9+7$ is not large enough
to avoid collisions. Use a larger prime modulus such as $2^{61}-1$ 
...
[here](https://codeforces.com/blog/entry/61587?#comment-455289) and 
[here](https://codeforces.com/blog/entry/60442).

</Warning>

In particular, a virtually unhackable way of choosing $B$ in the implementation
above is as follows:
...
Using the $M=10^9+9$ will not pass (see the note above
regarding the birthday paradox). Instead, we use $M=2^{61}-1$.
