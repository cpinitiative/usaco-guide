...
The two base-modulo pairs $(9973, 10^9 + 9)$ and $(69420, 998244353)$ work for this problem.
...
int main() {
	...
	for (int i = n.size(); i < h.size(); i++) {
		// Update frequency table
		freq_curr[h[i] - 'a']++;
		freq_curr[h[i - s.size()] - 'a']--;
		// Update polynomial hashes
		poly1 = (poly1 * P1 - p_pow1 * (h[i - s.size()] - 'a') % M1 + (h[i] - 'a') + M1) % M1;
		poly2 = (poly2 * P2 - p_pow2 * (h[i - s.size()] - 'a') % M2 + (h[i] - 'a') + M2) % M2;
...
An alternative solution without a sliding window would be to hash the substrings that we're trying to match. Since order doesn't matter, we need to modify our hash function slightly.

In particular, instead of computing the **polynomial** hash of the substrings, compute the product $(P + s_1)(P + s_2) \cdots (P + s_k) \bmod M$ as the hash (again, using two base-modulo pairs).

Alternative hashes (e.g. computing the sum $(P + s_1)^2 + (P + s_2)^2 + \dots + (P + s_k)^2 \bmod M$) also work for other problems, but the test cases are too strong for that to pass here.
