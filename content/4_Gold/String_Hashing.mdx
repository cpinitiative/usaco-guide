...
export const problems = {
	general: [
		...
		new Problem('Gold', 'Cownomics', '741', 'Normal', false, [], "Use two pointers; for a fixed $l$, keep extending $r$ to the right until the positions $l\ldots r$ explain spotiness. Hashing gives you a way to quickly check whether two substrings of different cow types are equal. So for a single $[l,r]$ pair you can check whether it works in $O(N\log N)$ time (and you only need to check $O(M)$ of these pairs in total). Actually, it's possible to pass $O(N^2M)$ (or even slower) solutions."),
		new Problem(
			'Gold',
			'Lights Out',
			'599',
...
preferably smth that can't be fakesolved
