...
Let $\texttt{dp[pos][k][under][started]}$ where $pos$ is the current position,
$k$ is a counter to see if you have at least half of the same digit, $under$ is
whether you have gone below the actual number, and $started$ is a boolean if you
still have leading zeros. The transition involves looping through 9 digits to
add to the current location and comparing with the digit we want to occupy at
...
public class Odometer {
     ...
     * @param pos     the starting position
     * @param k       counter for the number of the target digit targ
     * @param under   1 if the current sequence is smaller than the given upperbound
     * @param started 1 if the sequence has any digit other than leading zeros
     */
    public long solveDP(int pos, int k, int under, int started, int targ, int targ2) {
        // base case: the sequence has reached the maximal length
        if (pos == num.length()) {
            // edge case: the sequence only contains zeros, i.e. did not start
            if (started == 0) {
    ...
    /** Count interesting sequences that are less or equal ubound */
    public long countInterestingTo(long ubound) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Odometer solver = new Odometer();

        PrintWriter pw = new PrintWriter("odometer.out");
        pw.println(solver.countInterestingTo(Y) -
                solver.countInterestingTo(X - 1));
        pw.close();
    }
}
```
