...
```py
def solve_dp(pos, k, under, started, targ, targ2, dp, num):
    # base case: the sequence has reached the maximal length
    if pos == len(num):
        # edge case: the sequence only contains zeros, i.e. did not start
        if not started:
    ...
    ans = 0
    digit_diff = int(num[pos])
    for i in range(10):
        # If the sequence will be larger than the upperbound, then we can terminate
        if not under and i > digit_diff:
            break
        
        # If the new digit is smaller than the one in the upperbound, then any
        # child sequence can't be larger than the upperbound as this digit is
        # more significant than any following one.
        is_under = under
        if i < digit_diff:
            is_under = True
        ...
        # If we want to count how many sequences have exactly half digits of
        # targ and half of targ2, then any other number than targ or targ2 doesn't matter.
        ...
        ans += solve_dp(pos + 1, new_k, is_under, is_started, targ, targ2, dp, num)
    
    dp[pos][k][under][started] = ans
    return ans

def count_interesting_to(ubound):
    num = str(ubound)
    ans = 0
    dp = [[[[ -1 for _ in range(2)] for _ in range(50)] for _ in range(19)] for _ in range(2)]
    
    for i in range(10):
        ans += solve_dp(0, 20, False, False, i, -1, dp, num)
    ...
    for i in range(10):
        for j in range(10):
            duplicates += solve_dp(0, 20, False, False, i, j, dp, num)
...
<Problems problems="Easier Problems" />
