...
Let $\texttt{dp[i][und][k][is0]}$ where $i$ is the current position, $und$ is
whether you have gone below the actual number, $k$ is a counter to see if you
have at least half of the same digit, and $is0$ is a boolean if you still have
leading zeros. The transition involves looping through 9 digits to add to the
current location and comparing with the digit we want to occupy at least half of
the number.

Given the current state $\texttt{dp[i][und][k][is0]}$, we loop through all
digits from 0 to 9 and consider adding each of them to the current position. If
we add the digit $d$ to the current position, we update our state as follows:
...
ll dp[19][50][2][2];  // dp[pos][count][under][started]
string num;
...
ll count_interesting_to(ll ubound) {
	...
	for (int i = 0; i <= 9; i++) {
		for (int j = 0; j <= 9; j++) {
			reset();
			duplicates += solve_dp(0, 20, true, false, i, j);
		}
	}
