...
The **greatest common divisor (GCD)** of two integers $a$ and $b$ is the largest integer that is a factor of both $a$ and $b$. In order to find the GCD of two _non-negative_ integers, we use the **Euclidean Algorithm**, which is as follows:
