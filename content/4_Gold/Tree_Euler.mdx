...
Now we must resolve `dfs(2, 1)` and `dfs(3, 1)`. It does not matter which order we process these in, so for this example, we start with `dfs(2, 1)`. Since the timer value is 1, we set `st[2]` to 1 and increment the timer. However, because `2` has no children, we don't call `dfs`. Instead, we set `en[2]` to 1 because our current timer is now 2.
