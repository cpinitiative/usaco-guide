...
Before the DFS, our $\texttt{start}$ and $\texttt{end}$ arrays are initialized with
...
Notice that fter running $\text{dfs}$, each range $[\texttt{start}[i], \texttt{end}[i]]$
...
# BeginCodeSnip{Segment Tree (from PURS module)}
class SegmentTree:
...
g = [[] for i in range(MAX_N)]
st = [0 for i in range(MAX_N)]
en = [0 for i in range(MAX_N)]
timer = 1
...
for _ in range(q):
	...
	if query[0] == 1:
		st.add(st[query[1] - 1] - 1, query[2])
	else:
		print(st.range_sum(st[query[1] - 1] - 1, en[query[1] - 1]))
```
