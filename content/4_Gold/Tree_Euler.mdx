...
<video width="960" height="720" controls>
	<source src="/animations/euler_tour.mp4" type="video/mp4"/>
</video>
...
There are many ways to calculate the LCA of two nodes that are mentioned
in the [binary jumping](/plat/binary-jump) module. The one presented here
is known as LCA with RMQ, or colloquially known as $\mathcal{O}(1)$ LCA.

First, we need to preprocess our tree. To do this, we perform a standard
Euler tour. However, **when moving back up the tree we also increment
the timer**. This results in the nodes the Euler tour always being connected
and being more tour-like.
...
Here's some code to help understand how the tour is calculated.

```cpp
// tin[u] = timer when we first visit u
// et[u] = maps timer to current node
...
Let's say we want to calculate the LCA of two nodes $u$ and $v$.
Assume that $\texttt{tin}[u] < \texttt{tin}[v]$. Then, $\texttt{lca}(u, v)$
will equal to the node in the array $\texttt{et}$ at the lowest depth in the
range $[\texttt{tin}[u], \texttt{tin}[v]]$.
Here, $\texttt{et}[i]$ means that when the timer was equal to $i$, we were at
node $\texttt{et}[i]$.

To understand why this is true, consider the following:

> We know two facts about the LCA of $u$ and $v$:
> 1. The LCA is on the shortest path between $u$ and $v$.
> 2. The LCA is the node of minimal depth on the shortest path
> Also, our Euler tour is practically a shortest path: it's just that
> we happen to visit all the subtrees we pass by. Thus, this works.

After that, we can just reduce this to a RMQ problem. We can either use
a segment tree or a sparse table to handle our range-min queries.
...
// BeginCodeSnip{Min Segment Tree}
template <class T> struct MinSegmentTree {
	...
	void init(const vector<T> &arr) {
		len = arr.size();
		for (int i = 0; i < len; i++) { segtree[len + i] = arr[i]; }
...
class LCA {
  private:
	...
	MinSegmentTree<pair<int, int>> rmq;
  ...
  public:
	LCA(const vector<vector<int>> &adj)
	    : n(adj.size()), adj(adj), tin(n), et(2 * n), dep(n) {
		dfs(0, -1);
	...
	int query(int a, int b) {
		...
		return rmq.range_min(tin[a], tin[b]).second;
	}
};
```
...
template <typename T> struct SparseTable {
	...
	void init(const vector<T> &v) {
		len = v.size();
		const int max_log = 1 + __lg(len);
		st.emplace_back(v);
		for (int j = 1; j < max_log; j++) {
			st.emplace_back(len - (1 << j) + 1);
