...
After running $\text{dfs}()`, each range `[st[i], en[i]]` contains all ranges `[st[j], en[j]]` for each `j` in the subtree of `i`. Also, $\texttt{en}[i]-st[i]+1` is equal to the subtree size of `i`.
...
Now we must resolve $\text{dfs}(4, 3)$ and $\text{dfs}(5, 3)$. First, we resolve $\text{dfs}(4, 3)$ by setting $\texttt{st}[4]$ to the value of the timer (3 in this case) and incrementing the timer. Then, since `4` has no children, set $\texttt{en}[4]$ to 3.

Now the value of the timer is 4, and we must resolve $\text{dfs}(5, 3)$. Similar to before, we set $\texttt{st}[5]$ to 4. Since `5` also has no children, set $\texttt{en}[5]$ to 4.
...
Assumes that $\text{dfs}()` code is included. Uses a segment tree.
...
// Build the segment tree: run `build()` after running $\text{dfs}()`
...
// Make sure you run $\text{dfs}()` and `build()` before you run this
int lca(int a, int b) {
