...
<Resources>
  ...
  <Resource source="PAPS1" title="19.3 - Hashing">many applications</Resource>
...
For each distinct numerical value in the arrays, we generate a random positive 64-bit integer.
With this map, we can build the prefix XOR hashes for $a$ and $b$.

An issue we have to deal with is duplicate elements, as XORing an element with
itself will result in a value of $0$ and will be equivalent it never having
...
XOR hashing is susceptible to hacking on platforms like Codeforces. For more information, check out [this blog](https://codeforces.com/blog/entry/153335).

In summary, avoid using `std::rand()` in general (check out [this blog](https://codeforces.com/blog/entry/61587) for more info), and avoid directly using values from `mt19937`.
The following options should be safe:
- Using `mt19937` with `uniform_int_distribution`, where the range provided is **not** in the form of $[0, 2^n)$.
- Using [splitmix64](https://rosettacode.org/wiki/Pseudo-random_numbers/Splitmix64) or a similar random number generator.
