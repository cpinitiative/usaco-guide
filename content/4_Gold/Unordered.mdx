...
replacement for their `map` and `set` counterparts, as long as the type being
hashed does not require custom hashing.

The operations for `unordered_map` include:

- `insert`, `erase`, `count`, which are the same as
  [sorted sets](/bronze/intro-sets)
- `find`, which gets the iterator to an element.
...
[ordered map](/silver/intro-ordered) (which supports all of the functions used
...
The link provides an example of hashing pairs of strings. More examples (for
pairs of ints)
