...
We can also find the modular inverse from Euclidean Division. Given the prime modulus $m > a$ we have:
$m = k \cdot a + r$, where k = $floor(\frac{m}{a})$	and $r = m % a$. Then: $0 = k \cdot a + r mod m \iff r = -k \cdot a mod m
...
The advantage of this approach is that we can precompute the inverse modular of numbers in range $[1, MOD)$ in $\mathcal{O}(MOD)$.
