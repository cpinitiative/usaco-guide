...
being used many times, it is a good idea to precalculate it. Note the above problem could also be solved by precalculating invererses by first calculating the inverse of n! for a large n then finding all previous values by multiplying in $O(n)$.
