...
For example, the inverse of $2 \pmod{p}$ where $p=10^9+7$ is
$$
1\equiv p+1 \pmod{p}.
$$
...
<CPPSection>
To calculate $a \choose b$, we can precalculate the factorials to speed up the calculation of $a!$, $b!$, and $(a-b)!$. $a \choose b$ is equivalent to
