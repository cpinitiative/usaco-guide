...
being used many times, it is a good idea to precalculate it. Note the above problem could also be solved by precalculating invererses by finding the inverse of $N!$ for a large $N$ then calculating the inverses of $1...{N!-1}$ by multiplying each inverse starting from $N!$ with their respective values in $O(n)$.
