...
We can slightly modify the normal implementation of the [Euclidean Algorithm](/gold/divisibility?lang=cpp#gcd--lcm) to
return more information. It is possible to obtain the integers $x$ and $y$ by reversing the steps in the algorithm.
You can read a more in-depth explanation [here](/adv/extend-euclid).
...
```cpp
/* @return x[0] * a + x[1] * b = x[2], where x[2] = gcd(a, b) */
...
modulo $m_1$, respecively, thus the properties hold: $n_1 \cdot m_1 \equiv 1 \pmod{m_2}$ and $n_2 \cdot m_2 \equiv 1 \pmod{m_1}$.
...
**Time Complexity:** $\mathcal{O}(\log{MN})$ for each test case
