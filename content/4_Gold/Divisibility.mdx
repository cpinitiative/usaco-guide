...
$$
\sum_{1 \le a < b \le n} \gcd(a,b)
$$

A direct $O(n^2)$ enumeration over all pairs is too slow, so we use number theory.

First, define

$$
f(j)=\sum_{1 \le a,b \le j} \gcd(a,b),
$$

which is the sum of gcd values over all ordered pairs inside the square
$[1,j] \times [1,j]$.

To compute this efficiently, group pairs by their gcd value.
If $\gcd(a,b)=d$, then we can write $a=dx, b=dy$ with $\gcd(x,y)=1$.
After dividing both numbers by $d$, we only need to count coprime pairs.

The number of integers up to $k$ that are coprime with $k$ is given by Euler's totient function $\phi(k)$.
...
This counts all ordered pairs.
However, the problem only considers pairs with $a<b$.
The diagonal pairs $a=b$ contribute $\gcd(a,a)=a$, and there are exactly $j$ such terms, so we subtract them.

Thus the required contribution for each $j$ is $(f(j)-j)$.
...
so each query can be answered in $O(1)$.

The totient sieve runs in $O(n \log \log n)$, and the divisor enumeration for
computing $f$ runs in $O(n \log n)$.
Hence, the overall preprocessing complexity is $O(n \log n)$.
...
int main() {
	...
	// compute f[j] = sum_{a,b <= j} gcd(a,b)
