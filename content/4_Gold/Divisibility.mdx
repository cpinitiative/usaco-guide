...
The totient function is **multiplicative** meaning that $\phi(nm)=\phi(n) \cdot \phi(m)$, where $n$ and $m$ are coprime - $gcd(n, m)=1$
...
Let the factorization be $n=p_1^{q_1} \cdot p_2^{q_2} \cdot ... \cdot p_k^(q_k)$ where $p_i$ are prime factor of $n$, then:
$$\phi(n)=\phi(p_1^{q_1}) \cdot \phi(p_2^{q_2}) \cdot ... \cdot \phi(p_k^(q_k)) 
= p_1^{q_1-1}(p_1 - 1) \cdot p_2^{q_2-1}(p_2 - 1) \cdot .. \cdot p_k^{q_k-1}(q_k - 1) $$
