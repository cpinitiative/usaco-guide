...
<FocusProblem problem = "sumdiv"/>

The sum of all divisors of the integer $N$ can be efficiently computed in $\mathcal{O}(\sqrt{N})$ using [Divisor function](https://en.wikipedia.org/wiki/Divisor_function).
...
int main() {
    ...
    for(int d = 2; d * d <= n; d++) {
        ...
        if(d%MOD == 1) {
            div_sum = div_sum * (lgpow(d, p*m+1) - 1) % MOD;
        } else {
            div_sum = div_sum * (lgpow(d, p*m+1) + MOD - 1) * inv(d-1) % MOD;
        }
    }

    if(n > 1) {
        if(n % MOD == 1) {
            div_sum = div_sum * (lgpow(n, m+1) - 1) % MOD;
        } else {
            div_sum = div_sum * (lgpow(n, m+1) + MOD - 1) * inv(n-1) % MOD;
        }
    }
