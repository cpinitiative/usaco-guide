...
redirects:
  - /gold/divis
title: 'Divisibility'
author: Darren Yao, Michael Cao, Andi Qu, Kevin Sheng, and Mihnea Brebenel
...
int lgpow(int x, int y) {
	...
	for (int bit = 0; (1 << bit) <= y; bit++) {
		if ((1 << bit) & y) { div_sum = div_sum * aux % MOD; }
		aux = aux * aux % MOD;
	}
	return div_sum;
}
...
int main() {
	...
	for (int d = 2; d * d <= n; d++) {
		...
		// The Modular Inverse only exists if d % MOD != 0
		if (d % MOD == 1) {
			div_sum = div_sum * (lgpow(d, p * m + 1) - 1) % MOD;
		} else {
			div_sum =
			    div_sum * (lgpow(d, p * m + 1) + MOD - 1) * inv(d - 1) % MOD;
		}
	}
