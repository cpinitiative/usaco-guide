...
Our function can be computed with the normal phi function computation and prefix sums.
...
void precompute() {
	...
	for (int i = 1; i < MAXN; i++) {
		for (int j = i + i; j < MAXN; j += i) { s[j] = s[j] + 1LL * i * phi[j / i]; }
	}
