...
Sieve of Eratosthenes. Since it's almost the same with the Sieve of Eratosthenes the time complexity will be: $\mathcal{O}(N\log\log{N})$.
...
The queries ask us for the sum of the greatest common divisor between all pairs of numbers not exceeding n.

Let's define a function f(j) as the sum of gcd values over all ordered pairs 1 ≤ a, b ≤ j, i.e.
f(j) = Σ gcd(a, b).

To compute this efficiently, we group pairs according to their gcd.
If gcd(a, b) = d, then we can write a = d·x and b = d·y with gcd(x, y) = 1.
Thus, after dividing both numbers by d, we only need to count coprime pairs.

The number of integers up to k that are coprime with k is given by Euler's totient function φ(k).
Using this fact, we obtain the identity

$$
f(j) = \sum_{i \mid j} i \cdot \phi\!\left(\frac{j}{i}\right)
$$

where each divisor i contributes i multiplied by the number of pairs whose reduced gcd is 1.

However, this sum also includes pairs where a = b.
Since gcd(a, a) = a, there are exactly j such terms, which must be excluded because the problem only considers a < b.
Therefore, the contribution for each j becomes f(j) − j.

Finally, we precompute prefix sums

$$
\text{sum}[n] = \sum_{k=1}^{n} (f[k] - k)
$$

so every query can be answered in O(1) time after preprocessing.

The preprocessing runs in O(n log n) using a sieve-style computation of φ.
...
vector<ll> phi(MOD, 0), f(MOD, 0), sum(MOD, 0);

int main() {
	for (int i = 1; i < MOD; i++) phi[i] = i;

	for (int i = 2; i < MOD; i++)
		if (phi[i] == i)
			for (int j = i; j < MOD; j += i) phi[j] -= phi[j] / i;

	for (int i = 1; i < MOD; i++)
		for (int j = i; j < MOD; j += i) f[j] += i * phi[j / i];

	for (int i = 1; i < MOD; i++) sum[i] = sum[i - 1] + f[i] - i;
