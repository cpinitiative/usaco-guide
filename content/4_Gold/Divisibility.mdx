...
<Resources>
	...
	<Resource source="PAPS1" title="17.1, 17.2 - Number Theory" />
	<Resource
		source="MONT"
...
<!-- Since $F_n = \mathcal{O}(\varphi^n)$, the worst-case complexity of this algorithm is $\mathcal{O}(\log ab)$. -->
...
Coding $\operatorname{lcm}$ as `a * b / gcd(a, b)` might cause integer overflow if the
...
Let the factorization be $n=p_1^{q_1} \cdot p_2^{q_2} \cdot \dots \cdot p_k^{q_k}$ where $p_i$ is a prime factor of $n$, then:

$$
\phi(n)=\phi(p_1^{q_1}) \cdot \phi(p_2^{q_2}) \cdot \dots \cdot \phi(p_k^{q_k}) = p_1^{q_1-1}(p_1 - 1) \cdot p_2^{q_2-1}(p_2 - 1) \cdot \dots \cdot p_k^{q_k-1}(q_k - 1)
$$
