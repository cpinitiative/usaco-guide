...
- If n is a power of a prime number, $n=p^q$ where p is a prime number and $1 \leq q$ then there are exactly $p^{q-1}$ numbers divisible by $p$, so
...
$$\phi(n)=\phi(p_1^{q_1}) \cdot \phi(p_2^{q_2}) \cdot \dots \cdot \phi(p_k^{q_k})
= p_1^{q_1-1}(p_1 - 1) \cdot p_2^{q_2-1}(p_2 - 1) \cdot \ldots \cdot p_k^{q_k-1}(q_k - 1) $$
...
Usually in problems we need to precompute all the totient of all numbers between $1$ and $n$, then factorizing is not efficient. The idea is the same as the
...
The queries ask us: what is the sum of the greatest common divisor between all the numbers smaller than $n$ ? The following solution will illustrate the usefulness of
[Euler's totient function](https://en.wikipedia.org/wiki/Euler%27s_totient_function) in problems involving divisibility, greatest common divisor and lowest common multiple.
...
Defining the function this way gives us the following equation: $f(n,d)=f(\frac{n}{d}, 1)$. The reasong why is simple: By choosing two coprime numbers $a$ and $b$ with
$a,b \le \frac{n}{d}$ guarantees us that $a \cdot d, b \cdot d \le n$ and $gcd(a \cdot d, b \cdot d) = d$, thus contributing to the global answer. Shifting away to our
new function $f(\frac{n}{d}, 1)$ allows us to compute $f(n, d)$ more easily using a slightly modified phi function preprocessing. To find the sum simply multiply
$f(n, d)$ with $d$.

This way the global answer for a query is $\sum_{d=1}^{n}d \cdot f(n, d) = \sum_{d=1}^{n}d \cdot f(\frac{n}{d}, 1)$.
...
long long solve(long long n) {
	if (n < NMAX) { return phi[n]; }
	if (memo.count(n)) { return memo[n]; }
	long long d = sqrt(n), ans = 1LL * n * (n + 1) / 2;
	for (int i = 2; n / i > d; i++) { ans -= solve(n / i); }
	for (int i = d; i >= 1; i--) {
		ans -= 1LL * (n / i - n / (i + 1)) * solve(i);
	}
	memo[n] = ans;
	return ans;
}
...
int main() {
	...
	// Prefix sums of phi
	for (int i = 2; i < NMAX; i++) { phi[i] += phi[i - 1]; }
