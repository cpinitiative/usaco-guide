...
{/_ NOTE: general problems were RANDOMLY STARRED. please review! _/}
...
{/\* <Resource source="CPH" title="12.2 - Breadth-First Search">example,
...
{/_ Sometimes, when a Dijkstra runs too slowly, you need to use a 0/1 BFS
instead. _/}
...
In this problem, let's connect two cells $(u, v)$ in the grid if you can reach
$v$ from $u$ by flipping gravity, going left, or going right. Also, we should
only store cells where CB doesn't fall in the direction of gravity.

Now, we're given a weighted graph and we need to find the shortest path. Since
the edge weights on the graph are only 1 if we change gravity and 0 otherwise,
we can run a 0/1 BFS. \*/}

{/_ which runs much faster than Dijkstra's algorithm. _/}
