...
Now, we're given a weighted graph and we need to find the shortest path. However, since the edge weights on the graph are only 1 if we change gravity and 0 otherwise, we can run a 0/1 BFS which runs much faster than Dijkstra's algorithm.
