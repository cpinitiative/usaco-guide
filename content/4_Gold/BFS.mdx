...
Python's `queue.Queue()` uses Locks to maintain a threadsafe synchronization, thereby making it quite slow. To avoid TLE in many questions, rather use `collections.deque()` for a faster queue data structure.
