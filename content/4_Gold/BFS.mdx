...
Finally, we can backtrack from node $n \rightarrow 1$, in this case $5 \rightarrow 1$, pushing each value that we back track into a vector. The path we take is $5 \rightarrow 2 \rightarrow 1$ which corresponds to the vector $[5, 2, 1]$. We break at node 1 because we notice that its parent is 0. Finally, we reverse the vector and print out it's length (in this case, 3).
