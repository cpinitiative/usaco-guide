...
$A[i + 1] > L_i[j - 1]$, there is an increasing sequence of length $j+1$ ending
at $A[i + 1]$, as we can append $A[i + 1]$ to the end of the sequence of length
...
for x in range(1, n):
    ...
    if v < endpoints[-1]:
        ...
        index = bisect_left([-x for x in endpoints], -v)  # invert list in order to use bisect_left
        endpoints[index] = v
print(len(endpoints))
