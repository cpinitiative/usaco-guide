...
*Wrong Proof 1:* See [cp-algo](https://cp-algorithms.com/sequences/longest_increasing_subsequence.html#toc-tgt-13). However, it doesn't seem to be correct because the process of unhooking and reattaching is not guaranteed to terminate. For example, consider partitioning $A=(3,1,2)$ into the non-increasing subsequences $s_1=(3,1)$ and $s_2=(2)$ (note that this link is about partitioning $A$ into non-increasing subsequences rather than increasing subsequences). Then $3$ will be moved from the front of $s_1$ to $s_2$ on the first step, back to $s_1$ on the second step, and so on. 
