...
$L_i[j] \geq L_i[j + 1]$. Let $a_0, a_1, \dots, a_j$ be any increasing subsequence
of length $j+1$ ending on $L_i[j + 1]$ (so $a_j=L_i[j + 1]$). Notice that
$a_0, a_1, \dots, a_{j-1}$ is an increasing subsequence of length $j$.
Thus, $L_i[j]$ cannot be greater than $a_{j-1}$.
From that, we can write this equation: $L_i[j] \leq a_{j-1} < a_j = L_i[j+1]$,
