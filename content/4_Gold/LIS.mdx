...
Luckily for us, there's a simple solution to this (although it might not be obvious why it works).
...
*Proof -2:* See [cp-algo](https://cp-algorithms.com/sequences/longest_increasing_subsequence.html#toc-tgt-13). However, it doesn't seem to be correct because the process of unhooking and reattaching is not guaranteed to terminate. 

*Proof -1:* [This](https://math.stackexchange.com/questions/3739994/find-minimum-number-of-increasing-subsequences) also seems to be wrong?

*Proof 1:* Consider the greedy strategy to construct the collection of increasing subsequences (essentially [patience sorting](https://en.wikipedia.org/wiki/Patience_sorting)). For each element $x$ of $A$ from left to right, add it to the increasing subsequence with last element less than $x$ such that this last element is maximized. 
...
*Proof 2:* This is just a special case of [Dilworth's Theorem](https://en.wikipedia.org/wiki/Dilworth%27s_theorem). See the inductive proof.
