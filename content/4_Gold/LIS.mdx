...
contributors: Dong Liu, Siyong Huang, Aryansh Shrivastava, Eric Xu
prerequisites:
  - intro-dp
...
**Lemma 2:** The length of the LIS ending on $A[i + 1]$ is equal to $j+1$, where $j$ is the least
index (with 0-indexing) such that $L_i[j] \geq A[i + 1]$.

**Proof:** Let $j$ be defined as it is in the statement. Since $L_i$ is strictly increasing, 
$A[i + 1] > L_i[j - 1]$ (with $j>0$). From this, we can see that there is an increasing sequence of length $j+1$ ending on
$A[i + 1]$, as we can append $A[i + 1]$ to the end of the existing sequence $L_i[j - 1]$, which has length
$j$. Note that if $j=0$, it means that we cannot append $A[i+1]$ to any existing sequence. In that case, we can simply create a new LIS of length 1, so the lemma still holds. By Lemma 1, $L_i$ is strictly increasing, so $L_i[k] \geq A[i + 1]$ for all
...
**Proof:** Let $j$ be defined as it was in Lemma 2. We can update
$L_{i + 1}[j]$ to $A[i + 1]$ using Lemma 2 and the fact that $A[i + 1] \leq L_i[j]$.
...
list with binary search or an ordered set (as demonstrated in the
solution for PCB).
