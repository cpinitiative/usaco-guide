...
author: Michael Cao, Benjamin Qi, Andi Qu, Andrew Wang, Dong Liu
prerequisites:
  - intro-dp
...
Thus we can reorder the elements in $b$ such that $b[i]$ becomes $\texttt{pos}[b[i]]$ where $\texttt{pos}[i]$ represents the index of $i$ in $a$. After reordering $b$, our answer would be the LIS of $b$. 

Reordering $b$ takes $\mathcal O(N)$ and finding a LIS takes $\mathcal O(N\log N)$, so the overall time complexity in this algorithm is $\mathcal O(N\log N)$.
...
int main() {
	...
	for (int i = 1; i <= n; ++i) {
		int p = lower_bound(begin(lis), end(lis), b[i]) - begin(lis);
		if (p == lis.size())
			lis.push_back(b[i]);
		else
			lis[p] = b[i];
