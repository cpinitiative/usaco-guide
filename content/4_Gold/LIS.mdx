...
these being `[2]`, `[2, 5]`, and `[2, 5, 15]`.

**Lemma 1:** $L_i$ forms a strictly increasing sequence.

**Proof:** Assume for a contradiction that for some $j$, we have
$L_i[j] \geq L_i[j + 1]$. Define $a_i$ ($0 \leq i \leq j$) to be the $i$th element
of any increasing subsequence that ends with $L_i[j + 1]$. We can have an
increasing subsequence on length $j$ that ends on $a_{j - 1}$
which is less than $a_j = L_i[j + 1]$, so there is a contradiction.

**Lemma 2:** The length of the LIS ending on $A[i + 1]$ is equal to the least
index $j$ (with 0-indexing) such that $L_i[j] \geq A[i + 1]$.

**Proof:** Let $j$ be defined as it is in the statement. First of all,
since $A[i + 1] > L_i[j - 1]$, there is an increasing sequence of
length $j$ ending on $A[i + 1]$ as we can append $A[i + 1]$ to the end
of the sequence of length $j - 1. By Lemma 1, $L_i$ is strictly increasing,
so $L_i[k] \geq A[i + 1]$ for all $k \geq j$. This means that
the length of the LIS is $j$.

**Lemma 3:** At most 1 element differs between $L_i$ and $L_{i + 1}$.

**Proof:** Let $j$ be defined as it was in the previous lemma.
We need to set $L_{i + 1}[j]$ to be $A[i + 1]$ since
$A[i + 1] \leq L_i[j]$. We don't update anything else though, since
...
list with binary search, or we can use a sorted set (as demonstrated in the solution for PCB).
...
int find_lis(vector<int> a) {
	...
	for (int i : a) {
		...
		if (pos == dp.size()) {  // we can have a new, longer IS!
			dp.push_back(i);
...
def find_lis(arr: List[int]) -> int:
	...
	for i in arr:
		...
		if pos == len(min_endings):  # we can have a new, longer IS!
