...
an increasing subsequence on length $j$ that ends on $a_{j - 1}$ which is less
...
all $k \geq j$. This means that the length of the LIS is $j$.

**Lemma 3:** At most 1 element differs between $L_i$ and $L_{i + 1}$.

**Proof:** Let $j$ be defined as it was in the previous lemma. We need to set
$L_{i + 1}[j]$ to be $A[i + 1]$ since $A[i + 1] \leq L_i[j]$. We don't update
anything else though, since $A[i + 1] > L_i[k]$ for all $k < j$ and there are no
increasing sequences ending on $A[i + 1]$ of length greater than $j$.
