...
Let $L_i$ be an array (0-indexed) where $L_i[j]$ is the smallest element from the first $i$
elements of $A$ with an increasing sequence of length $j + 1$ ending on it (or
...
$L_i[j] \geq L_i[j + 1]$. Define $a_i$ ($0 \leq i \leq j$) to be the $i$-th
element of any increasing subsequence that ends with $L_i[j + 1]$. We can have
an increasing subsequence of length $j$ that ends on $a_{j - 1}$, which is less
than $a_j = L_i[j + 1]$, so there is a contradiction.
...
$j - 1$. By Lemma 1, $L_i$ is strictly increasing, so $L_i[k] \geq A[i + 1]$ for
all $k \geq j$. This means that there cannot be a LIS ending at $A[i + 1]$ longer than $j$.
...
since $A[i + 1] > L_i[k]$ for all $k < j$ and there are no
increasing sequences ending on $A[i + 1]$ for $k > j$.
