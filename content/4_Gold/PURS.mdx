...
class MinSegmentTree {
   private:
	...
	void set(int ind, T val, int at, int at_left, int at_right) {
		...
		if (ind < mid) {
			...
			set(ind, val, 2 * at + 2, mid, at_right);
		}
		segtree[at] =
			combine(segtree[2 * at + 1], segtree[2 * at + 2]);
	}
	...
	T range_min(int start, int end, int at, int at_left, int at_right) {
		...
		int mid = (at_left + at_right) / 2;
		T left_res = range_min(start, end, 2 * at + 1, at_left, mid);
		T right_res = range_min(start, end, 2 * at + 2, mid, at_right);
   ...
   public:
	MinSegmentTree(int len) : len(len) {
		...
		segtree = vector<T>(padded_len * 2 - 1, DEFAULT);
	};

	/** Sets the value at ind to val. */
	void set(int ind, T val) { set(ind, val, 0, 0, padded_len); }

	/** @return the minimum element in the range [start, end) */
	T range_min(int start, int end) { return range_min(start, end, 0, 0, padded_len); }
};
...
**Time Complexity:** Same as the recursive implementation.
...
/** A data structure that can answer point update & range sum queries. */
template <class T>
class SumSegmentTree {
   private:
	...
	T range_sum(int start, int end, int at, int at_left, int at_right) {
		...
		int mid = (at_left + at_right) / 2;
		T left_res = range_sum(start, end, 2 * at + 1, at_left, mid);
		T right_res = range_sum(start, end, 2 * at + 2, mid, at_right);
		return combine(left_res, right_res);
	}
   ...
   public:
	...
	/** @return the minimum element in the range [start, end) */
	T range_sum(int start, int end) { return range_sum(start, end, 0, 0, padded_len); }
};
