...
<Warning>
This Python segment tree is too slow for CSES and will result in TIME LIMIT EXCEEDED due to heavy recursion and 
...
class MinSegmentTree:
    ...
    def _range_min(self, start: int, end: int, at: int, at_left: int, at_right: int) -> int:
...
class SumSegmentTree:
    ...
    def _range_sum(self, start: int, end: int, at: int, at_left: int, at_right: int) -> int:
        if at_right < start or end < at_left:
            return self.DEFAULT
        if start <= at_left and at_right <= end:
            return self.segtree[at]
