...
A common archetype of DP Problems involves a 2D grid of square cells (like graph paper), and we have to analyze "paths" which have these properties:
 - A path is a sequence of cells such that each cell in the sequence shares an edge with the cell before it and the cell after it in the sequence (if those cells exist).
 - The path is restricted to one direction on the x-axis and one direction on the y-axis (e.g. You may only move down or to the right).
