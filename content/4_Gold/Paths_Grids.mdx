...
A common archetype of DP Problems involves a 2D grid of square cells (like graph paper), and we have to count the number of "paths" which have these properties:
...
Usually, the DP-state for grid problems will indicate how many possible paths there are starting from a specific point on the grid. Thus, the state will include at least two numbers: the x-coordinate and the y-coordinate. For some problems, it might be more convenient to think of x and y as the column and the row respectively.
