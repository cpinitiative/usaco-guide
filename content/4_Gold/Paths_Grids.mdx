...
Note how the coordinates in the code are in the form $\[x_{coordinate}\]\[y_{coordinate}\]$. Most of the time, it's more convenient to think of points as $\[row\]\[column\]$ instead, which swaps the order of the coordinates, though the code uses the former format to be consistent with the definition of $dp(x, y)$.
