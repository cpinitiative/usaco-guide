	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < N; i++)
			for (int j : graph[i])
				if (ind[j] <= ind[i]) {
					System.out.println(
					    "IMPOSSIBLE");  // topological sort wasn't valid
					System.exit(0);
				}
...
top_sort = [x + 1 for x in top_sort]
if flag == 0:
	print(*top_sort)
```
...
public class cycle {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 1; cycle.isEmpty() && i <= N; i++) { dfs(i); }
		if (cycle.isEmpty()) out.println("IMPOSSIBLE");
		else {
...
def dfs(node):
	...
	for u in adj[node]:
		if on_stack[u]:
			cycle.append(node)
			on_stack[node], on_stack[u] = False, False
			return True
...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		FastIO sc =
		    new FastIO(System.in);  // View "General - Fast I/O" for more
		                            // information about the "FastIO" class
	...
	static class FastIO {
		...
		public FastIO(InputStream is) throws Exception { dis = is; }
...
# Put all code in a function so it can run faster.
def main():
	...
	for _ in range(flights):
		...
		edge[u].append(v)
		in_degree[v] += 1
		back_edge[v].append(u)

	topo_order = []
	queue = deque([i for i, v in enumerate(in_degree) if v == 0])
	while queue:
		cur_node = queue.popleft()
		topo_order.append(cur_node)
		for neigh_node in edge[cur_node]:
			in_degree[neigh_node] -= 1
			if in_degree[neigh_node] == 0:
				queue.append(neigh_node)
