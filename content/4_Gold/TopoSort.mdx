...
```python
import sys
sys.setrecursionlimit(1000000000) #to set the recursion stack maximum limit
...
top_sort = top_sort[::-1] # Now 'top_sort' is sorted topologically.
...
for i in range(N):
	for j in graph[i]:
		if ind[j] <= ind[i]:
			flag = 1
			print("IMPOSSIBLE") # Topological sort isn't valid
			break
	if flag == 1:
		break
...
def dfs(node):
	visited[node], on_stack[node] = 1, 1
	for u in adj[node]:
		if on_stack[u] == 1:
			cycle.append(node)
			on_stack[node], on_stack[u] = 0, 0
			return True
		elif visited[u] == 0:
			if dfs(u):
				if on_stack[node] == 1:
...
for i in range(1,N+1):
	if cycle == []:
		...
		break
if cycle == []:
	print("IMPOSSIBLE") # Cycle isn't there.
else:
	print(len(cycle)+1)
