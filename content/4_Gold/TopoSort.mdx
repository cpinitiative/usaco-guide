...
def main():
	def bfs(n: int, G: List[List[int]]) -> List[int]:
		topo_order = []
		...
		count = 0
		q = deque([node for node in range(n) if in_deg[node] == 0])
		while q:
			node = q.popleft()
			...
			for neigh in G[node]:
				...
				if in_deg[neigh] == 0:
					q.append(neigh)
			count += 1
		return topo_order if count == n else []
	...
	ans = bfs(N, G)
	print("IMPOSSIBLE" if not ans else " ".join(map(str, [node + 1 for node in ans])))
