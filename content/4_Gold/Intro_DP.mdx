...
- **Push DP**: what states does the current state affect?
- **Pull DP**: what states affect the current state?
We present both approaches below.
<Spoiler title="Push DP">
...
- Jump from stone $i - 2$, incurring a cost of
  ...
  $$
**Base case:** $\texttt{dp}[0] = 0$ (start at stone 0)
<LanguageSection>
<CPPSection>
```cpp
#include <bits/stdc++.h>
