...
two, which results in an exponential time complexity. However, this can sped up
with dynamic programming by keeping track of "optimal states" in order to avoid
such expensive calculations - in this case, the minimum cost to reach stone $i$.
...
- Jump one stone, incurring a cost of $|h_i - h_{i+1}|$:

  $$
  \texttt{dp}[i + 1] = \min(\texttt{dp}[i + 1], \texttt{dp}[i] + |h_i - h_{i + 1}|)
  $$

- Jump two stones, incurring a cost of $|h_i - h_{i + 2}|$:
  $$
  \texttt{dp}[i + 2] = \min(\texttt{dp}[i + 2], \texttt{dp}[i] + |h_i - h_{i + 2}|)
  $$
...
int main() {
	...
	for(int i = 1; i <= N; ++i) {
		cin >> height[i];
		dp[i] = 1e9 + 7; // initially set our values to INF
	}

	dp[1] = 0; // base case: 0 cost for the first stone
	...
	cout << dp[N] << endl; // dp[N] is the minimum cost to get to stone N
}
```

<!-- Thoughts on including frog 2? -->
...
public class Main {
	...
	public static void main(String[] args) {
		...
		int N = io.nextInt();
		int[] height = new int[N + 1], dp = new int[N + 1];
