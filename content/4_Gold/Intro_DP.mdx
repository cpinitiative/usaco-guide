...
two, which results in an exponential time complexity. However, this can sped
up with dynamic programming by keeping track of "optimal states" in order
to avoid such expensive calculations - in this case, the minimum cost to
reach stone $i$.
...
- Jump one stone, incurring a cost of $|h_i - h_{i+1}|$:
  ...
  $$
  \texttt{dp}[i + 1] = \min(\texttt{dp}[i + 1], \texttt{dp}[i] + |\texttt{H}[i] - \texttt{H}[i + 1]|)
  $$

- Jump two stones, incurring a cost of $|h_i - h_{i + 2}|$:
  $$
  \texttt{dp}[i + 2] = \min(\texttt{dp}[i + 2], \texttt{dp}[i] + |\texttt{H}[i] - \texttt{H}[i + 2]|)
  $$
