...
However, notice that for the cases $N = 1$ and $N = 0$, there doesnt exist a $\texttt{dp}[N-2]$! These 2 cases are our base cases. For this problem, $N = 1$ and $N = 0$ are our only base cases, and we can simply set 
...
Lastly, we need to figure out which case of our dynamic programming is our answer. Since we defined $\texttt{dp}[x]$ as the $x$th term in the Fibonacci sequence and we want to find the $N$th term, our answer is simply
