...
public class Main {
	...
	static long[][] dp = new long[1 << MAX_N][MAX_N];
	// come_from[i] contains the cities that can fly to i
	static List<Integer>[] come_from = new List[MAX_N];
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < MAX_N; i++) { come_from[i] = new ArrayList<>(); }

		for (int f = 0; f < flight_num; f++) {
			int start = scanner.nextInt();
			int end = scanner.nextInt();
			come_from[end - 1].add(start - 1);
		}
		...
		for (int s = 2; s < (1 << city_num); s++) {
			// only consider subsets that have the first city
			if ((s & (1 << 0)) == 0) continue;
			// also only consider subsets with the last city if it's the full
			// subset
			if ((s & (1 << (city_num - 1))) == 0 && s != ((1 << city_num) - 1))
				continue;
		...
		System.out.println(dp[(1 << city_num) - 1][city_num - 1]);
	}
}
```
...
dp = [[0] * MAX_N for _ in range(1 << MAX_N)]
come_from = [[] for _ in range(MAX_N)]
...
def main():
	...
	for _ in range(flight_num):
		...
		come_from[end].append(start)
		index += 2

	dp[1][0] = 1
	for s in range(2, 1 << city_num):
		if not (s & (1 << 0)):
			continue
		if (s & (1 << (city_num - 1))) and s != ((1 << city_num) - 1):
			continue
		...
		for end in range(city_num):
			...
			prev = s - (1 << end)
			for j in come_from[end]:
				if s & (1 << j):
					dp[s][end] += dp[prev][j]
					dp[s][end] %= MOD

	print(dp[(1 << city_num) - 1][city_num - 1])
