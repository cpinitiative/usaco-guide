...
Due to Python's large constant factor, the same C++ solution that is accepted TLEs in Python.
...
dp = [[0 for _ in range(n)] for _ in range(1 << n)]  # Cities indexed from 0 to n-1
dp[1][0] = 1
