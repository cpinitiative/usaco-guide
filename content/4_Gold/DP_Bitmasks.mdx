...
// adj[i] contains the cities that can fly to i
...
int main() {
	...
	// NOTE: the bitmask is 0-indexed, but the cities aren't
	dp[1][1] = 1;
	for (int s = 2; s < 1 << city_num; s++) {
		...
		for (int end = 1; end <= city_num; end++) {
			if ((s & (1 << (end - 1))) == 0)
				continue;
			
			// the subset that didn't include the current end
			int prev = s - (1 << (end - 1));
			for (int j : adj[end]) {
				if ((s & (1 << (j - 1)))) {
					dp[s][end] += dp[prev][j];
