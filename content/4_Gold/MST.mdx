...
class kruskal{
   ...
   public static void main(String[] args) throws IOException
   {
      ...
      comp = N;
      long ans = 0;
      disjoint = new int[N+1]; Arrays.fill(disjoint, -1);
      size = new int[N+1]; Arrays.fill(size, 0);
      while(!pq.isEmpty()){
         Edge e = pq.poll();
...
Note that **Prim's algorithm** has complexity $O(E \log E)$ since in the worst case every edge will be checked and its corresponding vertex will be added to the priority queue. 
