...
def main():
    ...
    left = subset_sum(a, 0, n // 2 - 1)
    right = subset_num(a, n // 2, n - 1)
    ...
    ans = 0
    for i in left:
        low_index = lower_bound(right, x - i)
        high_index = upper_bound(right, x - i)
        ans += high_index - low_index
