...
Meet in the middle problems often have double the constraints of an already
optimized solution.
...
increase our answer. Worst case this does about $4^{40}$ operations, which is
way too slow.

### Meet in the Middle Solution

We can divide the given array into two separate arrays. Let's say that the
$left$ array runs from indexes $0$ to $\frac{n}{2}-1$, and the $right$ array
...
recombine them to search for our answer. For every $\texttt{sum}$ in the $left$,
we can simply check how many elements of $x - \texttt{sum}$ there are in
$right$. This can be done using simple binary search.
...
CSES has very tight time constraints for java, so the following code will barely pass in around ~0.98 ms. In addition, it might not even pass on first submits.
</Warning>
