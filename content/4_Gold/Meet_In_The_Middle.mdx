...
We can divide the given array into two separate arrays. Let's say that the $left$ array runs from indexes $0$ to $\frac{n}{2}-1$, and the $right$ array runs from indexes $\frac{n}{2}$ to $n-1$. Both arrays will have at most $20$ elements, so we can loop through all subsets of these two arrays in at most $2^{21}$ operations, which is perfectly fine.

Now that we've got the subset sums of these two separate arrays, we need to recombine them to search for our answer. For every $\texttt{sum}$ in the $left$, we can simply check how many elements of $x - \texttt{sum}$ there are in $right$. This can be done using simple binary search.
