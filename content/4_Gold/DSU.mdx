...
This implementation assumes that `disjoint` is an array that starts such that
`disjoint[i] = -1` for every $0 <= i < n$.
...
The solution below is more verbose than some of the templates above. Use
whichever implementation you are most comfortable with.
