...
export const metadata = {
  problems: {
    ...
    general: [
      ...
      new Problem("CF", "Level Generation", "problemset/problem/818/F", "Normal", false, [], "first find out which is the best way to construct the graph, then it's possible to see that the number of edges increase for some range and then decrease; so, using binary search find the last i such that f(i-1)<=f(i)"),
      new Problem("CF", "Packmen", "contest/847/problem/E", "Normal", false, [], "binary search on time and check if packmen can eat all keeping left and right endpoints"),
...
We want to find the last point at which the condition is satisfied.
...
while(l <= r) {
    ...
    if (check(mid) == true) { 
        // if mid works, then all numbers smaller than mid also work, so we only care about the part after mid
        res = mid; // update the answer
        l = mid + 1; // cut the part before mid, notice that we already handled mid itself so we cut it as well
    } else {
        // if mid does not work, greater values would not work too, so we don't care about them
        r = mid - 1; // cut mid and after
    }
}
/// now res is the answer
/// if res stayed as -1 that means no value in the range satisfies the condition
```
