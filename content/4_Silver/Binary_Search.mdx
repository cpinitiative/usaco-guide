...
The binary search function for this is very similar. We will need to do the same thing, but when the condition is satisfied, we will cut the right part, and when it's not, the left part will be cut. Here is a more concise implementation:
...
int main() {
  ps(fstTrue([](int x) { return 1; },2,10)); // 2
  ps(fstTrue([](int x) { return x >= 5; },2,10)); // 5
  ps(fstTrue([](int x) { return x >= 20; },2,10)); // 11
}
```
