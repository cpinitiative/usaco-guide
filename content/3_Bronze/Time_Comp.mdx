...
In programming contests, your program needs to finish running within a certain timeframe in order to receive credit. For USACO, this limit is $4$ seconds for Java submissions. A conservative estimate for the number of operations the grading server can handle per second $10^8$ but could be closer to $5 \cdot 10^8$ given good constant factors).
...
- Java Quicksort `Arrays.sort` function on primitives: $O(n^2)$ 
  - See "Introduction to Sorting" for details.
