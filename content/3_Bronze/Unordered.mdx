...
In USACO contests, unordered sets and maps generally fine, but the built-in hashing algorithm is vulnerable to pathological data sets causing abnormally slow runtimes, in turn causing failures on some test cases (see [neal's blog](https://codeforces.com/blog/entry/62393)). 

To fix this, use a different hashing algorithm. (how?)

(Java isn't vulnerable though)