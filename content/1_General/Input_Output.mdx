...
The first method of reading input can be far slower than using `stdin` (Hundreds of times slower!). Coupled with python's relatively slow execution speed, reading input efficienctly becomes incredibly important. 

```py
# import the sys module to use std in/out
import sys 
...
# for larger inputs, you can buffer the problem's input, and sort through it yourself. Fastest input method for python.
...
So taking three integers as input and printing their sum is quite simple. On a larger scale (thousands of integers), using stdin and stdout become far more important for speed:
