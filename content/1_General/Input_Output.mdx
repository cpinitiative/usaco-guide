...
The following solution, which uses `int` instead of `long long`, does
...
value for the `int` data type ($2^{31}-1$, as mentioned in the
[prerequisite module](/general/data-types)).
...
As noted in the resource above, this problem requires _64-bit integers_.
The following solution, which uses `int` instead of `long`, does
not pass all of the test cases.
...
<Info title="Choice of IDE">

We assume in this section that you are using an online IDE, such as
[USACO Guide IDE](https://ide.usaco.guide/). The instructions for using
a local IDE are similar, except you skip the download step.

</Info>

Do the following to submit on CSES. Submitting on other platforms (such as
USACO) is similar.

1. Run your solution code with the sample input, and make sure that it produces the sample output.<Asterisk>for problems with only one correct output</Asterisk>
...
5. Upload your solution file. For CSES, assuming the file had the proper
extension, the language should be automatically detected (for USACO, select
the language yourself). Some platforms (such as Codeforces) allow you
to paste your code into a text box instead of uploading a file.
6. Submit your solution. For CSES, you will be redirected to the results page
(for USACO, results will appear at the top of the page). If your solution is
correct on all the test cases, you're done! Otherwise, fix your code and start
again from step 1.

<JavaOnly>

<Warning title="File Naming for Java Users">

The name of the file you submit must match the name of your public class.
For example, if your public class is named `PublicClassName`, then the name of the
...
<Info title="Submitting to Past USACO Problems">

If you are using the [USACO Guide IDE](https://ide.usaco.guide/), you can
submit to some past USACO problems through that (Settings -> Judge).
You cannot use this to submit during a live contest.

</Info>

## File I/O

<Info title="USACO File I/O">
