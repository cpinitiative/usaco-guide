...
- **Redirects**: _Optional_. Takes a list of URLs that will redirect to the
  current module. Include this whenever the module URL changes.
...
`[insert text here](/general/code-con)`.
...
solution but the problem metadata isn't set to `'internal'`. The
