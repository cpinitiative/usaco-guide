...
```
Assertion failed: (false), function main, file prog.cpp, line 6.
zsh: abort      ./$1 $@[2,-1]
```

If you want to disable all asserts you can add `#define NDEBUG` to the start of your program.
...
For example, the following code results in a runtime error on 
[ide.usaco.guide](https://ide.usaco.guide), but runs without errors on my computer.
...
or `lldb`. See the section about [debuggers](#debuggers) for more information.
...
If your code is getting WA, one option is to run your buggy program against 
another program that you're relatively confident is correct on randomly 
...
In this section we'll go over some options you can add to your
`g++` compile command to aid in debugging. You can find the official documentation
...
This replaces each STL container with its corresponding debug container.
...
For example, the following `prog.cpp` gives a segmentation fault.
...
### C++: Printing More Information About Runtime Errors
...
section where the output didn't contain the line where the runtime error 
occurred. To output the line number, you can

1. Add `-g` to the compile command.
