...
mention line # [macros](https://gcc.gnu.org/onlinedocs/cpp/Standard-Predefined-Macros.html)
...
Here is an interactive script to test a Java program with input and output files:

```
read -p "Enter the name of the Java class to be tested: " name
read -p "Enter the number of test cases: " cases
...
for (( i = 1; i <= cases; i++ )); do
	echo "Testing: $i"
	cp $i.in $name.in
	java $name.java < $name.in > $name.out

	diffOut=$(diff -w $i.out $name.out 2>&1)
...
if [[ wrong -gt 0 ]]; then
	echo -e "$RED*** TEST CASE(S) FAILED ***$NC"
else
...
If you want to learn how to write these scripts yourself, you can check [here](https://usaco.guide/general/cpp-command#linux) (but take this advice with a grain of salt since I'm not so proficient myself ...).

<Warning>

This won't work if you're using Windows. Instead, you can use what [tourist](https://www.youtube.com/watch?v=JU3HY5GLVnY&t=58m58s) does:
...
You can also check what options [Errichto](https://codeforces.com/blog/entry/64993) and [ecnerwala](https://gist.github.com/ecnerwala/a3c6332ac626bc448165) use.

### Warning Options

<Resources>
	<Resource starred source="GCC" url="https://gcc.gnu.org/onlinedocs/gcc/Warning-Options.html" title="3.8 - Options to Request or Suppress Warnings">documentation for options below</Resource>
</Resources>
...
<Resources>
	<Resource source="LCPP" url="ttps://www.learncpp.com/cpp-tutorial/variable-shadowing-name-hiding/" title="6.5 - Variable Shadowing (Name Hiding)" starred> </Resource>
</Resources>
...
`-fsanitize` flags [don't work with mingw](https://github.com/msys2/MINGW-packages/issues/3163). If you're using Windows but still want to use these flags, consider using [an online compiler](https://repl.it/) (or installing Linux) instead.
...
According to [this issue](https://github.com/msys2/MINGW-packages/issues/3163), AddressSanitizer does not appear to be available for MinGW.
