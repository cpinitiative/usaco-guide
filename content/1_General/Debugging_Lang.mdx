...
Sometimes, you'd like to know around which line your code is errorring at.
...
C++ usually silently fails when you access or write to a
...
int main() {
	vector<int> valid_vec(1, 10);
	vector<int> invalid_vec(1, 5);
	for (int i = 0; i < 10; i++) {
		cout << i << " " << valid_vec[i] << "\n";
	}
...
[here](https://usaco.guide/general/cpp-command#linux).
