...
See [Working With MDX](/content/1_General/Working_MDX.mdx) for additional information.
...
2.  If it doesn't already exist, create a new `mdx` file in `solutions/`
    ...
    **Time Complexity:** $\mathcal{O}(N\log^2N)$
    ...
    [Working With MDX](/content/1_General/Working_MDX.mdx#problem-lists) for more examples of
    IDs.
...
- Use spaces to separate your operators. For example, type `a + b` instead of `a+b`.
