...
2.  If it doesn't already exist, create a new `mdx` file in `solutions/`
    ...
    **Time Complexity:** $\mathcal{O}(N\log^2N)$
    ...
    Keep file names and solution IDs consistent; namely, The ID for a USACO
    problem (such as the one above) is the number at the very end of the URL on
...
- Use universally understandable variable names, especially for Bronze and
  ...
  in-contest.
  - Do not name a data structure the data structure's name. For example, don't
    name a `std::set` "set."
- If you use a I/O template, make sure to collapse it with
...
Furthermore, avoid variable-length arrays as described
