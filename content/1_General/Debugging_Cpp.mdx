...
description: 'C++-Specific Debugging Tips'
---

## Checking for Out of Bounds

Writing to an invalid array index is known as 
...
or `lldb`. See the section on [debuggers](#debuggers) for more information.
...
We give examples for some of these below. You can find the official 
documentation for said options [here](https://gcc.gnu.org/onlinedocs/gcc/Warning-Options.html).
...
With `-fsanitize=undefined`, a more informative error message is produced:
...
## Example: Using the LLDB Debugger
...
occurred. Assuming the C++ source code is stored in `prog.cpp` and the executable
is named `prog`, to output the line number using `lldb` you can
