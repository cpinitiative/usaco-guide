...
Here is the script that was mentioned in the video. Set the commands at the top
based on how each program is run:

```
# A and B are solutions you want to compare, gen takes int
...
For example, after compiling a Java program, you can use `A="java A"`. For a
Python program, you can use `A="python3 A.py"`.
...
```
# A is the command for the solution you want to test
for ((i = 1; i <= 10; ++i)); do
    echo $i
    $A < $i.in > out
