...
```cpp
vi v = {2,4,1,5,3};
sort(1+all(v)); // {2,1,3,4,5}    this expands to sort(1+begin(v), end(v));
sort(all(v)); // {1,2,3,4,5}
```

Note how `1+all(v)` expands to `1+begin(v), end(v)`. If you find this confusing, just write `sort(1 + begin(v), end(v))`.
