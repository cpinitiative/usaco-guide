...
You are not allowed to use pre-written code for USACO contests, so you need to type out any code you use during the contest. If you want to use a template, it has to be written from scratch.
...
Note how `1+all(v)` expands to `1+begin(v),end(v)`.

<Warning>

This is potentially confusing, so it might not be a good idea to use this.
