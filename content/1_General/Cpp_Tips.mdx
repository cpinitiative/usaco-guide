...
<Warning>
	You are not allowed to use pre-written code for USACO contests, so you need to
	type out any code you use during the contest. If you want to use a template, it has to be written from scratch.
</Warning>
...
The syntax of `all(v)` can be confusing and is better written as `v.begin(),v.end()`. For example, `1+all(v)` is more clearly written as `v.begin()+1,v.end()`. Generally, don't use `all(v)` as it only saves a couple characters.
...
$10^9+7$ is a prime that appears quite frequently in programming contests. Interestingly, $(10^9+7-1)/2=5\cdot 10^8+3$ is also prime. $998244353$ is also a prime that shows up often in programming contests.
