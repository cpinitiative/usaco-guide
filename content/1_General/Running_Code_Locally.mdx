...
author: Benjamin Qi, Hankai Zhang, Anthony Wang, Nathan Wang, Nathan Chen, Owen Wang, Shourya Bansal, @wastingtime#1534
...
Your best options for text editors during competitions will probably be either Geany, CodeBlocks, or Visual Studio Code.
The decision comes down solely to your preference.

If you want to get the full functionality of C++ on your Windows Machine, you should install one of the following
compilers:

#### MinGW

If you want to keep the ease of use and simplicity of Linux C++, including the ability to use commands like `g++` and
easily include `<bits/stdc++.h>`, you can install the compiler MinGW. This is a version of the `g++` compiler from Linux
...
In order to get a similar library to `<bits/stdc++.h>` you can use the following template, which automatically swaps
between `<bits/stdc++.h>` and Microsoft's variant depending on the system running it:
...
subsequently includes the correct libraries for the system. This comes with the same consequences as using
`<bits/stdc++.h>` for the same reasons.

*Note: As USACO has now disallowed the use of templates during competitions, if you want to be able to get the
functionality of `<bits/stdc++.h>` on your local machine, you should use MinGW instead.*

#### Windows Subsystem for Linux

Windows Subsystem for Linux, commonly referred to as WSL, runs the linux kernel within you windows installation,
allowing you to use Linux binaries without having to use Linux as your main Operating System. This means that you can
install `g++`, use `g++`, and consequently include `<bits/stdc++.h>`.
