...
Vim is a good text editor for efficiency when programming. It has many keybinds associated with it$\text{\textemdash}$you can find a good cheat 
sheet [here](https://vim.rtorr.com/).
...
1. Open a file with `vim {file}`
2. Navigate to the desired location
3. Enter insert mode to edit (`i`)
4. Return to normal mode (`Esc`)
5. Save and quit with `:wq`, or quit without saving with `:q!`

Vim is a modal text editor, meaning there are different modes.
The most common modes are insert and normal. Opening Vim (by typing `vim` in
...
Vim allows you to run code via the terminal. The `:term` command (type `:term` in normal mode) opens
the terminal in a horizontal split. If you want a vertical split, type
...
insert mode is the same$\text{\textemdash}$when in normal mode, just press `i`. In normal mode, you can cycle
windows with `<Ctrl-w><w>`. In insert mode, you can close the terminal with `<Ctrl-D>`.

Vim also supports tabs. To open the terminal in a separate tab, use `:tab term`. You can cycle tabs
...
in the terminal, opening a tutor that can help you learn the keybinds.

Vim has many powerful customizability features. Through the
...
and run code. However, this is mostly personal preference.
...
Despite its steep learning curve, Vim is a powerful tool for many users. If you prefer
staying on the terminal, being able to customize your experience from the ground up, or using
your keyboard instead of your mouse, Vim is likely a good fit for you. If you prefer a GUI,
being able to manage large directories, and minimal friction 
to writing code, a more traditional editor like VS Code may 
be more suitable.
