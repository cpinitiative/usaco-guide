...
contributors:
  ...
  Shourya Bansal, Dong Liu, Ruben Jing
...
<Resources>
	<Resource
		...
		starred
	>
		See [PAPS 2.1](/PAPS.pdf#page=30) and the [docs](https://code.visualstudio.com/docs/cpp/config-linux) for C++ setup instructions.
	</Resource>
	<Resource
		source="Jetbrains"
		title="CLion"
	...
	>
		Windows & Linux. [Apparently](https://codeforces.com/blog/entry/61780) was unstable at IOI.
	</Resource>
	<Resource
		source="Apple"
		title="XCode"
...
<Spoiler title="Vim">
Good text editor for efficiency when programming. Vim has a lot
of keybinds associated with it, a good cheat sheet is [here](https://vim.rtorr.com/).
The more you use Vim, the easier it gets. A lot of these keybinds
aren't used in normal usage, so don't be scared by the amount.

A standard workflow would be opening a file with `vim {file}`,
navigating to a desired location, entering insert mode to edit parts of
the file, returning to normal mode,
then saving and quitting with `:wq`, or quitting without saving with `:q!`.

Vim is a modal text editor, meaning there are different modes.
The most common modes are insert and normal. Opening Vim--typing `vim` in
the command line--will have you start in normal mode. In normal mode,
you can move around with Vim motions (like `hjkl` for left/down/up/right)
or other forms of navigation (like `w` to jump to the next word and `f` to
search characters on the current line). After you get to your desired location, enter
insert mode with `i` (insert before cursor), `a` (insert after cursor), or other
keybinds. To return to normal mode when in insert mode, press `Esc`.

Additionally, tools like find-and-replace in VSCode exist in Vim as well.
Find-and-replace closely mimics the [sed](https://www.gnu.org/software/sed/manual/sed.html)
command's syntax. Undo and redo, copy and paste, indenting multiple lines,
and searching for patterns are also in Vim's normal mode. It's best to understand regex for
some of these tools. To learn regex, check out [RexEgg](https://www.rexegg.com/regex-quickstart.php).

In Vim, you can also run code. Vim has a `:term` command (type `:term` in normal mode) to open
the terminal. By default, this should be a horizontal split. If you want a vertical split, type
`:vert term`. Inside the terminal, you can run code as you normally would.

The terminal also has a normal/insert mode. To enter normal mode, use `<Ctrl-\><Ctrl-N>`. Entering
insert mode is the same - when in normal mode, just press `i`. In normal mode, you can cycle
windows with `<Ctrl-w><w>`. In insert mode, you can close the terminal with `<Ctrl-D>`.

Vim also has tabs. To open the terminal in a separate tab, use `:tab term`. You can cycle tabs
in normal mode with `gt`.

The keybinds for these functionalities are all documented in the cheat
sheet, linked above. Additionally, you can run the `vimtutor` command
in terminal, opening a tutor that can help you learn the keybinds.

Some more powerful features of Vim include customizability. Through the
`~/.vimrc` file, you can change how your Vim appears, functions, and
even create custom keybinds! Many people publish their `~/.vimrc`s online.
In your `~/.vimrc`, you can also streamline the code running experience.
The instructions above are a bit complicated, and some Vim users make
binds to easily open the terminal, while others make binds to compile
and run code. However, this is mostly your preference.

Vim has many forks as well, with a major one being [NeoVim](https://github.com/neovim/neovim).
The core functionality and keybinds remain nearly identical, but Vim forks may have
more features.

Despite it's steep learning curve, Vim can be a powerful tool for many users. If you prefer
staying on the terminal, being able to customize your experience from ground up, or using
your keyboard instead of your mouse, Vim would be a good fit for you. If you prefer a GUI,
being able to manage large directories, and just want to write code as soon as possible,
