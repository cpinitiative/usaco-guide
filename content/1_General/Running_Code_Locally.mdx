...
would normally expect from an IDE/text editor.
...
Run this command to install C/C++ compilers on Linux:
...
* The `-Wall` flag will show all warnings of the code, which can be helpful in debugging.
...
You can also [build from source and install](https://github.com/python/cpython#build-instructions).
