...
3.  Recompile your C++ file:

    ```
    g++ -add-flags-here -Winvalid-pch file.cpp
    ```

    The `-Winvalid-pch` will warn you if the precompiled header is not used. For
    example, if I precompiled `stdc++.h` with `-std=c++17`, but I compiled my
