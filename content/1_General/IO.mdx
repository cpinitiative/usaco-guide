...
From December 2020 onwards, USACO too switched to using standard IO; however to practice older problems, it will still be necessary. In USACO, the inout and output file names are given and follow the convention `problemname.in`. After the program is run, output must be printed to a file called `problemname.out`. Note that you'll have to rename the `.in` and `.out` files depending on the problem. For example, in the above problem you would use `paint.in` and `paint.out`.
...
The standard IO functions can also be redefined to work with file IO. The following template (which is commonly used on USACO solution pages) uses this redefinition to solve this example.
