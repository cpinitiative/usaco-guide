...
You will need the [`<cstdio>`](http://www.cplusplus.com/reference/cstdio/) library. The `freopen` statements reuse standard I/O for file I/O. Afterwards, you can simply use `cin` and `cout` (or `scanf` and `printf`) to read and write data.
