...
In most websites (such as CodeForces and CSES), input and output are **standard**. Here, we'll provide examples that take three integers as input and prints their sum. 
...
<Warning title="F-Strings">

F-Strings do not work on the USACO Grader and will result in a missing output file.
