...
export const problems = {
	weird: new Problem('CSES', 'Weird Algorithm', '1068', 'Very Easy'),
...
All of the examples below will read in three integers as part of a single line and output their sum. For example, given the input
...
This library is quite straightforward to use and the `cin` function simply assigns values sepearated by spaces or lines to different variables. Similarly, the `cout` function simply writes on the standard console and either an escape sequence (that's what the `\n` does below) or the `endl` command can enter a new line at the end.
...
These are faster because they buffer the input and output and handle it all at once as opposed to parsing each line individually. However, `BufferedReader` is harder to use than `Scanner`. It has quite a few more methods and the `io` library must be imported for its use as well.
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		// StringTokenizer breaks down a string into "tokens"
...
From December 2020 onwards, USACO switched to using standard IO; however to practice older problems, it will still be necessary. In USACO, the input and output file names are given and follow the convention `problemname.in`. After the program is run, output must be printed to a file called `problemname.out`. Note that you'll have to rename the `.in` and `.out` files depending on the problem. For example, in the above problem you would use `paint.in` and `paint.out`.
...
int main() {
	...
	// this creates/overwrites the output file

	// now can use cin or scanf
	int a, b, c; cin >> a >> b >> c;
	// here, the cin function reads from the file instead of standard input
	cout << "sum is " << a+b+c << "\n";
}
```

Essentially, the `freopen` statements change the standard IO to point to files instead, and this allows simple functions to be used for files as well. After re-defining the IO functions, then you can simply use `cin` and `cout` to read and write data.
