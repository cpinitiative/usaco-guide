...
The standard IO functions can also be redefined to work with file IO. The following template (which is commonly used on USACO solution pages) uses this redefinition to solve this example. After re-defining the IO functions, then you can simply use `cin` and `cout` to read and write data.
