...
5.  If you want to be able to compile with just `g++`, write a shell alias! Aliases are discussed [here](TODO: link to alias section). Avoid overriding the system `g++` as that will almost surely cause problems.
...
- `-O2` tells `g++` to compile your code to run more quickly (see [here](https://www.rapidtables.com/code/linux/gcc/gcc-o.html)). Note that `-O2` will slow down compilation time significantly. Think of it as a trade-off between runtime and compile time. 
