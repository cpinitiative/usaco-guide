...
[USACO](http://www.usaco.org/index.php?page=instructions) (and most contests) use [GCC](https://en.wikipedia.org/wiki/GNU_Compiler_Collection)'s `g++` to compile and run your code. You'll need `g++` specifically to use the `#include <bits/stdc++.h>` header file within the template provided in [Code Conventions](/general/code-con#template); see [C++ Tips & Tricks](/general/cpp-tips#include-bitsstdch) for details.
