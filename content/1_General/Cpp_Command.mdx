...
5.  If you want to be able to compile with just `g++`, write a shell alias! Put the following lines into your shell's rc file (`~/.bashrc` if you use `bash`, and `~/.zshrc` if you use `zsh`).
    ...
    Once you do so, `g++ --version` should now output the same thing as `g++-9 --version`.
...
Don't be afraid, the `./name` will only run if `g++ name.cpp -o name` finished successfully. Such is the magic of `&&`!
...
- `-O2` tells `g++` to compile your code to run more quickly (see [here](https://www.rapidtables.com/code/linux/gcc/gcc-o.html)). Note that `-O2` will slow down compilation time. Think of it as a trade-off between runtime and compile time. 
...
You should always compile with these flags (with the possible exception of `-O2`, as described above).
