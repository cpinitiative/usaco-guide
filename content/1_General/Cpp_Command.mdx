...
Now you can easily compile and run `name.cpp` from the command line with `co name.cpp && ./name` or `run name.cpp`. Note that all occurrences of `$1` in the function are replaced with `name`, while `${1%.*}` removes the file extension from `$1`.
