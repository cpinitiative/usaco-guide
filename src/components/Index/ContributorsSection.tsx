...
const MemberCard = ({ member }: { member: Member }) => {
  const socialMedia = {
    ...
    facebook: {
      ...
      link: (x: string) => `https://www.facebook.com/${x}`,
    },
    linkedin: {
      icon: (
    ...
    github: {
      ...
      link: (x: string) => `https://github.com/${x}`,
    },
    codeforces: {
      icon: (
      ...
      link: (x: string) => `https://codeforces.com/profile/${x}`,
    },
    website: {
      icon: (
        <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
    ...
    reddit: {
      icon: (
       ...
       link: (x: string) => `https://reddit.com/user/${x}`,
    },
  };
          ...
          <div className="flex flex-col gap-3 px-5 pb-5">
            <ul className="flex justify-center gap-3">
              {Object.keys(socialMedia)
                ...
                .map(sm => {
                  const memberValue = member[sm as keyof typeof member];
                  ...
                  return (
                    <li key={sm} className="h-[26.5px]">
                      <a
                        key={member.name + sm}
                        target="_blank"
                        href={socialMedia[sm as keyof typeof socialMedia].link(memberValue)}
                        rel="noopener noreferrer"
                        className="inline-block text-gray-700 hover:text-gray-500 dark:text-gray-400 dark:hover:text-gray-300"
                      >
                        {socialMedia[sm as keyof typeof socialMedia].icon}
                      </a>
                    </li>
                  );
                })}
